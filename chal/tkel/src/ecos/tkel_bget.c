/*
   ...
   *   Automatic   pool compaction, growth, and shrinkage by
   ...
   checking using the <BGET_ASSERT.h> mechanism; all these checks   can   be
...
/*#define BGET_ASSERTIONS*/
#ifdef BGET_ASSERTIONS
   #define BGET_ASSERT(x)       if(!(x)) printf("!!!bget assert %d\n", __LINE__)
   ...
   /*
      ...
      */
   #define SHOULD_NOT_BE_HERE() printf("bget should not be here! %d", __LINE__)
#else
...
/***********************************************************************************************/
void *TKEL_bget(bufsize requested_size)
{
   ...
   if (size < (long)SizeQ) {        /* Need at least room for the */
   ...
   while (b != &freelist) {
      ...
      if ((best == &freelist) || (b->bh.bsize < best->bh.bsize)) {
        best = b;
        if ((best->bh.bsize - size) < (long)(SizeQ + (sizeof(struct bhead))))
        {
          /* when a free buffer is large enough to store the data */
          /* but small enough to avoid split of the buffer */
   ...
   while (b != &freelist) {
      ...
      if ((b->bh.bsize - size) > (long)(SizeQ + (sizeof(struct bhead)))) {
        struct bhead *ba, *bn;
...
/************************************************************************************************/
void TKEL_bpool(void* buf, bufsize len)
{
