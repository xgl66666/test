...
typedef struct _CCAttrSettings
{
    MS_U8               enClosedCaptionOn;
    MS_U8               enCCPotsion;// Auto: 0, Top: 1, Bottom: 2
    MS_U8               enCCFontSize;//Default:0,Small:1,Medium:2, Large: 3
    MS_U8               enCCFontColor;
    MS_U8               enCCBGColor;
    MS_U8               enCCOpacity;//Default: 0, Solid: 1,
    MS_U8               enCCPosition; //auto:0 top:1 bottom:2
} stCCAttrSettings ;


#ifdef CHAKRA_DEPENDENCY
typedef enum
{
    CC_MODE_DTV             = 0x00,
    CC_MODE_ATV             = 0x01,
...
typedef enum
{
    E_APICC_HB_CTRL_ON = 0,
   	E_APICC_HB_CTRL_OFF,
   	E_APICC_HB_CTRL_RESET,
} APICC_HB_CTRL_TYPE;
...
void apiCC_SetCmd(MS_U8 u8CmdIdx, MS_U8 u8SrvIdx);
void apiCC_SetAttr(stCCAttrSettings *stCCSettingFromHB);
void apiCC_Control(MS_U8 u8CtrlType);
void apiCC_SetKoreanCodeFromPSIP(MS_U8 *u8KoreanCode);
void apiCC_GOPPathSetting(MS_U8 u8PathType);
void apiCC_IgnoreField(MS_BOOL ignoreField);
void apiCC_SetByPassMode(MS_BOOL bByPass);
MS_BOOL apiCC_GetByPassMode(void);
