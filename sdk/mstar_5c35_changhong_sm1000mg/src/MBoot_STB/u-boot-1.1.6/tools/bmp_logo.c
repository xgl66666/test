...
int main (int argc, char *argv[])
{
	 ...
	 */
	skip_bytes (fp, 8);
	tsize = fread (&data_offset, sizeof (uint16_t), 1, fp);
	skip_bytes (fp, 6);
	tsize = fread (&b->width,   sizeof (uint16_t), 1, fp);
	skip_bytes (fp, 2);
	tsize = fread (&b->height,  sizeof (uint16_t), 1, fp);
	skip_bytes (fp, 22);
	tsize = fread (&n_colors, sizeof (uint16_t), 1, fp);
	skip_bytes (fp, 6);
