...
int do_i2c_read ( cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
    ...
    i2c_read (chip, (uint)addr, alen, buffer, len);
...
int do_i2c_read_byte ( cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
    uchar devaddr;
    uchar regoffset[2];
    uchar value;
    ...
    i2c_read (devaddr, (uint)regoffset, 2, &value, 1);
    printf("value=%d\n", value);
    return 0;
}

int do_i2c_write_byte ( cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
    uchar devaddr;
    uchar regoffset[2];
    uchar value;
    ...
    i2c_write (devaddr, (uint)regoffset, 2, &value, 1);
	return 0;
}
...
U_BOOT_CMD(
	i2c_init,	1,	1,	do_i2c_init,
	"i2c_init  - i2c initialization\n",
	"\n    -i2c initialization!!\n"
);

U_BOOT_CMD(
	i2c_r,	6,	1,	do_i2c_read,
	"i2c_r   - i2c read \n",
	"\n command: i2c_r slaveID(hex) addr[0](hex) addr[1](hex) alen(hex) len(hex)\n"
);

U_BOOT_CMD(
	i2c_rb,	3,	1,	do_i2c_read_byte,
	"i2c_rb  - i2c read for one byte\n",
	"\n command: i2c_rb slaveID(hex) offset(hex)\n"
);

U_BOOT_CMD(
	i2c_wb,	4,	1,	do_i2c_write_byte,
	"i2c_wb  - i2c write for one byte\n",
	"\n command: i2c_wb slaveID(hex) offset(hex) value(hex)\n"
);
