...
unsigned int MDrv_SYS_GetRawUART(void)
{
    unsigned int i, c, ch, offset, force_e ;
    unsigned char uart_int ;
    unsigned char* bank = (unsigned char *)0xA0100000 ;
...
int do_disable_uart ( cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
    ///*((volatile unsigned int*)(0xBF200000+(0xF1F<<2))) |= 0x100 ;
    //*(volatile unsigned char*)(0xbf803c78)|= 0x10;
	printf("Disable UART\n");
	#ifdef __AEONR2__
	*(volatile MS_U32*)(0xA0200000+(0x0F53*4)) |= 0x000F;
	#else
...
int do_echo (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
    ...
    printf( "size=0x%08X, checksum=0x%02X\n", image_size, sum ) ;
