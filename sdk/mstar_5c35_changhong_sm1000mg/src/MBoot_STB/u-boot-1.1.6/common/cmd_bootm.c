...
#define CFG_BOOTM_LEN	0x4000000	/* use 8MByte as default max gunzip size */
#endif
...
#if (CONFIG_COMMANDS & CFG_CMD_SECURE)
    Secure_Kernel_Authentication(addr);
#endif
...
# error Unknown CPU type
	...
	case IH_COMP_NONE:
		if(ntohl(hdr->ih_load) == addr) {
			...
			printf ("   XIP %s ... ", name);
#endif
        } else if(ntohl(hdr->ih_load) == (addr + sizeof(image_header_t))) {
#if (!CONFIG_MSTAR_SILENT)
            printf ("   Shifting %s ...\n", name);
#endif
        } else {
            ...
            flush_cache(ntohl(hdr->ih_load), len);
...
#define GetUnCachedAddr(x)  ((U8*)((U32)(x) + 0x20000000))   // Access data in DRAM not Cache
        i = lzo_decompress((unsigned char *)data, GetUnCachedAddr(ntohl(hdr->ih_load)), len, unc_len); // decompress code into non-cacheable memory area

#else
#define GetUnCachedAddr(x)  (U8*)(U32)(x)                   // Access data in DRAM not Cache
        i = lzo_decompress((unsigned char *)data, GetUnCachedAddr(ntohl(hdr->ih_ep)), len, unc_len); // decompress code into non-cacheable memory area
#endif

        /* if decompress fail */
        if (0 /* LZO_E_OK */ != i)
        {
            printf ("LZO ERROR %d - must RESET board to recover\n", i);
			SHOW_BOOT_PROGRESS (-6);
			udelay(3000);
			do_reset (cmdtp, flag, argc, argv);
        }
    ...
    if (gd->flags & GD_FLG_SILENT)
    {
        ...
        if (getenv("MS_BIN_START") != NULL)
        {
            run_command("cpmsbin", 0);
        }
...
print_type (image_header_t *hdr)
{
    ...
    case IH_CPU_AEONR2:	arch = "AEONR2";	    break;
    ...
    case IH_COMP_LZO:	comp = "lzo compressed";	break;
