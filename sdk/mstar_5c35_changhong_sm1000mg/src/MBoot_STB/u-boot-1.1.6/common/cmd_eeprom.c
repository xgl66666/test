...
int do_eeprom ( cmd_tbl_t * cmdtp, int flag, int argc, char *argv[])
{
	...
	if (argc == 5) {
		...
		if (strcmp (argv[1], "read") == 0) {
			...
			msAPI_rmBurstReadBytes(off, (U8 *)addr, cnt);
#if (EEPROM_DEUBG)
            {
                char buffer[128];
                memset(buffer, 0 , 128);
                sprintf(buffer, "md.b 0x%x 0x%x", addr, cnt);
                printf("cmd: %s\n", buffer);
			...
			msAPI_rmPageWrite(off, (U8 *)addr, cnt);
#if (EEPROM_DEUBG)
            {
                char buffer[128];
                memset(buffer, 0 , 128);
