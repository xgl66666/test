...
DECLARE_GLOBAL_DATA_PTR;
    ...
    !defined(CFG_ENV_IS_IN_SPIFLASH)    && \
...
U_BOOT_CMD(
    enctest,    CFG_MAXARGS,    1,     do_encrypt_Test,
    "EncTest   - call encrypt function in utopia\n",
    "[enctest aescbc len]\n"
    "    - aesecb: AESDMA ECB encrypt\n"
    "    - aescbc: AESDMA CBC encrypt\n"
    "    - aesctr: AESDMA CTR encrypt\n"
);

int do_decrypt_Test (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[]);
U_BOOT_CMD(
    dectest,    CFG_MAXARGS,    1,     do_decrypt_Test,
    "DecTest   - call decrypt function in utopia\n",
    "[enctest aescbc len]\n"
    "    - aesecb: AESDMA ECB decrypt\n"
    "    - aescbc: AESDMA CBC decrypt\n"
    "    - aesctr: AESDMA CTR decrypt\n"
);

int do_SHA_Test (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[]);
U_BOOT_CMD(
    shatest,    CFG_MAXARGS,    1,     do_SHA_Test,
    "SAHTest   - call SHA function in utopia\n",
);

int do_getChipID_Test (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[]);
U_BOOT_CMD(
    getchipidtest,    CFG_MAXARGS,    1,     do_getChipID_Test,
    "GetChipIDTest   - get CHIP ID via mailbox function in utopia\n",
);

int do_getDeviceID_Test (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[]);
U_BOOT_CMD(
    getdeviceidtest,    CFG_MAXARGS,    1,     do_getDeviceID_Test,
    "GetDeviceIDTest   - get DEVICE ID via mailbox function in utopia\n",
);
#endif

int do_mstar (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[]);
U_BOOT_CMD(
    mstar ,    CFG_MAXARGS,    1,     do_mstar,
    "mstar   - update kernal & root file system automatically by script file\n",
    "[script_file_name [target_ipaddr [tftp_serverip [bootdelay]]]]\n"
    "    - script_file_name: auto update script file name\n"
    "    - target_ipaddr: the ip address of target board\n"
    "    - tftp_serverip: tftp server ip address\n"
    "    - bootdelay: boot delay setting\n"
);
//--------------------------------------------------------------------------
//
//  Add usb update command
//
//--------------------------------------------------------------------------
int do_ustar(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[]);
U_BOOT_CMD(
    ustar ,    CFG_MAXARGS,    1,     do_ustar,
...
U_BOOT_CMD(
    cusid ,  7,    0,     do_checkOUIValid,
    "cusid check the image is release by the valid guys\n",
    "command cusid <oui> <swModel> <swVer> <ChipVersion> <BoardSel> <BuildCL>\n"
);
...
U_BOOT_CMD(
    usb_miu ,    CFG_MAXARGS,    1,     do_usb_miu,
    "ustar   - update kernal & root file system automatically by script file\n",
    "[script_file_name] \n"
    "    - when use this command \n"
    "    - at first use usb start \n"
...
U_BOOT_CMD(
    net_miu ,    CFG_MAXARGS,    1,     do_net_miu,
    "ustar   - update kernal & root file system automatically by script file\n",
    "[script_file_name] \n"
    "    - when use this command \n"
    "    - at first use usb start \n"
...
U_BOOT_CMD(
    pnlinfo ,    CFG_MAXARGS,    1,     do_setpanelinfo,
...
U_BOOT_CMD(
    sysinfo ,    CFG_MAXARGS,    1,     do_setsysinfo,
    "sysinfo   - set system info and save to nand flahs\n",
      NULL
);

#if 0
int do_kipargs (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[]);
U_BOOT_CMD(
    kipargs ,    CFG_MAXARGS,    1,     do_kipargs,
    "kipargs   - set Kernel Init Parameter\n",
    NULL
);
#endif
...
U_BOOT_CMD(
    cpmsbin ,    CFG_MAXARGS,    1,     do_cpmsbin,
