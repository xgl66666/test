...
int usb_hub_probe(struct usb_device *dev, int ifnum, int *bfound);
 ...
 * wait_ms
 */
#define USB_MSEC_LOOP              ( CPU_CLOCK_RATE/10000/4 )

void __inline__ wait_ms(unsigned long ms)
{
#if defined (CONFIG_TITANIA8) || defined(CONFIG_TITANIA12) || defined(CONFIG_AMBER2) || defined(CONFIG_TITANIA9) || defined(CONFIG_URANUS4) || defined(CONFIG_JANUS2) || defined(CONFIG_TITANIA13) || defined(CONFIG_AMBER1) || defined(CONFIG_KRONUS) || defined(CONFIG_KAISERIN)
	...
	while(ms-->0)
		//udelay(1000);
         { volatile int i; for (i=0; i< USB_MSEC_LOOP; i++); }
#endif
}
/***************************************************************************
 * Init USB Device
...
#define CHIP_TOP_BASE 0xBF203C00
#endif
int UsbPortSelect;
int usb_init(int port)
{
	int result;
        ...
        chipVer = *((unsigned char volatile *)(CHIP_TOP_BASE+0x60*4+1));
        chipVer &= 0x1;
        if (chipVer)
            printf("Kronus 128-pin package\n");
#endif
#if 0
...
//printf("device descr\n");
	...
	if( err < 0 )   //Get desc failed at the 1st time, that means usb_new_device failed.
		return -1;
	dev->descriptor.bMaxPacketSize0 = desc->bMaxPacketSize0;
        if (UsbPortSelect==0)
           SetControl1MaxPacket(desc->bMaxPacketSize0);
#if defined(ENABLE_SECOND_EHC)		
        else if (UsbPortSelect==1)
           SetControl1MaxPacket2(desc->bMaxPacketSize0);
#endif		
#if defined(ENABLE_THIRD_EHC)
        else if (UsbPortSelect==2)
           SetControl1MaxPacket3(desc->bMaxPacketSize0);
#endif
#if defined(ENABLE_FOURTH_EHC)
            else if (UsbPortSelect==3)
               SetControl1MaxPacket4(desc->bMaxPacketSize0);
#endif
	...
	if (parent) {
		...
		//err = hub_port_reset(dev, port, &portstatus);
		if (err < 0) {
...
int usb_scan_devices(void)
{
	...
	USB_PRINTF("scan end\n");
#endif
if (dev->config.if_desc[0].bInterfaceClass == 8)        //mass storage
        //do mass storage
{}
#ifdef ENABLE_USB_LAN_MODULE
else
