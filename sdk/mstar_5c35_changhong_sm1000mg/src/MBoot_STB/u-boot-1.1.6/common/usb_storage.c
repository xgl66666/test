...
int usb_stor_scan(int mode)
{
	...
	for(i=0;i<USB_MAX_DEVICE;i++) {
		...
		if(usb_storage_probe(dev,0,&usb_stor[usb_max_devs])) { /* ok, it is a storage devices */
		  ...
		  ret= usb_stor_get_info(dev, &usb_stor[usb_max_devs], &usb_dev_desc[usb_max_devs]);
		  if(ret == 1)
				usb_max_devs++;
		  else if (ret == (-1))
		  	break;
...
int usb_stor_BBB_transport(ccb *srb, struct us_data *us)
{
	...
	if((result < 0) && (us->pusb_dev->status & USB_ST_STALLED)) {
		...
		result = usb_stor_BBB_clear_endpt_stall(us, dir_in? (us->ep_in|0x80) : us->ep_out);
		if (result >= 0)
		{		
		    if (dir_in)
				FirstBulkIn = 1;
...
int usb_stor_Bulk_max_lun(struct usb_device *dev, struct us_data *us)
{
	int result;
    char iobuf[512];
	...
	result = usb_control_msg(dev, usb_rcvctrlpipe(dev, 0),
				 ...
				 USB_RECIP_INTERFACE,
				 0, us->ifnum, iobuf, 1, USB_CNTL_TIMEOUT * 5);

	USB_STOR_PRINTF("GetMaxLUN command result is %d, data is %d\n",
		  result, iobuf[0]);
	...
	if (result == 1)
		return iobuf[0];
	if (result == -3) //USB_STOR_TRANSPORT_NODEV
        return (-1);
...
int usb_stor_get_info(struct usb_device *dev,struct us_data *ss,block_dev_desc_t *dev_desc)
{
	...
	if (
		...
		||
		/* SanDisk Corporation Cruzer Micro AX06096IB*/
		(dev->descriptor.idVendor == 0x0781 &&
...
printf("usb_stor_Bulk_max_lun: ");
	maxLun = usb_stor_Bulk_max_lun(dev, ss);
printf("%x \n", maxLun);
	//pccb->lun = dev_desc->lun = usb_stor_Bulk_max_lun(dev, ss);
	if (maxLun < 0)
		return (-1);
	USB_STOR_PRINTF(" address %d\n",dev_desc->target);
	wait_ms(5);

	for (pccb->lun=0; pccb->lun<=maxLun; pccb->lun++)
	{
		if (maxLun>0)
			printf("Lun:%d\n", pccb->lun); 
