...
//typedef MS_BOOL ( *Demodcallback ) (MS_BOOL Timeout);
...
extern EN_DEVICE_DEMOD_TYPE MDrv_CofdmDmd_GetCurrentDemodType(void);
extern MS_U8 MSB123X_GetDemodScanTypeStatus(void);
extern MS_BOOL MSB123X_SetDemodScanTypeStatus(MS_U8 status);
extern MS_BOOL MDrv_CofdmDmd_SetCurrentDemodType(EN_DEVICE_DEMOD_TYPE  type);
extern MS_BOOL FT2125_DTV_SetTune(double Freq, RF_CHANNEL_BANDWIDTH eBandWidth);
extern MS_BOOL MSB123X_IIC_Bypass_Mode(MS_BOOL enable);
extern MS_BOOL MDrv_Demod_Restart(DEMOD_MS_FE_CARRIER_PARAM *pParam);
extern EN_LOCK_STATUS MSB123X_DTV_DVB_T_GetLockStatus(void);
extern MS_BOOL MDrv_cofdmDmd_InitParameter(void);
extern MS_BOOL MDrv_CofdmDmd_GetPlpBitMap(MS_U8* u8PlpBitMap);
extern MS_BOOL MDrv_CofdmDmd_GetPlpGroupID(MS_U8 u8PlpID, MS_U8* u8GroupID);
extern MS_BOOL MDrv_CofdmDmd_SetPlpGroupID(MS_U8 u8PlpID, MS_U8 u8GroupID);
