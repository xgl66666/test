...
{
	struct ubifs_old_idx *old_idx=NULL, *o=NULL;
...
void destroy_old_idx(struct ubifs_info *c)
{
	...
	struct ubifs_old_idx *old_idx=NULL;
...
{
	int err=0, node_len=0;
	struct ubifs_ch *ch = buf;
	uint32_t crc=0, node_crc=0;
			      ...
			      struct ubifs_zbranch *zbr, void *node)
{
    ...
    if((type!=UBIFS_INO_KEY)&&(type!=UBIFS_DATA_KEY)&&(type!=UBIFS_DENT_KEY)&&(type!=UBIFS_XENT_KEY))
        return -EINVAL; 
	//ret = try_read_node(c, node, key_type(c, key), zbr->len, zbr->lnum,
    //		    zbr->offs);
    ret = try_read_node(c, node, type, zbr->len, zbr->lnum,
    		    zbr->offs);
	if (ret == 1) {
				      ...
				      const struct qstr *nm, int adding)
{
    ...
    int /*uninitialized_var(o_n),*/o_n=0, err=0, cmp=0, unsure = 0, nn = *n;
...
static struct ubifs_znode *dirty_cow_bottom_up(struct ubifs_info *c,
					       struct ubifs_znode *znode)
{
	struct ubifs_znode *zp=NULL;
	int *path = c->bottom_up_buf, p = 0;

	ubifs_assert(c->zroot.znode);
	ubifs_assert(znode);
    if(c->zroot.znode==NULL) return ERR_PTR(-ENOMEM);
    if(znode==NULL) return ERR_PTR(-ENOMEM);
...
{
	int found=0, n=0, err=0, safely = 0, gc_seq1=0;
	struct ubifs_znode *znode=NULL;
	struct ubifs_zbranch zbr, *zt=NULL;
...
{
    int n=0, err = 0, lnum = -1 ;//, uninitialized_var(offs);
    int offs=0 ;//int uninitialized_var(len);
    int len=0;
...
{
	int lnum = bu->zbranch[0].lnum, offs = bu->zbranch[0].offs, len=0, err=0, i=0;
	void *buf=NULL;
...
static void correct_parent_keys(const struct ubifs_info *c,
				struct ubifs_znode *znode)
{
	union ubifs_key *key=NULL, *key1=NULL;
...
{
	struct ubifs_znode *zn=NULL, *zi=NULL, *zp=NULL;
	int i=0, keep=0, move=0, appending = 0;
...
{
	int i=0, n=0, k=0, err = 0;
	struct ubifs_znode *znode=NULL;
	union ubifs_key *key=NULL;
...
int ubifs_tnc_remove_ino(struct ubifs_info *c, ino_t inum)
{
	...
	struct ubifs_dent_node *xent=NULL, *pxent = NULL;
...
struct ubifs_dent_node *ubifs_tnc_next_ent(struct ubifs_info *c,
					   ...
					   const struct qstr *nm)
{
	int n=0, err=0, type = key_type(c, key);
	struct ubifs_znode *znode=NULL;
	struct ubifs_dent_node *dent=NULL;
	struct ubifs_zbranch *zbr=NULL;
	union ubifs_key *dkey=NULL;
