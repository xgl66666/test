...
static int _read_bbt (struct mtd_info *mtd, loff_t from, size_t len, size_t *retlen, u_char *buf)
{
        ...
        err = mtd->read_ecc (mtd, from, len, retlen, buf, NULL, chip->autooob);
        if (err){
                if (err == -EUCLEAN) {
                /*
...
static void create_bbt (struct mtd_info *mtd, uint8_t *buf, struct nand_bbt_descr *bd, int chip)
{
	...
	for (i = startblock; i < numblocks;) {
              if (!(this->options & NAND_MLC)) {
                nand_read_raw (mtd, buf, from, readlen, ooblen);
                for (j = 0; j < len; j++) {
                    if (check_pattern (&buf[j * scanlen], scanlen, (int)mtd->oobblock, bd)) {
                        this->bbt[i >> 3] |= (uint8_t)(0x03 << (i & 0x6));
                        break;
                    }
                }
	    }
           ...
           {
                len = 1;
                readlen = (size_t)((unsigned int)len * mtd->oobblock);
                nand_read_raw (mtd, buf, from, readlen, ooblen);
                for (j = 0; j < len; j++) {
                   if (check_pattern (&buf[j * scanlen], scanlen, (int)mtd->oobblock, bd)) {
                   this->bbt[i >> 3] |= (uint8_t)(0x03 << (i & 0x6));
                ...
                nand_read_raw (mtd, buf, (from + (loff_t)(1 << this->bbt_erase_shift) - (loff_t)(mtd->oobblock)), readlen, ooblen);
                for (j = 0; j < len; j++) {
                   if (check_pattern (&buf[j * scanlen], scanlen, (int)mtd->oobblock, bd)) {
                   this->bbt[i >> 3] |= (uint8_t)(0x03 << (i & 0x6));
                       break;
...
#else // somebody write wrong data to bbt, should be clarified & removed
	//tmpptr = kmalloc (16, GFP_KERNEL);
	this->bbt = kmalloc (len, GFP_KERNEL);
#endif
...
int nand_update_bbt (struct mtd_info *mtd, loff_t offs)
{
	...
	/* Do we have a bbt per chip ? */
	if (td->options & NAND_BBT_PERCHIP) {
            chip = (int)((offs >> (loff_t)this->phys_erase_shift) / (loff_t)this->blocknum);
            chipsel = chip;
	} else {
		chip = 0;
		chipsel = -1;
...
int nand_update_td(struct mtd_info *mtd, loff_t offs)
{
        ...
        len = mtd->size >> (chippr->bbt_erase_shift + 2);
        /* Allocate a temporary buffer for one eraseblock incl. oob */
        ...
        /* Do we have a bbt per chip ? */
        if ((td->options) & NAND_BBT_PERCHIP) {
                chip = (int)((offs >> (loff_t)chippr->phys_erase_shift) / (loff_t)chippr->blocknum);
                chipsel = chip;
        } else {
                chip = 0;
                chipsel = -1;
