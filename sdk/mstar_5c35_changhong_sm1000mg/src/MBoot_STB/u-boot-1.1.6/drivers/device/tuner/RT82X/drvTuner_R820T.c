...
static MS_BOOL I2C_Write(I2C_TYPE *I2C_Info)
{
    ...
    if (FALSE == MDrv_IIC_WriteBytes(u8SlaveID, 1, paddr, u16size, pu8data))
...
R828_ErrCode R828_SetFrequency(R828_Set_Info R828_INFO, R828_SetFreq_Type R828_SetFreqMode)
{
  ...
  /*  if ((R828_INFO.R828_Standard == ATSC_4063) || (R828_INFO.R828_Standard == ATSC_5070))
    ...
    {
        if ((R828_SetFreqMode == TUNER_NORMAL_MODE) || (R828_SetFreqMode == FAST_MODE))
        {
            // PRE_DECT on
            R828_Arry[1]  = (R828_Arry[1] & 0xBF) | SysFreq_Info1.PRE_DECT;
            R828_I2C.RegAddr  = 0x06;
...
{
    TunerMode.Femode = mode.Femode;
    TunerMode.Fetype = mode.Fetype;
}

FEMode MDrv_Tuner_GetTunerMode(void)
{
    FEMode mode;
    mode.Femode = TunerMode.Femode;
    mode.Fetype = TunerMode.Fetype;
    return mode;
}
MS_U32 MDrv_Tuner_SetTuner(MS_U32 u32Freq, MS_U8 feBw)
{
    R828_Standard_Type type = DVB_T_8M;
    R828_Set_Info R828_INFO;
    FEMode mode;

    if ((u32Freq < 42000) || (u32Freq > 1002000))
    {
        PRINTE(("input frequency error\n"));
        return FALSE;
    }

    mode = MDrv_Tuner_GetTunerMode();
    printf("\n u32Freq %d, ucBw %d, tuner type =%d\n", u32Freq, feBw, mode.Fetype);
    if (E_TUNE_TYPE_T2 == mode.Fetype)
    {
        switch (feBw)
        {
        case 6:
    ...
    else if (E_TUNE_TYPE_T == mode.Fetype)
    {
        switch (feBw)
        {
        case 6:
    ...
    else if (E_TUNE_TYPE_ISDBT == mode.Fetype)
    {
        type = ISDB_T;
    }
    else
    ...
    if (FALSE == R828_SetStandard(type))
    {
        PRINTE(("R828_Standard error\n"));
    }
    R828_INFO.RF_KHz = u32Freq;
    R828_INFO.R828_Standard = type;
    R828_INFO.RT_Input = SIGLE_IN;
    R828_INFO.RT_InputMode = AIR_IN;
    if (FALSE == R828_SetFrequency(R828_INFO, mode.Femode))
    {
        PRINTE(("R282 Set Freq error\n"));
        return FALSE;
    }
    return TRUE;
}
...
MS_BOOL MDrv_Tuner_Init(void)
{
    ...
    TunerMode.Femode = TUNER_NORMAL_MODE;
