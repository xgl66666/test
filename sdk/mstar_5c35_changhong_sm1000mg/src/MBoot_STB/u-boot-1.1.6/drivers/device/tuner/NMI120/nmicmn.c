                               ...
                               bool_t is_stereo, tTnrTuneMode tuneMode, tNmiDacSel dacSel, bool_t rfinvert, bool_t fmovermod)
{
...

    {
        ...
        **/
        ...
        {
            if (!ISNM120(pp->chipid)) //add by wang 5-21-2012
                wReg32(0x27c, 0x1010);
        }
    }
...
static void mn131_get_rssi(tTnrGain *pg)
{
    ...
    if (pp->std == nFM_RADIO)
    {
        ...
        if ((pp->frequency > 94400000) && (pp->frequency <= 100100000))
        {
            ...
            else if ((dagc > 119) && (dagc <= 135))
            {
                rssi_offsetx100 = 140;
            }
            else if ((dagc > 135) && (dagc <= 148))
            {
                rssi_offsetx100 = 0;
            }
            else if ((dagc > 148) && (dagc <= 158))
            {
                rssi_offsetx100 =  -200;
            }
            else if ((dagc > 158) && (dagc <= 164))
            {
                rssi_offsetx100 =  -400;
            }
            else if (dagc > 164)
            ...
            {
                if ((pg->bblicode == 0xf1) || (pg->bblicode == 0xf0))
                {
                    rssi_offsetx100 = 430;
                }
                else if (pg->bblicode == 0xef)
                {
                    rssi_offsetx100 = 520;
                }
                else
        ...
        {
            if ((dagc > 112) && (dagc <= 142))
            {
                rssi_offsetx100 = 380;
            }
            else if ((dagc > 142) && (dagc <= 176))
            {
                rssi_offsetx100 = 330;
            }
            else if ((dagc > 176) && (dagc <= 206))
            {
                rssi_offsetx100 = 250;
            }
            else if ((dagc > 206) && (dagc <= 232))
            {
                rssi_offsetx100 = 110;
            }
            else if ((dagc > 232) && (dagc <= 252))
            {
                rssi_offsetx100 = -240;
            }
            else if (dagc > 252)
...
static const uint16_t is_readonly[] =
{
    0x100,
    0x11c,
    0x120,
...
void dump_nmi_regiest(void)
{
    uint32_t val32;
    int adr32;
    nmi_tuner_os_log("dump_nmi_regiest start~~~~~~~~~~~\n");
    nmi_tuner_os_log("rf_regs_addr, rf_reg_value\n");
    for (adr32 = 0; adr32 <= 0x3c; adr32++)
    {
        val32 = rReg8(adr32);
        nmi_tuner_os_log("0x%02x, %2x\n", adr32, val32);
    }

    nmi_tuner_os_log("bb_regs_addr, bb_reg_value\n");
    for (adr32 = 0x100; adr32 <= 0x3FC; adr32 += 4)
    {
        int idx;
        for (idx = 0; idx < sizeof(is_readonly) / sizeof(is_readonly[0]); idx++)
        {
            if (is_readonly[idx] == adr32)
            {
                break;
            }
        }
        if (idx < sizeof(is_readonly) / sizeof(is_readonly[0]))
        {
            continue;
        }
        ...
        nmi_tuner_os_log("0x%04x, %8x\n", adr32, val32);
    }

    nmi_tuner_os_log("dump_nmi_regiest end~~~~~~~~~~~\n");
}
...
static void nm131_tuner_config(uint32_t in_rffrequency, tNmiTvStd std, tNmiOutput output, uint32_t *clk_off_f, tNmiDacSel dac_sel, bool_t rfinvert)
{
    ...
    for (retry = 0; retry < 2; retry++)
    {
        ...
        **/
        ...
        nmi_tuner_os_log("rffreq%x", (U16)(rffrequency >> 16));
        nmi_tuner_os_log(" %x\n", (U16)((rffrequency << 16) >> 16));
...
void nm131_set_scan_level(int level)
{
    ...
    {
    case 1:
        chip.priv.alocklevelth = 65;
        break;
    case 2:
        chip.priv.alocklevelth = 60;
        break;
    case 3:
        chip.priv.alocklevelth = 55;
        break;
    case 4:
        chip.priv.alocklevelth = 50;
        break;
    case 5:
        chip.priv.alocklevelth = 45;
        break;
    case 6:
        chip.priv.alocklevelth = 40;
        break;
    case 7:
        chip.priv.alocklevelth = 35;
        break;
    default:
        chip.priv.alocklevelth = 45;
    }
...
static uint32_t nm131_get_chipid(void)
{
    ...
    chipid = rReg32(0x3fc);
    //nmi_tuner_os_log("%x:",(uint16_t)(chipid>>16));
...
static int nm131_chip_init(void)
{
    ...
    // nmi_tuner_os_log("nm131_chip_init id(%d)\n",pp->chipid);
