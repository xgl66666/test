...
/******************************************************************************/
void msAPI_Tuner_Initialization(BOOL bSrcChg)
{
...
BOOLEAN msAPI_Tuner_GetSignalModulMode(WORD *wModul_Mode)
{
#if ENABLE_T_C_COMBO
...
{
    MDrv_CofdmDmd_GetPlpBitMap(u8PlpBitMap);
}
BOOLEAN msAPI_GetPlpGroupID(U8 u8PlpID, U8* u8GroupID)
{
    return MDrv_CofdmDmd_GetPlpGroupID(u8PlpID,u8GroupID);

}

BOOLEAN msAPI_SetPlpGroupID(U32 u32PlpID, U32 u32GroupID)
{
    return MDrv_CofdmDmd_SetPlpGroupID(u32PlpID,u32GroupID);
}
#else
void msAPI_GetPlpBitMap(U8* u8PlpBitMap)
{
