...
static U32                      DSI_OC_Transaction_id;
static U8                       u8ObjKeyLength = 0;
static U8                       u8ObjKey[OBJECT_KEY_MAX_LENGTH] = {0};
static BIOP_FILE_INFO           FileInfo;
//static U8 code                  ZipArea[3800];

extern U8 g_OAD_OUI[3];
extern U8 OAD_OUI_ASCII[7];
extern BIOP_BODY_PROFILE_IOR_INFO  OAD_OUI_DIR_IOR;
extern BIOP_BODY_PROFILE_IOR_INFO  OAD_FILE_IOR;

/******************************************************************************/
U32 process_service_domain( U32 Addr, U8 DataType );
U32 process_iop_ior( U8 byte_order, U32 Addr, struct biop_iop_ior *ior, U8 DataType );
U32 process_service_context_list( U32 Addr, U8 DataType );
void process_biop_dir( U8 byte_order, U32 Addr, U8 DataType) ;


/******************************************************************************/
U8 biop_uint08( U32 Addr, U8 DataType )
{
    U8  *p;
    U8  Buf[1];
    U32 dstaddr= 0;
    ...
    {
        dstaddr = (U32)Buf;
        //MApi_BDMA_Copy((U32)(Addr), XD2PHY(dstaddr), (U32)(1), MIU_SDRAM2SDRAM);
        //MDrv_BDMA_MemCopy((MS_U32)Addr, (MS_U32)dstaddr,(MS_U32)1) ;
        memcpy((U8*)dstaddr,(U8*)Addr,(U32)1) ;
    }
 ...
 */
U16 biop_uint16( U8 byte_order, U32 Addr, U8 DataType )
{
    U8  *p;
    U16 val;
    U8  Buf[2];
    ...
    {
        dstaddr = (U32)Buf;
        //MApi_BDMA_Copy((U32)(Addr), XD2PHY(dstaddr), (U32)(2), MIU_SDRAM2SDRAM);
        //MDrv_BDMA_MemCopy((MS_U32)Addr, (MS_U32)dstaddr,(MS_U32)2) ;
        memcpy((U8*)dstaddr,(U8*)Addr,(U32)2) ;
    }
...
U32 biop_uint32( U8 byte_order, U32 Addr, U8 DataType )
{
    U8  *p;
    U32 val;
    U8  Buf[4];
    U32 dstaddr= 0, temp1= 0, temp2= 0;
    ...
    {
        dstaddr = (U32)Buf;
        //MApi_BDMA_Copy((U32)(Addr), XD2PHY(dstaddr), (U32)(4), MIU_SDRAM2SDRAM);
        //MDrv_BDMA_MemCopy((MS_U32)Addr, (MS_U32)dstaddr,(MS_U32)4) ;
        memcpy((U8*)dstaddr,(U8*)Addr,(U32)4) ;
    }
 ...
 */
U32 biop_sequence255( U32 Addr, struct biop_sequence *out, U8 DataType )
{
    U8  *p;
    U8  Buf[1];
    U32 dstaddr = 0;
...
U32 biop_sequence65535( U8 byte_order, U32 Addr, struct biop_sequence *out, U8 DataType )
{
    U8  Buf[2];
    U32 dstaddr= 0;
    ...
    out->size       = (U32)biop_uint16( byte_order, dstaddr, DataType );
    out->data_p     = (U32)(Addr + 2);
...
U32 biop_sequence( U8 byte_order, U32 Addr, struct biop_sequence *out, U8 DataType )
{
    U8  Buf[4];
    U32 dstaddr = 0;
    ...
    out->size       = (U32)biop_uint32(byte_order, dstaddr, DataType);
    out->data_p     = (U32)(Addr + 4);
 ...
 */
U32 biop_align32( U32 size )
{
    return (size % 4) != 0 ? (4 - (size % 4)) : 0;
}

void biop_MessageHeader(U32 Addr, struct BIOPMessageHeader *header, U8 DataType)
{
    U8      *p;
    U32     dstaddr= 0, size= 0;
    U8      Buf[12];
    ...
    {
        dstaddr = (U32)Buf;
        //MApi_BDMA_Copy((U32)(Addr), XD2PHY(dstaddr), (U32)(size), MIU_SDRAM2SDRAM);
        //MDrv_BDMA_MemCopy((MS_U32)Addr, (MS_U32)dstaddr,(MS_U32)size) ;
        memcpy((U8*)dstaddr,(U8*)Addr,(U32)size) ;
    }
    ...
    dstaddr += 4;
    header->biop_version.u8major  = biop_uint08((U32)(dstaddr), SDRAM_TYPE); dstaddr+=1;
    header->biop_version.u8minor  = biop_uint08((U32)(dstaddr), SDRAM_TYPE); dstaddr+=1;
    header->byte_order          = biop_uint08((U32)(dstaddr), SDRAM_TYPE); dstaddr+=1;
    header->message_type        = biop_uint08((U32)(dstaddr), SDRAM_TYPE); dstaddr+=1;
    header->message_size        = biop_uint32(header->byte_order, (U32)(dstaddr), SDRAM_TYPE); dstaddr+=4;
...
*/
void biop_rawdata( U32 Addr, U8 * Buf, U8 len, U8 DataType )
{
    U32 dstaddr= 0;
    ...
    {
        dstaddr = (U16)(Addr);
        memcpy(Buf, (U8 xdata *)dstaddr, (U16)len);
    }
    else if ( DataType == SDRAM_TYPE )
    {
        //MApi_BDMA_Copy((U32)(Addr), XD2PHY(Buf), (U32)(len), MIU_SDRAM2SDRAM);
        //MDrv_BDMA_MemCopy((MS_U32)Addr, (MS_U32)dstaddr,(MS_U32)len) ;
        memcpy((U8*)Buf,(U8*)Addr,(U32)len) ;
    }
}
...
void process_biop_dir( U8 byte_order, U32 Addr, U8 DataType )
{
    ...
    for(i=0; i<nbindings; i++)
    {
        ...
        for(j=0; j<nnames; j++)
        {
            ...
            data_p += biop_sequence255((U32)data_p, &name, DataType);
            //OAD_BIOP_DBG(printf("  name %u: '%.*s'", j, name.size, name.data_p));
            data_p += biop_sequence255((U32)data_p, &kind, DataType);
...
BOOLEAN process_biop( U32 Addr, U32 size, U8 DataType )
{
    U32     bytes_left, msgheaderSize;
    U32     data_p, start, subhdr;
    ...
    S8      TypeID[4];
    U8      ObjectKey[OBJECT_KEY_MAX_LENGTH];
    ...
    while(bytes_left != 0)
    {
        ...
        biop_rawdata(kind.data_p, (U8*)TypeID, sizeof(TypeID), DataType);
...
U32 process_iop_ior( U8 byte_order, U32 Addr, struct biop_iop_ior *ior, U8 DataType )
{
    ...
    U32 id_length;
    U32 kind_length;
    U32 initialContextLength;
    U8 vsn_major, vsn_minor;

    data_p = start = Addr;

    /*[01]type_id =====================================================================*/
    /* typeId - "dir\0", "fil\0", etc */
    data_p += biop_sequence(byte_order, (U32)data_p, &type, DataType);

    /*[02]taggedProfiles_count ========================================================*/
    nprofiles = biop_uint32(byte_order, (U32)data_p, DataType);
    data_p += 4;
    ...
    for(i=0; i<nprofiles; i++)
    {
        ...
        /*[03-1]###profile header### ==================================================*/
        /*[03-1a]profileId_tag ========================================================*/
        tag = biop_uint32(byte_order, (U32)data_p, DataType);

        data_p += 4;

        /*[03-1b]profile_data =========================================================*/
        data_p += biop_sequence(byte_order, (U32)data_p, &profile, DataType);
        if(tag == TAG_BIOP)
        {
            ...
            if (biop_uint32(profile_bo, (U32)profile.data_p, DataType) != TAG_ObjectLocation)
                OAD_BIOP_ASSERT(printf("Expecting BIOP::ObjectLocation"));
            profile.data_p += 4;

            /*[03-2b]component_data_length ============================================*/
            /* component_data_length = *(profile.data_p); */
            OAD_BIOP_D1(printf("\nComponent Data Length: 0x%02bx", biop_uint08((U32)profile.data_p, DataType)));
            profile.data_p += 1;

            /*[03-2c]carouselId =======================================================*/   //(1)===>Look Up DownloadInfoIndication
            ior->carousel_id = biop_uint32(profile_bo, (U32)profile.data_p, DataType);
            OAD_BIOP_D1(printf("\nCar_ID: 0x%08LX", ior->carousel_id));
            profile.data_p += 4;

            /*[03-2d]moduleId =========================================================*/   //(2)===>Look Up DownloadInfoIndication
            ior->module_id = biop_uint16(profile_bo, (U32)profile.data_p, DataType);
            ...
            profile.data_p += biop_sequence255((U32)profile.data_p, &ior->key, DataType);

            /*[03-3]###ConnBinder### ==================================================*/
            /*[03-3a]DSM::ConnBinder: componentId_tag =================================*/
            if (biop_uint32(profile_bo, (U32)profile.data_p, DataType) != TAG_ConnBinder)
            ...
            if(taps_count > 0)
            {
                ...
                /*[03-4c]use ==========================================================*/
                if (biop_uint16(profile_bo, (U32)profile.data_p, DataType) != BIOP_DELIVERY_PARA_USE)
                    OAD_BIOP_ASSERT(printf("Expecting BIOP_DELIVERY_PARA_USE"));
                profile.data_p += 2;

                /*[03-4d]association_tag ==============================================*/   //(1)===>Look Up PMT
                ior->association_tag = biop_uint16(profile_bo, (U32)profile.data_p, DataType);
                ...
                /*[03-4f]selector_type ================================================*/
                if (biop_uint16(profile_bo, (U32)profile.data_p, DataType) != SELECTOR_TYPE_MESSAGE)
                    OAD_BIOP_ASSERT(printf("Expecting selector_type MESSAGE"));
                profile.data_p += 2;

                /*[03-4g]transactionId ================================================*/
                transaction_id = biop_uint32(profile_bo, (U32)profile.data_p, DataType);
                OAD_BIOP_D1(printf("\ntransaction_id: 0x%08LX", transaction_id));
                DSI_OC_Transaction_id = transaction_id;
                profile.data_p += 4;

                /*[03-4h]timeout ======================================================*/
                timeout = biop_uint32(profile_bo, (U32)profile.data_p, DataType);
            ...
            while(taps_count > 0)
            {
                OAD_BIOP_D1(printf("\nOther Tap"));
                /*[03-5a]id ===========================================================*/
                /* id = biop_uint16(profile_bo, *((U16 *) profile.data_p)) */
                profile.data_p += 2;
                ...
                association_tag = biop_uint16(profile_bo, (U32)profile.data_p, DataType);
                profile.data_p += 2;
            ...
            while((ncomponents-2)>0)
            {
                ...
                OAD_BIOP_DBG(printf("componentId_tag=0x%08x ", biop_uint32(profile_bo, (U32)profile.data_p, DataType)));
                profile.data_p += 4;
        ...
        else if(tag == TAG_LITE_OPTIONS)
        {
            ...
            if (biop_uint32(profile_bo, (U32)profile.data_p, DataType) != TAG_ServiceLocation)
                OAD_BIOP_ASSERT(printf("Expecting BIOP::ServiceLocation"));
            profile.data_p += 4;

            /*[03-2b]component_data_length ============================================*/
            /* component_data_length = biop_uint32(profile_bo, *((U32 *) profile.data_p)); */   //<<=== spec is incorrect
            ...
            nameComponents_count = biop_uint32(profile_bo, (U32)profile.data_p, DataType);
            profile.data_p += 4;

            for(i=0; i< nameComponents_count; i++)
            {
                /*[03-4b]id_length ====================================================*/
                id_length = biop_uint32(profile_bo, (U32)profile.data_p, DataType);
                profile.data_p += 4;
                ...
                /*[03-4d]kind_length ==================================================*/
                kind_length = biop_uint32(profile_bo, (U32)profile.data_p, DataType);
                profile.data_p += 4;
            ...
            initialContextLength = biop_uint32(profile_bo, (U32)profile.data_p, DataType);
            profile.data_p += 4;
        ...
        {
            OAD_BIOP_ASSERT(printf("\nUnknown IOP::IOR profileId_tag (0x%x)", tag));
        }
    }
...
*/
U32 process_biop_SGWInfo_Taps_Skip(U32 Addr, U8 DataType)
{
    U8  downloadTapsCount, selectorLength;      // variable of DSM::Tap()
    U8  i;
    U32 data_p, start;
...
U32 process_service_context_list_Skip(U32 Addr, U8 DataType)
{
    U8      i;
    U8      contextListCount;       // variable of serviceContextList
    U32     contextId;
    U16     contextDataLength;
    U32     data_p, start;
...
U32 process_profile_Body_GetTransID(void)
{
...
U32 process_userinfo_MIS(U32 Addr, U8 DataType)
{
    U16     userInfoLength, manufacturer_data_length;     // variable of userInfo
    U16   i, j;
    U8      manufacturer_info_tag;
    U32     manufacturer_id, version_id;
    U32     data_p, start;
    ...
    userInfoLength = biop_uint16(BIOP_BIGENDIAN, (U32)data_p, DataType); data_p += 2;
    ...
    {
        manufacturer_id = 0;
        manufacturer_id += ((U32)biop_uint08(data_p, DataType) << 16);
        manufacturer_id += ((U32)biop_uint08(data_p+1, DataType) << 8);
...
void process_biop_service_gateway_info(unsigned char *section)
{
    ...
    section += (U32)process_iop_ior(BIOP_BIGENDIAN, (U32)section, &sg_ior, SDRAM_TYPE);
    section += (U32)process_biop_SGWInfo_Taps_Skip((U32)section, SDRAM_TYPE);
    section += (U32)process_service_context_list_Skip((U32)section, SDRAM_TYPE);
    section += (U32)process_userinfo_MIS((U32)section, SDRAM_TYPE);
