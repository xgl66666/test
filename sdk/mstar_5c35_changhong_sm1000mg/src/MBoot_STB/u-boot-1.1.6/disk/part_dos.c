...
/* Only support FAT32 */
static inline int is_supported(int part_type)
{
    if (part_type == 0x4 || part_type == 0x6 ||part_type == 0xe)
    {
        printf("Not fat32, might contains problem\n");
        return 1;
    }
    return (part_type == 0x1 || part_type == 0xb ||
	    part_type == 0xc || part_type == 0x1c);
}
				 ...
				 int which_part, disk_partition_t *info)
{
	...
	for (i = 0; i < 4; i++, pt++) {
		 ...
		 */
		if ((pt->sys_ind != 0) &&
		    (le32_to_int(pt->size4) != 0) &&
		    (part_num == which_part) &&
		    (is_extended(pt->sys_ind) == 0) &&
		    (is_supported(pt->sys_ind) == 1)) {
			info->blksz = 512;
