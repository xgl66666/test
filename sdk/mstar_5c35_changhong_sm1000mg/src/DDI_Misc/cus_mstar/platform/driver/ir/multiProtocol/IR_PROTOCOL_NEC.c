...
IR_DECODE_STATUS _ParseNEC(U32 *pu32CustCode, U16 *pu16KeyCode, U8 *pu8State, U8 *pu8Reserved)
{
    ...
    if((MsOS_GetSystemTime() - _ulNecPreDecodeTime) > ((IR_TIME_NEC_TIMEOUT-IR_TIME_UPD(IR_TIME_NEC_REPEAT_SHOT_1+IR_TIME_NEC_REPEAT_SHOT_2+IR_TIME_NEC_REPEAT_SHOT_3, IR_TIME_NEC_LEADCODE_TOLERENCE))/1000))
    {//>110ms timeout
        _u32NecLastDecodeData=0;
    }

    _ulNecPreDecodeTime = MsOS_GetSystemTime();
    ...
    else
    {
        ...
        while(_MulProtCommon_PeekShot(u8StartIndex+IR_NEC_LEADCODE_SHOT_COUNT, 1, &u32ShotValue, &bNegShot)==TRUE)//only use peek here
        {
        ...
        if(u8NecBits==IR_NEC_BITS && bLastShotN==TRUE)
        {
            _NecRAW2Format(u32NecData, pu32CustCode, pu16KeyCode, pu8State, pu8Reserved);
            _u32NecLastDecodeData=u32NecData;
            return E_IR_DECODE_DATA_OK;
        }
