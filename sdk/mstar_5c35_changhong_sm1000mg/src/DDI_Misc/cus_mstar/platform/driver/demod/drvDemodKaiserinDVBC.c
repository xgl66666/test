...
DRV_DEMOD_TABLE_TYPE GET_DEMOD_ENTRY_NODE(DEMOD_MSKAISERIN_DVBC)  DDI_DRV_TABLE_ENTRY(demodtab);
static MS_BOOL bInited = FALSE;
static MS_S32 _s32MutexId = -1;
static DEMOD_MS_INIT_PARAM K2_DVBC_INIT_PARAM;
...
MS_BOOL MDrv_Demod_DVBC_Init(MS_U8 u8DemodIndex,DEMOD_MS_INIT_PARAM* pParam)
{
    ...
    K2_DVBC_INIT_PARAM.pstTunertab = pParam->pstTunertab;
...
MS_BOOL MDrv_Demod_DVBC_Open(MS_U8 u8DemodIndex)
{
...
MS_BOOL MDrv_Demod_DVBC_Close(MS_U8 u8DemodIndex)
{
    MS_BOOL ret;
...
MS_BOOL MDrv_Demod_DVBC_SetSerialControl(MS_U8 u8DemodIndex,MS_BOOL bEnable)
{
    MS_BOOL ret;
...
MS_BOOL MDrv_Demod_DVBC_PowerOnOff(MS_U8 u8DemodIndex,MS_BOOL bPowerOn)
{
    if (HB_ObtainMutex(_s32MutexId, COFDMDMD_MUTEX_TIMEOUT) == FALSE)
    {
        DMD_ERR(("%s: Obtain mutex failed.\n", __FUNCTION__));
...
MS_BOOL MDrv_Demod_DVBC_GetLock(MS_U8 u8DemodIndex,EN_LOCK_STATUS *peLockStatus)
{
...
MS_BOOL MDrv_Demod_DVBC_GetSNR(MS_U8 u8DemodIndex,float *pfSNR)
{
    MS_BOOL ret;
...
MS_BOOL MDrv_Demod_DVBC_GetBER(MS_U8 u8DemodIndex,float *pfBER)
{
    MS_BOOL ret;
...
MS_BOOL MDrv_Demod_DVBC_GetPWR(MS_U8 u8DemodIndex,MS_S32 *ps32Signal)
{
    MS_BOOL ret;
    if (HB_ObtainMutex(_s32MutexId, COFDMDMD_MUTEX_TIMEOUT) == FALSE)
    {
...
MS_BOOL MDrv_Demod_DVBC_GetSignalQuality(MS_U8 u8DemodIndex,MS_U16 *pu16quality)
{
    MS_BOOL ret;
    if (HB_ObtainMutex(_s32MutexId, COFDMDMD_MUTEX_TIMEOUT) == FALSE)
    {
...
MS_BOOL MDrv_Demod_DVBC_Config(MS_U8 u8DemodIndex,MS_U8 *pRegParam)
{
    if (HB_ObtainMutex(_s32MutexId, COFDMDMD_MUTEX_TIMEOUT) == FALSE)
    {
        DMD_ERR(("%s: Obtain mutex failed.\n", __FUNCTION__));
...
MS_BOOL MDrv_Demod_DVBC_GetParam(MS_U8 u8DemodIndex, DEMOD_MS_FE_CARRIER_PARAM* pParam)
{
...
MS_BOOL MDrv_Demod_DVBC_Restart(MS_U8 u8DemodIndex, DEMOD_MS_FE_CARRIER_PARAM* pParam,MS_U32 u32BroadCastType)
{
    ...
    if(!K2_DVBC_INIT_PARAM.pstTunertab->GetTunerIF(u8DemodIndex, &u32IF_Freq))
    {
       u32IF_Freq = _u32IFrequency;
       DMD_DBG(("K2 Demod Get Tuner IF FAIL, use default Tuner IF %d kHz\n",(int)u32IF_Freq));
    }
    else
    {
       DMD_DBG(("K2 Demod Get Tuner IF %d kHz\n", (int)u32IF_Freq));
    }
...
MS_BOOL MDrv_Demod_DVBC_SetMode(MS_U8 u8DemodIndex, Demod_Mode* pMode)
{
#if 0
    DMD_Mode dmdMod;
...
MS_BOOL MDrv_Demod_DVBC_SetOutoutPath(MS_U8 u8DemodIndex, DEMOD_INTERFACE_MODE path)
{
    if (HB_ObtainMutex(_s32MutexId, COFDMDMD_MUTEX_TIMEOUT) == FALSE)
    {
        DMD_ERR(("%s: Obtain mutex failed.\n", __FUNCTION__));
...
DEMOD_INTERFACE_MODE MDrv_Demod_DVBC_GetOutoutPath(MS_U8 u8DemodIndex)
{
    if (HB_ObtainMutex(_s32MutexId, COFDMDMD_MUTEX_TIMEOUT) == FALSE)
    {
        DMD_ERR(("%s: Obtain mutex failed.\n", __FUNCTION__));
...
MS_BOOL DEMOD_MSKAISERIN_DVBC_Extension_Function(MS_U8 u8DemodIndex, DEMOD_EXT_FUNCTION_TYPE fuction_type, void *data)
{
    switch(fuction_type)
    {
        default:
...
MS_BOOL MDrv_Demod_DVBC_I2C_ByPass(MS_U8 u8DemodIndex,MS_BOOL bOn)
{
  if(GET_DEMOD_ENTRY_NODE(DEMOD_MSKAISERIN_DVBC).I2CByPassPreSetting != NULL)
  {
      return GET_DEMOD_ENTRY_NODE(DEMOD_MSKAISERIN_DVBC).I2CByPassPreSetting(u8DemodIndex,bOn);
  }
  else
...
DRV_DEMOD_TABLE_TYPE GET_DEMOD_ENTRY_NODE(DEMOD_MSKAISERIN_DVBC)  DDI_DRV_TABLE_ENTRY(demodtab) =
{
     .name                         = "DEMOD_MSKAISERIN_DVBC",
     .data                         = DEMOD_MSKAISERIN_DVBC,
     .init                         = MDrv_Demod_DVBC_Init,
     .GetLock                      = MDrv_Demod_DVBC_GetLock,
     .GetSNR                       = MDrv_Demod_DVBC_GetSNR,
     .GetBER                       = MDrv_Demod_DVBC_GetBER,
     .GetPWR                       = MDrv_Demod_DVBC_GetPWR,
     .GetQuality                   = MDrv_Demod_DVBC_GetSignalQuality,
     .GetParam                     = MDrv_Demod_DVBC_GetParam,
     .Restart                      = MDrv_Demod_DVBC_Restart,
     .I2CByPass                    = MDrv_Demod_DVBC_I2C_ByPass,
     .I2CByPassPreSetting          = NULL,
     .Extension_Function           = DEMOD_MSKAISERIN_DVBC_Extension_Function,
...
#endif // (FRONTEND_DEMOD_TYPE == DEMOD_MSKAISERIN_DVBC)
