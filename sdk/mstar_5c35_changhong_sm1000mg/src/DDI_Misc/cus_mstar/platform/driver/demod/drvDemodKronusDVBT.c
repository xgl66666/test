...
DRV_DEMOD_TABLE_TYPE GET_DEMOD_ENTRY_NODE(DEMOD_MSKRONUS_DVBT) DDI_DRV_TABLE_ENTRY(demodtab);
...
MS_BOOL MDrv_Demod_MSKRONUS_Init(MS_U8 u8DemodIndex,DEMOD_MS_INIT_PARAM* pParam)
{
    ...
    static MS_U8 u8DMD_DVBT_InitExt[]={ 4, // version
                                        ...
                                        FRONTEND_DEMOD_IQ_TYPE, // u8ADCIQMode : 0=I path, 1=Q path, 2=both IQ //DEMOD_IQ_I_IN
...
MS_BOOL MDrv_Demod_MSKRONUS_Open(MS_U8 u8DemodIndex)
{
...
MS_BOOL MDrv_Demod_MSKRONUS_Close(MS_U8 u8DemodIndex)
{
    MS_BOOL ret;
...
MS_BOOL MDrv_Demod_MSKRONUS_PowerOnOff(MS_U8 u8DemodIndex, MS_BOOL bPowerOn)
{
    if (HB_ObtainMutex(_s32MutexId, COFDMDMD_MUTEX_TIMEOUT) == FALSE)
    {
        DMD_ERR( ("%s: Obtain mutex failed.\n", __FUNCTION__));
...
MS_BOOL MDrv_Demod_MSKRONUS_GetLock(MS_U8 u8DemodIndex, EN_LOCK_STATUS *peLockStatus)
{
...
MS_BOOL MDrv_Demod_MSKRONUS_GetSNR(MS_U8 u8DemodIndex, float *pfSNR)
{
    MS_BOOL ret;
...
MS_BOOL MDrv_Demod_MSKRONUS_GetBER(MS_U8 u8DemodIndex, float *pfBER)
{
    MS_BOOL ret;
...
MS_BOOL MDrv_Demod_MSKRONUS_GetPWR(MS_U8 u8DemodIndex, MS_S32 *ps32Signal)
{
    MS_BOOL ret;
...
MS_BOOL MDrv_Demod_MSKRONUS_GetSignalQuality(MS_U8 u8DemodIndex, MS_U16* u16quality)
{
    MS_BOOL ret;
    ...
    ret = MDrv_DMD_DVBT_GetSignalQuality(u16quality);
    HB_ReleaseMutex(_s32MutexId);
    return ret;
}
...
MS_BOOL MDrv_Demod_MSKRONUS_GetParam(MS_U8 u8DemodIndex, DEMOD_MS_FE_CARRIER_PARAM* pParam)
{
    if (HB_ObtainMutex(_s32MutexId, COFDMDMD_MUTEX_TIMEOUT) == FALSE)
    {
        DMD_ERR( ("%s: Obtain mutex failed.\n", __FUNCTION__));
...
MS_BOOL MDrv_Demod_MSKRONUS_Restart(MS_U8 u8DemodIndex, DEMOD_MS_FE_CARRIER_PARAM* pParam,MS_U32 u32BroadCastType)
{
...
MS_BOOL MDrv_Demod_SetOutoutPath(MS_U8 u8DemodIndex, DEMOD_INTERFACE_MODE path)
{
    //Dummy function
    if (HB_ObtainMutex(_s32MutexId, COFDMDMD_MUTEX_TIMEOUT) == FALSE)
    {
...
DEMOD_INTERFACE_MODE MDrv_Demod_GetOutoutPath(MS_U8 u8DemodIndex)
{
    //Dummy function
...
MS_BOOL DEMOD_MSKRONUS_DVBT_Extension_Function(MS_U8 u8DemodIndex, DEMOD_EXT_FUNCTION_TYPE fuction_type, void *data)
{
    switch(fuction_type)
    { 
        default:
...
MS_BOOL MDrv_Demod_MSKRONUS_I2C_ByPass(MS_U8 u8DemodIndex,MS_BOOL bOn)
{
  if(GET_DEMOD_ENTRY_NODE(DEMOD_MSKRONUS_DVBT).I2CByPassPreSetting != NULL)
  {
      return GET_DEMOD_ENTRY_NODE(DEMOD_MSKRONUS_DVBT).I2CByPassPreSetting(u8DemodIndex,bOn);
  }
  else
...
DRV_DEMOD_TABLE_TYPE GET_DEMOD_ENTRY_NODE(DEMOD_MSKRONUS_DVBT) DDI_DRV_TABLE_ENTRY(demodtab) = 
{  
     .name                         = "DEMOD_MSKRONUS_DVBT",
     .data                         = DEMOD_MSKRONUS_DVBT,        
     .init                         = MDrv_Demod_MSKRONUS_Init,
     .GetLock                      = MDrv_Demod_MSKRONUS_GetLock,
     .GetSNR                       = MDrv_Demod_MSKRONUS_GetSNR,
     .GetBER                       = MDrv_Demod_MSKRONUS_GetBER,
     .GetPWR                       = MDrv_Demod_MSKRONUS_GetPWR,
     .GetQuality                   = MDrv_Demod_MSKRONUS_GetSignalQuality,
     .GetParam                     = MDrv_Demod_MSKRONUS_GetParam,
     .Restart                      = MDrv_Demod_MSKRONUS_Restart,
     .I2CByPass                    = MDrv_Demod_MSKRONUS_I2C_ByPass,
     .I2CByPassPreSetting          = NULL,
     .Extension_Function           = DEMOD_MSKRONUS_DVBT_Extension_Function,
...
#endif // (FRONTEND_DEMOD_TYPE == DEMOD_MSKRONUS_DVBT)
