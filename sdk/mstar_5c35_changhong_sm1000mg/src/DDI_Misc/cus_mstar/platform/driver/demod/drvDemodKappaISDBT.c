...
MAPI_BOOL INTERN_ISDBT_Power_On_Initialization(MS_U8 u8DemodIndex,DEMOD_MS_INIT_PARAM* pParam)
{
...
MAPI_BOOL INTERN_ISDBT_Lock(MS_U8 u8DemodIndex,EN_LOCK_STATUS *peLockStatus)
{
...
MAPI_BOOL INTERN_ISDBT_Config(RF_CHANNEL_BANDWIDTH BW, MAPI_BOOL bSerialTS, MAPI_BOOL bPalBG, MAPI_BOOL bLPSel)
{
    BW = BW;
    bSerialTS = bSerialTS;
    bPalBG = bPalBG;
...
MAPI_BOOL INTERN_ISDBT_Active(MAPI_BOOL bEnable)
{
    return MDrv_DMD_ISDBT_SetConfig(bEnable);
}

MAPI_BOOL INTERN_ISDBT_Serial_Control(MAPI_BOOL bEnable)
{
...
MAPI_BOOL INTERN_ISDBT_GetSNR(MS_U8 u8DemodIndex,float *pfSNR)
{
    float  f_snr;

    if (MDrv_DMD_ISDBT_GetSNR(&f_snr)== FALSE){
        printf("MDrv_DMD_ISDBT_GetSNR fail \n", f_snr);
        return FALSE;
    }
    //printf("SNR value = %f\n", f_snr);
    *pfSNR = f_snr;
...
MAPI_BOOL INTERN_ISDBT_WriteReg(MS_U16 RegAddr, MS_U8 u8Data)
{
    return MDrv_DMD_ISDBT_SetReg(RegAddr, u8Data);
}
MAPI_BOOL INTERN_ISDBT_ReadReg(MS_U16 RegAddr, MS_U8* pu8Data)
{
    return MDrv_DMD_ISDBT_GetReg(RegAddr, pu8Data);
}

MAPI_BOOL INTERN_ISDBT_Exit ( void )
{
...
MAPI_BOOL INTERN_ISDBT_Power_Save(void)
{
...
MS_BOOL MDrv_Kappa_ISDBT_Demod_GetSNR(MS_U8 u8DemodIndex, float* pfSNR)
{
    MS_BOOL bRtn = FALSE;
...
MS_BOOL MDrv_Kappa_ISDBT_Demod_GetBER(MS_U8 u8DemodIndex, float* pfBER)
{
    MS_BOOL bRtn = FALSE;
    bRtn = MDrv_DMD_ISDBT_GetPostViterbiBer(E_ISDBT_Layer_A, pfBER);
    return bRtn;
}

MS_BOOL MDrv_Kappa_ISDBT_Demod_GetPWR(MS_U8 u8DemodIndex,MS_S32 *ps32Signal)
{
    MS_BOOL bRtn = FALSE;
...
MS_BOOL MDrv_Kappa_ISDBT_Demod_GetSignalQuality(MS_U8 u8DemodIndex, MS_U16* u16Quality)
{  
    return MDrv_DMD_ISDBT_GetSignalQuality();
}

MS_BOOL MDrv_Kappa_ISDBT_Demod_Restart(MS_U8 u8DemodIndex, DEMOD_MS_FE_CARRIER_PARAM* pParam,MS_U32 u32BroadCastType)
{
    MS_BOOL bRtn = FALSE;
    bRtn = INTERN_ISDBT_Active(TRUE);
    return bRtn;
}

MS_BOOL DEMOD_MSKAPPA_ISDBT_Extension_Function(MS_U8 u8DemodIndex, DEMOD_EXT_FUNCTION_TYPE fuction_type, void *data)
{
    switch(fuction_type)
    {
        default:
...
MS_BOOL MDrv_Kappa_ISDBT_Demod_I2C_ByPass(MS_U8 u8DemodIndex,MS_BOOL bOn)
{
  if(GET_DEMOD_ENTRY_NODE(DEMOD_MSKAPPA_ISDBT).I2CByPassPreSetting != NULL)
  {
      return GET_DEMOD_ENTRY_NODE(DEMOD_MSKAPPA_ISDBT).I2CByPassPreSetting(u8DemodIndex,bOn);
  }
  else
...
DRV_DEMOD_TABLE_TYPE GET_DEMOD_ENTRY_NODE(DEMOD_MSKAPPA_ISDBT) DDI_DRV_TABLE_ENTRY(demodtab) = 
{  
     .name                         = "DEMOD_MSKAPPA_ISDBT",
     .data                         = DEMOD_MSKAPPA_ISDBT,        
     .init                         = INTERN_ISDBT_Power_On_Initialization,
     .GetLock                      = INTERN_ISDBT_Lock,
     .GetSNR                       = MDrv_Kappa_ISDBT_Demod_GetSNR,
     .GetBER                       = MDrv_Kappa_ISDBT_Demod_GetBER,
     .GetPWR                       = MDrv_Kappa_ISDBT_Demod_GetPWR,
     .GetQuality                   = MDrv_Kappa_ISDBT_Demod_GetSignalQuality,
     .GetParam                     = MDrv_Demod_null_GetParam,
     .Restart                      = MDrv_Kappa_ISDBT_Demod_Restart,
     .I2CByPass                    = MDrv_Kappa_ISDBT_Demod_I2C_ByPass,
     .I2CByPassPreSetting          = NULL,
     .Extension_Function           = DEMOD_MSKAPPA_ISDBT_Extension_Function,
