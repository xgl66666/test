...
AVL_DVBSx_ErrorCode AVL_DVBSx_DiSEqC_SetLNBOut(AVL_uchar ucLow)
{
	...
	else
	{
	    ...
	    {
		     HB_printf("LNB output not change --- Fail !\n");
...
AVL_DVBSx_ErrorCode AVL_DVBSx_DiSEqC_SetCmd(AVL_puchar pucCmd,AVL_uchar ucCmdSize)
{
	...
	if(AVL_DVBSx_EC_OK != r)
	{
		HB_printf("Output  modulation bytes --- Fail!\n");
	}
	else
	{
	    do
...
AVL_DVBSx_ErrorCode AVL_DVBSx_SetMpegMode(AVL_uchar bParallelTS,AVL_uchar bLockChannel)
{
	...
	if(AVL_DVBSx_EC_OK != r)
	{
		HB_printf("Set MPEG output mode failed !\n");
              return (r);
	}
       if(bParallelTS == 0)
       {
...
AVL_DVBSx_ErrorCode AVL_DVBSx_Initialize(void)
{
	...
	if( AVL_DVBSx_EC_OK !=r )
	{
		HB_printf("Chip Object Initialization failed !\n");
		return (r);
	}
	...
	if( AVL_DVBSx_EC_OK !=r )
	{
		HB_printf("IBase Initialization failed !\n");
		return (r);
	}
	...
	if(AVL_DVBSx_EC_OK != r)
	{
		HB_printf("Demod Initialization failed !\n");
		return (r);
	}
...
#if (FRONTEND_TUNER_TYPE == TUNER_TDA20152)
	...
	if(AVL_DVBSx_EC_OK != r)
    {
        HB_printf("Set RF AGC Polar failed !\n");
        return (r);
    }
    ...
    if(AVL_DVBSx_EC_OK != r)
    {
        HB_printf("Driver RF AGC failed !\n");
        return (r);
    }
...
#if (FRONTEND_TUNER_TYPE == TUNER_SHARP_BS2S7HZ6306)
...
#endif
#if (FRONTEND_TUNER_TYPE == TUNER_SHARP_BS2S7VZ7801)
   pTuner->m_pInitializeFunc = &ExtSharpBS2S7VZ7801_Initialize;
	...
	if(AVL_DVBSx_EC_OK != r)
	{
		HB_printf("Tuner Initialization failed !\n");
		return (r);
	}
	...
	if(AVL_DVBSx_EC_OK != r)
	{
		HB_printf("Diseqc Initialization failed !\n");
	}
	return (r);
}
...
#if (FRONTEND_TUNER_TYPE == TUNER_AV2011)||(FRONTEND_TUNER_TYPE == TUNER_TDA20152)
...
AVL_DVBSx_ErrorCode AVL_DVBSx_SetFrequency(AVL_uint16 uiFreq_MHz,AVL_uint32 uiSymbolRate_Hz)
{
	  ...
	  if (AVL_DVBSx_EC_OK != r)
	  {
		HB_printf("Reset error status failed !\n");
		return (r);
       }
...
AVL_DVBSx_ErrorCode AVL_DVBSx_GetSinalLevel(AVL_int16 *piRFSignalLevel)
{
	...
	if (AVL_DVBSx_EC_OK != r)
	{
		HB_printf("Get SignalLevel --- Fail !\n");
        uiRFSignalDBM = 0;
    }
	else
	{
		for(i=0; i<47; i++)
		{
			if(uiRFSignalLevel<= SignalLevel[i].SignalLevel)
			{
				...
				{
					HB_printf("RFSignalLevel is too weak !\n");
				}
				else
				{
...
AVL_DVBSx_ErrorCode AVL_DVBSx_Blind_start(AVL_int16 u16StartFreq,AVL_int16 u16EndFreq)
{
	   ...
	   //r = AVL_DVBSx_II2C_Read16(pAVLChip, rc_tuner_max_LPF_100kHz_addr, &uiMaxLPF);     //Get the right LPF for blind scan to regulate the freq_step.
          ...
          if(AVL_DVBSx_EC_OK != r)
          {
                HB_printf("Set blindscan spectrum inversion failed !\n");
                return (r);
          }
#if 0
...
AVL_DVBSx_ErrorCode AVL_DVBSx_Blind_WaitCurFreqFinished(AVL_uchar* u8Progress,AVL_uchar* ucFindNum)
{
      ...
      *ucFindNum = 0;
      ...
      {
  	       HB_printf("Get scan status failed !\n");
...
#endif //#if (FRONTEND_DEMOD_TYPE == DEMOD_AVL6211)
