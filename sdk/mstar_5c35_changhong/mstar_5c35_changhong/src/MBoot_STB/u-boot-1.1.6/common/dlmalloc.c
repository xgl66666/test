...
void malloc_bin_initial(void)
{
	...
	for (i=0; i<NAV;i++)
	{
		*p++=(unsigned long)bin_at(i);
		*p++=(unsigned long)bin_at(i);//this duplication is correct, not typo
	}
...
#else
        if (value != 0) return 0;
        else  n_mmaps_max = value; return 1;
#endif
