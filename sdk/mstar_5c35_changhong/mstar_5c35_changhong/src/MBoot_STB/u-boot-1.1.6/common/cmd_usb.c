...
int do_usb (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
...
#if 0 //Colin, For performance test
	if (strcmp(argv[1],"performance") == 0) {
		...
		if (argc==5) {
			unsigned long addr = 0x80400000;
			unsigned long blksize  = simple_strtoul(argv[3], NULL, 10);
			unsigned long testcount  = simple_strtoul(argv[4], NULL, 10);
			unsigned long n, ioblks, ii;
			int testtype; // 1: read, 2: write
			unsigned int time_sta, time_end;

			if ( (strncmp(argv[2], "R", 1) == 0) ||
				(strncmp(argv[2], "r", 1) == 0) )
				testtype = 1;
			else if ( (strncmp(argv[2], "W", 1) == 0) ||
				(strncmp(argv[2], "w", 1) == 0) )
				testtype = 2;
			else
				testtype = 0;
			...
			ioblks = (blksize + stor_dev->blksz - 1) / stor_dev->blksz;

			printf ("\nUSB performance: %s %d bytes for %d loops\n",
					(testtype == 1) ? "read" : "write", blksize, testcount);

			time_sta = MsOS_GetSystemTime();

			for (ii=0; ii<testcount; ii++)
			{
				if (testtype == 1)
					n = stor_dev->block_read(usb_stor_curr_dev, 200+ii*ioblks, ioblks, (ulong *)addr);
				else
					n = stor_dev->block_write(usb_stor_curr_dev, 200+ii*ioblks, ioblks, (ulong *)addr);

				if (n < ioblks)
				{
					printf("read/write length is less than requested.\n");
					return 1;
				}
			}
			time_end = MsOS_GetSystemTime();

			//printf("Transfer %d bytes, %d sec\n", ioblks*testcount*512, time_end-time_sta);
			printf("Transfer rate: %d bytes/sec\n", (ioblks*testcount*stor_dev->blksz) / ((time_end-time_sta) / 1000) );
			return 0;
		}
	}
#endif //#if 0 //Colin, For performance test
...
U_BOOT_CMD(
	...
	"usb performance r/w blksize loopcount - do read/write performance test\n"
	"    transfer blksize bytes for loopcount loops\n"
);
