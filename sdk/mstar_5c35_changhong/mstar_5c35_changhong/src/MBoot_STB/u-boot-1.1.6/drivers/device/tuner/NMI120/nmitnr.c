...
int nmi_bus_read(uint8_t DeviceAddr, uint8_t * pArray, uint32_t count)
{
    return nmi_tuner_os_bus_read(DeviceAddr, pArray, count);
}


int nmi_bus_write(uint8_t DeviceAddr, uint8_t * pArray, uint32_t count)
{
    return nmi_tuner_os_bus_write(DeviceAddr, pArray, count);
}

uint32_t nmi_bus_get_read_blksz(void)
{
    //return UINT_MAX;
    return 0;
}

uint32_t nmi_bus_get_write_blksz(void)
{
    return 14;
}
