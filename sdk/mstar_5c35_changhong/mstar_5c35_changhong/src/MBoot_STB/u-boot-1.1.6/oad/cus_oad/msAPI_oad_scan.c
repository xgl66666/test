...
extern unsigned int gCountryType; 

extern unsigned int gCustomerOUI;
        ...
        if (streamType == 0x0B)
        {
            ...
            for (i = curPos; i < esInfoLen+curPos; )
            {
                ...
                if (esDescTag == 0x66 && descLen == 18)
                {
                    ...
                    {
                        if(pu8Section[i+3] == (MS_U8)(gCustomerOUI>>16)
                           &&pu8Section[i+4] == (MS_U8)(gCustomerOUI>>8)
                           &&pu8Section[i+5] == (MS_U8)gCustomerOUI && !memcmp(&pu8Section[i+9], "MSTAR OAD", 9))
                        {
                            *pOADPid = esPid;
...
int MApp_BL_OADGetRFChanInfo(CHANINFO_T *pRFChan)
{
    ...
    minChNum = MApi_RfChannel_GetMinRfChNum(gCountryType);
    maxChNum = MApi_RfChannel_GetMaxRfChNum(gCountryType);
    pRFChan->m_countryType = gCountryType;
