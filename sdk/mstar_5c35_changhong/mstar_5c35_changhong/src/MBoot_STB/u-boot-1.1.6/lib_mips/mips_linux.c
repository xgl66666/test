...
#include <asm/addrspace.h>
#include <ms_ver.h>/*MBoot version information*/
...
/*Modified by Brian.Lin*/
/*
* Change linux_env_set() that environment sending by u-boot 
* to fit the structure of linux kernel environment format
*
*#FORMAT#
*(char **)env_p
*(char *)env_name
...
static void linux_env_set (char *env_name, char *env_val)
{
	if (linux_env_idx < LINUX_MAX_ENVS - 1) {
		...
		linux_env[linux_env_idx] = linux_env_p;//insert environment name
		strcpy (linux_env_p, env_name);
		linux_env_p += strlen (env_name)+1;

		linux_env[++linux_env_idx] = linux_env_p;//insert environment value
		strcpy (linux_env_p, env_val);
		linux_env_p += strlen (env_val)+1;
