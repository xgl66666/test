...
MXL_STATUS MxLWare603_API_CfgDevSoftReset(UINT8 u8TunerIndex,UINT8 devId)
{
  ...
  status = MxLWare603_OEM_WriteRegister(u8TunerIndex, devId, AIC_RESET_REG, 0x00);
...
MXL_STATUS MxLWare603_API_CfgDevOverwriteDefaults(UINT8 u8TunerIndex,
                                                  UINT8 devId,
                                                  MXL_BOOL singleSupply_3_3V)
{
  ...
  status |= MxL603_Ctrl_ProgramRegisters(u8TunerIndex,devId, MxL603_OverwriteDefaults);

  status |= MxLWare603_OEM_WriteRegister(u8TunerIndex,devId, 0x00, 0x01);
  status |= MxLWare603_OEM_ReadRegister(u8TunerIndex,devId, 0x31, &readData);
  readData &= 0x2F;
  readData |= 0xD0;
  status |= MxLWare603_OEM_WriteRegister(u8TunerIndex,devId, 0x31, readData);
  ...
  if (MXL_ENABLE == singleSupply_3_3V)
    status |= MxLWare603_OEM_WriteRegister(u8TunerIndex,devId, MAIN_REG_AMP, 0x04);
...
MXL_STATUS MxLWare603_API_CfgDevXtal(UINT8 u8TunerIndex,UINT8 devId, MXL603_XTAL_SET_CFG_T xtalCfg)
{
  ...
  if ((xtalCfg.xtalFreqSel == MXL603_XTAL_16MHz) || (xtalCfg.xtalFreqSel == MXL603_XTAL_24MHz))
  {
    ...
    status = MxLWare603_OEM_WriteRegister(u8TunerIndex,devId, XTAL_CAP_CTRL_REG, control);
    ...
    if (MXL_ENABLE == xtalCfg.singleSupply_3_3V)
      status |= MxLWare603_OEM_WriteRegister(u8TunerIndex,devId, MAIN_REG_AMP, 0x14);
  }
  else
...
MXL_STATUS MxLWare603_API_ReqDevVersionInfo(UINT8 u8TunerIndex, UINT8 devId,
                                            MXL603_VER_INFO_T* mxlDevVerInfoPtr)
{
  ...
  if (mxlDevVerInfoPtr)
  {
    status |= MxLWare603_OEM_ReadRegister(u8TunerIndex, devId, CHIP_ID_REQ_REG, &readBack);
    mxlDevVerInfoPtr->chipId = (readBack & 0xFF);

    status |= MxLWare603_OEM_ReadRegister(u8TunerIndex, devId, CHIP_VERSION_REQ_REG, &readBack);
...
MXL_STATUS MxLWare603_API_ReqDevGPOStatus(UINT8 u8TunerIndex, UINT8 devId,
...
MXL_STATUS MxLWare603_API_CfgTunerMode(UINT8 u8TunerIndex, UINT8 devId,
                                       MXL603_TUNER_MODE_CFG_T tunerModeCfg)
{
  ...
  switch(tunerModeCfg.signalMode)
  {
    ...
    case MXL603_DIG_J83B:
      ...
      {
        // Low power
        status |= MxLWare603_OEM_WriteRegister(u8TunerIndex,devId, DIG_ANA_IF_CFG_0, 0xFE);
        status |= MxLWare603_OEM_WriteRegister(u8TunerIndex,devId, DIG_ANA_IF_CFG_1, 0x10);
      }
      else
      {
        // High power
        status |= MxLWare603_OEM_WriteRegister(u8TunerIndex,devId, DIG_ANA_IF_CFG_0, 0xD9);
        status |= MxLWare603_OEM_WriteRegister(u8TunerIndex,devId, DIG_ANA_IF_CFG_1, 0x16);
      }
    ...
    case MXL603_DIG_ISDBT_ATSC:
      ...
      {
        // Low power
        status |= MxLWare603_OEM_WriteRegister(u8TunerIndex,devId, DIG_ANA_IF_CFG_0, 0xF9);
        status |= MxLWare603_OEM_WriteRegister(u8TunerIndex,devId, DIG_ANA_IF_CFG_1, 0x18);
        status |= MxLWare603_OEM_WriteRegister(u8TunerIndex,devId, DIG_ANA_IF_PWR, 0xF1);
      }
      else
      {
        // High power
        status |= MxLWare603_OEM_WriteRegister(u8TunerIndex,devId, DIG_ANA_IF_CFG_0, 0xD9);
        status |= MxLWare603_OEM_WriteRegister(u8TunerIndex,devId, DIG_ANA_IF_CFG_1, 0x16);
        status |= MxLWare603_OEM_WriteRegister(u8TunerIndex,devId, DIG_ANA_IF_PWR, 0xB1);
      }
      ...
      status |= MxLWare603_OEM_WriteRegister(u8TunerIndex,devId, DFE_DACIF_GAIN, dfeRegData);
  ...
  {
    // XTAL calibration
    status |= MxLWare603_OEM_WriteRegister(u8TunerIndex,devId, XTAL_CALI_SET_REG, 0x00);
    status |= MxLWare603_OEM_WriteRegister(u8TunerIndex,devId, XTAL_CALI_SET_REG, 0x01);
...
MXL_STATUS MxLWare603_API_CfgTunerAGC(UINT8 u8TunerIndex,UINT8 devId, MXL603_AGC_CFG_T agcCfg)
{
      ...
      (agcCfg.agcType <= MXL603_AGC_EXTERNAL))
  {
    ...
    status |= MxLWare603_OEM_ReadRegister(u8TunerIndex,devId, AGC_CONFIG_REG, &regData);
    regData &= 0xF2; // Clear bits <3:2> & <0>
    regData = (UINT8) (regData | (agcCfg.agcType << 2) | 0x01);
    status |= MxLWare603_OEM_WriteRegister(u8TunerIndex,devId, AGC_CONFIG_REG, regData);

    // AGC set point <6:0>
    status |= MxLWare603_OEM_ReadRegister(u8TunerIndex,devId, AGC_SET_POINT_REG, &regData);
    regData &= 0x80; // Clear bit <6:0>
    regData |= agcCfg.setPoint;
    status |= MxLWare603_OEM_WriteRegister(u8TunerIndex,devId, AGC_SET_POINT_REG, regData);

    // AGC Polarity <4>
    status |= MxLWare603_OEM_ReadRegister(u8TunerIndex,devId, AGC_FLIP_REG, &regData);
    regData &= 0xEF; // Clear bit <4>
    regData |= (agcCfg.agcPolarityInverstion << 4);
    status |= MxLWare603_OEM_WriteRegister(u8TunerIndex,devId, AGC_FLIP_REG, regData);
  }
  else
    status = MXL_INVALID_PARAMETER;
                                           ...
                                           MXL603_CHAN_TUNE_CFG_T chanTuneCfg)
{
  ...
  // Abort Tune
  status |= MxLWare603_OEM_WriteRegister(u8TunerIndex,devId, START_TUNE_REG, 0x00);
  ...
  if (chanTuneCfg.startTune == MXL_ENABLE)
  {
    if (chanTuneCfg.signalMode <= MXL603_DIG_J83B)
    {
      ...
      {
        status |= MxLWare603_OEM_WriteRegister(u8TunerIndex,devId, 0x7C, 0x9F);
        if ((chanTuneCfg.signalMode == MXL603_DIG_DVB_C) || (chanTuneCfg.signalMode == MXL603_DIG_J83B))
          regData = 0xD1;
        else
          regData = 0x91;
      ...
      switch(chanTuneCfg.bandWidth)
      {
        ...
        case MXL603_TERR_BW_8MHz:
            ...
            status |= MxLWare603_OEM_WriteRegister(u8TunerIndex,devId, CHAN_TUNE_HI_REG, (UINT8)((freq >> 8 ) & 0xFF));
            break;
      ...
      status |= MxLWare603_OEM_WriteRegister(u8TunerIndex,devId, START_TUNE_REG, 0x01);

      // Sleep 15 ms
      MxLWare603_OEM_Sleep(15);

      // Wait another 1000 ms
      //MxLWare603_OEM_Sleep(1000);
...
MXL_STATUS MxLWare603_API_CfgTunerIFOutParam(UINT8 u8TunerIndex,UINT8 devId, MXL603_IF_OUT_CFG_T ifOutCfg)
{
  ...
  status = MxLWare603_OEM_ReadRegister(u8TunerIndex,devId, IF_FREQ_SEL_REG, &readData);
  ...
  if (ifOutCfg.manualFreqSet == MXL_ENABLE)
  {
    ...
    readData |= 0x20;
    status = MxLWare603_OEM_WriteRegister(u8TunerIndex,devId, IF_FREQ_SEL_REG, readData);

    // Manual IF freq set
    ifFcw = (UINT16)(ifOutCfg.manualIFOutFreqInKHz * 8192 / 216000);
    control = (ifFcw & 0xFF); // Get low 8 bit
    status |= MxLWare603_OEM_WriteRegister(u8TunerIndex,devId, IF_FCW_LOW_REG, control);

    control = ((ifFcw >> 8) & 0x0F); // Get high 4 bit
    status |= MxLWare603_OEM_WriteRegister(u8TunerIndex,devId, IF_FCW_HIGH_REG, control);
  }
  ...
  if (status == MXL_SUCCESS)
  {
    ...
    if (ifOutCfg.ifInversion <= MXL_ENABLE)
    {
      ...
      status |= MxLWare603_OEM_WriteRegister(u8TunerIndex,devId, IF_PATH_GAIN_REG, control);
    }
    else
      status |= MXL_INVALID_PARAMETER;
  }
...
MXL_STATUS MxLWare603_API_ReqTunerAGCLock(UINT8 u8TunerIndex,UINT8 devId, MXL_BOOL* agcLockStatusPtr)
{
  ...
  if (agcLockStatusPtr)
  {
    status = MxLWare603_OEM_ReadRegister(u8TunerIndex,devId, AGC_SAGCLOCK_STATUS_REG, &regData);
...
MXL_STATUS MxLWare603_API_ReqTunerLockStatus(UINT8 u8TunerIndex,UINT8 devId, MXL_BOOL* rfLockPtr,
                                                          MXL_BOOL* refLockPtr)
{
  MXL_STATUS status = MXL_SUCCESS;
  UINT8 regData = 0;
  ...
  if ((rfLockPtr) && (refLockPtr))
  {
    ...
    TUNER_DBG((" RfSynthStatus = %d, RefSynthStatus = %d \n", (UINT8)rfLockStatus,(UINT8)refLockStatus));
...
MXL_STATUS MxLWare603_API_ReqTunerRxPower(UINT8 u8TunerIndex,UINT8 devId, REAL32* rxPwrPtr)
{
  ...
  if (rxPwrPtr)
  {
    ...
    status = MxLWare603_OEM_ReadRegister(u8TunerIndex,devId, RFPIN_RB_LOW_REG, &regData);
    tmpData = regData;

    // RF input power high <1:0>
    status |= MxLWare603_OEM_ReadRegister(u8TunerIndex,devId, RFPIN_RB_HIGH_REG, &regData);
