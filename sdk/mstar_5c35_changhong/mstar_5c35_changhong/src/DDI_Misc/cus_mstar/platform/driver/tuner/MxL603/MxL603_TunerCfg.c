...
MXL_STATUS MxL603_Ctrl_ProgramRegisters(UINT8 u8TunerIndex,UINT8 devId, PMXL603_REG_CTRL_INFO_T ctrlRegInfoPtr)
{
  ...
  while (status == MXL_TRUE)
  {
    ...
    {
      status = MxLWare603_OEM_ReadRegister(u8TunerIndex,devId,ctrlRegInfoPtr[i].regAddr, &tmp);
    ...
    status = MxLWare603_OEM_WriteRegister(u8TunerIndex,devId,ctrlRegInfoPtr[i].regAddr, tmp);
    if (status != MXL_TRUE) break;
...
MXL_STATUS MxL603_Ctrl_WriteRegField(UINT8 u8TunerIndex,UINT8 devId, PMXL603_REG_CTRL_INFO_T ctrlRegInfoPtr)
{
  ...
  {
      status = MxLWare603_OEM_ReadRegister(u8TunerIndex,devId, ctrlRegInfoPtr->regAddr, &tmp);
  ...
  status = MxLWare603_OEM_WriteRegister(u8TunerIndex,devId, ctrlRegInfoPtr->regAddr, tmp);
