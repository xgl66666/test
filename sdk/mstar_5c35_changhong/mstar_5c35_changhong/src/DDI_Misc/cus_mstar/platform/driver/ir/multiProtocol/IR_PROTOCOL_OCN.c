...
IR_DECODE_STATUS _ParseOCN(U32 *pu32CustCode, U16 *pu16KeyCode, U8 *pu8State, U8 *pu8Reserved)
{
    ...
    if (MsOS_GetSystemTime() - _ulOcnPreDecodeTime> (IR_TIME_OCN_TIMEOUT/1000))//>108ms timeout
    {
        _u64OcnLastDecodeData=0;
    }

    _ulOcnPreDecodeTime = MsOS_GetSystemTime();
    ...
    else
    {
        ...
        while(_MulProtCommon_PeekShot(u8StartIndex+IR_OCN_LEADCODE_SHOT_COUNT, 1, &u32ShotValue, &bNegShot)==TRUE)//only use peek here
        {
            ...
            else//positive
            {
                ...
                {
                    if(u8OcnBits<IR_OCN_CUSTOM_BITS)
                    {
                    ...
                    else if(u8OcnBits>=IR_OCN_CUSTOM_BITS&&u8OcnBits<IR_OCN_BITS)
                    {
                        if(b2ndLendCodeExist==FALSE)
                        {
        ...
        if(u8OcnBits==IR_OCN_BITS && bLastShotN==TRUE)
        {
            _OcnRAW2Format(u64OcnData, pu32CustCode, pu16KeyCode, pu8State, pu8Reserved);
            _u64OcnLastDecodeData=u64OcnData;
            return E_IR_DECODE_DATA_OK;
        }
