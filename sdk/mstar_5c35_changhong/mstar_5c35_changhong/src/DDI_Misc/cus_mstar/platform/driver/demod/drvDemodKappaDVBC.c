...
static DEMOD_MS_INIT_PARAM Kappa_DVBC_INIT_PARAM;
static MS_U32 _u32IFrequency = 5000;
DRV_DEMOD_TABLE_TYPE GET_DEMOD_ENTRY_NODE(DEMOD_MSKAPPA_DVBC) DDI_DRV_TABLE_ENTRY(demodtab);

//#if ((FRONTEND_TUNER_TYPE == TUNER_PHILIPS_TDA18250HN || FRONTEND_TUNER_TYPE == TUNER_MXL603 || FRONTEND_TUNER_TYPE == TUNER_TDA18250B|| FRONTEND_TUNER_TYPE == TUNER_TDA18250A|| FRONTEND_TUNER_TYPE == TUNER_TDA18260 || FRONTEND_TUNER_TYPE == TUNER_MS221))
...
MS_BOOL MDrv_Kappa_Demod_Init(MS_U8 u8DemodIndex,DEMOD_MS_INIT_PARAM* pParam)
{
    ...
    Kappa_DVBC_INIT_PARAM.pstTunertab = pParam->pstTunertab;
...
MS_BOOL MDrv_Kappa_Demod_Open(MS_U8 u8DemodIndex)
{
...
MS_BOOL MDrv_Kappa_Demod_Close(MS_U8 u8DemodIndex)
{
    MS_BOOL ret;
...
MS_BOOL MDrv_Kappa_Demod_PowerOnOff(MS_U8 u8DemodIndex,MS_BOOL bPowerOn)
{
    if (HB_ObtainMutex(_s32MutexId, COFDMDMD_MUTEX_TIMEOUT) == FALSE)
    {
        DMD_ERR(("%s: Obtain mutex failed.\n", __FUNCTION__));
...
MS_BOOL MDrv_Kappa_Demod_GetLock(MS_U8 u8DemodIndex,EN_LOCK_STATUS *peLockStatus)
{
...
MS_BOOL MDrv_Kappa_Demod_GetSNR(MS_U8 u8DemodIndex,float *pfSNR)
{
    MS_BOOL ret;
...
MS_BOOL MDrv_Kappa_Demod_GetBER(MS_U8 u8DemodIndex,float *pfBER)
{
    MS_BOOL ret;
...
MS_BOOL MDrv_Kappa_Demod_GetPWR(MS_U8 u8DemodIndex,MS_S32 *ps32Signal)
{
    MS_BOOL ret;
    if (HB_ObtainMutex(_s32MutexId, COFDMDMD_MUTEX_TIMEOUT) == FALSE)
    {
...
MS_BOOL MDrv_Kappa_Demod_GetSignalQuality(MS_U8 u8DemodIndex,MS_U16 *pu16quality)
{
    MS_BOOL ret;
    if (HB_ObtainMutex(_s32MutexId, COFDMDMD_MUTEX_TIMEOUT) == FALSE)
    {
...
MS_BOOL MDrv_Demod_Config(MS_U8 u8DemodIndex,MS_U8 *pRegParam)
{
    if (HB_ObtainMutex(_s32MutexId, COFDMDMD_MUTEX_TIMEOUT) == FALSE)
    {
        DMD_ERR(("%s: Obtain mutex failed.\n", __FUNCTION__));
...
MS_BOOL MDrv_Kappa_Demod_GetParam(MS_U8 u8DemodIndex, DEMOD_MS_FE_CARRIER_PARAM* pParam)
{
...
MS_BOOL MDrv_Kappa_Demod_Restart(MS_U8 u8DemodIndex, DEMOD_MS_FE_CARRIER_PARAM* pParam,MS_U32 u32BroadCastType)
{
    ...
    if(!Kappa_DVBC_INIT_PARAM.pstTunertab->GetTunerIF(u8DemodIndex, &u32IF_Freq))
    {
       u32IF_Freq = _u32IFrequency;
       DMD_DBG(("Kappa DVBC demod Get Tuner IF FAIL, use default IF %d kHz\n", (int)u32IF_Freq));
    }
    else
    {
       DMD_DBG(("Kappa DVBC demod Get Tuner IF %d kHz\n", (int)u32IF_Freq));
    }
...
MS_BOOL MDrv_Demod_SetMode(MS_U8 u8DemodIndex, Demod_Mode* pMode)
{
#if 0
    DMD_Mode dmdMod;
...
MS_BOOL MDrv_Kappa_DVBT_Demod_SetOutoutPath(MS_U8 u8DemodIndex, DEMOD_INTERFACE_MODE path)
{
    if (HB_ObtainMutex(_s32MutexId, COFDMDMD_MUTEX_TIMEOUT) == FALSE)
    {
        DMD_ERR(("%s: Obtain mutex failed.\n", __FUNCTION__));
...
DEMOD_INTERFACE_MODE MDrv_Kappa_DVBT_Demod_GetOutoutPath(MS_U8 u8DemodIndex)
{
    if (HB_ObtainMutex(_s32MutexId, COFDMDMD_MUTEX_TIMEOUT) == FALSE)
    {
        DMD_ERR(("%s: Obtain mutex failed.\n", __FUNCTION__));
...
MS_BOOL DEMOD_MSKAPPA_DVBC_Extension_Function(MS_U8 u8DemodIndex, DEMOD_EXT_FUNCTION_TYPE fuction_type, void *data)
{
    switch(fuction_type)
    {
        default:
...
MS_BOOL MDrv_Kappa_Demod_I2C_ByPass(MS_U8 u8DemodIndex,MS_BOOL bOn)
{
  if(GET_DEMOD_ENTRY_NODE(DEMOD_MSKAPPA_DVBC).I2CByPassPreSetting != NULL)
  {
      return GET_DEMOD_ENTRY_NODE(DEMOD_MSKAPPA_DVBC).I2CByPassPreSetting(u8DemodIndex,bOn);
  }
  else
...
DRV_DEMOD_TABLE_TYPE GET_DEMOD_ENTRY_NODE(DEMOD_MSKAPPA_DVBC) DDI_DRV_TABLE_ENTRY(demodtab) =
{
     .name                         = "DEMOD_MSKAPPA_DVBC",
     .data                         = DEMOD_MSKAPPA_DVBC,
     .init                         = MDrv_Kappa_Demod_Init,
     .GetLock                      = MDrv_Kappa_Demod_GetLock,
     .GetSNR                       = MDrv_Kappa_Demod_GetSNR,
     .GetBER                       = MDrv_Kappa_Demod_GetBER,
     .GetPWR                       = MDrv_Kappa_Demod_GetPWR,
     .GetQuality                   = MDrv_Kappa_Demod_GetSignalQuality,
     .GetParam                     = MDrv_Kappa_Demod_GetParam,
     .Restart                      = MDrv_Kappa_Demod_Restart,
     .I2CByPass                    = MDrv_Kappa_Demod_I2C_ByPass,
     .I2CByPassPreSetting          = NULL,
     .Extension_Function           = DEMOD_MSKAPPA_DVBC_Extension_Function,
