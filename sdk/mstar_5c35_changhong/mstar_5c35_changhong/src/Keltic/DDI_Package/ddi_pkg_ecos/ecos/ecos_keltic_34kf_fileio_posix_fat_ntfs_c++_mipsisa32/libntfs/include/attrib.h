...
extern s64 ntfs_attr_pwrite(ntfs_attr *na, const s64 pos, s64 count,
		const void *b, bool not_use_blib_cache);
...
#if PREFETCH_NTFS_FREE_CLUSTER_LIST
extern void ntfs_prefetch_freecluster_runlist(ntfs_volume *vol);
extern void ntfs_prefetch_reserve_free_clusters_runlist(ntfs_volume *vol, LCN pos, s64 length);
extern s64 ntfs_prefetch_search_first_free_cluster(ntfs_volume *vol, LCN search_start, s64 search_length);
extern int ntfs_prefetch_add_free_clusters_runlist(ntfs_volume *vol, LCN pos, s64 length);
extern void ntfs_prefetch_dump_free_cluster_runlist(ntfs_volume *vol);
#endif
#ifdef CYGCFG_FS_NTFS_QUICK_MOUNT
extern s64 ntfs_free_clusters_get_and_init(ntfs_volume *vol);
#endif
