...
#define IO_BLOCK_SIZE  (CYGCFG_FS_IO_BLOCK_NUM * 512)//256k
...
struct ntfs_device {
    ...
    ntfs_atime_t atime;                 // enable/diable acess time
    ntfs_hash_table_t ihash;            // for inode cache
...
extern struct ntfs_device *ntfs_device_alloc(const char *devname);
...
extern s64 ntfs_pwrite(struct ntfs_device *dev, const s64 pos, s64 count,
		const void *b);
extern s64 ntfs_pwrite_not_use_blib_cache(struct ntfs_device *dev, const s64 pos, s64 count,
		const void *b);
...
extern s64 ntfs_partition_size_get(struct ntfs_device *dev, int block_size);
