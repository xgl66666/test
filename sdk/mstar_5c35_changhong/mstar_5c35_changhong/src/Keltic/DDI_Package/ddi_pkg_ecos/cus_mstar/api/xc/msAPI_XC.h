...
//  Macro and Define
//-------------------------------------------------------------------------------------------------
#define MSAPI_XC_MUTEX
#define MSAPI_XC_MUTEX_DBG                  0
#define _MSAPI_XC_MUTEX_TIME_OUT            MSOS_WAIT_FOREVER

// Mutex function
#define _MSAPI_XC_ENTRY()     _MSAPI_XC_ENTRY_MUTEX(_MSAPI_XC_Mutex)
#define _MSAPI_XC_RETURN()    _MSAPI_XC_RETURN_MUTEX(_MSAPI_XC_Mutex)

#ifdef MSAPI_XC_MUTEX
    #if(MSAPI_XC_MUTEX_DBG)
        #define _MSAPI_XC_ENTRY_MUTEX(_mutex_)                                          \
                printf("1,==========================\n");                               \
                printf("[%s][%s][%06d]\n",__FILE__,__FUNCTION__,__LINE__);              \
                if(!MsOS_ObtainMutex(_mutex_, _MSAPI_XC_MUTEX_TIME_OUT))                \
                {                                                                       \
                    printf("==========================\n");                             \
                    printf("[%s][%s][%06d] Mutex taking timeout\n",__FILE__,__FUNCTION__,__LINE__);     \
                }
        #define _MSAPI_XC_RETURN_MUTEX(_mutex_)                                         \
                printf("0,==========================\n");                               \
                printf("[%s][%s][%06d] \n",__FILE__,__FUNCTION__,__LINE__);             \
        ...
        #define _MSAPI_XC_RETURN_MUTEX(_mutex_)    MsOS_ReleaseMutex(_mutex_);
    #endif
#else // #if not def MSAPI_XC_MUTEX
    #define _MSAPI_XC_ENTRY_MUTEX(_mutex_)     while(0)
    #define _MSAPI_XC_RETURN_MUTEX(_mutex_)    while(0)
#endif // #ifdef MSAPI_XC_MUTEX
...
typedef enum
{
    ...
    E_MSAPI_XC_INPUT_SOURCE_NUM,           ///<41 number of the source
...
typedef enum
{
    ...
    E_MSAPI_HDMITX_FORMAT_MAX_NUM = 64,
...
//-------------------------------------------------------------------------------------------------
E_MSAPI_XC_RESULT msAPI_XC_SetHDMIByPassOnOff(MS_BOOL bEnable);
