...
//  Local Defines
//-------------------------------------------------------------------------------------------------
#define SC_T0_SEND_TIMEOUT                  10
#define SC_T0_RECV_TIMEOUT                  40
#define SC_WAIT_DELAY                       20
#define SC_NULL_DELAY                       5
...
MS_BOOL _MApi_SC_T0_GetBytes(MS_U8 u8SCID, MS_U8 *pChar,MS_U16 *pu16Size,MS_U32 StartTime,MS_U32 TimeOut)
{
    ...
    while (1)
    {
        if (E_SC_OK == MDrv_SC_Recv(u8SCID, (pChar+nIndex),pu16Size, SC_T0_RECV_TIMEOUT))
        {
            nIndex+=(*pu16Size);
...
//[DEBUG]
    ...
    *pu16ReadDataLen = 0;
    ...
    eResult = MDrv_SC_Send(u8SCID, pu8SendData, 4, SC_T0_SEND_TIMEOUT);
    if (eResult != E_SC_OK)
    {
    ...
    eResult = MDrv_SC_Send(u8SCID,&u8Temp1, 1, SC_T0_SEND_TIMEOUT);
    if (eResult != E_SC_OK)
    {
        SC_API_DBG("%s, %d: Send LcLe Fail\n",__FUNCTION__,__LINE__);
        return E_API_SC_FAIL;
    }
    MsOS_DelayTask(SC_WAIT_DELAY);
    if (u16Lc)
    {
