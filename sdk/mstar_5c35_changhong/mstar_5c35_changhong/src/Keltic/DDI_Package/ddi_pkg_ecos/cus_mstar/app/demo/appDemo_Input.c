...
static void _appDemo_Input_IRCallback(MS_U8 u8Key, MS_U8 u8RepeatFlag)
{
    printf("%04d %s u8Key = 0x%02x, u8RepeatFlag = 0x%02x\n", __LINE__, __FUNCTION__, u8Key, u8RepeatFlag);
...
static void _appDemo_Input_Task(void)
{
    while(1)
    {
...
static void _appDemo_FreeInputResource(void)
{
    MS_BOOL bRet = FALSE;
    ...
    {
        bRet = MsOS_FreeMemory(u8InputTaskStack,gs32NonCachedPoolID);
        u8InputTaskStack = MsOS_AllocateMemory(APPINPUT_TASK_STACK_SIZE,gs32NonCachedPoolID);
    ...
    if (s32InpuTaskId > 0)
    {
        bRet = MsOS_DeleteTask(s32InpuTaskId);
        s32InpuTaskId = -1;
        printf("[%s][%d] bRet %d \n",__FUNCTION__,__LINE__,bRet);
    }
...
///
//------------------------------------------------------------------------------
MS_BOOL appDemo_Input_Init(void)
{
    ...
    MDrv_IR_HK_SetCallback(_appDemo_Input_IRCallback);
    ...
    if (s32InpuTaskId < 0)
    {
        s32InpuTaskId = MsOS_CreateTask( (TaskEntry) _appDemo_Input_Task,
                                      (MS_U32)NULL,
                                      E_TASK_PRI_HIGH,
                                      TRUE,
                                      u8InputTaskStack,
...
MS_BOOL appDemo_Input_Exit(void)
{
...
///
//------------------------------------------------------------------------------
MS_BOOL appDemo_Input_Help(void)
{
