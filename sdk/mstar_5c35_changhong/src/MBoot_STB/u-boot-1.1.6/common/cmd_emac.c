...
#if ((CONFIG_COMMANDS & CFG_CMD_NET) && (!defined(ENABLE_USB_LAN_MODULE)))
...
int do_dumpMac(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
    if (!emac_init)
    {
...
int do_bistMac(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
...
int do_loadspi (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
    ulong ram_start;
    ulong spi_start;
    ulong len,i;
    ...
    printf("copy from spi(%x) to ram(%x) with len=%x\n",spi_start,ram_start,len);
