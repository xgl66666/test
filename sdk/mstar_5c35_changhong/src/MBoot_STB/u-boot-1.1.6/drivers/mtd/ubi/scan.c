...
//       ubi_msg("[%d] lsb(%d), paired_page(%d)\r\n", j, ga_tUbiPairedPageMap[j].lsb, ga_tUbiPairedPageMap[j].paired_page);
     ...
     */
        ...
        ubi_rb_for_each_entry(rb, seb, &sv->root, u.rb) {
            ...
            backup_seb = seb;
            pnum = seb->pnum;
            for (offset = ubi->leb_start; offset < ubi->peb_size; offset+=ubi->min_io_size) {
                ...
                if (err >= 0) {
                    ...
                    if(check_pattern(ubi->oobbuf, 0xFF, ubi->mtd->oobavail))
                        break;
                }
        ...
        if(corrupt) {
            ...
            retry:
                ...
                err = add_to_list(si, old_seb->pnum, old_seb->ec, &si->corr);
                if (err){
        ...
        if (1 == scrub)
        {
            /* if bitflip found in backup block, we can erase it
                directly instead of scrub it, its data is useless anymore */
            add_to_list(si, backup_seb->pnum, backup_seb->ec, &si->erase);
        }
