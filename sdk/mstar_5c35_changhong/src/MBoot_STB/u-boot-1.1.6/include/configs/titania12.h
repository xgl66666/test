...
#include "../../../sboot/inc/titania12/board/Board.h"
...
#define MTDPARTS_DEFAULT    {"mtdparts=edb64M-nand:0x40000(NPT),0x40000(KL_BP),0x500000(KL),0x7500000(UBI)"}
                                                  //{"mtdparts=edb64M-nand:256k(nand_partition_tbl),256k(linux_boot_param),8m(kernel),512k(chakra_boot_param),8m(charkra_bin),8m(rootfs),39m(usr)"} // {"mtdparts=edb64M-nand:512k(uboot),5m(kernel0),5m(kernel1),2m(rootfs),51m(app)"} // {"mtdparts=edb64M-nand:8m(kernel),8m(rootfs),48m(user)"}
#else
...
#else
#define CFG_ENV_OFFSET		    0x3FEC000       /* end of NAND */
#define CFG_ENV_SIZE		    0x4000          /* 16K bytes */
#define CFG_ENV_BLOCK_NUM       1
#endif
