...
u32_t g_u32ArpTimerCount = 0;
u32_t g_u32TcpTimerCount = 0;
u32_t g_u32DnsTmrCount = 0;
u8_t g_bIsInitialized = FALSE;
...
u32_t sys_now(void)
{
    u32_t u32CurrentTime = 0;
    u32CurrentTime = MsOS_GetSystemTime();
    return u32CurrentTime;
}

/*
    sys_timer_tick called at TimerISR(void) @ drvISR.c
*/
void sys_timer_tick()
{
    u32_t u32IsrStat;
    ...
    if(g_bIsInitialized)
    {
        if (++g_u32ArpTimerCount >= ARP_TMR_INTERVAL)
        {
            etharp_tmr();
            g_u32ArpTimerCount = 0;
        }

        if(++g_u32TcpTimerCount >= TCP_TMR_INTERVAL)
        {
            tcp_tmr();
            g_u32TcpTimerCount = 0;
        }

        if(++g_u32DnsTmrCount >= DNS_TMR_INTERVAL)
        {
            dns_tmr();
            g_u32DnsTmrCount = 0;
        }
