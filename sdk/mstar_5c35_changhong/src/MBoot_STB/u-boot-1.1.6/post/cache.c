...
void cache32 (unsigned long uAddrStart,unsigned long uleng)
{
    unsigned long uAddrStartAlign, uAddrEnd;
    uAddrStartAlign = (U32)uAddrStart & 0xFFFFF800;

    //printf("uAddrStart=%08X\n", uAddrStart);

    uAddrEnd = ((uAddrStartAlign + uleng) & 0xFFFFF800) + 0x7FF;
    CACHE32_UNROLL32_ALIGN2;
	for (;uAddrStartAlign < uAddrEnd; uAddrStartAlign += 0x400 * 2)
	{
		cache32_unroll32(uAddrStartAlign, 0x01);
	}
