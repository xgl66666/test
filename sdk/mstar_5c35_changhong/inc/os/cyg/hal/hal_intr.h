...
__externC CYG_WORD32 hal_disable_interrupts(void);
...
__externC void hal_restore_interrupts( CYG_WORD32 old );
#define HAL_RESTORE_INTERRUPTS(_old_) hal_restore_interrupts( _old_ )

__externC CYG_WORD32 hal_query_interrupts(void);
...
// Callable versions of the above macros. Mainly for the use of
// platform specific implementations that need to also work in MIPS16
// mode, and hence cannot inline this code. These routines will only
// be included if they are called.
__externC void hal_interrupt_mask_im( cyg_uint32 vector );
__externC void hal_interrupt_unmask_im( cyg_uint32 vector );
__externC void hal_interrupt_acknowledge_im( cyg_uint32 vector );
...
__externC void hal_clock_initialize( CYG_WORD32 period );
...
__externC void hal_clock_reset( CYG_WORD32 period, cyg_uint32 *count);
#define HAL_CLOCK_RESET( _vector_, _period_, _count_)   \
CYG_MACRO_START \
    hal_clock_reset(_period_, _count_); \
CYG_MACRO_END

__externC CYG_WORD32 hal_clock_read(void);
#define HAL_CLOCK_READ( _pvalue_ )              \
CYG_MACRO_START                                 \
    register CYG_WORD32 result;                 \
    result = hal_clock_read();                  \
    *(_pvalue_) = result;                       \
CYG_MACRO_END
