...
#define PQ_REG_FUNC_xc(u32Addr, u8Value, u8Mask) \
    do{ \
            if(u32Reg %2){ \
...
void MDrv_PQBin_DumpGeneralRegTable(MS_PQBin_IP_Table_Info *pTabInfo)
{
    MS_U8  u8Bank;
    MS_U8  u8Addr;
    MS_U8  u8Mask;
...
void MDrv_PQBin_DumpCombRegTable(MS_PQBin_IP_Table_Info *pTabInfo)
{
    MS_U16 i;
    MS_U8 u8Addr;
    MS_U8 u8Mask;
...
void MDrv_PQBin_DumpScalerRegTable(MS_PQBin_IP_Table_Info *pTabInfo)
{
    MS_U32 u32RegAddr;
    MS_U8 u8Mask;
    MS_U8 u8Addr;
...
void MDrv_PQBin_DumpSWDriverRegTable(MS_PQBin_IP_Table_Info *pTabInfo)
{
    MS_U16 u16Addr, i;
    MS_U8 u8Mask;
    MS_U8 u8Addr;
    ...
    PQ_BIN_DBG(printf("tab: sc\n"));
    if(pTabInfo->u16GroupIdx >= pTabInfo->u16GroupNum)
    {
        PQ_BIN_DUMP_DBG(printf("[PQBin]IP_Info error: SWDriver Reg Table\n"));
        return;
    }
    ...
    for(i=0; i<pTabInfo->u16RegNum; i++)
    {
        ...
        else
        {
            ...
            u16Addr = BK_SCALER_BASE | (MS_U16)u8Addr;
            PQ_BIN_DUMP_DBG(printf("[addr=%04x, msk=%02x, val=%02x]\n", u16Addr, u8Mask, u8Value));
...
void MDrv_PQBin_DumpTable(MS_PQBin_IP_Table_Info* pTabInfo)
{
    ...
    case PQ_TABTYPE_SWDRIVER:
        MDrv_PQBin_DumpSWDriverRegTable(pTabInfo);
        break;
    ...
    case PQ_TABTYPE_C_SRAM4:
        ...
        break;
    default:
        PQ_BIN_DUMP_FILTER_DBG(printf("[PQBin]DumpTable:unknown type\n"));
        break;
    }
}
    ...
    MS_PQBin_Header_Info * pPQBinHeader)
{
    ...
    _u8PQBinfunction = PQ_FUNC_DUMP_REG;
#else
    u16PQSrcType = u16PQSrcType;
    u16PQIPIdx = u16PQIPIdx;
    u16PQPnlIdx = u16PQPnlIdx;
    pPQBinHeader = pPQBinHeader;
#endif
}
