...
#include "K1C_Main.h"              // table config parameter
#include "mhal_pq.h"
//-------------------------------------------------------------------------------------------------
...
{
    MS_U8 u8Bank;
    MS_U16 i, j, x, step,phase;
    MS_U8 u8Ramcode[8];
    MS_U32 u32Addr;

    //In STB only support SRAM1 & SRAM2
    if( (u8sramtype != SC_FILTER_VE_V_SRAM1) &&
        (u8sramtype != SC_FILTER_VE_V_SRAM2) )
    {
        printf("Unknown y sram type %u\n", u8sramtype);
        return;
    }
    ...
    // selec table 0 or table1
    if(u8sramtype == SC_FILTER_VE_V_SRAM1)
    {
      MApi_XC_W2BYTEMSK(REG_SC_BK62_49_L,BIT(10) , 0x0400);
    }
    else    //SC_FILTER_VE_Y_SRAM2
    {
      MApi_XC_W2BYTEMSK(REG_SC_BK62_49_L, ~(BIT(10)), 0x0400);
    }
    ...
    for ( i=0; i<phase; )
    {
        ...
        MApi_XC_W2BYTEMSK(REG_SC_BK62_4A_L, (((MS_U16)u8Ramcode[1])<<8|(MS_U16)u8Ramcode[0]), 0xFFFF);
        MApi_XC_W2BYTEMSK(REG_SC_BK62_4B_L, (((MS_U16)u8Ramcode[3])<<8|(MS_U16)u8Ramcode[2]), 0xFFFF);
        MApi_XC_W2BYTEMSK(REG_SC_BK62_4C_L, ((MS_U16)u8Ramcode[4]), 0x00FF);

        //address
        MApi_XC_W2BYTEMSK(REG_SC_BK62_49_L, i, BIT(0)|BIT(1)|BIT(2)|BIT(3)|BIT(4)|BIT(5)|BIT(6));

        // enable write
        MApi_XC_W2BYTEMSK(REG_SC_BK62_49_L, BIT(8), 0x0100);

        if(u8sramtype == SC_FILTER_VE_V_SRAM2)
        {
            i+=2;
        }
        else
...
void Hal_PQ_set_ve_h_sram_table(MS_U8 u8sramtype, void *pData)
{
    ...
    //In STB only support SRAM1 & SRAM2
    if( (u8sramtype != SC_FILTER_VE_H_SRAM1) &&
        (u8sramtype != SC_FILTER_VE_H_SRAM2) )
    {
        printf("Unknown y sram type %u\n", u8sramtype);
        return;
    }
    ...
    if(u8sramtype == SC_FILTER_VE_H_SRAM1)
    {
      MApi_XC_W2BYTEMSK(REG_SC_BK62_0E_L,BIT(10) , 0x0400);
    }
    else    //SC_FILTER_VE_Y_SRAM2
    {
      MApi_XC_W2BYTEMSK(REG_SC_BK62_0E_L, ~(BIT(10)), 0x0400);
    ...
    for ( i=0; i<phase; )
    {
        ...
        MApi_XC_W2BYTEMSK(REG_SC_BK62_08_L, (((MS_U16)u8Ramcode[1])<<8|(MS_U16)u8Ramcode[0]), 0xFFFF);
        MApi_XC_W2BYTEMSK(REG_SC_BK62_09_L, (((MS_U16)u8Ramcode[3])<<8|(MS_U16)u8Ramcode[2]), 0xFFFF);
        MApi_XC_W2BYTEMSK(REG_SC_BK62_0A_L, ((MS_U16)u8Ramcode[4]), 0x00FF);

        //address
        MApi_XC_W2BYTEMSK(REG_SC_BK62_0E_L, i, BIT(0)|BIT(1)|BIT(2)|BIT(3)|BIT(4)|BIT(5)|BIT(6));

        // enable write
        MApi_XC_W2BYTEMSK(REG_SC_BK62_0E_L, BIT(8), 0x0100);

        if(u8sramtype == SC_FILTER_VE_H_SRAM2)
        {
            i+=2;
        }
        else
...
MS_U8 Hal_PQ_get_ve_h_sram_table_index(MS_U8 u8type)
{
    switch(u8type)
    {
        default:
        case 0: //default, PQ_DE_FLICK_DEFAULT
            return PQ_IP_VE_HSD_SRAM_FC100_Main;
        case 1: //low, PQ_DE_FLICK_LOW
            return PQ_IP_VE_HSD_SRAM_FC100_Main;
        case 2: //mid, PQ_DE_FLICK_MID
            return PQ_IP_VE_VSD_SRAM_C121_128_Main;
        case 3: //high, PQ_DE_FLICK_HIGH
            return PQ_IP_VE_HSD_SRAM_C565_Main;
    }

    return PQ_IP_VE_HSD_SRAM_FC100_Main;
}

MS_U8 Hal_PQ_get_ve_v_sram_table_index(MS_U8 u8type)
{
    switch(u8type)
    {
        default:
        case 0: //default, PQ_DE_FLICK_DEFAULT
            return PQ_IP_VE_VSD_SRAM_C121_128_Main;
        case 1: //low, PQ_DE_FLICK_LOW
            return PQ_IP_VE_VSD_SRAM_FC100_Main;
        case 2: //mid, PQ_DE_FLICK_MID
            return PQ_IP_VE_VSD_SRAM_C565_Main;
        case 3: //high, PQ_DE_FLICK_HIGH
            return PQ_IP_VE_VSD_SRAM_AVG_Main;
    }

    return PQ_IP_VE_VSD_SRAM_C121_128_Main;
}

void Hal_PQ_set_ve_hvsp_v_sram_table(MS_U8 u8sramtype, void *pData)
{
    MS_U8 u8Bank;
    MS_U16 i, j, x, step,phase;
    MS_U8 u8Ramcode[8];
    ...
    //In STB only support SRAM1 & SRAM2
    if( (u8sramtype != SC_FILTER_VE_HVSP_V_SRAM1) &&
        (u8sramtype != SC_FILTER_VE_HVSP_V_SRAM2) )
    {
        printf("Unknown y sram type %u\n", u8sramtype);
        return;
    }
    ...
    if(u8sramtype == SC_FILTER_VE_HVSP_V_SRAM1)
    {
      MApi_XC_W2BYTEMSK(REG_SC_BK5D_49_L,BIT(10) , 0x0400);
    }
    else    //SC_FILTER_VE_Y_SRAM2
    {
      MApi_XC_W2BYTEMSK(REG_SC_BK5D_49_L, ~(BIT(10)), 0x0400);
    ...
    for ( i=0; i<phase; )
    {
        ...
        MApi_XC_W2BYTEMSK(REG_SC_BK5D_4A_L, (((MS_U16)u8Ramcode[1])<<8|(MS_U16)u8Ramcode[0]), 0xFFFF);
        MApi_XC_W2BYTEMSK(REG_SC_BK5D_4B_L, (((MS_U16)u8Ramcode[3])<<8|(MS_U16)u8Ramcode[2]), 0xFFFF);
        MApi_XC_W2BYTEMSK(REG_SC_BK5D_4C_L, ((MS_U16)u8Ramcode[4]), 0x00FF);

        //address
        MApi_XC_W2BYTEMSK(REG_SC_BK5D_49_L, i, BIT(0)|BIT(1)|BIT(2)|BIT(3)|BIT(4)|BIT(5)|BIT(6));

        // enable write
        MApi_XC_W2BYTEMSK(REG_SC_BK5D_49_L, BIT(8), 0x0100);

        if(u8sramtype == SC_FILTER_VE_HVSP_V_SRAM2)
        {
            i+=2;
        }
        else
...
void Hal_PQ_set_ve_hvsp_h_sram_table(MS_U8 u8sramtype, void *pData)
{
    MS_U8 u8Bank;
    MS_U16 i, j, x, step,phase;
    MS_U8 u8Ramcode[8];
    ...
    //In STB only support SRAM1 & SRAM2
    if( (u8sramtype != SC_FILTER_VE_HVSP_H_SRAM1) &&
        (u8sramtype != SC_FILTER_VE_HVSP_H_SRAM2) )
    {
        printf("Unknown y sram type %u\n", u8sramtype);
        return;
    }
    ...
    if(u8sramtype == SC_FILTER_VE_HVSP_H_SRAM1)
    {
      MApi_XC_W2BYTEMSK(REG_SC_BK5D_0E_L,BIT(10) , 0x0400);
    }
    else    //SC_FILTER_VE_Y_SRAM2
    {
      MApi_XC_W2BYTEMSK(REG_SC_BK5D_0E_L, ~(BIT(10)), 0x0400);
    ...
    for ( i=0; i<phase; )
    {
        ...
        MApi_XC_W2BYTEMSK(REG_SC_BK5D_08_L, (((MS_U16)u8Ramcode[1])<<8|(MS_U16)u8Ramcode[0]), 0xFFFF);
        MApi_XC_W2BYTEMSK(REG_SC_BK5D_09_L, (((MS_U16)u8Ramcode[3])<<8|(MS_U16)u8Ramcode[2]), 0xFFFF);
        MApi_XC_W2BYTEMSK(REG_SC_BK5D_0A_L, ((MS_U16)u8Ramcode[4]), 0x00FF);

        //address
        MApi_XC_W2BYTEMSK(REG_SC_BK5D_0E_L, i, BIT(0)|BIT(1)|BIT(2)|BIT(3)|BIT(4)|BIT(5)|BIT(6));

        // enable write
        MApi_XC_W2BYTEMSK(REG_SC_BK5D_0E_L, BIT(8), 0x0100);

        if(u8sramtype == SC_FILTER_VE_HVSP_H_SRAM2)
        {
            i+=2;
        }
        else
...
MS_U8 Hal_PQ_get_ve_hvsp_h_sram_table_index(MS_U8 u8type)
{
    switch(u8type)
    {
        default:
        case 0: //default, PQ_DE_FLICK_DEFAULT
            return PQ_IP_VE_HSP_SRAM_FC100_Main;
        case 1: //low, PQ_DE_FLICK_LOW
            return PQ_IP_VE_HSP_SRAM_FC100_Main;
        case 2: //mid, PQ_DE_FLICK_MID
            return PQ_IP_VE_VSP_SRAM_C121_128_Main;
        case 3: //high, PQ_DE_FLICK_HIGH
            return PQ_IP_VE_HSP_SRAM_C565_Main;
    }

    return PQ_IP_VE_HSP_SRAM_FC100_Main;
}

MS_U8 Hal_PQ_get_ve_hvsp_v_sram_table_index(MS_U8 u8type)
{
    switch(u8type)
    {
        default:
        case 0: //default, PQ_DE_FLICK_DEFAULT
            return PQ_IP_VE_VSP_SRAM_C121_128_Main;
        case 1: //low, PQ_DE_FLICK_LOW
            return PQ_IP_VE_VSP_SRAM_FC100_Main;
        case 2: //mid, PQ_DE_FLICK_MID
            return PQ_IP_VE_VSP_SRAM_C565_Main;
        case 3: //high, PQ_DE_FLICK_HIGH
            return PQ_IP_VE_VSP_SRAM_AVG_Main;
    }
...
MS_U16 Hal_PQ_get_madi_idx(MS_MADI_TYPE etype)
{
    ...
    switch(etype)
    {
        ...
        case MS_MADI_FBL_EODi:
            ...
            break;
            //Kayla PQ excel file not support below enum
        ...
        case MS_MADI_24_2R_P:
        default:
            u16ret = 0xFFFF;
            PQ_DBG(printf("!!Invalid!!, Please check the index in [%s][%d]\n", __FUNCTION__, __LINE__));	
            break;
    }
    return u16ret;
}
...
MS_U16 Hal_PQ_get_sram_size(MS_U16 u16sramtype)
{
    ...
    case SC_FILTER_C_SRAM2:
        ...
        break;
    case SC_FILTER_VE_V_SRAM1:
    case SC_FILTER_VE_V_SRAM2:
        u16ret = PQ_IP_VE_VSD_SRAM_SIZE_Main;
        break;
    case SC_FILTER_VE_H_SRAM1:
    case SC_FILTER_VE_H_SRAM2:
        u16ret = PQ_IP_VE_HSD_SRAM_SIZE_Main;
        break;
    case SC_FILTER_VE_HVSP_V_SRAM1:
    case SC_FILTER_VE_HVSP_V_SRAM2:
        u16ret = PQ_IP_VE_VSP_SRAM_SIZE_Main;
        break;
    case SC_FILTER_VE_HVSP_H_SRAM1:
    case SC_FILTER_VE_HVSP_H_SRAM2:
        u16ret = PQ_IP_VE_HSP_SRAM_SIZE_Main;
        break;
