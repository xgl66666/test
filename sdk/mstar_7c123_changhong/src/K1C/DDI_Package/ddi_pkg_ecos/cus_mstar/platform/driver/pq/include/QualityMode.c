...
MS_U16 QM_InputSourceToIndex(PQ_WIN eWindow, MS_U8 enInputSourceType)
{
    ...
    PQMODE_DBG(printf("H=%u,V=%u,VFreq=%u,IsInterlace=%u,dispH=%u,dispV=%u\n", u16Input_HSize,u16Input_VSize,u16Input_VFreq, (MS_U16)bIsInterlaced,u16Disp_HSize,u16Disp_VSize));

#if (PQ_QM_3D)
    if (QM_Is3DVideo())
    {
        u16SrcType = (bIsInterlaced == TRUE) ?
                     QM_3D_Video_interlace_Main :
                     QM_3D_Video_progressive_Main;
    }
    else
#endif
    ...
    {
        if (QM_IsYPbPr_720x480_60I(eWindow)){
        ...
        else if (QM_IsYPbPr_1920x1080_24P(eWindow) || QM_IsYPbPr_1920x1080_30P(eWindow)){
    ...
    else if (QM_IsSourceDTV(enInputSourceType))
    {
        ...
        else // QM_IsDTV_H264(eWindow)
        {
            ...
            else
            {
                ...
                {
#if (PQ_QM_4K2K)
                    if( (u16Input_HSize > 1920) && (u16Input_VSize > 1088) )
                    {
...
#if (PQ_QM_4K2K)
            else if( (u16Input_HSize > 1920) && (u16Input_VSize > 1080) )
            {
