...
#ifndef PQ_QM_VE_HVSP
#define PQ_QM_VE_HVSP 0
#endif
...
#if(PQ_QM_VE_HVSP == 1)
extern MS_U32 gu32LastPQIPVEHSPSRAM1_Main;
extern MS_U32 gu32LastPQIPVEHSPSRAM2_Main;
extern MS_U32 gu32LastPQIPVEVSPSRAM1_Main;
extern MS_U32 gu32LastPQIPVEVSPSRAM2_Main;
#endif
...
static void _MDrv_PQ_DumpFilterTable(EN_IP_Info* pIP_Info)
{
    ...
    case PQ_TABTYPE_VE_VSD_SRAM:
        ...
        break;
#if(PQ_QM_VE_HVSP == 1)
   case PQ_TABTYPE_VE_HSP_SRAM:
        //load table0 & table1 (128 phase & 64 phase)
        Hal_PQ_set_ve_hvsp_h_sram_table(SC_FILTER_VE_HVSP_H_SRAM1, (void *)((MS_U32)&pIP_Info->pIPTable[pIP_Info->u8TabIdx * Hal_PQ_get_sram_size(SC_FILTER_VE_HVSP_H_SRAM1)]));
        Hal_PQ_set_ve_hvsp_h_sram_table(SC_FILTER_VE_HVSP_H_SRAM2, (void *)((MS_U32)&pIP_Info->pIPTable[pIP_Info->u8TabIdx * Hal_PQ_get_sram_size(SC_FILTER_VE_HVSP_H_SRAM2)]));
        gu32LastPQIPVEHSPSRAM1_Main  = (MS_U32)&pIP_Info->pIPTable[pIP_Info->u8TabIdx * Hal_PQ_get_sram_size(SC_FILTER_VE_HVSP_H_SRAM1)];
        gu32LastPQIPVEHSPSRAM2_Main  = (MS_U32)&pIP_Info->pIPTable[pIP_Info->u8TabIdx * Hal_PQ_get_sram_size(SC_FILTER_VE_HVSP_H_SRAM2)];
        break;

    case PQ_TABTYPE_VE_VSP_SRAM:
        //load table0 & table1 (128 phase & 64 phase)
        Hal_PQ_set_ve_hvsp_v_sram_table(SC_FILTER_VE_HVSP_V_SRAM1, (void *)((MS_U32)&pIP_Info->pIPTable[pIP_Info->u8TabIdx * Hal_PQ_get_sram_size(SC_FILTER_VE_HVSP_V_SRAM1)]));
        Hal_PQ_set_ve_hvsp_v_sram_table(SC_FILTER_VE_HVSP_V_SRAM2, (void *)((MS_U32)&pIP_Info->pIPTable[pIP_Info->u8TabIdx * Hal_PQ_get_sram_size(SC_FILTER_VE_HVSP_V_SRAM2)]));
        gu32LastPQIPVEVSPSRAM1_Main  = (MS_U32)&pIP_Info->pIPTable[pIP_Info->u8TabIdx * Hal_PQ_get_sram_size(SC_FILTER_VE_HVSP_V_SRAM1)];
        gu32LastPQIPVEVSPSRAM2_Main  = (MS_U32)&pIP_Info->pIPTable[pIP_Info->u8TabIdx * Hal_PQ_get_sram_size(SC_FILTER_VE_HVSP_V_SRAM2)];
        break;
#endif
    default:
...
#if(PQ_QM_VE_HVSP == 1)
    static MS_U8 _u8VE_HVSP_V_SRAMTable=0xFF;
    static MS_U8 _u8VE_HVSP_H_SRAMTable=0xFF;
#endif
...
#if(PQ_QM_VE_HVSP == 1)
    case PQ_TABTYPE_VE_HSP_SRAM:
        if (_u8VE_HVSP_H_SRAMTable != pIP_Info->u8TabIdx)
        {
            PQ_DUMP_FILTER_DBG(printf("old sram1: %u, new sram1: %u\n",
                (MS_U16)_u8VE_HVSP_H_SRAMTable, (MS_U16)pIP_Info->u8TabIdx));
            _u8VE_HVSP_H_SRAMTable = pIP_Info->u8TabIdx;
            _MDrv_PQ_DumpFilterTable(pIP_Info);
        }
        else
        {
            PQ_DUMP_FILTER_DBG(printf("use the same VE HSP SRAM: %u\n", (MS_U16)pIP_Info->u8TabIdx));
        }
        break;
    case PQ_TABTYPE_VE_VSP_SRAM:
        if (_u8VE_HVSP_V_SRAMTable != pIP_Info->u8TabIdx)
        {
            if(!_bUserDeFlicker)
            {
                PQ_DUMP_FILTER_DBG(printf("old sram1: %u, new sram1: %u\n",
                    (MS_U16)_u8VE_HVSP_V_SRAMTable, (MS_U16)pIP_Info->u8TabIdx));
                _u8VE_HVSP_V_SRAMTable = pIP_Info->u8TabIdx;
                _MDrv_PQ_DumpFilterTable(pIP_Info);
            }
        }
        else
        {
            PQ_DUMP_FILTER_DBG(printf("use the same VE VSP SRAM: %u\n", (MS_U16)pIP_Info->u8TabIdx));
        }
        break;
#endif
    default:
...
#if(PQ_QM_VE_HVSP == 1)
            ||(ip_Info.u8TabType == PQ_TABTYPE_VE_HSP_SRAM)
            ||(ip_Info.u8TabType == PQ_TABTYPE_VE_VSP_SRAM)
#endif
           )
        {
            continue;
