...
#include "MsOS.h"
#include "drvMMIO.h"
#include "bond.h"  // pad index, and gpio mapping

#define _TEMP_PM_       0

#define _END_OF_TBL_        0xFF
...
//---------------------------------------------------------------------
    ...
    _RVM1(0x1E05, TS1_MODE, BITMASK(1:0)),

    #define NAND_MODE ((PADS_NAND_MODE == PAD_TS1_VLD) ? (BIT0) : \
                       (PADS_NAND_MODE == PAD_TS1_D7) ? (BIT1) : \
...
void MDrv_WriteRegMaskTbl(const MS_U8 *pPadConfig)
{
    MS_U32 u32NonPMAddr = 0, u32PMAddr = 0, u32Size = 0;

    MDrv_MMIO_GetBASE(&u32NonPMAddr, &u32Size, MS_MODULE_HW);
    MDrv_MMIO_GetBASE(&u32PMAddr, &u32Size, MS_MODULE_PM);
    ...
    while (1)
    {
        ...
        while (1)
        {
            MS_U32 addr = u16Index;
            volatile MS_U8 *reg = (MS_U8 *)(u32MapBase + (addr << 1) - (addr & 1));

            /*printf("#%-3d %p 0x%08lx 0x%02x 0x%02x 0x%02x\n", i++, reg, addr, u8Length, pPadConfig[0], pPadConfig[1]);
            fflush(NULL);
            sync();
            tcdrain(1);
            tcdrain(2);
            ...
            *reg = (*reg & ~pPadConfig[1]) | (pPadConfig[0] & pPadConfig[1]);
...
{
    MDrv_WriteRegMaskTbl(padInitTbl + 10);
}
