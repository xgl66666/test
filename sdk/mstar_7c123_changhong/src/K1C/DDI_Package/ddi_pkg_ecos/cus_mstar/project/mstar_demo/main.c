...
//    MApi_DMX_FlowSet(DMX_FLOW_PLAYBACK, DMX_FLOW_INPUT_EXT_INPUT0, FALSE, TRUE, TRUE);
    printf("Looping...\n");
#if defined (PC_CARD)
//    RBL011_ASYNC_ProcessIT();
      RBL011_ASYNC_DumpCIS();
      if(RBL011_ASYNC_LoopTest())
          printf("CI LoopTest Failed...\n");
      else
          printf("CI LoopTest OK...\n");
#endif
    while(1)
    {
        MsOS_DelayTask(10000);
        if (Demo_Main_Release_Check()== TRUE)
        {
            break;
        }
    }
...
int AppMain_main (void *pPoolStart)
{
    ...
    //MS_BOOL msBool; //fix tool 4.8.2 compiling warning

    //----------------------------------------------------------------------------------------------
    //  System Initialization
    //----------------------------------------------------------------------------------------------
    //msBool =
    MApi_SystemInit_Init(pPoolStart);
