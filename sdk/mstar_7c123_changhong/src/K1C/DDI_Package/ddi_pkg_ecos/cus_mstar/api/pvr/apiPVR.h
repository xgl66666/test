...
typedef enum
{
    ...
    EN_APIPVR_FILE_SYSTEM_TYPE_EXT4,
...
typedef enum
{
    ...
    EN_APIPVR_AV_CODEC_AUD_DRA,             ///DRA (audio)
...
typedef struct PVRProgramInfo_s{
    ...
    MS_BOOL bLive;
    MS_BOOL bRecordAppend;
    MS_U64 u64FileSliceSize;
}PVRProgramInfo_t;
...
typedef struct PVRBoundedLinearRecordInfo_s{
    MS_U8 u8RecordIdx;
    MS_BOOL bBoundedLinearRecord;
}PVRBoundedLinearRecordInfo_t;

typedef enum
{
    EN_APIPVR_PVR_CFG_NONE,
    // bAutoPlaybackProgramChg: TRUE: enable; FALSE: disable, pCfgPara is unused
    EN_APIPVR_CFG_AUTO_PB_PROGRAM_CHG,
    // data type: MS_U32, *pCfgPara: 0: do not use record queue; 1~0xFFFFFFFF: time slice for record queue, unit: sec
    EN_APIPVR_CFG_TIMESHIFT_DURATION_PRECISION,
    EN_APIPVR_CFG_RECORD_DURATION_PRECISION = EN_APIPVR_CFG_TIMESHIFT_DURATION_PRECISION,
    // data type: PVRBoundedLinearRecordInfo_t
    // *pCfgPara.u8RecordIdx: record program index, *pCfgPara.bBoundedLinearRecord: TRUE: Bounded linear record
    EN_APIPVR_CFG_IS_BOUNDED_LINEAR_RECORD,
    // Use *pCfgPara as MS_U32 parameter
    // For eCos:
    // if *pCfgPara = 0, PVR record would do flush after write.
    // if *pCfgPara > 0, PVR record would do flush every pCfgPara ms.
    // For Linux:
    // if *pCfgPara = 0, PVR record would do sync after write.
    // if *pCfgPara > 0. PVR record wouldn't do sync after write. Let Linux kernel do the job.
    EN_APIPVR_CFG_FILE_SYNC_CONFIG
...
typedef struct
{
    PVRProgramInfo_t*       pProgramInfo;
    MS_BOOL                 bBackgroundRecord;
    MS_U8                   u8RecordProgramIdx;
    MS_U32                  u32AESDMAKey[4];
}PVR_ProgramInfo;

typedef enum
{
    EN_APIPVR_ACCESS_METADATA_NONE,
    // data type: PVR_ProgramInfo
    // pAccessPara->pProgramInfo is program information
    // pAccessPara->bBackgroundRecord, TRUE:background record metadata, FALSE:record metadata
    // pAccessPara->u8RecordProgramIdx is recording program index
    // pAccessPara->u32AESDMAKey is user key
    EN_APIPVR_ACCESS_METADATA_PROGRAM_INFO,
    // data type: char, *pAccessPara is filename
    EN_APIPVR_ACCESS_METADATA_FILE_NAME,
    // data type: MS_U32, *pAccessPara is file start time(The unit: seconds)
    EN_APIPVR_ACCESS_METADATA_FILE_START_TIME,
} EN_APIPVR_ACCESS_METADATA_CMD;

typedef struct
{
    MS_BOOL                         bSet;
    EN_APIPVR_ACCESS_METADATA_CMD   eAccessCmd;
    void*                           pAccessPara;
} PVR_AccessMetaDataParam;

// ---------------------------
// PVR Initialization and Exit
// ---------------------------
...
// Timeshifting
// ---------------------------
// Timeshift Record Setting
MS_BOOL MApi_PVR_FreezeScreenInTimeshiftRecord(EN_APIPVR_ACTION_TYPE enAction,MS_BOOL *bFreeze); // Obsolete, please use MApi_PVR_EX_FreezeScreenInTimeshiftRecord instead
...
MS_BOOL MApi_PVR_SetTimeshiftFileSize(MS_U64 u64TimeShiftFileSizeInKB); // Obsolete, please use MApi_PVR_SetRecordFileSize instead
...
MS_BOOL MApi_PVR_SetBackRecordMetaData(PVRProgramInfo_t *pProgramInfo,MS_U8 u8RecordProgramIdx,MS_U32 u32AESDMAKey0,MS_U32 u32AESDMAKey1,MS_U32 u32AESDMAKey2,MS_U32 u32AESDMAKey3); // Obsolete, please use MApi_PVR_AccessMetaData instead
MS_BOOL MApi_PVR_BackRecordStop(MS_U8 *pu8hRecord,MS_U8 u8RecordProgramIdx);

// Timeshift Record/Play
MS_BOOL MApi_PVR_IsTimeShift(void); // Obsolete, please use MApi_PVR_EX_IsTimeShift instead
MS_BOOL MApi_PVR_EX_IsTimeShift(MS_U8 u8PlaybackProgramIdx);
MS_BOOL MApi_PVR_TimeshiftRecordStart(MS_U8 *pu8hRecord,MS_U8 u8RecordProgramIdx); // Obsolete, please use MApi_PVR_EX_TimeshiftRecordStart instead
...
MS_BOOL MApi_PVR_TimeshiftRecordStop(MS_U8 *pu8hRecord,MS_U8 u8RecordProgramIdx); // Obsolete, please use MApi_PVR_EX_TimeshiftRecordStop instead
...
MS_BOOL MApi_PVR_TimeshiftPlaybackStart(MS_U8 *pu8hPlayback, char *fileName, MS_U32 u32PlaybackTimeInSec, MS_U32 u32PlaybackPTS); // Obsolete, please use MApi_PVR_EX_TimeshiftPlaybackStart instead
...
MS_BOOL MApi_PVR_TimeshiftPlaybackStop(MS_U8 *pu8hPlayback,MS_U8 u8RecordProgramIdx); // Obsolete, please use MApi_PVR_EX_TimeshiftPlaybackStop instead
MS_BOOL MApi_PVR_EX_TimeshiftPlaybackStop(MS_U8 *pu8hPlayback,MS_U8 u8RecordProgramIdx,MS_U8 u8PlaybackProgramIdx);

MS_BOOL MApi_PVR_TimeshiftStop(MS_U8 *pu8hRecord,MS_U8 *pu8hPlayback,MS_U8 u8RecordProgramIdx); // Obsolete, please use MApi_PVR_EX_TimeshiftStop instead
...
// ------------------------------------
// Unbounded Linear Record and Playback
// ------------------------------------
MS_BOOL MApi_PVR_RecordStart(MS_U8 *pu8hRecord,MS_U8 u8RecordProgramIdx,APIPVR_FILE_FORMAT pvrFileFormat, MS_U32 u32FreeDiskSpaceInMB,MS_BOOL bRecordAll); // Obsolete, please use MApi_PVR_EX_RecordStart instead
...
MS_BOOL MApi_PVR_PlaybackOpen(MS_U8 u8CurProgramPath, MS_U8 *pu8hPlayback, void *pAVStreamID, char *fileName); // Obsolete, please use MApi_PVR_EX_PlaybackStart instead
MS_BOOL MApi_PVR_PlaybackStart(MS_U8 *pu8hPlayback, char *fileName, MS_U32 u32PlaybackTimeInSec, MS_U32 u32PlaybackPTS); // Obsolete, please use MApi_PVR_EX_PlaybackStart instead
...
MS_BOOL MApi_PVR_SetMetaData(PVRProgramInfo_t *pProgramInfo,MS_U8 u8RecordProgramIdx,MS_U32 u32AESDMAKey0,MS_U32 u32AESDMAKey1,MS_U32 u32AESDMAKey2,MS_U32 u32AESDMAKey3); // Obsolete, please use MApi_PVR_AccessMetaData instead
MS_BOOL MApi_PVR_SetRecordFileSize(MS_U8 u8RecordProgramIdx, MS_U64 u64RecordFileSizeInKB);
...
// Merge Stream
MS_BOOL MApi_PVR_SetRecordSourceID(MS_U8 u8RecordProgramIdx, MS_U8 u8SourceID);
...
// ---------------------------
// Playback related operation
// ---------------------------
// Playback Info.
MS_BOOL MApi_PVR_IsPlaybacking(void); // Obsolete, please use MApi_PVR_EX_IsPlaybacking instead
...
MS_BOOL MApi_PVR_GetProgramInfo(char *fileName,PVRProgramInfo_t * programInfo); // Obsolete, please use MApi_PVR_AccessMetaData instead
...
void MApi_PVR_NotifyTrickModeDoneMsg(void);
...
MS_BOOL MApi_PVR_PlaybackPause(MS_U8 u8hPlayback); // Obsolete, please use MApi_PVR_PlaybackSetSpeed instead
MS_BOOL MApi_PVR_PlaybackResume(MS_U8 u8hPlayback); // Obsolete, please use MApi_PVR_PlaybackSetSpeed instead
MS_BOOL MApi_PVR_PlaybackFastForward(MS_U8 u8hPlayback); // Obsolete, please use MApi_PVR_PlaybackSetSpeed instead
MS_BOOL MApi_PVR_PlaybackFastBackward(MS_U8 u8hPlayback); // Obsolete, please use MApi_PVR_PlaybackSetSpeed instead
...
// Capture Video
MS_BOOL MApi_PVR_CaptureVideo(MS_U32 u32PhysicalAddr, MS_U32 u32Width, MS_U32 u32Height,MS_U32 *capturepts); // Obsolete, please use MApi_PVR_EX_CaptureVideo instead
...
// AB Loop and Skip Table
MS_BOOL MApi_PVR_PlaybackABLoop(char *fileName,MS_U32 u32ABLoopBeginTime, MS_U32 u32ABLoopEndTime); // Obsolete, please use MApi_PVR_EX_PlaybackABLoop instead
MS_BOOL MApi_PVR_EX_PlaybackABLoop(MS_U8 u8hPlayback, char *fileName,MS_U32 u32ABLoopBeginTime, MS_U32 u32ABLoopEndTime);
MS_BOOL MApi_PVR_PlaybackABLoopReset(void); // Obsolete, please use MApi_PVR_EX_PlaybackABLoopReset instead
MS_BOOL MApi_PVR_EX_PlaybackABLoopReset(MS_U8 u8hPlayback);
MS_BOOL MApi_PVR_SkipTableGetIndex(MS_U32 *pu32SkipBeginTime,MS_U32 *pu32Idx, MS_U32 *pu32SkipEndTime); // Obsolete, please use MApi_PVR_EX_SkipTableGetIndex instead
MS_BOOL MApi_PVR_EX_SkipTableGetIndex(MS_U8 u8hPlayback, MS_U32 *pu32SkipBeginTime,MS_U32 *pu32Idx, MS_U32 *pu32SkipEndTime);
MS_BOOL MApi_PVR_SkipTableAdd(char *fileName, MS_U32 u32SkipBeginTime, MS_U32 u32SkipEndTime); // Obsolete, please use MApi_PVR_EX_SkipTableAdd instead
MS_BOOL MApi_PVR_EX_SkipTableAdd(MS_U8 u8hPlayback, char *fileName, MS_U32 u32SkipBeginTime, MS_U32 u32SkipEndTime);
MS_BOOL MApi_PVR_SkipTableRemove(char *fileName,MS_U32 u32Index); // Obsolete, please use MApi_PVR_EX_SkipTableRemove instead
MS_BOOL MApi_PVR_EX_SkipTableRemove(MS_U8 u8hPlayback,char *fileName,MS_U32 u32Index);
MS_BOOL MApi_PVR_SetPlaybackRetentionLimit(char *fileName, MS_U32 *pu32RetentionLimitTime); // Obsolete, please use MApi_PVR_EX_SetPlaybackRetentionLimit instead
...
MS_BOOL MApi_PVR_RenameFileName(char *fileName,char *newFileName); // Obsolete, please use MApi_PVR_AccessMetaData instead

// Change the Field of Meta File
MS_BOOL MApi_PVR_SaveAge(char *filename,MS_U8 u8Age);
void MApi_PVR_SaveCrossProgramInfo(char *fileName,MS_U32 u32ProDuration,MS_U16 *u16pProName);
...
MS_BOOL MApi_PVR_AccessMetaData(char *fileName, PVR_AccessMetaDataParam *pstPVRAccessMetadataParam);
