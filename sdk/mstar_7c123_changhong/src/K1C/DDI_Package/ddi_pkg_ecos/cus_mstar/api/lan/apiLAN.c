...
{
extern MS_U32 MDrv_EMAC_Init(void);
...
MS_BOOL MApi_LAN_MacConfig(MS_U8* mac)
{
	...
	if(_MApi_LAN_SetIfParam(ifr, SIOCSIFHWADDR)){
		...
		{
extern void MDrv_EMAC_ChangeMacAddr(MS_U8* mac);
...
{
extern void EMAC_PowerOn_Set(void);
    EMAC_PowerOn_Set();
return(TRUE);
}
//-------------------------------------------------------------------------------------------------
...
{
extern void EMAC_PowerOff_Set(void);
    EMAC_PowerOff_Set();
return(TRUE);
}
...
{
extern void EMAC_Resume(void);
    EMAC_Resume();
}

void MApi_LAN_Suspend(void)
{
extern void EMAC_Suspend(void);
    EMAC_Suspend();
}

MS_U32 MApi_LAN_QueryInformation(EMAC_OID xoid,MS_U8 *xbuf,MS_U32 xlen,MS_U32 *xread)
{
extern MS_U32 QueryInformation(EMAC_OID xoid,MS_U8 *xbuf,MS_U32 xlen,MS_U32 *xread);
    return QueryInformation(xoid, xbuf, xlen, xread);
}

MS_U32 MApi_LAN_SetInformation(EMAC_OID xoid,MS_U8 *xbuf,MS_U32 xlen,MS_U32 *xwrit)
{
extern MS_U32 SetInformation(EMAC_OID xoid,MS_U8 *xbuf,MS_U32 xlen,MS_U32 *xwrit);
    return SetInformation(xoid, xbuf, xlen, xwrit);
}
...
{
extern int CheckEMAC_Initialized(void);
