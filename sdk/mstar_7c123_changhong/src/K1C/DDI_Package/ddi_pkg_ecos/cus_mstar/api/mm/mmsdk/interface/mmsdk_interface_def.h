...
///physical address
typedef MMSDK_U64  MMSDK_PHY;       // 8 bytes
...
/// define the max number of the size of the texture mode traveling buffer, defined MAX_TRAVELING_BUFFER_COUNT in OMXVideoRenderer.h
#define MMSDK_MAX_TRAVELING_BUFFER_COUNT (8)
...
/// Define orientation angle.(Photo)
typedef enum
{
    /// photo orientation is 0 degree.
    E_MMSDK_ORIENTATION_0 = 0,
    /// photo orientation is 90 degree.
    E_MMSDK_ORIENTATION_90,
    /// photo orientation is 180 degree.
    E_MMSDK_ORIENTATION_180,
    /// photo orientation is 270 degree.
    E_MMSDK_ORIENTATION_270,
    /// photo orientation is none.
    E_MMSDK_ORIENTATION_NOT_SUPPORT
} EN_MMSDK_ORIENTATION_ANGLE;
...
typedef enum
{
    ...
    /// OPUS
    E_MMSDK_AUDIO_CODEC_OPUS = 0x1300,
...
typedef enum
{
    ...
    /// Video codec type is AVS2.
    E_MMSDK_VIDEO_CODEC_AVS2,
    /// Video codec type is AVC Dolby Vision.
    E_MMSDK_VIDEO_CODEC_H264_DV,
...
typedef enum
{
    ...
    ///ABGR8888
    E_MMSDK_COLOR_FORMAT_ABGR8888,
    ///RGB565
    E_MMSDK_COLOR_FORMAT_RGB565,
    ///YUV420_NV12
    E_MMSDK_COLOR_FORMAT_YUV420_NV12,
    ///YUV422_UYVY
    E_MMSDK_COLOR_FORMAT_YUV422_UYVY,
...
typedef enum
{
    ...
    /// Rename as audio specific config(ex. Vorbis Header Info, OPUS Audio Specific Config Data)
    E_MMSDK_ES_DATA_TYPE_AUDIO_SPECIFIC_CONFIG = E_MMSDK_ES_DATA_TYPE_VORBIS_HEADER,
...
typedef enum
{
    ...
    ///Music: Set music album mode
    E_MMSDK_INFO_MUSIC_ALBUM_MODE,
    /*==================== Comment ====================*/
    ...
    /// Support drm use shared memory mode in the version
    E_MMSDK_INFO_SUPPORT_DRM_SHARED_MEMORY_MODE = 0x080F,
    /// Support textute mode(VR360); Param: MMSDK_U32
    E_MMSDK_INFO_SUPPORT_VIDEO_TEXTURE_MODE = 0x0810,
    /// Get the video texture mode capability; Param: ST_MMSDK_VIDEO_TEXTURE_CAP_INFO
    E_MMSDK_INFO_GET_VIDEO_TEXTURE_MODE_CAPABILITY = 0x0811,
    /// Set the video texture mode property; Param: ST_MMSDK_VIDEO_TEXTURE_PROPERTY
    E_MMSDK_INFO_SET_VIDEO_TEXTURE_MODE_PROPERTY = 0x0812,
    /// Get the video texture mode required buffer size; Param: MMSDK_U32
    E_MMSDK_INFO_GET_VIDEO_TEXTURE_REQUIRE_BUFFER_SIZE = 0x0813,
    /// Get the video texture mode traveling buffer count and buffer addresses with tthe buffer information and start address; Param: ST_MMSDK_TRAVELING_BUFFER_INFO
    E_MMSDK_INFO_GET_VIDEO_TEXTURE_TRAVELING_BUFFER_INFO = 0x0814,
    ...
    /// Get ES audio buffer level; Param: MMSDK_U32
    E_MMSDK_INFO_MOVIE_AUDIO_BUFFER_LEVEL,
    /// HLS element get DISCONTINUITY tag
    E_MMSDK_INFO_MOVIE_GET_DISCONTINUITY_TAG,
    /// set video scan type of index table. Only used for SetPlayPosition; Param:MMSDK_U8(0:backward, 1:forward)
    E_MMSDK_INFO_MOVIE_SET_PLAY_POSITION_IS_FORWARD,
    /// Free frame buffer after render done in texture mode; Param: ST_MMSDK_TEXTURE_FRAMEBUFFER_INFO
    E_MMSDK_INFO_MOVIE_FREE_TEXTURE_MODE_FRAME_BUFFER,
    /// current video pid; Param: MMSDK_U32
    E_MMSDK_INFO_MOVIE_VIDEO_PID,
    /// current audio pid; Param: MMSDK_U32
    E_MMSDK_INFO_MOVIE_AUDIO_PID,
    /// Set scrambling key for NSTV drm; Param: EN_MMSDK_SCRAMBLING_KEY_TYPE
    E_MMSDK_INFO_MOVIE_SET_SCRAMBING_KEY,
    /// No more segment in the stream; Param: MMSDK_BOOL
    E_MMSDK_INFO_MOVIE_SET_NO_MORE_SEGMENT,
    /// get the subtitle component id for ts file(with track id), track id for other container; input: track id; Param: MMSDK_U32
    E_MMSDK_INFO_MOVIE_SUBTITLE_COMPONENT_ID,
    /*----------divx----------------*/
    ...
    /// photo orientation info; Param: EN_MMSDK_ORIENTATION_ANGLE
    E_MMSDK_INFO_PHOTO_ORIENTATION,
    /*==================== Photo End ====================*/
}EN_MMSDK_INFO;
...
typedef enum
{
    ...
    /// Set init seek pts; Param: MMSDK_U32
    E_MMSDK_MEDIAINFO_EXT_INIT_SEEK_PTS,
    /// Enable Feature Texture Mode Flow for VR360; Param: MMSDK_BOOL
    E_MMSDK_MEDIAINFO_EXT_ENABLE_TEXTURE_MODE,
...
/// Define the scrambling key Type
typedef enum
{
    /// '00' = Not scrambled.
    E_MMSDK_SCRAMBLING_KEY_TYPE_NONE = 0x0,
    /// '01' = Reserved
    E_MMSDK_SCRAMBLING_KEY_TYPE_RESERVED = 0x1,
    /// '10' = Scrambled with even key.
    E_MMSDK_SCRAMBLING_KEY_TYPE_EVEN = 0x2,
    /// '11' = Scrambled with odd key.
    E_MMSDK_SCRAMBLING_KEY_TYPE_ODD = 0x3,
    /// maximum Scrambling key value.
    E_MMSDK_SCRAMBLING_KEY_TYPE_MAX = E_MMSDK_SCRAMBLING_KEY_TYPE_ODD,
}EN_MMSDK_SCRAMBLING_KEY_TYPE;
...
typedef enum
{
    ...
    /// for hls element push next fragment
    /// Param: ST_MMSDK_CALLBACK_NOTIFY_INFO
    E_MMSDK_CALLBACK_GET_NEXT_FRAGMENT = 35,
    /// Allocate texture frame buffer from browser, blocking call; ST_MMSDK_CALLBACK_TEXTURE_FRAME_BUFFER_INFO
    E_MMSDK_CALLBACK_TEXTURE_FRAMEBUFFER_ALLOCATE = 36,
    /// Render texture frame buffer to browser; ST_MMSDK_CALLBACK_TEXTURE_FRAME_BUFFER_INFO
    /// The browser with set E_MMSDK_INFO_MOVIE_FREE_TEXTURE_MODE_FRAME_BUFFER after render done
    E_MMSDK_CALLBACK_TEXTURE_FRAMEBUFFER_RENDER = 37,
    /// Notify create pipeline done
    E_MMSDK_CALLBACK_CREATE_PIPELINE_DONE = 38,
    /// Notify get next segment
    E_MMSDK_CALLBACK_GET_NEXT_SEGMENT = 39,
    /// for dlna element get Music Album.
    E_MMSDK_CALLBACK_MUSIC_INCLUDE_ALBUM = 40,
    /// Report video decoder started to that the upper layer start to dequeue video input buffer in the SVP path
    E_MMSDK_CALLBACK_VIDEO_DECODER_START = 41,
...
///Define Callback Message Type Parameter structure: Music Album information
typedef struct
{
    /// Media item.
    MEDIAITEM Item;
    /// photo offset
    MMSDK_U32 u32JPGOffset;
    /// photo size
    MMSDK_U32 u32JPGSize;
}ST_MMSDK_MUSIC_ALBUM_INFO;
...
/// Define the information for texture frame buffer.
typedef struct
{
    /// Allocate Address. (Physical Address)
    MMSDK_U64 u64Address;
    ...
    /// Color Format.
    EN_MMSDK_COLOR_FORMAT enColorFormat;
    /// Is Physical buffer continuity
    MMSDK_U8 IsPhyContinuity;
} ST_MMSDK_TEXTURE_FRAMEBUFFER_INFO;

/// Define Callback Message Type Parameter structure: Texture Frame Buffer Information
typedef struct
{
    /// Media item.
    MEDIAITEM Item;
    /// Pointer to the structure of texture frame buffer information
    ST_MMSDK_TEXTURE_FRAMEBUFFER_INFO *pstTextureFrameBufferInfo;
} ST_MMSDK_CALLBACK_TEXTURE_FRAME_BUFFER_INFO;

/// Define the video texture mode
typedef enum
{
    /// HW support per-frame format convert
    E_MMSDK_VIDEO_TEXTURE_MODE_SINGLE = 0x00000001,
    /// HW support continuous frame capture onto continuous frame buffer
    E_MMSDK_VIDEO_TEXTURE_MODE_TRAVELING = 0x00000002,
    /// GPU support direct accessing
    E_MMSDK_VIDEO_TEXTURE_MODE_DIRECT_ACCESSING = 0x00000004,
} EN_MMSDK_VIDEO_TEXTURE_MODE;

/// Define the structure of video texture mode capability info; FIXME: the capability maybe have more than one setting
typedef struct
{
    /// The video texure mode to query
    EN_MMSDK_VIDEO_TEXTURE_MODE enVideoTextureMode;
    /// video texure mode capabitlity color formats
    EN_MMSDK_COLOR_FORMAT enVideoTextureColorFormat;
    /// video texure capability reserved 1
    MMSDK_U32 u32Reserved1;
    /// video texure capability reserved 2
    MMSDK_U32 u32Reserved2;
    /// video texure capability MAX width
    MMSDK_U16 u16VideoTextureCapMaxWidth;
    /// video texure capability MAX height
    MMSDK_U16 u16VideoTextureCapMaxHeight;
    /// video texure capability MIN width
    MMSDK_U16 u16VideoTextureCapMinWidth;
    /// video texure capability MIN height
    MMSDK_U16 u16VideoTextureCapMinHeight;
    /// video texure capability frame rate
    MMSDK_U8 u8VideoTextureCapFrameRate;
    /// support video texure capability rotation
    MMSDK_BOOL bSupportVideoTextureRotation;
} ST_MMSDK_VIDEO_TEXTURE_CAP_INFO;

/// Define the structure of video texture mode preperty
typedef struct
{
    /// The video texure mode to query
    EN_MMSDK_VIDEO_TEXTURE_MODE enVideoTextureMode;
    /// video texure mode color formats
    EN_MMSDK_COLOR_FORMAT enColorFormat;
    /// video texure reserved 1
    MMSDK_U32 u32Reserved1;
    /// video texure reserved 2
    MMSDK_U32 u32Reserved2;
    /// video texure width
    MMSDK_U16 u16Width;
    /// video texure height
    MMSDK_U16 u16Height;
    /// video texure frame rate
    MMSDK_U8 u8VideoFrameRate;
    /// video texure rotaion
    EN_MMSDK_ROTATE_ANGLE eVideoTextureRotation;
} ST_MMSDK_VIDEO_TEXTURE_PROPERTY;

/// Define the information for video texture mode traveling buffer information.
typedef struct
{
    /// Allocate Address. (Bus Address)
    MMSDK_U64 u64Address;
    /// Allocate Size.
    MMSDK_U32 u32Size;
    /// Color Format.
    EN_MMSDK_COLOR_FORMAT enColorFormat;
    /// Buffer count (output)
    MMSDK_U8 u8BufferCount;
    /// Buffer Bus Address (output)
    MMSDK_U64 au64TravelingBufferAddress[MMSDK_MAX_TRAVELING_BUFFER_COUNT];
} ST_MMSDK_TRAVELING_BUFFER_INFO;
...
typedef enum
{
    ...
    /// encrypt type
    E_MMSDK_TEE_ENCRYPT_TYPE_NSTV_DRM     = 0x00010000,
    /// encrypt type is shared memory mode
    E_MMSDK_TEE_ENCRYPT_CTL_FLAG_SHM_MODE   = 0x00000040,
...
/// Define download module information
typedef enum
{
    /// Probe size for av_find_stream_info
    E_MMSDK_DOWNLOADMODULE_PROBESIZE,
    /// Duration size for av_find_stream_info
    E_MMSDK_DOWNLOADMODULE_MAX_ANALYZE_DURATION,
    /// Check Info is set
    E_MMSDK_DOWNLOADMODULE_ENABLE_SET_DOWNLOADMODULE_INFO,
    /// Enable rety mode, only for hls protocol
    E_MMSDK_DOWNLOADMODULE_ENABLE_HLS_RETRY_MODE,
    /// Probe size for "hls meida file" av_find_stream_info(), only for hls protocol
    E_MMSDK_DOWNLOADMODULE_HLS_PROBESIZE,
    /// Duration size for "hls medie file" av_find_stream_info(), only for hls protocol
    E_MMSDK_DOWNLOADMODULE_HLS_MAX_ANALYZE_DURATION,
    /// Timeout value, set by the caller before "hls medie file" avformat_open_input(), only for hls protocol
    E_MMSDK_DOWNLOADMODULE_HLS_AVOPEN_TIMEOUT_VALUE,
} EN_MMSDK_DOWNLOADMODULE_INFO;
...
{
    /// DS mode NONE
    E_MMSDK_DS_MODE_NONE = 0,
    /// DS mode normal
    E_MMSDK_DS_MODE_NORMAL,
    /// DS mode, force I and orignal I
    E_MMSDK_DS_MODE_FORCE_I_ORG_I,
    /// DS mode, force I and orignal P
    E_MMSDK_DS_MODE_FORCE_I_ORG_P,
    /// DS mode, force P and orignal I
    E_MMSDK_DS_MODE_FORCE_P_ORG_I,
    /// DS mode, force P and orignal P
    E_MMSDK_DS_MODE_FORCE_P_ORG_P,
    /// DS mode number
    E_MMSDK_DS_MODE_NUM,
} EN_MMSDK_DS_MODE;
