...
#define MAX_SUPPORTED_PARTITION_COUNT 8
...
struct  cyg_disk_partition_t
{
    ...
    // wyc.rel-090814, for format to write partition id to record partition sector
    cyg_uint16 poffset; // offset in partition table
    cyg_uint32 start;   // first sector number
    cyg_uint32 end;     // last sector number
    cyg_uint32 size;    // size in sectors
    cyg_uint32 psector; // sector num of partition table
};

struct  cyg_disk_info_t
{
    cyg_disk_identify_t         ident;         // identify data
    cyg_disk_partition_t        partitions[MBR_PART_NUM_MAX]; // partitions
    cyg_addrword_t              devs[MBR_PART_NUM_MAX+ 1];       // device instances
    ...
    struct cyg_devtab_entry     devtab[MBR_PART_NUM_MAX];     // Device table entries for partitions
    disk_channel                diskchan[MBR_PART_NUM_MAX];   // Disk channels for partitions
};
