...
#define AUDIO_DO_ALIGNMENT(Value, AlignmentSize) \
    ...
    #define AUDIO_PRINT(fmt, args...)    LOGI("<<android>>      " fmt, ## args)
    #define AUDIO_ERROR(fmt, args...)    LOGE("<<android>>      " fmt, ## args)
#else
    #define AUDIO_PRINT(fmt, args...)    //printf("[[utopia]]      " fmt, ## args)
    #define AUDIO_ERROR(fmt, args...)    printf("[[utopia]][audio]      " fmt, ## args)
    #define AUDIO_DBG(_f, _a...)         //printf("\033[1;35m[[utopia]][audio]"_f"\033[0m", ##_a)
#endif


#define AUDIO_CHECK_SHM_IF_INVALID_THEN_RETURN_ERROR_VALUE(bRet) \
    do { \
        if (g_AudioVars2 == NULL) \
        { \
            AUDIO_ERROR("%s() : Warning! g_AudioVars2 should not be NULL !!!\n", __FUNCTION__); \
            AUDIO_ERROR("%s() : Perform SHM Init here !!!\n", __FUNCTION__); \
            MDrv_AUDIO_SHM_Init(); \
            if (g_AudioVars2 == NULL) \
            { \
                return bRet; \
...
#define AUDIO_CHECK_SHM_IF_INVALID_THEN_RETURN \
    do { \
        if (g_AudioVars2 == NULL) \
        { \
            AUDIO_ERROR("%s() : Warning! g_AudioVars2 should not be NULL !!!\n", __FUNCTION__); \
...
// Define
//=====================================================
#define AUDIO_DRIVER_VERSION_MAJOR    2 /* API change or new feature */
#define AUDIO_DRIVER_VERSION_MINOR    2 /* defect */
#define AUDIO_DRIVER_VERSION_REVISION    3 /* refine */
