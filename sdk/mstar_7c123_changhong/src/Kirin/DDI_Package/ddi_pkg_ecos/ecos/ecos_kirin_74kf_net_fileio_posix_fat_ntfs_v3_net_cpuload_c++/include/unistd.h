...
#if (__GNUC__ == 3) && (__GNUC_MINOR__ == 4) && (__GNUC_PATCHLEVEL__ == 4)
...
// Notice that These are UTF16 versions, not wchar_t versions
extern int  wrename( const utf16 *path1, const utf16 *path2 );
extern int 	wlink(const utf16 *path1, const utf16 *path2);
extern int 	wunlink(const utf16 *wpath);
extern int 	wrmdir(const utf16 *path);
extern int 	waccess(const utf16 *path, int amode);
extern int 	wchown(const utf16 *path, uid_t owner, uid_t group);
extern long wpathconf(const utf16 *path, int vname);
    
extern int 	wchdir(const utf16 *path);
extern utf16 	*wgetcwd(utf16 *buf, size_t size);

extern int 	wexecl(WCHAR *path,...);
extern int 	wexecv(WCHAR *path,char **argv);
extern int 	wexecle(WCHAR *path,...);
extern int 	wexeclp(WCHAR *file,...);
extern int 	wexecvp(WCHAR *file, char **argv);
extern int 	wexecve(WCHAR *name, char **argv, char **envv);
