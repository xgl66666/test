...
#define LDR_PUTS(val)
   ...
   (defined(CONFIG_LD_BFN_V3)) && (defined(CONFIG_MSTAR_KERES))
#include "drvNAND.h"
...
static MS_U8 pu8_SpareBuf[256]   __attribute__((aligned(16)));
...
static MS_U32 _NAND_ReadPage(MS_U8 *buf, MS_U32 pageNum, MS_U8 u8_RandCmdDis)
{
    MS_U32 i = 0, ret = 0;

    MS_U16 readSctrCnt = NandDrv.u16_PageSectorCnt;
    MS_U16 sctrSize = NandDrv.u16_SectorByteCnt;

    if(u8_RandCmdDis == 1)
    {
        //special case for bfn_miu.bin
        readSctrCnt = 1;
        sctrSize = 0x400;
    }

    for(i = 0 ; i<readSctrCnt; i++)
    {
        //DBGREG(0x2000 + i, 2);
        ret = NC_ReadSector(&NandDrv, //ret = NC_ReadSector_RIUMode(&NandDrv,
                pageNum,
                i,
                (U16*)(buf+i*sctrSize),
                pu8_SpareBuf,
...
{
    MS_U32 cnt =0, i = 0, max_redundancy = 256;
    MS_U32 ret = 0;
    ...
    nand_clock_setting(&NandDrv, NFIE_CLK_54M, BIT_NC_LATCH_DATA_1_5_T);

    for(cnt = 0; cnt < max_redundancy; cnt++)
    {
        //read page 0
        ret = _NAND_ReadPage(pu8_RamBuf, cnt*NandDrv.u16_BlkPageCnt, 1);
        //LDR_PUTX(cnt);
        //LDR_PUTC(':');
        //LDR_PUTC(pu8_RamBuf[0]);LDR_PUTC('\n');LDR_PUTC('\r');
        ...
        if(memcmp((const void *)(pu8_RamBuf), "MSTARSEMIUNFDCIS", 16) == 0)
        {
            LDR_PUTS("Hit CIS\n");
            WREG_DBG(DBG_F_LDR_HITCIS, 1);
            pNandInfo = (NAND_FLASH_INFO_t *)(pu8_RamBuf);
            ret = nand_set_param (&NandDrv,
            ...
            if(ret != 0)
            {
                ...
                WREG_DBG(DBG_E_LDR_SETPARA_FAIL, 2); 
                continue;
            }else
            {
                return 1;
...
MS_U32 BOOT_NAND_2nd(void)
{
    MS_U32 BINSize = 0, ret = 0, MIUPos = 0, BIN_Entry = 0;
    MS_U32 max_redundancy = 256;
    MS_U32 pageNum = 0;
    MS_U32 blk = 1;

    WREG_DBG(DBG_F_LDR_NAND_2ND, 1);
  
    for(; blk < max_redundancy; blk++ )
    {

        //read page 0 for each block
        pageNum = blk*NandDrv.u16_BlkPageCnt;
        ret =_NAND_ReadPage(pu8_RamBuf, pageNum, 0);
        ...
        //find APP tag
        if(memcmp((const void *)(pu8_RamBuf), "0NIB", 4) == 0)
        {
            LDR_PUTS("Hit BIN\n");
            WREG_DBG(DBG_F_LDR_HITCIS, 1);
            MIUPos = *((MS_U32*)(pu8_RamBuf+0x4));
            BINSize = *((MS_U32*)(pu8_RamBuf+0x14));
            BIN_Entry = *((MS_U32*)(pu8_RamBuf+0x24));
            ...
            while (BINSize)
            {
                ret = _NAND_ReadPage((MS_U8 *)MIUPos, pageNum++, 0);
                if(ret != 0)
                {
                    break;
                }

                MIUPos += NandDrv.u16_PageByteCnt;
                BINSize -= ((BINSize < NandDrv.u16_PageByteCnt) ? BINSize : NandDrv.u16_PageByteCnt);
            }

            /* old version
            MS_U16 u16OneBlkPageCnt = 0;
            while(BINSize)
            {
                _NAND_ReadPage((MS_U8 *)MIUPos, pageNum+u16OneBlkPageCnt, 0);

                if (u16OneBlkPageCnt==0)
                {
                    if (*((MS_U32*)(MIUPos)) != BINIDTAG)
                    {
                        pageNum += NandDrv.u16_BlkPageCnt;
                    }
                    else
                    {
                        u16OneBlkPageCnt++;
                ...
                MIUPos += NandDrv.u16_PageByteCnt;
                BINSize -= ((BINSize < NandDrv.u16_PageByteCnt)?BINSize:NandDrv.u16_PageByteCnt);
                if (u16OneBlkPageCnt==NandDrv.u16_BlkPageCnt)
                {
                    pageNum += NandDrv.u16_BlkPageCnt;
                    u16OneBlkPageCnt=0;
                }
            ...
            if(BINSize == 0) return BIN_Entry;
            else continue;
        }

    }

    if(blk == max_redundancy){
        LDR_PUTS("Hit border\n");
        WREG_DBG(DBG_E_LDR_HIT_BORDER, 2); 
        while(1){}
    }
...
void LDR_Entry_NAND(void)
{
    ...
    pfEntry = (PF_AP_Entry)BOOT_NAND_2nd(); //load APP bin to DRAM
...
#elif  (defined(CONFIG_LD_BFN_V2))&& ((!defined(CONFIG_MSTAR_KELTIC)) && (!defined(CONFIG_MSTAR_KAPPA)) && (!defined(CONFIG_MSTAR_KRITI)))
