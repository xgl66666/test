...
#ifdef    GUI_SPECIFIC
typedef enum _OWNERSHIP_TYPE   {
    OWNERSHIP_FREE = 0,
    OWNERSHIP_OWNER,
    OWNERSHIP_NOT_OWNER,
    OWNERSHIP_NOT_LOCKED,
    OWNERSHIP_ALREADY_OWNER,
    OWNERSHIP_LOCKED_BY_ANOTHER,
    OWNERSHIP_TIMEOUT,
    OWNERSHIP_ABANDONED,
    OWNERSHIP_CRITICAL,
    OWNERSHIP_MUTEX_RELEASE_ERROR,
    OWNERSHIP_DEFAULT
}  OWNERSHIP_TYPE;

typedef enum _COMMUNICATION_MODE   {
    REGISTER_COMMUNICATION = 0,
    COMMAND_COMMUNICATION,
    UNKNOWN_COMMUNICATION_MODE
} COMMUNICATION_MODE;
...
#endif /* LINUX_ST_SDK2_I2C */
#ifdef    GUI_SPECIFIC
    COMMUNICATION_MODE currentMode;
    char           *semaphoreName;
    signed   int    trackDebug;
    signed   int    COM_ownership;
    signed   int    COM_critical;
    signed   int    switchRegisterBytesCount;
    unsigned char   switchRegisterBytesBuffer[2];
    unsigned char  *switchRegisterBytes;
    signed   int    switchCommandBytesCount;
    unsigned char   switchCommandBytesBuffer[2];
    unsigned char  *switchCommandBytes;
#endif /* GUI_SPECIFIC */
} L0_Context;
...
typedef struct L1_Context {
  ...
  typedef struct _firmware_struct {
...
#endif /* GUI_SPECIFIC */
#ifdef    GUI_SPECIFIC
signed   int CurrentCommunicationMode      (L0_Context* i2c, char *rettxt);
signed   int L0_RegisterSwitchBytes        (L0_Context* i2c, signed   int iNbBytes, unsigned char *pucDataBuffer);
signed   int L0_CommandSwitchBytes         (L0_Context* i2c, signed   int iNbBytes, unsigned char *pucDataBuffer);
signed   int L0_CommandSwitchInfo          (L0_Context* i2c, unsigned char **ppBuffer);
signed   int L0_RegisterSwitchInfo         (L0_Context* i2c, unsigned char **ppBuffer);
signed   int L0_RegisterSwitchForce        (L0_Context* i2c);
signed   int L0_CommandSwitchForce         (L0_Context* i2c);
#endif /* GUI_SPECIFIC */
...
#ifdef    GUI_SPECIFIC
    signed   int     L0_Cypress_SPI_debug (void);
#endif /* GUI_SPECIFIC */
