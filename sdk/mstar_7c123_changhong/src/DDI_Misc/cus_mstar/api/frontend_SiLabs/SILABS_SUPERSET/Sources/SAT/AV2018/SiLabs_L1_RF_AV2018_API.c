...
/* This is an estimation of RSSI based on the IF_AGC (some dBm of error are possible according to the RF and the input level) */
  context = context; /* To avoid compiler warning while keeping a common prototype for all tuners */
/*int   AV2018_agc         [] = {     0,   115,   145,   160,   179,   195,   211,   255}; */
/*int   AV2018_level_dBm_10[] = {    90,  -288,  -387,  -487,  -587,  -682,  -738,  -890}; */
  /* VDAPPS-367: in case tuner reg[8]=0X3F then the IF_agc is reduced, so the tables must be updated */
  /*int   AV2018_agc         [] = {     0,  82,   100,  128,  170,  187,  210,  223,  254,  255};*/
  /*int   AV2018_level_dBm_10[] = {    90, -50,  -261, -461, -661, -761, -861, -891, -904, -910};*/
  /* VDAPPS-416: reg[3]_D7 is Frac<0>, D6~D0 is 0x54, For single RXIQ out (for AV2018), so the tables have been updated */
  int   AV2018_agc         [] = {     0,  82,   100,  116,  140,  162,  173,  187,  210,  223,  254,  255};
  int   AV2018_level_dBm_10[] = {    90, -50,  -263, -361, -463, -563, -661, -761, -861, -891, -904, -910};
  int   index;
  int   table_length;
  int  *x;
  int  *y;
  int   slope;
  SiTRACE("L1_RF_AV2018_RSSI if_agc %d\n",if_agc);
  ...
  SiTRACE("L1_RF_AV2018_RSSI context->rssi %d dBm*10\n",context->rssi);
  return (int)(context->rssi);
}

char* L1_RF_AV2018_TAG_TEXT       (void) {
  /* Set here the version string for the tuner */
  return (char *)"AV2018 driver SVN5712";
}
