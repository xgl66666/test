...
mem_shared_struct   *COMtoken;
mem_shared_struct   *pModeShare;
...
void storeBytes(unsigned char **dest, int nbBytes, unsigned char *str, int *count) {
   int i;
   *dest = (unsigned char*) malloc(sizeof(unsigned char)*(nbBytes));
   for (i=0; i<nbBytes; i++) { (*dest)[i]= str[i]; }
   *count = nbBytes;
}
signed   int Emb_L0_CreateShare            (mem_shared_struct **ppShare, char *name) {
    /* shared memory area creation */
    if ((*ppShare = (mem_shared_struct*) malloc(sizeof(mem_shared_struct))) == NULL) {
        SiTRACE("unable to allocate the shared memory '%s'\n", name);
        return FALSE;
    }
    (*ppShare)->hMutex = CreateMutex(
    NULL,                       /* default security attributes */
    FALSE,                      /* initially not owned         */
    NULL);                      /* unnamed mutex               */

    if ((*ppShare)->hMutex == NULL) {
        SiTRACE("CreateMutex error for '%s' : %d\n", name, GetLastError());
    }
    (*ppShare)->hFileMapping = CreateFileMapping(INVALID_HANDLE_VALUE , NULL, PAGE_READWRITE, 0, (DWORD) 2000, (LPSTR)name);
    if ((*ppShare)->hFileMapping == NULL) {
        free((*ppShare)->name);
        free((*ppShare));
        SiTRACE ("'%s' RC_SHM_NOT_CREATED\n", name);
        return FALSE;
    }
    (*ppShare)->location = MapViewOfFile((*ppShare)->hFileMapping, FILE_MAP_READ | FILE_MAP_WRITE, 0, 0, 0);
    if ((*ppShare)->location == NULL) {
        CloseHandle((*ppShare)->hFileMapping);
        free((*ppShare)->name);
        free((*ppShare));
        SiTRACE ("'%s' RC_OBJECT_NOT_CREATED\n", name);
        return FALSE;
    }
    store(&((*ppShare)->name), name);
    return TRUE;
}
signed   int GetCommunicationOwnership     (L0_Context* i2c, HANDLE hMutex, const char *name) {
    DWORD dwWaitResult;
    double value;
    double *pValue;
    int w;
    if (i2c->COM_ownership == 1) {
      if (i2c->trackDebug) SiTRACE("GetCommunicationOwnership : 0x%02x already owning COM ownership...\n", i2c->address);
      return OWNERSHIP_ALREADY_OWNER;
    }
    value = 1;
    /* Request ownership of mutex. */
    dwWaitResult = WaitForSingleObject( hMutex, 20L);   /* 20 millisecond time-out interval */
    switch (dwWaitResult)     {
        /* The thread got mutex ownership. */
        case WAIT_OBJECT_0:
            pValue = (double*)COMtoken->location;
            if (*pValue == 0) {
                CopyMemory((double*)COMtoken->location, &value, sizeof(double));
                if (!ReleaseMutex(hMutex)) {
                    SiTRACE("\n ERROR when releasing %s mutex!\n",name);
                    return OWNERSHIP_MUTEX_RELEASE_ERROR;
                }
                i2c->COM_ownership = 1;
                if (i2c->trackDebug) SiTRACE("GetCommunicationOwnership 0x%02x i2c->COM_ownership = true\n", i2c->address);
                return OWNERSHIP_OWNER;
            } else {
                if (! ReleaseMutex(hMutex)) {
                    SiTRACE("\nGetCommunicationOwnership ERROR when releasing %s mutex!\n",name);
                }
                w =  ((clock()%4+1)*20);
                SiTRACE ("!");
                if (i2c->trackDebug) SiTRACE ("GetCommunicationOwnership %s is already in use, sorry 0x%02x!\n", name, i2c->address);
                system_wait(w);
                return OWNERSHIP_LOCKED_BY_ANOTHER;
            }
        /* Cannot get mutex ownership due to time-out. */
        case WAIT_TIMEOUT:
            SiTRACE("\n WAIT_TIMEOUT when taking %s mutex!\n",name);
            return OWNERSHIP_TIMEOUT;
        /* Got ownership of the abandoned mutex object. */
        case WAIT_ABANDONED:
            SiTRACE("\n WAIT_ABANDONED when taking %s mutex!\n",name);
            return OWNERSHIP_ABANDONED;
        default :
          return OWNERSHIP_DEFAULT;
    }
}
signed   int UseCommunicationMode          (L0_Context* i2c, COMMUNICATION_MODE mode  ) {
   DWORD dwWaitResult;
   int    index;
   int    ret;
   double value;
   double *pValue;
   int memIndexSize;
   memIndexSize = i2c->indexSize;
   value = mode;
   index = (int)i2c->address;
   if (index == 0x00) {
    SiERROR("UseCommunicationMode called with add 0x00 (invalid i2c address) !!\n");
    return UNKNOWN_COMMUNICATION_MODE;
   }
   /* Request ownership of mutex. */
    dwWaitResult = WaitForSingleObject( pModeShare->hMutex, 5000L);   /* five-second time-out interval */
    switch (dwWaitResult) {
        /* The thread got mutex ownership. */
        case WAIT_OBJECT_0:
            ret = mode;
            pValue = (double*)pModeShare->location + index;
            if (*pValue == value) {
               switch (mode) {
                   case COMMAND_COMMUNICATION:
                     if (i2c->trackDebug) {SiTRACE("0x%02x already in COMMAND mode\n", i2c->address);}
                     break;
                   case REGISTER_COMMUNICATION:
                     if (i2c->trackDebug) {SiTRACE("0x%02x already in REGISTER mode\n", i2c->address);}
                     break;
                   default:
                     SiTRACE("UseCommunicationMode: unknown CommunicationMode %d (same as current mode)\n",mode);
                     break;
               };
            } else {
               switch (mode) {
                   case COMMAND_COMMUNICATION:
                     SiTRACE("0x%02x needs to switch to COMMAND mode\n", i2c->address);
                     if (i2c->switchCommandBytesCount > 0) {
                        i2c->indexSize = 0;
                        ret = L0_WriteBytes(i2c, 0x00, i2c->switchCommandBytesCount, i2c->switchCommandBytes);
                        i2c->indexSize = memIndexSize;
                     } else {ret = 0;}
                     if (ret == i2c->switchCommandBytesCount) {
                        CopyMemory((double*)pModeShare->location + index, &value, sizeof(double));
                        ret = COMMAND_COMMUNICATION;
                     } else {
                        ret = UNKNOWN_COMMUNICATION_MODE;
                     }
                     break;
                   case REGISTER_COMMUNICATION:
                     SiTRACE("0x%02x needs to switch to REGISTER mode\n", i2c->address);
                     if (i2c->switchRegisterBytesCount > 0) {
                        i2c->indexSize = 0;
                        ret = L0_WriteBytes(i2c, 0x00, i2c->switchRegisterBytesCount, i2c->switchRegisterBytes);
                        i2c->indexSize = memIndexSize;
                     } else {ret = 0;}
                     if (ret == i2c->switchRegisterBytesCount) {
                        CopyMemory((double*)pModeShare->location + index, &value, sizeof(double));
                        ret = COMMAND_COMMUNICATION;
                     } else {
                        ret = UNKNOWN_COMMUNICATION_MODE;
                     ...
                     break;
                   default:
                     SiTRACE("UseCommunicationMode: unknown CommunicationMode %d (different from current mode %d)\n",mode, *pValue);
                     ret = UNKNOWN_COMMUNICATION_MODE;
                     break;
               };
            }
            /* Release ownership of the mutex object. */
            if (! ReleaseMutex(pModeShare->hMutex)) {SiTRACE("\n ERROR when releasing %s mutex in UseCommunicationMode %d!\n", pModeShare->name, index);}
            return ret;
            break;
            /* Cannot get mutex ownership due to time-out. */
        case WAIT_TIMEOUT:
            SiTRACE("\n WAIT_TIMEOUT when releasing %s mutex in UseCommunicationMode %d!\n", pModeShare->name, index);
            return FALSE;
            /* Got ownership of the abandoned mutex object. */
       case WAIT_ABANDONED:
            SiTRACE("\n WAIT_ABANDONED when releasing %s mutex in UseCommunicationMode %d!\n", pModeShare->name, index);
            return FALSE;
        default:
            SiTRACE("UseCommunicationMode dwWaitResult 'default' = 0x%x\n", dwWaitResult);
            return FALSE;
    }
   return UNKNOWN_COMMUNICATION_MODE;
}
signed   int ForceCommunicationMode        (L0_Context* i2c, COMMUNICATION_MODE mode  ) {
   DWORD dwWaitResult;
   int    index;
   int    ret;
   double value;
   ...
   /* Request ownership of mutex. */
    ...
    switch (dwWaitResult) {
        ...
        case WAIT_OBJECT_0:
            pValue = (double*)pModeShare->location + index;
            if (i2c->trackDebug) {SiTRACE("mode %d\n", mode);}
            switch (mode) {
                case COMMAND_COMMUNICATION:
                  if (i2c->trackDebug) {SiTRACE("forcing switch to COMMAND mode\n");}
                  if (i2c->switchCommandBytesCount > 0) {
                     i2c->indexSize = 0;
                     if (i2c->trackDebug) {SiTRACE("writing %d COMMAND bytes\n",i2c->switchCommandBytesCount);}
                     ret = L0_WriteBytes(i2c, 0x00, i2c->switchCommandBytesCount, i2c->switchCommandBytes);
                     i2c->indexSize = memIndexSize;
                  } else {ret = 0;}
                  if (ret == i2c->switchCommandBytesCount) {
                  ...
                  break;
                case REGISTER_COMMUNICATION:
                  if (i2c->trackDebug) {SiTRACE("forcing switch to REGISTER mode\n");}
                  if (i2c->switchRegisterBytesCount > 0) {
                     i2c->indexSize = 0;
                     if (i2c->trackDebug) {SiTRACE("writing %d REGISTER bytes\n",i2c->switchRegisterBytes);}
                     ret = L0_WriteBytes(i2c, 0x00, i2c->switchRegisterBytesCount, i2c->switchRegisterBytes);
                     i2c->indexSize = memIndexSize;
                  } else {ret = 0;}
                  if (ret == i2c->switchRegisterBytesCount) {
   ...
   /* Request ownership of mutex. */
    ...
    switch (dwWaitResult) {
        ...
        case WAIT_OBJECT_0:
            ...
            mode = (int)(*pValue);
            ret  = mode;
            if (i2c->trackDebug) SiTRACE("CurrentCommunicationMode: mode = %d\n", mode);
            switch (mode) {
                case COMMAND_COMMUNICATION : sprintf(rettxt, "Command Mode" ); break;
                case REGISTER_COMMUNICATION: sprintf(rettxt, "Register Mode"); break;
                default                    : sprintf(rettxt, "UNKNOWN Mode" ); ret  = UNKNOWN_COMMUNICATION_MODE; break;
            };
            /* Release ownership of the mutex object. */
            if (! ReleaseMutex(pModeShare->hMutex)) {SiTRACE("\n ERROR when releasing %s mutex in UseCommunicationMode %d!\n", pModeShare->name, index);}
            return ret;
...
signed   int ReleaseCommunicationOwnership (L0_Context* i2c, HANDLE hMutex, const char *name) {
   if (i2c->COM_critical == 1 ) {
      if (i2c->trackDebug) SiTRACE("ReleaseCommunicationOwnership: communication critical. Must NOT release COM ownership...\n");
      return OWNERSHIP_CRITICAL;
   }
   if (i2c->COM_ownership == 0) {
      if (i2c->trackDebug) SiTRACE("ReleaseCommunicationOwnership: not owning COM ownership...\n");
      return OWNERSHIP_NOT_OWNER;
   }
    DWORD dwWaitResult;
    double value;
    double *pValue;
    value = 0;
    /* Request ownership of mutex. */
    dwWaitResult = WaitForSingleObject( hMutex, 500L);   /* 500 millisecond time-out interval */
    switch (dwWaitResult)     {
        /* The thread got mutex ownership. */
        case WAIT_OBJECT_0:
            pValue = (double*)COMtoken->location;
            if (*pValue == 1) {
                ...
                i2c->COM_ownership = 0;
                if (i2c->trackDebug) SiTRACE("ReleaseCommunicationOwnership i2c->COM_ownership = false\n");
                return OWNERSHIP_FREE;
            } else {
                SiTRACE ("?");
                if (! ReleaseMutex(hMutex)) {
                  SiTRACE("\n ERROR when releasing %s mutex!\n",name);
                }
               return OWNERSHIP_NOT_LOCKED;
            }
        /* Cannot get mutex ownership due to time-out. */
        case WAIT_TIMEOUT:
            SiTRACE("\n WAIT_TIMEOUT when releasing %s mutex!\n",name);
            return OWNERSHIP_TIMEOUT;
        /* Got ownership of the abandoned mutex object. */
        case WAIT_ABANDONED:
            SiTRACE("\n WAIT_ABANDONED when releasing %s mutex!\n",name);
            return OWNERSHIP_ABANDONED;
         default :
            return OWNERSHIP_DEFAULT;
    }
}
signed   int L0_RegisterSwitchBytes        (L0_Context* i2c, int iNbBytes, unsigned char *pucDataBuffer) {
   int i;
   if (i2c->trackDebug) {
      SiTRACE("old switchRegisterBytes : ");
      for (i = 0; i < i2c->switchRegisterBytesCount; i++) {SiTRACE ("0x%02x ",i2c->switchRegisterBytes[i]);}
      SiTRACE("\n");
   }

   storeBytes(&i2c->switchRegisterBytes, iNbBytes, pucDataBuffer, &i2c->switchRegisterBytesCount);

   if (i2c->trackDebug) {
      SiTRACE("new switchRegisterBytes : ");
      for (i = 0; i < i2c->switchRegisterBytesCount; i++) {SiTRACE ("0x%02x ",i2c->switchRegisterBytes[i]);}
      SiTRACE("\n");
   }
  return 1;
}
signed   int L0_CommandSwitchBytes         (L0_Context* i2c, int iNbBytes, unsigned char *pucDataBuffer) {
   int i;
   if (i2c->trackDebug) {
      SiTRACE("old switchCommandBytes : ");
      for (i = 0; i < i2c->switchCommandBytesCount; i++) {SiTRACE ("0x%02x ",i2c->switchCommandBytes[i]);}
      SiTRACE("\n");
   }

   storeBytes(&i2c->switchCommandBytes, iNbBytes, pucDataBuffer, &i2c->switchCommandBytesCount);

   if (i2c->trackDebug) {
      SiTRACE("new switchCommandBytes : ");
      for (i = 0; i < i2c->switchCommandBytesCount; i++) {SiTRACE ("0x%02x ",i2c->switchCommandBytes[i]);}
      SiTRACE("\n");
   }
...
int L0_RegisterSwitchInfo         (L0_Context* i2c, unsigned char **ppBuffer) {
   int i;
   *ppBuffer = (unsigned char*)realloc(*ppBuffer, sizeof(unsigned char)*i2c->switchRegisterBytesCount);
   for (i = 0; i < i2c->switchRegisterBytesCount; i++) {
      (*ppBuffer)[i] = i2c->switchRegisterBytes[i];
   }
   return i2c->switchRegisterBytesCount;
}
int L0_CommandSwitchInfo          (L0_Context* i2c, unsigned char **ppBuffer) {
   int i;
   *ppBuffer = (unsigned char*)realloc(*ppBuffer, sizeof(unsigned char)*i2c->switchCommandBytesCount);
   for (i = 0; i < i2c->switchCommandBytesCount; i++) {
      (*ppBuffer)[i] = i2c->switchCommandBytes[i];
   }
   return i2c->switchCommandBytesCount;
}
int L0_RegisterSwitchForce        (L0_Context* i2c) {
   int ret;
   int waitcount;
   waitcount = 0;
   ret = 0;
   while (waitcount++ < 10) {
      if (i2c->COM_ownership) break;
      GetCommunicationOwnership(i2c, COMMutex, i2c->semaphoreName);
   }
   if (i2c->COM_ownership) {
      ForceCommunicationMode(i2c,REGISTER_COMMUNICATION);
      ReleaseCommunicationOwnership(i2c, COMMutex, i2c->semaphoreName);
      ret = 1;
  }
  return ret;
}
int L0_CommandSwitchForce         (L0_Context* i2c) {
   int ret;
   int waitcount;
   waitcount = 0;
   ret = 0;
   ...
   if (i2c->COM_ownership) {
      ForceCommunicationMode(i2c,COMMAND_COMMUNICATION);
      ReleaseCommunicationOwnership(i2c, COMMutex, i2c->semaphoreName);
      ret = 1;
  }
  return ret;
...
#endif /* SiTRACES */
#ifdef    GUI_SPECIFIC
  i2c->semaphoreName    = (char*) malloc(20*sizeof(char));
  sprintf(i2c->semaphoreName,"COM_W1");
  i2c->COM_critical     = 0;
  i2c->COM_ownership    = 0;
  i2c->currentMode = UNKNOWN_COMMUNICATION_MODE;

  COMMutex = CreateMutex(
  NULL,                       /* default security attributes */
  FALSE,                      /* initially not owned         */
  NULL);                      /* unnamed mutex               */


  if (COMMutex == NULL) {SiTRACE("CreateMutex error for COMMutex : %d\n", GetLastError());}

  if ((COMtoken = (mem_shared_struct*) malloc(sizeof(mem_shared_struct))) == NULL) {
      SiTRACE("unable to allocate the shared memory 'COMtoken'\n");
  }
      COMtoken->hFileMapping = CreateFileMapping(INVALID_HANDLE_VALUE , NULL, PAGE_READWRITE, 0, (DWORD) 2000, (LPSTR) i2c->semaphoreName);
  if (COMtoken->hFileMapping == NULL) {
      free(COMtoken);
      SiTRACE ("'%s' RC_SHM_NOT_CREATED\n",i2c->semaphoreName);
  }

  COMtoken->location = MapViewOfFile(COMtoken->hFileMapping, FILE_MAP_READ | FILE_MAP_WRITE, 0, 0, 0);

  if (COMtoken->hFileMapping  != NULL && GetLastError() == ERROR_ALREADY_EXISTS) {
/*        SiTRACE("'%s' already exists\n", (*ppContext)->semaphoreName);*/
  } else {
/*        SiTRACE("'%s' has just been created\n", (*ppContext)->semaphoreName);*/
  }

  if (COMtoken->location == NULL) {
      CloseHandle(COMtoken->hFileMapping);
      free(COMtoken);
      SiTRACE ("'%s' RC_OBJECT_NOT_CREATED\n",i2c->semaphoreName);
  }

  if (Emb_L0_CreateShare(&pModeShare, "L0_MODE")) {
      if (GetLastError() == ERROR_ALREADY_EXISTS) {
/*            SiTRACE("'L0_MODE' ALREADY_EXISTS\n");*/
      } else {
/*            SiTRACE("'L0_MODE' has just been created\n");*/
      }
  } else {
    SiTRACE("'L0_MODE' shared could not be created!\n");
    SiERROR("'L0_MODE' shared could not be created!\n");
    exit(0);
  }
...
int     L0_SetAddress        (L0_Context* i2c, unsigned int add, int addSize) {
  ...
  unsigned char reg_bytes[2];
  cmd_bytes[0] = 0xfe;  cmd_bytes[1] = 0x00;
  reg_bytes[0] = 0xff;  reg_bytes[1] = 0x00;
  if ((add >= 0xc8) && (add <= 0xce)) {
    L0_RegisterSwitchBytes (i2c, 2, reg_bytes);
    L0_CommandSwitchBytes  (i2c, 2, cmd_bytes);
  }
#endif /* GUI_SPECIFIC */
...
int     L0_ReadBytes         (L0_Context* i2c, unsigned int iI2CIndex, int iNbBytes, unsigned char *pucDataBuffer) {
  ...
  switch (i2c->connectionType) {
    ...
    case CUSTOMER:
      ...
      */
      if(MDrv_IIC_ReadBytes(getI2CPort(0), i2c->address, i2c->indexSize, pucAddressBuffer, iNbBytes, pucDataBuffer))
      {
         //printf("IIC R OK, slave ID = 0x%02x\n", i2c->address);
         nbReadBytes = iNbBytes;
      }
      else
      {
         printf("IIC R Error, slave ID = 0x%02x\n", i2c->address);
      }
      break;
    case SIMU:
...
int     L0_WriteBytes        (L0_Context* i2c, unsigned int iI2CIndex, int iNbBytes, unsigned char *pucDataBuffer) {
  ...
  if (i2c->COM_ownership == 0) {
    SiERROR("\n\nL0_WriteBytes couldn't get COM_ownership!!!\n\n");
    return 0;
  }
  if (i2c->COM_ownership == 1) {
    if (i2c->indexSize > 0) { UseCommunicationMode(i2c,REGISTER_COMMUNICATION); }
  }
#endif /* GUI_SPECIFIC */
    ...
    case CUSTOMER:
      nbWrittenBytes = 0;
        ...
        */
        ...
        if(MDrv_IIC_WriteBytes(getI2CPort(0), i2c->address, i2c->indexSize, pucAddressBuffer, iNbBytes, pucDataBuffer))
        {
           //printf("IIC W OK\n");
           nbWrittenBytes = iNbBytes + i2c->indexSize;
        }
        ...
        {
           printf("IIC W Error, slave ID = 0x%02x\n", i2c->address);
           write_error++;
        }
...
#endif /* SiTRACES */
#ifdef    GUI_SPECIFIC
  if (i2c->COM_ownership == 1) { ReleaseCommunicationOwnership(i2c, COMMutex, i2c->semaphoreName); }
#endif /* GUI_SPECIFIC */
...
int     L0_ReadCommandBytes  (L0_Context* i2c, int iNbBytes, unsigned char *pucDataBuffer) {
  ...
  if (i2c->COM_ownership == 0) { return 0; }
  UseCommunicationMode(i2c,COMMAND_COMMUNICATION);
#endif /* GUI_SPECIFIC */
...
#ifdef    GUI_SPECIFIC
  rawI2C->semaphoreName  = i2c->semaphoreName;
  rawI2C->COM_ownership  = i2c->COM_ownership;
#endif /* GUI_SPECIFIC */
  if (i2c->indexSize >0) {
    for (i=0;i<i2c->indexSize;i++) {
      r = 8*(i2c->indexSize -1 -i);
...
long    L0_ReadRegister      (L0_Context *i2c, unsigned int iI2CIndex, unsigned char offset, unsigned char nbbit, unsigned int isSigned) {
  ...
  if (i2c->COM_ownership == 0) {
    SiERROR("L0_ReadRegister: Could not get COM_ownership\n\n");
    return 0;
  }
  UseCommunicationMode(i2c,REGISTER_COMMUNICATION);
#endif /* GUI_SPECIFIC */
...
long    L0_ReadRegisterTrace (L0_Context *i2c, char* name, unsigned int iI2CIndex, unsigned char offset, unsigned char nbbit, unsigned int isSigned) {
  ...
  __attribute__((unused)) unsigned int iNbBytes;
#ifdef    SiTRACES
  trace_skip_info = 1;
  iNbBytes = (nbbit + offset + 7)/8;
...
const char*   Si_I2C_TAG_TEXT      (void) { return (char *)"V5.0.3"; }
/************************************************************************************************************************
  L0_FastI2C  function
...
#ifdef    GUI_SPECIFIC
int     L0_Cypress_SPI_debug (void) {
#ifdef   USB_Capability
  #ifdef   CYPRESS_SPI_Capability
  unsigned char EP0BUF[30];
  int len;
  len = 30;
  Cypress_USB_VendorRead      (0xDE, 0, 0x90, EP0BUF, &len);
  if (len) {
  SiTRACE ("GPIFTRIG           0x%02x\n", EP0BUF [ 0]);
  SiTRACE ("GPIFADRH:L         0x%02x%02x\n", EP0BUF [ 1], EP0BUF [ 2] );
  SiTRACE ("EP6GPIFFLGSEL      0x%02x\n", EP0BUF [ 3]);
  SiTRACE ("GPIFREADYCFG       0x%02x\n", EP0BUF [ 4]);
  SiTRACE ("GPIFCTLCFG         0x%02x\n", EP0BUF [ 5]);
  SiTRACE ("GPIFIDLECS         0x%02x (GPIFDONE %d)\n", EP0BUF [ 6], (EP0BUF [ 6])>>7&0x01 );
  SiTRACE ("GPIFIDLECTL        0x%02x\n", EP0BUF [ 7]);
  SiTRACE ("GPIFWFSELECT       0x%02x\n", EP0BUF [ 8]);
  SiTRACE ("GPIFREADYSTAT      0x%02x\n", EP0BUF [ 9]);
  SiTRACE ("EP2GPIFFLGSEL      0x%02x\n", EP0BUF [10]);
  SiTRACE ("EP2GPIFPFSTOP      0x%02x\n", EP0BUF [11]);
  SiTRACE ("EP2GPIFTRIG        0x%02x\n", EP0BUF [12]);
  SiTRACE ("XGPIFSGLDATLX      0x%02x\n", EP0BUF [13]);
  SiTRACE ("XGPIFSGLDATLNOX    0x%02x\n", EP0BUF [14]);
  SiTRACE ("GPIFABORT          0x%02x\n", EP0BUF [15]);
  SiTRACE ("EP2FIFOCFG         0x%02x \n",EP0BUF [16]);
  SiTRACE ("EP2FIFOFLGS        0x%02x\n", EP0BUF [17]);
  SiTRACE ("EP2FIFOBCH:L       0x%02x 0x%02x (%d)\n", EP0BUF [18], EP0BUF [19], (EP0BUF [18]<<8) + EP0BUF [19] );
  SiTRACE ("GPIFTCB      0x%02x%02x%02x%02x (%d)\n", EP0BUF [20], EP0BUF [21], EP0BUF [22], EP0BUF [23], (EP0BUF [20]<<24) + (EP0BUF [21]<<16) + (EP0BUF [22]<<8) + EP0BUF [23]);
  SiTRACE ("IFCONFIG           0x%02x\n", EP0BUF [24]);
  SiTRACE ("EP2CS              0x%02x (NPAK %d, FULL %d, EMPTY %d, STALL %d)\n", EP0BUF [25], (EP0BUF[25]>>4)&0x7, (EP0BUF[25]>>3)&0x1, (EP0BUF[25]>>2)&0x1, (EP0BUF[25]>>0)&0x1 );
  SiTRACE ("EP6CS              0x%02x (NPAK %d, FULL %d, EMPTY %d, STALL %d)\n", EP0BUF [26], (EP0BUF[26]>>4)&0x7, (EP0BUF[26]>>3)&0x1, (EP0BUF[26]>>2)&0x1, (EP0BUF[26]>>0)&0x1 );
  SiTRACE ("EP2BCH:L           0x%02x 0x%02x (%d)\n", EP0BUF [27], EP0BUF [28], (EP0BUF [27]<<8) + EP0BUF [28] );
  } else {
    SiTRACE("Couldn't read the GPIF registers\n");
  }
  #endif /* CYPRESS_SPI_Capability */
#endif /* USB_Capability */
  return 1;
