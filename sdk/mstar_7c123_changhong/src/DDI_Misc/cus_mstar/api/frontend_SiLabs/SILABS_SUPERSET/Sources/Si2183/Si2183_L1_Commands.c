...
/* Change log:
 ...
 As from V0.2.1.0:
  <new_feature>[DUALS/TS_Bonding] Upgrading Si2183_L1_DD_TS_PINS to allow channel bonding
...
*/
        ...
        if ( pByteBuffer[1] == 0x10) { SiTRACE("BAD_BOOTMODE\n"); }
        if ( pByteBuffer[1] == 0x10) { SiTRACE("BAD_PATCH\n");    }
        if ( pByteBuffer[1] == 0x10) { SiTRACE("BAD_NVM\n");      }
  #endif /* SiTRACES */
      }
                                                   ...
                                                   unsigned long   master_freq)
{
    ...
    if ((primary_ts_mode       > Si2183_DD_TS_PINS_CMD_PRIMARY_TS_MODE_MAX      ) ) {error_code++; SiTRACE("\nOut of range: ");};
      if (primary_ts_mode   == Si2183_DD_TS_PINS_CMD_PRIMARY_TS_MODE_DRIVE_TS   ) { SiTRACE("PRIMARY_TS_MODE    DRIVE_TS  " ); }
      if (primary_ts_mode   == Si2183_DD_TS_PINS_CMD_PRIMARY_TS_MODE_NOT_USED   ) { SiTRACE("PRIMARY_TS_MODE    NOT_USED  " ); }
      if (primary_ts_mode   == Si2183_DD_TS_PINS_CMD_PRIMARY_TS_MODE_NO_CHANGE  ) { SiTRACE("PRIMARY_TS_MODE    NO_CHANGE " ); }
    ...
    if ((primary_ts_dir        > Si2183_DD_TS_PINS_CMD_PRIMARY_TS_DIR_MAX       ) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("PRIMARY_TS_DIR %d "       , primary_ts_dir        );
    if ((secondary_ts_mode     > Si2183_DD_TS_PINS_CMD_SECONDARY_TS_MODE_MAX    ) ) {error_code++; SiTRACE("\nOut of range: ");};
      if (secondary_ts_mode == Si2183_DD_TS_PINS_CMD_SECONDARY_TS_MODE_DRIVE_TS ) { SiTRACE("SECONDARY_TS_MODE  DRIVE_TS  " ); }
      if (secondary_ts_mode == Si2183_DD_TS_PINS_CMD_SECONDARY_TS_MODE_NOT_USED ) { SiTRACE("SECONDARY_TS_MODE  NOT_USED  " ); }
      if (secondary_ts_mode == Si2183_DD_TS_PINS_CMD_SECONDARY_TS_MODE_NO_CHANGE) { SiTRACE("SECONDARY_TS_MODE  NO_CHANGE " ); }
    ...
    if ((secondary_ts_dir      > Si2183_DD_TS_PINS_CMD_SECONDARY_TS_DIR_MAX     ) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("SECONDARY_TS_DIR %d "     , secondary_ts_dir      );
    if ((demod_role            > Si2183_DD_TS_PINS_CMD_DEMOD_ROLE_MAX           ) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("DEMOD_ROLE %d "           , demod_role            );
    if ((master_freq           > Si2183_DD_TS_PINS_CMD_MASTER_FREQ_MAX          ) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("MASTER_FREQ %d "          , master_freq           );
    if (error_code) {
      SiTRACE("%d out of range parameters\n", error_code);
      return ERROR_Si2183_PARAMETER_OUT_OF_RANGE;
    }
                                                   ...
                                                   unsigned char   wake_up)
{
    ...
    if (reset == Si2183_POWER_UP_CMD_RESET_RESET) {
      Si2183_pollForResponse(api, 1, rspByteBuffer);
    }
