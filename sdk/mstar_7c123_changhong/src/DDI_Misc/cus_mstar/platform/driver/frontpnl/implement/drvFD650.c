...
void MDrv_FD650_Display(char *FPStr, MS_U8 sec_flag)
{
    ...
    FD650_Write(FD650_SYSON_8);// ¿ªÆôÏÔÊ¾ºÍ¼üÅÌ£¬8¶ÎÏÔÊ¾·½Ê½
    //·¢ÏÔÊ¾Êý¾Ý
    FD650_Write( FD650_DIG0 | (MS_U8)data[0] ); //µãÁÁµÚÒ»¸öÊýÂë¹Ü

    if(sec_flag)
        FD650_Write( FD650_DIG1 | (MS_U8)data[1] | FD650_DOT ); //µãÁÁµÚ¶þ¸öÊýÂë¹Ü
    else
        FD650_Write( FD650_DIG1 | (MS_U8)data[1] );

    FD650_Write( FD650_DIG2 | (MS_U8)data[2]|FD650_DOT); //µãÁÁµÚÈý¸öÊýÂë¹Ü

    FD650_Write( FD650_DIG3 | (MS_U8)data[3] ); //µãÁÁµÚËÄ¸öÊýÂë¹Ü
...
void Mdrv_FD650_LockLed_OnOff(MS_U8 bLightOn)
{
    ...
    FD650_Write(FD650_SYSON_8);// ¿ªÆôÏÔÊ¾ºÍ¼üÅÌ£¬8¶ÎÏÔÊ¾·½Ê½
    if(bLightOn)
        FD650_Write( FD650_DIG3 | sec_vel | FD650_DOT ); //µãÁÁµÚËÄ¸öÊýÂë¹Ü
    else
        FD650_Write( FD650_DIG3 | sec_vel  ); //µãÁÁµÚËÄ¸öÊýÂë¹Ü
