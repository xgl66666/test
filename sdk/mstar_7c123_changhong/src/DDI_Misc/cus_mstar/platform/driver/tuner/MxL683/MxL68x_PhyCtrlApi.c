                ...
                MXL68x_PWR_SUPPLY_E devPwrSupplyCfg)
{
  ...
  if (devContextPtr)
  {
    if (devContextPtr->driverStatus)
    {
      ...
      status = Ctrl_ReadRegister(devId, MPEG_CLK_PHASE_REG - 1, &regData1);  // High 16 bit 
      status |= Ctrl_ReadRegister(devId, MPEG_CLK_PHASE_REG, &regData);   // Low 16 bit 
      regData |= 0x03;  // Set Bit<1> and Bit<0> with 1 
      status |= Ctrl_WriteRegister(devId, MPEG_CLK_PHASE_REG - 1, regData1);   // High 16 bit 
      status |= Ctrl_WriteRegister(devId, MPEG_CLK_PHASE_REG, regData);   // Low 16 bit 

      // Perform Demod soft reset
      status |= Ctrl_CfgDevReset(devId, MXL68x_DEMOD_RESET);
  
      // Inquire MxL68x device type 
      status |= Ctrl_ReadRegister(devId, DMD_ID_VERSION_REG, &regData);
      ...
      status |= Ctrl_WriteRegister(devId, POWER_SUPPLY_REG, regData);        
    }
    else
		            ...
		            MXL_BOOL_E enableXtalCal)                                      
{
  ...
  if ((devContextPtr) && (xtalCap > 0) && (xtalCap < 32))
  {
    if (devContextPtr->driverStatus)
    {
      status |= Ctrl_ReadRegister(devId, XTAL_CFG_REG, &regData); 
      ...
      status |= Ctrl_WriteRegister(devId, XTAL_CFG_REG, regData);      
      regData = 0;

      if(MXL_ENABLE == enableXtalCal) // For Master Only
      {
        //Program Xtal Aplitude to a higher value
        status |= Ctrl_WriteRegister(devId, DFE_DIG_ANA_XTAL_TH, 0x0003);   
      }

      status |= Ctrl_ReadRegister(devId, XTAL_SHARING_REG, &regData); 
      ...
      status |= Ctrl_WriteRegister(devId, XTAL_SHARING_REG, regData);    

      if(MXL_ENABLE == enableXtalCal)
      {
        // Program Xtal external bias register 
        regData = 0;
        status |= Ctrl_ReadRegister(devId, DFE_XTAL_EXT_BIAS_REG, &regData); 
        regData = ((regData & 0x00C0) | 0x000A);  // Program Bit<5:0> with 0x0A 
        status |= Ctrl_WriteRegister(devId, DFE_XTAL_EXT_BIAS_REG, regData); 
      ...
      else
      {
        ...
        status |= Ctrl_WriteRegister(devId, DFE_XTAL_EXT_BIAS_REG, 0x008A); 
      }
      ...
      // XTAL calibration
      status |= Ctrl_WriteRegister(devId, XTAL_CALI_SET_REG, 0x0000); 
      status |= Ctrl_WriteRegister(devId, XTAL_CALI_SET_REG, 0x0001);	  
      // Add 1 ms waiting time 
      MxLWare68x_OEM_SleepInUs(1000);	  
      status |= Ctrl_WriteRegister(devId, XTAL_CALI_SET_REG, 0x0000);
    ...
    if (devContextPtr->driverStatus)
    {
      switch(deviceOpMode)
      {
        case MXL68x_TUNER_ONLY: 
          ...
          // Enable Tuner 
          status |= Ctrl_WriteRegister(devId, TUNER_ENABLE_REG, MXL_ENABLE);

          // START_TUNE = 0
          status |= Ctrl_WriteRegister(devId, START_TUNE_REG, MXL_DISABLE);
        ...
        case MXL68x_TUNER_DEMOD: 
          ...
          status |= Ctrl_ReadRegister(devId, DMD_GPIO_FUNC_MODE_REG - 1, &msbRegData);
          status |= Ctrl_ReadRegister(devId, DMD_GPIO_FUNC_MODE_REG, &lsbRegData);
          lsbRegData &= 0xFFFD; // Set Bit<1> as 0 
          
          // Write high 16 bit 
          status |= Ctrl_WriteRegister(devId, DMD_GPIO_FUNC_MODE_REG - 1, msbRegData); 
          // Write low 16 bit 
          status |= Ctrl_WriteRegister(devId, DMD_GPIO_FUNC_MODE_REG, lsbRegData);

          // Enable demodulator clock to access demodulator registers, 0x3003 write 0xF89 
          // Read high 16 bit 
           
          // Read DMD_CCTL_RSTRQ_N_REG, read MSB 
          status |= Ctrl_ReadRegister(devId, DMD_CCTL_RSTRQ_N_REG - 1, &msbRegData); 

          // Write high 16 bit 
          status |= Ctrl_WriteRegister(devId, DMD_CCTL_RSTRQ_N_REG - 1, msbRegData); 
          // Write low 16 bit 
          status |= Ctrl_WriteRegister(devId, DMD_CCTL_RSTRQ_N_REG, 0x0F89);
                       ...
                       MXL68x_CLK_SWITCH_E clkType)
{
  ...
  if (clkType == MXL68x_PLL_CLOCK)
  {
    while (k < SEQ_STATE_RB_POLLING_TIMES)
    {
      ...
      status |= Ctrl_ReadRegister(devId, DFE_SEQ_STATE_RB_REG, &lsbRegData);     
    ...
    {
      MxL_DLL_DEBUG0("Err: Switching to PLL clock, SEQ_STATE_RB not equals 0x0A! \n");
      status = MXL_FAILURE; 
      devContextPtr->pllClkReady = MXL_FALSE;
    }
  }
  ...
  if (MXL_SUCCESS == status)
  {
    ...
    status |= Ctrl_ReadRegister(devId, DMD_CCTL_RSTRQ_N_REG - 1, &msbRegData); 
    // Read low 16 bits 
    status |= Ctrl_ReadRegister(devId, DMD_CCTL_RSTRQ_N_REG, &lsbRegData);
    ...
    // Write low 16 bit 
    status |= Ctrl_WriteRegister(devId, DMD_CCTL_RSTRQ_N_REG - 1, msbRegData);
    status |= Ctrl_WriteRegister(devId, DMD_CCTL_RSTRQ_N_REG, (lsbRegData|0x0008));
  }
                ...
                MXL_BOOL_E enableRfLoopThru)
{
  ...
  if (devContextPtr)
  {
    if (devContextPtr->driverStatus)
    {
      ...
      {       
        // Enable Tuner
        status = Ctrl_WriteRegister(devId, TUNER_ENABLE_REG, 0x0001);

        // Start tune = 1
        status |= Ctrl_WriteRegister(devId, START_TUNE_REG, 0x0001); 
      ...
      if (devContextPtr->firmwareStatus != MXL68x_FW_STATUS_UNKNOWN)
      {
        ...
        {
          // Select IF source
          status |= Ctrl_ReadRegister(devId, IF_FREQ_SEL_REG, &regData);
          regData = (ifSrc == MXL68x_IF_EXTERNAL)? (regData | 0xC0) : (regData & 0x3F);  // Bit<7:6>
          status |= Ctrl_WriteRegister(devId, IF_FREQ_SEL_REG, regData);

          status |= Ctrl_ReadRegister(devId, DIG_ANA_IF_CFG_0, &regData);
          regData = (ifSrc == MXL68x_IF_EXTERNAL)? (regData & 0xBF) : (regData | 0x40);  // Bit<6> = 0 
          status |= Ctrl_WriteRegister(devId, DIG_ANA_IF_CFG_0, regData);

          status |= Ctrl_ReadRegister(devId, DIG_INT_AAF_EN_REG, &regData);
          regData = (ifSrc == MXL68x_IF_EXTERNAL)? (regData & 0xBF) : (regData | 0x40);  // Bit<6> = 0 
          status |= Ctrl_WriteRegister(devId, DIG_INT_AAF_EN_REG, regData);
        }
        ...
        if(devContextPtr->mxl68xType == MXL683_DEVICE)
        {
          ...
          status |= Ctrl_ReadRegister(devId, DFE_LT_CONFIG_REG, &regData);
          ...
          if (MXL_FALSE == enableRfLoopThru)
            ...
            regData |= 0x10; 
          status |= Ctrl_WriteRegister(devId, DFE_LT_CONFIG_REG, regData);  
  ...
  if ((devContextPtr) && (mbinBufferPtr) && (mbinBufferSize))
  {
    if (devContextPtr->driverStatus)
    {
      ...
      MxLWare68x_OEM_SleepInUs(100000); 
	    ...
	    Ctrl_ReadRegister(devId, 0xA8CC, &mailbox);
     
      if(mailbox!=0xABCD)
      {
	      MxL_DLL_DEBUG0("FW Download Error Mailbox 0x%x [%d]\n",mailbox,devId);
		    status = MXL_FAILURE;
	    }
      else
      {
	      MxL_DLL_DEBUG0("FW Download Success Mailbox 0x%x [%d]\n",mailbox,devId);
      }
    }
    else
      status = MXL_NOT_INITIALIZED;
    ...
    if (devContextPtr->driverStatus)
    {
      switch(powerMode)
      {
        case MXL68x_PWR_MODE_ACTIVE: 
          ...
          status |= Ctrl_WriteRegister(devId, TUNER_ENABLE_REG, MXL_ENABLE);
          status |= Ctrl_WriteRegister(devId, START_TUNE_REG, MXL_ENABLE);
		  
#ifdef NEW_EAD_SPEC
    		  if(devContextPtr->mxl68xType == MXL683_DEVICE)
    		  {
      			MxL_DLL_DEBUG0("MxLWare68x_API_CfgDevPowerMode: LT Disabled.\n");
      			status |= Ctrl_ReadRegister(devId, DFE_LT_CONFIG_REG, &regData);
      			// For MxL683 RF LT EN default is high 
      			// Bit<4> = 1, turn on loop through : 0, turn off loop through
      			regData &= 0xFFEF;	
      			status |= Ctrl_WriteRegister(devId, DFE_LT_CONFIG_REG, regData);  
      			devContextPtr->rfLoopThrough = MXL_FALSE;
    		  }
        ...
        case MXL68x_PWR_MODE_STANDBY: 
          ...
          status |= Ctrl_SendHostCommand(devId, &pwrmodeSet);
    		  ...
    		  if(devContextPtr->mxl68xType == MXL683_DEVICE)
    		  {
    	        status |= Ctrl_ReadRegister(devId, DFE_SEQ_CDC_RF1_BO_REG, &regData);
    	        regData = (regData & 0xC0) | 0x02;  // Bit<5:0> = 02
    	        status |= Ctrl_WriteRegister(devId, DFE_SEQ_CDC_RF1_BO_REG, regData);
    		  
    			    MxL_DLL_DEBUG0("MxLWare68x_API_CfgDevPowerMode: LT Enabled.\n");
    	        status |= Ctrl_ReadRegister(devId, DFE_LT_CONFIG_REG, &regData);
    	        // For MxL683 RF LT EN default is high 
    	        // Bit<4> = 1, turn on loop through : 0, turn off loop through
    			    regData |= 0x10; 
    	        status |= Ctrl_WriteRegister(devId, DFE_LT_CONFIG_REG, regData);  
    	        devContextPtr->rfLoopThrough = MXL_TRUE;
    		  }
#endif
          break;
...
MXL_STATUS_E Ctrl_CfgDemodEnable(UINT8 devId, MXL_BOOL_E demodEnable)
{
  ...
  status |= Ctrl_ReadRegister(devId, DMD_CCTL_DEMRQ_REG - 1, &msbRegData);
  status |= Ctrl_ReadRegister(devId, DMD_CCTL_DEMRQ_REG, &lsbRegData);
  
  // Enable or Disable demod
  lsbRegData = (demodEnable == MXL_ENABLE) ? 0x0001 : 0x0000;
  
  status |= Ctrl_WriteRegister(devId, DMD_CCTL_DEMRQ_REG - 1, 0x0000); 
  status |= Ctrl_WriteRegister(devId, DMD_CCTL_DEMRQ_REG, lsbRegData);  
                ...
                MXL68x_GPIO_STATE_E gpioState)
{
  ...
  if (devContextPtr)
  {
    ...
    {
      // Read MSB
      status |= Ctrl_ReadRegister(devId, DMD_GPIO_FUNC_MODE_REG - 1, &regData); 
      gpioRegData = (regData << 16);
      
      // Read LSB
      status |= Ctrl_ReadRegister(devId, DMD_GPIO_FUNC_MODE_REG, &regData);  
      gpioRegData |= regData;
      ...
      status |= Ctrl_WriteRegister(devId, DMD_GPIO_FUNC_MODE_REG - 1, regData); 

      // Write low 16 bit 
      regData = (UINT16)(gpioRegData & 0x0FFFF); 
      status |= Ctrl_WriteRegister(devId, DMD_GPIO_FUNC_MODE_REG, regData); 
                ...
                UINT8 bootloaderVer[MXL68X_VERSION_SIZE])
{
  ...
  if (devContextPtr)
  {
    if (devContextPtr->driverStatus)
    {
      ...
      status = Ctrl_ReadRegister(devId, DMD_ID_VERSION_REG - 1, &lsbRegData);
      // Read low 16 bit , bit<15-12>: chip version, Bit<1:0>: Chip ID
      status |= Ctrl_ReadRegister(devId, DMD_ID_VERSION_REG, &lsbRegData);
        ...
        *chipVersionPtr = (lsbRegData >> 12) & 0x0F; 
        ...
        if (devContextPtr->firmwareStatus == MXL68x_FW_STATUS_LOADED)
        {
              ...
              (devVerInfoRsp.payloadLen == MXL68X_VERSION_SIZE * 2))
          {
            ...
            status |= Ctrl_ReadRegister(devId, MAILBOX_REG_FIRMWARE_RC_VER, &lsbRegData);
                ...
                UINT8* intrStatusPtr)
{
  ...
  if (devContextPtr)
  {
    if (devContextPtr->driverStatus)
    {
      status = Ctrl_ReadRegister(devId, DMD_INTERRUPT_MAILBOX_REG, &readBack);
                ...
                UINT8 numFrames)
{
  ...
  if (devContextPtr)
  {
    if (devContextPtr->driverStatus)
    {
      ...
      status = Ctrl_ReadRegister(devId, DMD_RS_BER_PER_FRAME, &regData);
      regData = (regData & 0xFF00) | (numFrames & 0x00FF);
      status |= Ctrl_WriteRegister(devId, DMD_RS_BER_PER_FRAME, regData);
      status |= Ctrl_WriteRegister(devId, DMD_RS_BER_PER_FRAME_MAILBOX_REG, regData);
      regData = 0;

      // Start Ber stat is moved from BER measurement (toggle 0 to 1)
      status |= Ctrl_ReadRegister(devId, DMD_RS_BER_PER_START, &regData);
      regData &= 0xFFFE; // Bit<0> = 0
      status |= Ctrl_WriteRegister(devId, DMD_RS_BER_PER_START, regData); 
                ...
                MXL_BOOL_E *berEndFlagPtr)
{
  ...
  if (devContextPtr)
  {
    if (devContextPtr->driverStatus)
    {
      status |= Ctrl_ReadRegister(devId, DMD_RS_BER_PER_END_FLG, &regData);
                ...
                UINT8 devId)
{
  ...
  if (devContextPtr)
  {
    if (devContextPtr->driverStatus)
    {
      status |= Ctrl_ReadRegister(devId, DMD_RS_BER_PER_CLR, &regData);
      ...
      status |= Ctrl_WriteRegister(devId, DMD_RS_BER_PER_CLR, regData); 
    }
    else
      status = MXL_NOT_INITIALIZED;
  }
												    ...
												    MXL_BOOL_E enableSkipNullPacket)
{
  ...
  if (devContextPtr)
  {
    if (devContextPtr->driverStatus)
    {
						...
						status |= Ctrl_ReadRegister(devId, DMD_CNST_CTRL, &readBack);
						...
						status |= Ctrl_WriteRegister(devId, DMD_CNST_CTRL, control);
    }
    else
      status = MXL_NOT_INITIALIZED;
  }
                ...
                MXL_BOOL_E enableRsDecoder)
{
  ...
  if (devContextPtr)
  {
    if (devContextPtr->driverStatus)
    {
      status |= Ctrl_ReadRegister(devId, DMD_FEC_OUT_CTRL, &regData);
      ...
      status |= Ctrl_WriteRegister(devId, DMD_FEC_OUT_CTRL, regData); 
                ...
                MXL68x_TS_PKT_SIZE_E tsPktSize)
{
  ...
  if (devContextPtr)
  {
    if (devContextPtr->driverStatus)
    {
      status |= Ctrl_ReadRegister(devId, MPEG_OUT_CFG_REG, &regData);  
      ...
      status |= Ctrl_ReadRegister(devId, MPEG_CLK_CFG_REG, &tmpData);
      ...
      status |= Ctrl_WriteRegister(devId, MPEG_CLK_CFG_REG, tmpData);  
      ...
      status |= Ctrl_WriteRegister(devId, MPEG_OUT_CFG_REG, regData);  
      status |= Ctrl_WriteRegister(devId, MPEG_OUT_CFG_MAILBOX_REG, regData);

      // Config MPEG CLK Phase <7>
      status |= Ctrl_ReadRegister(devId, MPEG_CLK_PHASE_REG-1, &regData);  
      status |= Ctrl_ReadRegister(devId, MPEG_CLK_PHASE_REG, &tmpData);  
      ...
      status |= Ctrl_WriteRegister(devId, MPEG_CLK_PHASE_REG - 1, regData);  
      status |= Ctrl_WriteRegister(devId, MPEG_CLK_PHASE_REG, tmpData);        
    }
    else
      status = MXL_NOT_INITIALIZED;
  }
                ...
                MXL68x_TS_DRV_STR_E tsDriveStrength)
{
  ...
  if (devContextPtr)
  {
    if (devContextPtr->driverStatus)
    {
      status |= Ctrl_ReadRegister(devId, MPEG_TS_DRV_STR_REG_1, &regData_1);  
      status |= Ctrl_ReadRegister(devId, MPEG_TS_DRV_STR_REG_2, &regData_2);  
	  ...
	  if(status == MXL_SUCCESS)
	  {
	      status |= Ctrl_WriteRegister(devId, MPEG_TS_DRV_STR_REG_1, regData_1);  
	      status |= Ctrl_WriteRegister(devId, MPEG_TS_DRV_STR_REG_2, regData_2);      
	  }
    }
    else
      status = MXL_NOT_INITIALIZED;
  ...
  if (devContextPtr)
  {
    if (devContextPtr->driverStatus)
    {
      *cnrPtr = 0 ; 
      ...
      status |= Ctrl_ReadRegister(devId, DMD_CNR_RB_LO_MAILBOX, &regData);
      cnrRaw = regData; // Low 16 bit 

      // High 16 bit, total 32 bits  
      status |= Ctrl_ReadRegister(devId, DMD_CNR_RB_HI_MAILBOX, &regData);
      cnrRaw += ((regData) << 16 );
                ...
                MXL_BOOL_E *layerCRSLockPtr)
{
  ...
  MxL_DLL_DEBUG0("MxLWare68x_API_ReqDemodLockStatus [%d]\n",devId);
  ...
  if (devContextPtr)
  {
    if (devContextPtr->driverStatus)
    {
      ...
      if (tmccLockPtr)
      { 
        status |= Ctrl_ReadRegister(devId, DMD_STATUS, &regData);
      ...
      {
        status |= Ctrl_ReadRegister(devId, DMD_RS_LOCK_A_B, &regData);
      ...
      if (layerCRSLockPtr)
      {
        status |= Ctrl_ReadRegister(devId, DMD_RS_LOCK_C, &regData);
																...
																MXL_BOOL_E* enableRsDecoder)
{
  ...
  if (devContextPtr) 
  {
    if (devContextPtr->driverStatus)
    { 
		    status = Ctrl_ReadRegister(devId, DMD_FEC_OUT_CTRL, &readBack);
                ...
                MXL68x_TS_PKT_SIZE_E *tsPktSizePtr)
{
  ...
  if (devContextPtr) 
  {
    if (devContextPtr->driverStatus)
    { 
		    status = Ctrl_ReadRegister(devId, MPEG_OUT_CFG_REG, &readBack);
      ...
      *mpegModePtr = (MXL68x_MPEG_MODE_E)((readBack & 0x04) >> 2); // Bit<2>
		    ...
		    *mpegErrIndicatorPtr = (MXL68x_MPEG_ERROR_IND_E)((readBack & 0x200) >> 9); // Bit<9>
		    ...
		    status |= Ctrl_ReadRegister(devId, MPEG_CLK_PHASE_REG - 1, &readBack);
		    status |= Ctrl_ReadRegister(devId, MPEG_CLK_PHASE_REG, &readBack);
                ...
                MXL68x_TS_DRV_STR_E *tsDriveStrengthPtr)
{
  ...
  if (devContextPtr)
  {
    if (devContextPtr->driverStatus)
    {
		status |= Ctrl_ReadRegister(devId, MPEG_TS_DRV_STR_REG_1, &readBack);
		tsDrvStr1 = (readBack & 0x003c) >> 2; // 0x3007<5:2>
		status |= Ctrl_ReadRegister(devId, MPEG_TS_DRV_STR_REG_2, &readBack);
              ...
              for (i = 0; i < MXL68x_ISDBT_LAYER_ALL; i++)
              {
                if (berInfoPtr[i])
                {
                  ...
                  else 
                  {
                    ...
                    // Read bit error count before Iterative decoding 
                    status |= Ctrl_ReadRegister(devId, DMD_RS_BER_PER_DATA_LO_MAILBOX, &regData);
                    berInfoPtr[i]->bitErrCount_WO_Iter = regData; // Low 16 bit 

                    // High 8 bit, total 32 bits  
                    status |= Ctrl_ReadRegister(devId, DMD_RS_BER_PER_DATA_HI_MAILBOX, &regData);
                    berInfoPtr[i]->bitErrCount_WO_Iter += ((regData) << 16 );

                    // Read bit error count after Iterative decoding  
                    status |= Ctrl_ReadRegister(devId, DMD_RS_BER_PER_ITER_DATA_LO_MAILBOX, &regData);
                    berInfoPtr[i]->bitErrCount_W_Iter = regData; // Low 16 bit 

                    status |= Ctrl_ReadRegister(devId, DMD_RS_BER_PER_ITER_DATA_HI_MAILBOX, &regData);
                ...
                MXL68x_TMCC_INFO_T *tmccInfoLayerCPtr)
{
  ...
  if (devContextPtr)
  {
    if (devContextPtr->driverStatus)
    {
      status |= Ctrl_ReadRegister(devId, DMD_TMCC_INFO_REG_0, &demodStatusReg0);
      ...
      status |= Ctrl_ReadRegister(devId, DMD_STATUS_INFO_REG, &tmp);
      ...
      if ((tmccInfoLayerAPtr) || (tmccInfoLayerBPtr) || (tmccInfoLayerCPtr))
      {
        ...
        status |= Ctrl_ReadRegister(devId, DMD_TMCC_INFO_REG_1, &demodStatusReg1);
        status |= Ctrl_ReadRegister(devId, DMD_TMCC_INFO_REG_2, &demodStatusReg2);
        status |= Ctrl_ReadRegister(devId, DMD_TMCC_INFO_REG_3, &demodStatusReg3);
	               ...
	               SINT32 *samplingFreqOff)
{
  ...
  if (devContextPtr) 
  {
    if (devContextPtr->driverStatus)
    {
        status |= Ctrl_ReadRegister(devId, DMD_CARRIER_FREQ_OFF_LO_MAILBOX, &regData);
        *carrierFreqOff = regData; // Low 16 bit 

        status |= Ctrl_ReadRegister(devId, DMD_CARRIER_FREQ_OFF_HI_MAILBOX, &regData);
        *carrierFreqOff += ((regData & 0xFFFF) << 16 );

        status |= Ctrl_ReadRegister(devId, DMD_SAMP_FREQ_OFF_LO_MAILBOX, &regData);
        *samplingFreqOff = regData; // Low 16 bit 

        status |= Ctrl_ReadRegister(devId, DMD_SAMP_FREQ_OFF_HI_MAILBOX, &regData);
                ...
                MXL_BOOL_E *isSpectrumInverted)
{
  ...
  *isSpectrumInverted = MXL_FALSE;
  ...
  if (devContextPtr) 
  {
    if (devContextPtr->driverStatus)
    {
      status |= Ctrl_ReadRegister(devId, DFE_GCOMP_IQ_SWITCH, &regData);
  ...
  if (devContextPtr)
  {
    if (devContextPtr->driverStatus)
    {
      ...
      if (i == MAX_PRESET_IF_FREQ) 
      {
        ...
        status |= Ctrl_WriteRegister(devId, IF_FCW_LOW_REG, (ifManualFreq & 0xFF)); 
        status |= Ctrl_WriteRegister(devId, IF_FCW_HIGH_REG, ((ifManualFreq >> 8) & 0x0F));
      ...
      if (status == MXL_SUCCESS)
      {
        ...
        status |= Ctrl_WriteRegister(devId, IF_PATH_GAIN_REG, regData); 
                ...
                MXL_BOOL_E agcPolarityInversion)
{
  ...
  if (devContextPtr)
  {
    if (devContextPtr->driverStatus)
    {
      ...
      status = Ctrl_ReadRegister(devId, AGC_CONFIG_REG, &regData); 
      regData &= 0xFFF2; // Clear bit<3:2> and bit<0>
      regData |= (UINT16)((agcType << 2) | 0x01);  // Only has AGC1 
      status |= Ctrl_WriteRegister(devId, AGC_CONFIG_REG, regData);

      // AGC set point <6:0>
      status |= Ctrl_ReadRegister(devId, AGC_SET_POINT_REG, &regData);
      regData &= 0xFF80; // Clear bit <6:0>
      regData |= setPoint;
      status |= Ctrl_WriteRegister(devId, AGC_SET_POINT_REG, regData);

      // AGC Polarity <4>
      status |= Ctrl_ReadRegister(devId, AGC_FLIP_REG, &regData);
      regData &= 0xFFEF; // Clear bit <4>
      regData |= (agcPolarityInversion << 4);
      status |= Ctrl_WriteRegister(devId, AGC_FLIP_REG, regData);
    }
    else
      status = MXL_NOT_INITIALIZED;
                ...
                MXL68x_SIGNAL_MODE_E signalMode)
{
  ...
  if (devContextPtr)
  {
    if (devContextPtr->driverStatus)
    {
      ...
      status |= Ctrl_ReadRegister(devId, DMD_GPIO_FUNC_MODE_REG-1, &control1);
      status |= Ctrl_ReadRegister(devId, DMD_GPIO_FUNC_MODE_REG, &control2);
      status |= Ctrl_WriteRegister(devId, DMD_GPIO_FUNC_MODE_REG-1, control1);
      status |= Ctrl_WriteRegister(devId, DMD_GPIO_FUNC_MODE_REG, (control2 & 0xFFFD));
      ...
      if ((MXL68x_FW_STATUS_LOADED == devContextPtr->firmwareStatus))
      {
        ...
        dfeRegData = ((devContextPtr->xtalFreqInMHz & 0x01) << 13) + \
                      ...
                      (signalMode & 0x0F);
        status |= Ctrl_WriteRegister(devId, MXL68X_FW_MAIL_BOX_REG, dfeRegData);
        ...
        status |= Ctrl_ReadRegister(devId, MXL68X_FW_MAIL_BOX_REG, &dfeRegData);
        ...
        if (dfeRegData == 0xFFFF)
        {
          ...
          status |= Ctrl_WriteRegister(devId, MXL68X_FW_MAIL_BOX_REG, 0x00);
          ...
          switch (signalMode)
          {
            ...
            case MXL68x_DIG_J83B:
              ...
              status |= Ctrl_WriteRegister(devId, DFE_TT_SPUR_ENABLE, 0);   
       
              //Make sure the CPU is Clock Gated for cable modes
               status |= Ctrl_ReadRegister(devId, DMD_GPIO_FUNC_MODE_REG-1, &control1);
               status |= Ctrl_ReadRegister(devId, DMD_GPIO_FUNC_MODE_REG, &control2);
               status |= Ctrl_WriteRegister(devId, DMD_GPIO_FUNC_MODE_REG-1, control1);
               status |= Ctrl_WriteRegister(devId, DMD_GPIO_FUNC_MODE_REG, (control2 | 0x0002));
             break; 
      ...
      else  // Implement tuner application setting in host side 
      {
        ...
        switch(signalMode)
        {
          case MXL68x_ANA_NTSC_MODE:
            ...
            if (devContextPtr->ifOutFreqInKHz < MXL68x_IF_FREQ_35250_KHZ)
            {
              ...
              status |= Ctrl_WriteRegister(devId, DIG_ANA_IF_CFG_0, 0xFE);
#else 
              status |= Ctrl_WriteRegister(devId, DIG_ANA_IF_CFG_0, 0x7E);
#endif 
              status |= Ctrl_WriteRegister(devId, DIG_ANA_IF_CFG_1, 0x10);
              ...
              status |= Ctrl_WriteRegister(devId, 0xDD, dfeRegData);    
            }
            else
            {
              // High power
              status |= Ctrl_WriteRegister(devId, DIG_ANA_IF_CFG_0, 0xD9);
              status |= Ctrl_WriteRegister(devId, DIG_ANA_IF_CFG_1, 0x16);
          ...
          case MXL68x_ANA_PAL_I:
            status |= Ctrl_WriteRegister(devId, 0x0C, 0x0B);
          ...
          case MXL68x_ANA_PAL_D:
            status |= Ctrl_WriteRegister(devId, 0x0C, 0x0C);
          ...
          case MXL68x_DIG_J83B: 
            ...
            status |= Ctrl_WriteRegister(devId, DFE_CSF_SS_SEL, dfeRegData);
          ...
          case MXL68x_DIG_ATSC:
            ...
            {
              // Low power if settings
              status |= Ctrl_WriteRegister(devId, DIG_ANA_IF_CFG_0, 0xF9);
              status |= Ctrl_WriteRegister(devId, DIG_ANA_IF_CFG_1, 0x18);
              status |= Ctrl_WriteRegister(devId, DIG_ANA_IF_PWR, 0xF1);
            }
            else
            {
              // High power if settings
              status |= Ctrl_WriteRegister(devId, DIG_ANA_IF_CFG_0, 0xD9);
              status |= Ctrl_WriteRegister(devId, DIG_ANA_IF_CFG_1, 0x16);
              status |= Ctrl_WriteRegister(devId, DIG_ANA_IF_PWR, 0xB1);
            }
            ...
            status |= Ctrl_WriteRegister(devId, DFE_DACIF_GAIN, dfeRegData);
        ...
        {
          // XTAL calibration
          status |= Ctrl_WriteRegister(devId, XTAL_CALI_SET_REG, 0x00);   
          status |= Ctrl_WriteRegister(devId, XTAL_CALI_SET_REG, 0x01);   
          status |= Ctrl_WriteRegister(devId, XTAL_CALI_SET_REG, 0x00); 
                ...
                MXL_BOOL_E useChanTuneInt) 
{
  ...
  if (devContextPtr)
  {
    if (devContextPtr->driverStatus)
    {
      ...
      status |= Ctrl_WriteRegister(devId, DMD_CHAN_SCAN_TUNE_XTAL_REG, 0x0000); // set all bits to 0 (including AutoSpecInv)
      status |= Ctrl_WriteRegister(devId, DMD_CHAN_SCAN_TUNE_FREQ_REG, 0x0000);

      if (freqInHz < 700000000) 
      {
        // Program cable mode settings - IF related
        status |= Ctrl_WriteRegister(devId, 0x7C, 0x1F);
      ...
      {
        // Program cable mode settings - IF related
        status |= Ctrl_WriteRegister(devId, 0x7C, 0x9F);
      ...
      // Program IF register
      status |= Ctrl_WriteRegister(devId, 0x131, regData);

      // frequency dependent XTAL Bias Settings for all analog modes
      if(devContextPtr->signalType <= MXL68x_ANA_SECAM_L)
      {
        status |= Ctrl_ReadRegister(devId, ANA_DIG_REFSX_XTAL_AMP_RB_REG, &ampRB);
        status |= Ctrl_ReadRegister(devId, ANA_DIG_REFSX_XTAL_AMP_LIN_RB_REG, &ampLinRB);
        ...
        status |= Ctrl_WriteRegister(devId, DIG_ANA_XTAL_XTAL_EXT_BIAS, regData);
      ...
      switch(devContextPtr->signalType)
      {
        ...
        case MXL68x_ANA_PAL_BG:
          ...
          if ((bandWidth == MXL68x_BW_7MHz) || (bandWidth == MXL68x_BW_8MHz))
            status |= Ctrl_WriteRegister(devId, 0x0C, regData);
      ...
      {         
        ...
        {
          status |= Ctrl_ReadRegister(devId, DFE_AGC_AUTO_REG, &regData);
          control = (regData & 0x80) | 0x0E;  // Bit<6> = 0, Bit<5:0> = 14 
          status |= Ctrl_WriteRegister(devId, DFE_AGC_AUTO_REG, control);

          status |= Ctrl_ReadRegister(devId, DFE_SEQ_TUNE_RF1_BO_REG, &regData);
          control = (regData & 0xC0) | 0x0E;  // Bit<5:0> = 14 
          status |= Ctrl_WriteRegister(devId, DFE_SEQ_TUNE_RF1_BO_REG, control);

          status |= Ctrl_ReadRegister(devId, DFE_SEQ_CDC_RF1_BO_REG, &regData);
          control = (regData & 0xC0) | 0x0E;  // Bit<5:0> = 14 
          status |= Ctrl_WriteRegister(devId, DFE_SEQ_CDC_RF1_BO_REG, control);
        }
        else 
        {
          status |= Ctrl_ReadRegister(devId, DFE_AGC_AUTO_REG, &regData);
        ...
        status |= Ctrl_WriteRegister(devId, DMD_CHAN_SCAN_TUNE_FREQ_REG, (UINT16)freq);
        ...
        status |= Ctrl_ReadRegister(devId, DMD_CHAN_SCAN_TUNE_XTAL_REG, &regData);
        ...
        status |= Ctrl_WriteRegister(devId, DMD_CHAN_SCAN_TUNE_XTAL_REG, fwXtalBwSettings);
        ...
        if(useChanTuneInt == MXL_FALSE)
        {
          ...
          if(rxPwr >=-9200)
          {
            ...
            //  MxLWare68x_OEM_SleepInUs(10000);
              ...
              status |= Ctrl_ReadRegister(devId, DMD_CHAN_SCAN_TUNE_XTAL_REG, &tuneDone);
      ...
      else   // Analog Modes, perform channel tune by host
      {
        ...
        status |= Ctrl_ReadRegister(devId, TUNE_MODE_REG, &regData);
        regData &= 0xFE;

        if (MXL68x_SCAN_MODE == tuneType) regData |= 0x01;

        status |= Ctrl_WriteRegister(devId, TUNE_MODE_REG, regData);

        // Bandwidth <1:0>
        status |= Ctrl_ReadRegister(devId, CHAN_TUNE_BW_REG, &regData);
        regData = (UINT16)((regData & 0xFFFC) | bandWidth);

        status |= Ctrl_WriteRegister(devId, CHAN_TUNE_BW_REG, regData);

        // Set RF  
        status |= Ctrl_WriteRegister(devId, CHAN_TUNE_LOW_REG, (UINT8)(freq & 0xFF));
        status |= Ctrl_WriteRegister(devId, CHAN_TUNE_HI_REG, (UINT8)((freq >> 8 ) & 0xFF));

        if (devContextPtr->signalType <= MXL68x_ANA_SECAM_L)
        {
          status |= MxLWare68x_API_CfgTunerEnableFineTune(devId, MXL_DISABLE);
          status |= Ctrl_WriteRegister(devId, FINE_TUNE_CTRL_REG_1, 0);
          status |= Ctrl_WriteRegister(devId, FINE_TUNE_CTRL_REG_1, 0x20); 
        }
                ...
                MXL_BOOL_E enableFineTune)
{
  ...
  if (devContextPtr)
  {
    if (devContextPtr->driverStatus)
    {
      status = Ctrl_ReadRegister(devId, FINE_TUNE_CTRL_REG_0, &fineTuneReg0);
      status |= Ctrl_ReadRegister(devId, FINE_TUNE_CTRL_REG_1, &fineTuneReg1);
      ...
      status |= Ctrl_WriteRegister(devId, FINE_TUNE_CTRL_REG_0, fineTuneReg0);    
      status |= Ctrl_WriteRegister(devId, FINE_TUNE_CTRL_REG_1, fineTuneReg1);    
    }
    else
      status = MXL_NOT_INITIALIZED;
                ...
                MXL68x_FINE_TUNE_STEP_E fineTuneStep)
{
  ...
  if (devContextPtr)
  {
    if (devContextPtr->driverStatus)
    {
      status = Ctrl_ReadRegister(devId, FINE_TUNE_SET_REG, &regData);

      // Fine tune <1:0>
      regData &= 0xFFFC;
      status |= Ctrl_WriteRegister(devId, FINE_TUNE_SET_REG, regData);
  ...
  if (devContextPtr)
  {
    if ((devContextPtr->driverStatus) && (rawAgcGainPtr != NULL))
    {
      *rawAgcGainPtr = 0;
      ...
      // Read low 8 bit 
      status = Ctrl_ReadRegister(devId, RF_TOTAL_GAIN_RB_REG, &regData);
      tempData = regData & 0xFF;

      // Read high 4 bit 
      status |= Ctrl_ReadRegister(devId, RF_TOTAL_GAIN_RB_REG + 1, &regData);
      regData = ((regData & 0x0F) << 8) + tempData;

      if (MXL_SUCCESS == status) *rawAgcGainPtr = regData; 
      
      // Readback AGC lock status
      status |= Ctrl_ReadRegister(devId, AGC_SAGCLOCK_STATUS_REG, &regData);  
                ...
                MXL_BOOL_E* refSynthLockPtr)
{
  ...
  MxL_DLL_DEBUG0("MxLWare68x_API_ReqTunerLockStatus [%d]\n",devId);
  ...
  if (devContextPtr)
  {
    if (devContextPtr->driverStatus)
    {
      ...
      status = Ctrl_ReadRegister(devId, RF_REF_STATUS_REG, &regData);  
      ...
      MxL_DLL_DEBUG0(" RfSynthStatus = %d, RefSynthStatus = %d", (UINT8)*rfSynthLockPtr,(UINT8)*refSynthLockPtr); 
  ...
  if (devContextPtr)
  {
    ...
    {   
      ...
      else
      {
        ...
        status = Ctrl_ReadRegister(devId, RFPIN_RB_LOW_REG, &regData);
        tmpData = regData;

        // RF input power high <1:0>
        status |= Ctrl_ReadRegister(devId, RFPIN_RB_HIGH_REG, &regData);
                ...
                UINT16 mask)
{
  ...
  if (devContextPtr)
  {
    if (devContextPtr->driverStatus)
    {   
      status |= Ctrl_WriteRegister(devId, CUST_SET_REG, mask);
    }
    else
      status = MXL_NOT_INITIALIZED;
  }
