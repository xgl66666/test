...
static MS_BOOL IIC_READ(MS_U8 u8SlaveID, MS_U8* paddr, MS_U8 u8AddrNum, MS_U8* pu8data, MS_U16 u16size)
{
    ...
    if (FALSE == MDrv_IIC_Read(u8SlaveID, paddr, u8AddrNum, pu8data, u16size))
#else
...
#else
    paddr[0] = 0x00;
    u8SlaveID = R828_ADDRESS;
    regsize = 1;
#endif

    if (FALSE == IIC_READ(u8SlaveID, paddr, regsize, pu8data, u16size))
    {
...
static MS_BOOL I2C_Write(I2C_TYPE *I2C_Info)
{
    ...
    paddr[0] = 0xFE;
    pu8data[0] = 0x34;
    pu8data[1] = I2C_Info->RegAddr;
    pu8data[2] = I2C_Info->Data;
    u16size  = 3;
    if (FALSE == MDrv_IIC_Write(u8SlaveID, paddr, 1, pu8data, u16size))
#else
    u8SlaveID = R828_ADDRESS;
    u16size = 1;
    pu8data[0] = I2C_Info->Data;
    paddr[0] = I2C_Info->RegAddr;
...
#if(FRONTEND_DEMOD_TYPE ==DEMOD_TC90517)
    u8SlaveID = 0x30;   //demeod iic addr
    paddr[0] = 0xFE;
    pu8data[0] = 0x34;
    pu8data[1] = I2C_Info->RegAddr;
    for (DataCunt = 0; DataCunt < I2C_Info->Len; DataCunt++)
    {
        pu8data[DataCunt + 2] = I2C_Info->Data[DataCunt];
    }
    u16size =  I2C_Info->Len + 2;
    if (FALSE == MDrv_IIC_Write(u8SlaveID, paddr, 1, pu8data, u16size))
#else
    u8SlaveID = R828_ADDRESS;
    u16size  = I2C_Info->Len;
    paddr[0] = I2C_Info->RegAddr;
    for (DataCunt = 0; DataCunt < I2C_Info->Len; DataCunt++)
    {
        pu8data[DataCunt] = I2C_Info->Data[DataCunt];
    }
    if (FALSE == MDrv_IIC_WriteBytes(FRONTEND_TUNER_PORT, u8SlaveID, 1, paddr, u16size, pu8data))
#endif
    {
...
Sys_Info_Type R828_Sys_Sel(R828_Standard_Type R828_Standard)
{
    ...
    case DVB_T2_8M:
        ...
        R828_Sys_Info.FILT_CAL_LO = 67500;
        R828_Sys_Info.FILT_GAIN = 0x10; //+3dB, 6MHz on
        R828_Sys_Info.IMG_R = 0x00; //image negative
        R828_Sys_Info.FILT_Q = 0x00;    //R10[4]:low Q(1'b0) //
    ...
    case ATSC_4063:
        R828_Sys_Info.IF_KHz = 4063;
        R828_Sys_Info.BW = BW_6M;
        R828_Sys_Info.FILT_CAL_LO = 57000;
        R828_Sys_Info.FILT_GAIN = 0x10; //+3dB, 6MHz on
        R828_Sys_Info.IMG_R = 0x00;     //image negative
        R828_Sys_Info.FILT_Q = 0x00;    //R10[4]:low Q(1'b0) //
    ...
    case ATSC_5070:
        ...
        R828_Sys_Info.BW = BW_6M;
        R828_Sys_Info.FILT_CAL_LO = 67000;
        R828_Sys_Info.FILT_GAIN = 0x10; //+3dB, 6MHz on
        R828_Sys_Info.IMG_R = 0x00;     //image negative
        R828_Sys_Info.FILT_Q = 0x00;    //R10[4]:low Q(1'b0) //
        R828_Sys_Info.HP_COR = 0x06;    // 1.7M disable, no cap, -8@1.75MHz
        R828_Sys_Info.EXT_ENABLE = 0x00; //R30[6], ext disable; R30[5]:0 ext at LNA max
        R828_Sys_Info.LOOP_THROUGH = 0x00; //R5[7], LT ON
        R828_Sys_Info.LT_ATT = 0x00;     //R31[7], LT ATT enable
        R828_Sys_Info.FLT_EXT_WIDEST = 0x00; //R15[7]: FLT_EXT_WIDE OFF
...
SysFreq_Info_Type R828_SysFreq_Sel(R828_Standard_Type R828_Standard, UINT32 RF_freq)
{
    ...
    case DVB_T_8M:
        if ((RF_freq == 506000) || (RF_freq == 666000) || (RF_freq == 818000))
        {
            ...
            R828_SysFreq_Info.LNA_TOP = 0xDD;   // Detect BW 3, LNA TOP:3, PreDet Top:2
            R828_SysFreq_Info.CP_CUR = 0x28;          //101, 0.2
            R828_SysFreq_Info.DIV_BUF_CUR = 0x20; // 10, 200u
            R828_SysFreq_Info.LNA_VTH_L = 0x73;     // LNA VTH 1.04 ,  VTL 0.64
        }
        else if (RF_freq == 786000)
        {
            R828_SysFreq_Info.MIXER_TOP = 0x24;     // MIXER TOP:13 , TOP-1, low-discharge
            R828_SysFreq_Info.LNA_TOP = 0x1E;       // Detect BW 0, LNA TOP:3, PreDet Top:1
        ...
        {
            R828_SysFreq_Info.MIXER_TOP = 0x24;     // MIXER TOP:13 , TOP-1, low-discharge
            R828_SysFreq_Info.LNA_TOP = 0x1D;   // Detect BW 0, LNA TOP:3, PreDet Top:2
            R828_SysFreq_Info.CP_CUR = 0x38;          //111, auto
            R828_SysFreq_Info.DIV_BUF_CUR = 0x30; // 11, 150u
            R828_SysFreq_Info.LNA_VTH_L = 0x73;     // LNA VTH 1.04 ,  VTL 0.64
        }
    ...
    case ATSC_5070:
        ...
        R828_SysFreq_Info.LNA_TOP = 0xDD;   // Detect BW 3, LNA TOP:3, PreDet Top:2
        R828_SysFreq_Info.LNA_VTH_L = 0x73;     // LNA VTH 1.04 ,  VTL 0.64
        R828_SysFreq_Info.MIXER_VTH_L = 0x85; // MIXER VTH 1.14, VTL 0.84
        R828_SysFreq_Info.AIR_CABLE1_IN = 0x00;
        R828_SysFreq_Info.CABLE2_IN = 0x00;
        R828_SysFreq_Info.PRE_DECT = 0x40;
        R828_SysFreq_Info.LNA_DISCHARGE = 14;
        R828_SysFreq_Info.CP_CUR = 0x38;          // 111, auto
        R828_SysFreq_Info.DIV_BUF_CUR = 0x22; // (10), 200u;  PLL cur=D_mid,Buf_max(10)
        R828_SysFreq_Info.FILTER_CUR = 0x40;  // 10, low
        break;
...
R828_ErrCode R828_SetFrequency(R828_Set_Info R828_INFO, R828_SetFreq_Type R828_SetFreqMode)
{
    ...
    if (R828_INFO.R828_Standard > SECAM_L1)
    {
        if (R828_SetFreqMode == FAST_MODE1)    //FAST mode
        {
    ...
    {
        if ((R828_SetFreqMode == NORMAL_MODE1) || (R828_SetFreqMode == FAST_MODE1))
        {
            // PRE_DECT on
            R828_Arry[1]  = (R828_Arry[1] & 0xBF) | SysFreq_Info1.PRE_DECT;
            R828_I2C.RegAddr  = 0x06;
...
UINT8 MDrv_Tuner_GetRfGain(void)
{
    UINT8  RF_gain1, RF_gain2, RF_gain_comb;
    ...
    RF_gain1 = (R828_I2C_Len.Data[3] & 0x0F);
    RF_gain2 = ((R828_I2C_Len.Data[3] & 0xF0) >> 4);
    RF_gain_comb = RF_gain1 * 2 + RF_gain2;
...
MS_BOOL MDrv_Tuner_R820T_Init(MS_U8 u8TunerIndex, TUNER_MS_INIT_PARAM* pParam)
{
	TunerMode.Femode = NORMAL_MODE1;
#if (DTV_SYSTEM_STYLE == DVBT2_STYLE)
	TunerMode.Fetype = E_TUNE_TYPE_T;
#elif((DTV_SYSTEM_STYLE == ISDBT_STYLE))
	TunerMode.Fetype = E_TUNE_TYPE_ISDBT;
#endif
    ...
    {
         R82X_DBG("Tuner R820T Init error\n");

         return FALSE;
    }

    //R828 Standard Setting
    if (RT_Success != R828_SetStandard(ISDB_T))
    {
        R82X_DBG("[%s] R828_Standard error\n", __func__);
        return FALSE;
    }
...
MS_BOOL MDrv_Tuner_R820T_Tune(MS_U8 u8TunerIndex,MS_U32 u32Freq /*Khz*/, MS_U8 ucBw /*MHz*/)
{
	...
	R828_Info_Msg.RT_Input		= SIGLE_IN;
	R828_Info_Msg.RT_InputMode  = AIR_IN;
	R828_Info_Msg.R828_Standard = R828_Standard_Set;
#else
    R828_Info_Msg.RF_KHz        = u32Freq; // unit: kHz
    R828_Info_Msg.RT_Input      = LOOP_THROUGH;
    R828_Info_Msg.R828_Standard = DVB_C_8M;
#endif
    if (RT_Success != R828_SetFrequency(R828_Info_Msg, NORMAL_MODE1))
    {
...
MS_BOOL MDrv_Tuner_R820T_CheckExist(MS_U8 u8TunerIndex, MS_U32* pu32channel_cnt)
{
    ...
    printf("[R820T] read id =0x%x\n",R828_I2C_Len.Data[0]);
    if((R828_I2C_Len.Data[0] == R820T_CHIP_ID) && (pu32channel_cnt != NULL))
        *pu32channel_cnt += 1;
    return (R828_I2C_Len.Data[0] == R820T_CHIP_ID) ;

}

MS_BOOL MDrv_Tuner_R820T_GetTunerIF(MS_U8 u8TunerIndex, MS_U32* pu32IF_Freq)
{
...
//*--------------------------------------------------------------------------------------
//*--------------------------------------------------------------------------------------
MS_BOOL MDrv_Tuner_R820T_GetPowerLevel(MS_U8 u8TunerIndex,float *Level)
{
    hwi2c_port = getI2CPort(u8TunerIndex);
    if(Level == NULL)
    {
...
MS_BOOL MDrv_Tuner_R820T_Extension_Function(MS_U8 u8TunerIndex, TUNER_EXT_FUNCTION_TYPE fuction_type, void *data)
{
    if(fuction_type == TUNER_EXT_FUNC_GET_POWER_LEVEL)
    {
        ...
        MDrv_Tuner_R820T_GetPowerLevel(u8TunerIndex,(float *)data);
    }
    return TRUE;
}


DRV_TUNER_TABLE_TYPE GET_TUNER_ENTRY_NODE(TUNER_R820T) DDI_DRV_TUNER_TABLE_ENTRY(tunertab) =
{
    .name               = "TUNER_R820T",          // demod system name
    .data               = TUNER_R820T,            // private data value
    .Init               = MDrv_Tuner_R820T_Init,
    .SetFreq            = MDrv_Tuner_Null_SetFreq,
    .SetFreqS2          = MDrv_Tuner_Null_SetFreq_S2,
    .GetLock            = MDrv_Tuner_Null_GetLock,
    .SetTuner           = MDrv_Tuner_R820T_Tune,
    .CheckExist         = MDrv_Tuner_R820T_CheckExist,
    .GetTunerIF         = MDrv_Tuner_R820T_GetTunerIF,
    .Extension_Function = MDrv_Tuner_R820T_Extension_Function
};
