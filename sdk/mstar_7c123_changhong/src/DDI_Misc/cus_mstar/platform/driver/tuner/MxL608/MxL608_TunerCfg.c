...
MXL608_REG_CTRL_INFO_T MxL608_DigitalIsdbtAtsc[] = 
{
  ...
  {0xC2, 0xFF, 0xA9},
  {0xC5, 0xFF, 0x7C},
  //{0xCD, 0xFF, 0xEB},// Youda modify
  //{0xCE, 0xFF, 0x7F},// Youda modify
  {0xD5, 0xFF, 0x03},
  {0xD9, 0xFF, 0x04},
  {0,    0,    0}
};
...
MXL_STATUS MxL608_Ctrl_ProgramRegisters(UINT8 u8TunerIndex,UINT8 devId, PMXL608_REG_CTRL_INFO_T ctrlRegInfoPtr)
{
    ...
    while (status == MXL_TRUE)
    {
        ...
        {
            status = MxLWare608_OEM_ReadRegister(u8TunerIndex,devId,ctrlRegInfoPtr[i].regAddr, &tmp);
        ...
        status = MxLWare608_OEM_WriteRegister(u8TunerIndex,devId,ctrlRegInfoPtr[i].regAddr, tmp);
        if (status != MXL_TRUE) break;
...
MXL_STATUS MxL608_Ctrl_WriteRegField(UINT8 u8TunerIndex,UINT8 devId, PMXL608_REG_CTRL_INFO_T ctrlRegInfoPtr)
{
    ...
    {
        status = MxLWare608_OEM_ReadRegister(u8TunerIndex,devId, ctrlRegInfoPtr->regAddr, &tmp);
    ...
    status = MxLWare608_OEM_WriteRegister(u8TunerIndex,devId, ctrlRegInfoPtr->regAddr, tmp);
...
MXL_STATUS MxL608_Ctrl_SetRfFreqLutTblReg(UINT8 u8TunerIndex,UINT8 devId, UINT32 FreqInHz, PMXL608_CHAN_DEPENDENT_FREQ_TABLE_T freqLutPtr)
{
  ...
  if (freqLutPtr)
  {
    ...
    for (idx = 0; 0 != freqLutPtr->centerFreqHz; idx++, freqLutPtr++)
    {
      if (freqLutPtr->centerFreqHz == 1)  
      {
		...
		status = MxLWare608_OEM_ReadRegister(u8TunerIndex, devId, DFE_REFLUT_BYP_REG, &regSetData[0]); 
  ...
  for (idx = 0; idx < MxL608_SPUR_REGISTER.SpurRegNum; idx++)
    status |= MxLWare608_OEM_WriteRegister(u8TunerIndex,devId, MxL608_SPUR_REGISTER.SpurRegAddr[idx], regSetData[idx]);
