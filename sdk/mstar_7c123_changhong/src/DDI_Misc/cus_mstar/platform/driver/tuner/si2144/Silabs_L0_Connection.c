...
int    L0_ReadBytes              (L0_Context* i2c, unsigned int iI2CIndex, int iNbBytes, unsigned char *pucDataBuffer)
{
    ...
    if (SI214X_ucI2cReadOnly(i2c->address,pucDataBuffer,iNbBytes)!=TRUE)
    {
        printf(("[Silabs]:<= I2C Read Error\n"));
        return 0;
    }

    //printf("SI214X_ucI2cReadOnly:i2c->address =%x;iNbBytes=%d\n",i2c->address,iNbBytes);
    //for(i=0;i<iNbBytes;i++)
    //{
    //    printf("pucDataBuffer[%x]\n",pucDataBuffer[i]);
    //}
    return iNbBytes;
}
...
int      L0_WriteBytes             (L0_Context* i2c, unsigned int iI2CIndex, int iNbBytes, unsigned char *pucDataBuffer)
{
    ...
    if (SI214X_ucI2cWriteOnly(i2c->address,pucDataBuffer,iNbBytes) !=TRUE)
    {
        printf(("[Silabs]:<= I2C Write Error\n"));
        return 0;
    }
    // ICtrlBus_I2cTunerWrite(0x100,i2c->address,pucDataBuffer,iNbBytes);
   // printf("SI214X_ucI2cWriteOnly:i2c->address =%x;iI2CIndex=%x;iNbBytes=%d\n",i2c->address,iI2CIndex,iNbBytes);
    //for(i=0;i<iNbBytes;i++)
    //{
    //    printf("pucDataBuffer[%x]\n",pucDataBuffer[i]);
    //}
