...
#if IF_THIS_TUNER_INUSE(TUNER_R850)
...
{      
  ...
  //   8,    9,   10,   11,   12,   13,   14,   15
    0x8E, 0xBB, 0xF8, 0xB0, 0xD2, 0x81, 0xCD, 0x46,
  //   0x10  0x11  0x12  0x13  0x14  0x15  0x16  0x17 
  // 16,   17,   18,   19,   20,   21,   22,   23
         0x37, 0x40, 0x89, 0x8B, 0x00, 0x80, 0x04, 0x23,
  ...
  // 40,   41,   42,   43,   44,   45,   46,   47
{
  ...
  //   0x08  0x09  0x0A  0x0B  0x0C  0x0D  0x0E  0x0F
  //	  8,    9,   10,   11,   12,   13,   14,   15
    0x0E, 0xBB, 0xF8, 0xB0, 0xF0, 0x61, 0xCD, 0x14,
  //   0x10  0x11  0x12  0x13  0x14  0x15  0x16  0x17 
  //	 16,   17,   18,   19,   20,   21,   22,   23
         0x37, 0x42, 0x81, 0x92, 0x00, 0x40, 0x05, 0x23,
  //   0x18  0x19  0x1A  0x1B  0x1C  0x1D  0x1E  0x1F 
  //	 24,   25,   26,   27,   28,   29,   30,   31,
...
R850_Sys_Info_Type R850_Sys_Sel(R850_Standard_Type R850_Standard)
{
    ...
    case R850_DVB_T2_10M:
        ...
        R850_Sys_Info.HPF_COR=12;                    //R24[3:0]=12 => R24[7:4]
        R850_Sys_Info.FILT_EXT_ENA=0;//ext disable
        R850_Sys_Info.FILT_COMP=1; //R24[3:2] [0~3 ; input: "0~3"]
        break;
    ...
    case R850_DVB_T2_1_7M_IF_5M:
        ...
        R850_Sys_Info.BW=2;                                //BW=6M
        R850_Sys_Info.FILT_CAL_IF=6000;           //CAL IF
        R850_Sys_Info.HPF_COR=1;                      //R24[3:0]=1 => R24[7:4]
        R850_Sys_Info.FILT_EXT_ENA=0;//ext disable
        R850_Sys_Info.FILT_COMP=1; //R24[3:2] [0~3 ; input: "0~3"]
        break;
    ...
    case R850_ISDB_T_IF_5M:
        ...
        R850_Sys_Info.HPF_COR=3;                      //R24[3:0]=4 (2.25M) => R24[7:4]
        if(R850_Chip == R850_MP)
    ...
    case R850_FM:
        ...
        R850_Sys_Info.HPF_COR=2;                      //R24[3:0]=2 => R24[7:4]
        R850_Sys_Info.FILT_EXT_ENA=0; //ext disable
        R850_Sys_Info.FILT_COMP=1; //R24[3:2] [0~3 ; input: "0~3"]
        break;
...
R850_ErrCode R850_Init(MS_U8 u8TunerIndex, R850_Standard_Type R850_Standard)
{
    ...
    R850_Sys_Init = R850_Sys_Sel(R850_Standard);
...
R850_ErrCode R850_InitReg(MS_U8 u8TunerIndex)//ok
{
    ...
    //Clock Out Enable
    R850_I2C_Len.Data[34] = R850_GET_REG_Array(u8TunerIndex, 34) & 0xFB;
    R850_SET_REG_Array(u8TunerIndex, 34, 0xFB, 0);
#endif
...
R850_ErrCode R850_MUX(MS_U8 u8TunerIndex, MS_U32 LO_KHz, MS_U32 RF_KHz, R850_Standard_Type R850_Standard)
{
    ...
    *(pFreq_Info1 + u8TunerIndex) = R850_Freq_Sel(LO_KHz, RF_KHz, R850_Standard);
    ...
    //Set_IMR
    if(*(pR850_IMR_done_flag + u8TunerIndex) == TRUE)
    {
        if(R850_Mixer_Mode==R850_IMR_NOR)
        {
...
R850_ErrCode R850_SetStandard(MS_U8 u8TunerIndex, R850_Standard_Type RT_Standard)
{
    ...
    if(RT_Standard != (*ppre_standard))
    {
      ...
      *pSys_Info = R850_Sys_Sel(RT_Standard);
...
MS_BOOL MDrv_Tuner_R850_Init(MS_U8 u8TunerIndex,TUNER_MS_INIT_PARAM* pParam)
{
     ...
     *(pSys_Info1 + u8TunerIndex) = R850_Sys_Sel(R850_ISDB_T_IF_5M); //set as default
...
MS_BOOL R850_CheckExist(MS_U8 u8TunerIndex, MS_U32* pu32channel_cnt)
{
    ...
    TUNER_DBG(("[R850]CheckExist\n"));
    hwi2c_port = getI2CPort(u8TunerIndex);

    for(i=0; i< sizeof(R850_Address_List); i++)
    {
        InitParam[u8TunerIndex].u8SlaveID = R850_Address_List[i];
        R850_I2C_Len.Len = 1;
        R850_I2C_Len.Data[0] = 0;
        if(R850_I2C_Read_Len(u8TunerIndex, &R850_I2C_Len) != TRUE)
        {
            TUNER_DBG(("[R850] Read chip ID fail with slave ID 0x%x\n", InitParam[u8TunerIndex].u8SlaveID));
        }
        else
        {
    ...
    TUNER_DBG(("[R850] read id =0x%x I2C slave ID = 0x%02x\n",regData, InitParam[u8TunerIndex].u8SlaveID));
