...
MXL_STATUS_E Ctrl_DfeRegRead(UINT8 devId, UINT16 regAddr, UINT8 numRegs, UINT32* regDataPtr )
{
  ...
  status |= Ctrl_ReadRegister(devId,DFE_FW_REG_BUFFER_MSB, &tmp);
  *regDataPtr = tmp;
  
  status |= Ctrl_ReadRegister(devId,DFE_FW_REG_BUFFER_LSB, &tmp);
...
MXL_STATUS_E Ctrl_ProgramRegisters(UINT8 devId, PREG_CTRL_INFO_T ctrlRegInfoPtr)
{
  ...
  while (1)   
  {
    ...
    {
      status = Ctrl_ReadRegister(devId,ctrlRegInfoPtr[i].regAddr, &tmp);
    ...
    status = Ctrl_WriteRegister(devId,ctrlRegInfoPtr[i].regAddr, tmp);
    if (status != MXL_SUCCESS) break;
...
MXL_STATUS_E Ctrl_SetRfFreqLutReg(UINT8 devId, UINT32 FreqInHz, PMXL68x_CHAN_DEP_FREQ_TABLE_T freqLutPtr)
{
  ...
  for (idx = 0; idx < MxL68x_SPUR_REGISTER.SpurRegNum; idx++)
  {
    ...
    {
      status |= Ctrl_ReadRegister(devId, MxL68x_SPUR_REGISTER.SpurRegAddr[idx], &readBack);
      regSetData[idx] |= (readBack & 0x00DF);  // Protect bit <4:0>, clear bit<5>, 1101 1111 = 0xDF  
    }
    status |= Ctrl_WriteRegister(devId, MxL68x_SPUR_REGISTER.SpurRegAddr[idx], regSetData[idx]);
  }
    ...
    if (devContextPtr->driverStatus)
    {
      ...
      devContextPtr->fwIntRetData.DataLength = ISDBT_AC_DATA_LENGTH;
						...
						if ((readBack & DMD_INT_MASK_AC_DATA) == DMD_INT_MASK_AC_DATA)
						{
								...
								status |= Ctrl_WriteRegister(devId, DMD_INTERRUPT_MAILBOX_REG, control);
...
MXL68x_FW_STATUS_E Ctrl_CheckFirmwareStatus(UINT8 devId)
{
  ...
  mxlStatus |= Ctrl_ReadRegister(devId, FIRMWARE_LOADED_CHECK_REG - 1, &fwDlStatus);
  mxlStatus |= Ctrl_ReadRegister(devId, FIRMWARE_LOADED_CHECK_REG, &fwDlStatus);

  // Readk back 0x300B bit<1> to know if CPU is clock gated
  mxlStatus |= Ctrl_ReadRegister(devId, DMD_GPIO_FUNC_MODE_REG - 1, &fwClkStatus);
  mxlStatus |= Ctrl_ReadRegister(devId, DMD_GPIO_FUNC_MODE_REG, &fwClkStatus);
...
MXL_STATUS_E Ctrl_WriteRegister(UINT8 devId, UINT16 regAddr, UINT16 regData)
{
  UINT8 status = MXL_SUCCESS;
  UINT16 pageNumber;
  PMXL68x_DEV_CONTEXT_T devContextPtr = MxL68x_Ctrl_GetDeviceContext(devId);
...
MXL_STATUS_E Ctrl_ReadRegister(UINT8 devId, UINT16 regAddr, UINT16 *dataPtr)
{
  UINT16 pageNumber;
  UINT8 status = MXL_SUCCESS;
  PMXL68x_DEV_CONTEXT_T devContextPtr = MxL68x_Ctrl_GetDeviceContext(devId);
