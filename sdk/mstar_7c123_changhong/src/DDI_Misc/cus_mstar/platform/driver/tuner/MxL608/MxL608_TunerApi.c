...
MXL_STATUS MxLWare608_API_CfgDevSoftReset(UINT8 u8TunerIndex,UINT8 devId)
{
  ...
  status = MxLWare608_OEM_WriteRegister(u8TunerIndex, devId, AIC_RESET_REG, 0x00);
...
MXL_STATUS MxLWare608_API_CfgDevOverwriteDefaults(UINT8 u8TunerIndex,
                                                  UINT8 devId,
                                                  MXL_BOOL singleSupply_3_3V)
{
  ...
  status |= MxL608_Ctrl_ProgramRegisters(u8TunerIndex,devId, MxL608_OverwriteDefaults);

  status |= MxLWare608_OEM_WriteRegister(u8TunerIndex,devId, 0x00, 0x01);
  status |= MxLWare608_OEM_ReadRegister(u8TunerIndex,devId, 0x31, &readData);
  readData &= 0x2F;
  readData |= 0xD0;
  status |= MxLWare608_OEM_WriteRegister(u8TunerIndex,devId, 0x31, readData);
  status |= MxLWare608_OEM_WriteRegister(u8TunerIndex,devId, 0x00, 0x00);


  /* If Single supply 3.3v is used */
  if (MXL_ENABLE == singleSupply_3_3V)
    status |= MxLWare608_OEM_WriteRegister(u8TunerIndex,devId, MAIN_REG_AMP, 0x04);
...
MXL_STATUS MxLWare608_API_CfgDevXtal(UINT8 u8TunerIndex,UINT8 devId, MXL608_XTAL_SET_CFG_T xtalCfg)
{
    UINT8 status = MXL_SUCCESS;
    UINT8 control = 0;
  ...
  TUNER_DBG(("\n%s\n", __FUNCTION__));
    ...
    if ((xtalCfg.xtalFreqSel == MXL608_XTAL_16MHz) || (xtalCfg.xtalFreqSel == MXL608_XTAL_24MHz))
    {
        ...
        if (xtalCfg.XtalSharingMode == MXL_ENABLE)
        {
            ...
            status |= MxLWare608_OEM_WriteRegister(u8TunerIndex, devId, XTAL_ENABLE_DIV_REG, control);
            status |= MxLWare608_OEM_WriteRegister(u8TunerIndex, devId, XTAL_EXT_BIAS_REG, 0x80);
        }
        else
        {
            control &= 0x01;
            // program Clock out div & Xtal sharing
            status |= MxLWare608_OEM_WriteRegister(u8TunerIndex,devId, XTAL_ENABLE_DIV_REG, control);
            status |= MxLWare608_OEM_WriteRegister(u8TunerIndex,devId, XTAL_EXT_BIAS_REG, 0x0A);
        }

        // Main regulator re-program
        if (MXL_ENABLE == xtalCfg.singleSupply_3_3V)
            status |= MxLWare608_OEM_WriteRegister(u8TunerIndex,devId, MAIN_REG_AMP, 0x14);
    }
    else
...
MXL_STATUS MxLWare608_API_CfgDevPowerMode(UINT8 u8TunerIndex,UINT8 devId, 
                                          ...
                                          UINT8 standbyLt)
{
    ...
    case MXL608_PWR_MODE_ACTIVE:
      status |= MxLWare608_OEM_WriteRegister(u8TunerIndex,devId, TUNER_ENABLE_REG, MXL_ENABLE);
      status |= MxLWare608_OEM_WriteRegister(u8TunerIndex,devId, START_TUNE_REG, MXL_ENABLE);
      status |= MxLWare608_OEM_WriteRegister(u8TunerIndex,devId, PAGE_CHANGE_REG, 0x01);
      if (enableLoopthrough == MXL_ENABLE)     
        status |= MxLWare608_OEM_WriteRegister(u8TunerIndex,devId,DFE_SEQ_TUNE_RF1_BO_REG,0x0E);
      else 
        status |= MxLWare608_OEM_WriteRegister(u8TunerIndex,devId,DFE_SEQ_TUNE_RF1_BO_REG,0x37);
      status |= MxLWare608_OEM_WriteRegister(u8TunerIndex,devId, PAGE_CHANGE_REG, 0x00); 
      break;

    case MXL608_PWR_MODE_STANDBY:
      status |= MxLWare608_OEM_WriteRegister(u8TunerIndex,devId, START_TUNE_REG, MXL_DISABLE);
      status |= MxLWare608_OEM_WriteRegister(u8TunerIndex,devId, TUNER_ENABLE_REG, MXL_DISABLE);
      if ((standbyLt != 0) && (enableLoopthrough == MXL_ENABLE))
      {
        status |= MxLWare608_OEM_WriteRegister(u8TunerIndex,devId, PAGE_CHANGE_REG, 0x01);
        status |= MxLWare608_OEM_WriteRegister(u8TunerIndex,devId,DFE_SEQ_TUNE_RF1_BO_REG,(standbyLt & 0x3F));
        status |= MxLWare608_OEM_WriteRegister(u8TunerIndex,devId, PAGE_CHANGE_REG, 0x00);
      }	
      break;
...
MXL_STATUS MxLWare608_API_CfgDevGPO(UINT8 u8TunerIndex,UINT8 devId, MXL608_GPO_STATE_E gpoState)
{
    UINT8 status = MXL_SUCCESS;
    UINT8 regData = 0;
...
MXL_STATUS MxLWare608_API_ReqDevVersionInfo(UINT8 u8TunerIndex,UINT8 devId,
                                            MXL608_VER_INFO_T* mxlDevVerInfoPtr)
{
    ...
    if (mxlDevVerInfoPtr)
    {
        status |= MxLWare608_OEM_ReadRegister(u8TunerIndex,devId, CHIP_ID_REQ_REG, &readBack);
        mxlDevVerInfoPtr->chipId = (readBack & 0xFF);

        status |= MxLWare608_OEM_ReadRegister(u8TunerIndex,devId, CHIP_VERSION_REQ_REG, &readBack);
        mxlDevVerInfoPtr->chipVersion = (readBack & 0xFF);

        TUNER_DBG(("Chip ID = 0x%d, Version = 0x%d \n", mxlDevVerInfoPtr->chipId, mxlDevVerInfoPtr->chipVersion));
...
MXL_STATUS MxLWare608_API_ReqDevGPOStatus(UINT8 u8TunerIndex, UINT8 devId,
...
MXL_STATUS MxLWare608_API_ReqDevPllState(UINT8 u8TunerIndex,UINT8 devId, MXL608_PLL_STATE_E* PllStatePtr)
{
  ...
  *PllStatePtr = MXL608_PLL_STATE_NA; 
  ...
  for (k = 0; k < 7; k++)
  {
    ...
    status |= MxLWare608_OEM_ReadRegister(u8TunerIndex,devId, regAddr[k], &regData[k]);
  }
                                       ...
                                       MXL608_TUNER_MODE_CFG_T tunerModeCfg)
{
    ...
    TUNER_DBG(("%s: Signal Mode = %d, IF Freq = %d, xtal = %d, IF Gain = %d\n",
                                                __FUNCTION__,
    ...
    case MXL608_DIG_J83B:
        ...
        status = MxL608_Ctrl_ProgramRegisters(u8TunerIndex,devId, tmpRegTable);
        ...
        status |= MxLWare608_OEM_WriteRegister(u8TunerIndex,devId, DFE_CSF_SS_SEL, dfeRegData);
    ...
    case MXL608_DIG_ISDBT_ATSC:
      ...
      switch(tunerModeCfg.ifOutGainLevel)
      {
        ...
        status |= MxLWare608_OEM_WriteRegister(u8TunerIndex,devId, DFE_DACIF_GAIN, dfeRegData);
    ...
    {
        // XTAL calibration
        status |= MxLWare608_OEM_WriteRegister(u8TunerIndex,devId, XTAL_CALI_SET_REG, 0x00);
        status |= MxLWare608_OEM_WriteRegister(u8TunerIndex,devId, XTAL_CALI_SET_REG, 0x01);
...
MXL_STATUS MxLWare608_API_CfgTunerAGC(UINT8 u8TunerIndex,UINT8 devId, MXL608_AGC_CFG_T agcCfg)
{
    UINT8 status = MXL_SUCCESS;
    UINT8 regData = 0;

    TUNER_DBG(("%s, AGC sel = %d, attack point set = %d, Flip = %d \n",
                                                __FUNCTION__,
                                                agcCfg.agcType,
                                                agcCfg.setPoint,
      ...
      (agcCfg.agcType <= MXL608_AGC_EXTERNAL))
    {
        ...
        status |= MxLWare608_OEM_ReadRegister(u8TunerIndex,devId, AGC_CONFIG_REG, &regData);
        regData &= 0xF2; // Clear bits <3:2> & <0>
        regData = (UINT8) (regData | (agcCfg.agcType << 2) | 0x01);
        status |= MxLWare608_OEM_WriteRegister(u8TunerIndex,devId, AGC_CONFIG_REG, regData);

        // AGC set point <6:0>
        status |= MxLWare608_OEM_ReadRegister(u8TunerIndex,devId, AGC_SET_POINT_REG, &regData);
        regData &= 0x80; // Clear bit <6:0>
        regData |= agcCfg.setPoint;
        status |= MxLWare608_OEM_WriteRegister(u8TunerIndex,devId, AGC_SET_POINT_REG, regData);

        // AGC Polarity <4>
        status |= MxLWare608_OEM_ReadRegister(u8TunerIndex,devId, AGC_FLIP_REG, &regData);
        regData &= 0xEF; // Clear bit <4>
        regData |= (agcCfg.agcPolarityInverstion << 4);
        status |= MxLWare608_OEM_WriteRegister(u8TunerIndex,devId, AGC_FLIP_REG, regData);
    }
    else
        status = MXL_INVALID_PARAMETER;
...
MXL_STATUS MxLWare608_API_CfgTunerLoopThrough(UINT8 u8TunerIndex,UINT8 devId, MXL_BOOL loopThroughCtrl, UINT8* gainArray, UINT8* attArray)
{
  UINT8 status = MXL_SUCCESS;
  UINT8 regData;
  ...
  if (loopThroughCtrl <= MXL_ENABLE)
  {
    ...
    status |= MxLWare608_OEM_ReadRegister(u8TunerIndex,devId, DIG_ANA_GINJO_LT_REG, &regData);

    if (loopThroughCtrl == MXL_ENABLE)
      regData |= 0x10;  // Bit<4> = 1       
    else
      regData &= 0xEF;  // Bit<4> = 0  
    status |= MxLWare608_OEM_WriteRegister(u8TunerIndex,devId, DIG_ANA_GINJO_LT_REG, regData);

    if ((loopThroughCtrl == MXL_ENABLE) && (gainArray != NULL) && (attArray != NULL))
    {
      regData = (gainArray[3]<<6 ) | (gainArray[2] <<4 ) | (gainArray[1] <<2 ) | gainArray[0] ;
      status |= MxLWare608_OEM_WriteRegister(u8TunerIndex,devId, DFE_SEQ_DIGANA_LT_GAIN_REG, regData);
      regData = (attArray[3]<<6 ) | (attArray[2] <<4 ) | (attArray[1] <<2 ) | attArray[0] ;
      status |= MxLWare608_OEM_WriteRegister(u8TunerIndex,devId, DFE_SEQ_DIGANA_LT_ATTN_REG, regData);
    }
    status |= MxLWare608_OEM_WriteRegister(u8TunerIndex,devId, PAGE_CHANGE_REG, 0x00);
  }
  else
...
MXL_STATUS MxLWare608_API_CfgTunerSouthAfricaLT(UINT8 u8TunerIndex,UINT8 devId)
{
    UINT8 status = MXL_SUCCESS;
  ...
  status |= MxLWare608_OEM_WriteRegister(u8TunerIndex,devId, PAGE_CHANGE_REG, 0x01);
  status |= MxLWare608_OEM_WriteRegister(u8TunerIndex,devId, DFE_SEQ_DIGANA_LT_GAIN_REG, 0x95);
  status |= MxLWare608_OEM_WriteRegister(u8TunerIndex,devId, DFE_SEQ_DIGANA_LT_ATTN_REG, 0x00);
  status |= MxLWare608_OEM_WriteRegister(u8TunerIndex,devId, PAGE_CHANGE_REG, 0x00);
...
MXL_STATUS MxLWare608_API_CfgTunerChanTune(UINT8 u8TunerIndex,UINT8 devId,
  ...
  if (chanTuneCfg.signalMode == MXL608_DIG_DVB_T_DTMB){
    ...
    if (chanTuneCfg.startTune == MXL_ENABLE)
    {
        if (chanTuneCfg.signalMode <= MXL608_DIG_J83B)
        {
            ...
            {
                status |= MxLWare608_OEM_WriteRegister(u8TunerIndex,devId, 0x7C, 0x1F);
            ...
            {
                status |= MxLWare608_OEM_WriteRegister(u8TunerIndex,devId, 0x7C, 0x9F);
                if ((chanTuneCfg.signalMode == MXL608_DIG_DVB_C) || (chanTuneCfg.signalMode == MXL608_DIG_J83B))
                    regData = 0xD1;
                else
                    regData = 0x91;
      ...
      status |= MxLWare608_OEM_WriteRegister(u8TunerIndex,devId, 0x00, 0x01);
      status |= MxLWare608_OEM_WriteRegister(u8TunerIndex,devId, 0x31, regData);
      status |= MxLWare608_OEM_WriteRegister(u8TunerIndex,devId, 0x00, 0x00);
      ...
      status |= MxLWare608_OEM_WriteRegister(u8TunerIndex,devId, FINE_TUNE_INIT1_REG, dfeAgcRssispData); 
#endif
      ...
      switch(bw)
      {
        ...
        case MXL608_TERR_BW_8MHz:
            ...
            // Set RF  
            status |= MxLWare608_OEM_WriteRegister(u8TunerIndex,devId, CHAN_TUNE_LOW_REG, (UINT8)(freq & 0xFF));
            status |= MxLWare608_OEM_WriteRegister(u8TunerIndex,devId, CHAN_TUNE_HI_REG, (UINT8)((freq >> 8 ) & 0xFF));
            break;
      ...
      status |= MxLWare608_OEM_WriteRegister(u8TunerIndex,devId, TUNER_ENABLE_REG, 0x01);
            ...
            status |= MxLWare608_OEM_ReadRegister(u8TunerIndex,devId, 0xB6, &agcData);
            status |= MxLWare608_OEM_WriteRegister(u8TunerIndex,devId, PAGE_CHANGE_REG, 0x01);
            status |= MxLWare608_OEM_ReadRegister(u8TunerIndex,devId, 0x60, &dfeTuneData);
            status |= MxLWare608_OEM_ReadRegister(u8TunerIndex,devId, 0x5F, &dfeCdcData);
            ...
            status |= MxLWare608_OEM_WriteRegister(u8TunerIndex,devId, 0x60, dfeTuneData);
            status |= MxLWare608_OEM_WriteRegister(u8TunerIndex,devId, 0x5F, dfeCdcData);
            status |= MxLWare608_OEM_WriteRegister(u8TunerIndex,devId, PAGE_CHANGE_REG, 0x00);
            status |= MxLWare608_OEM_WriteRegister(u8TunerIndex,devId, 0xB6, agcData);

            // Bit <0> 1 : start , 0 : abort calibrations
            status |= MxLWare608_OEM_WriteRegister(u8TunerIndex,devId, START_TUNE_REG, 0x01);
...
MXL_STATUS MxLWare608_API_CfgTunerIFOutParam(UINT8 u8TunerIndex,UINT8 devId, MXL608_IF_OUT_CFG_T ifOutCfg)
{
    UINT16 ifFcw;
    UINT8 status = MXL_SUCCESS;
    UINT8 readData = 0;
    ...
    TUNER_DBG(("%s, Manual set = %d \n", __FUNCTION__, ifOutCfg.manualFreqSet));

    // Read back register for manual IF Out
    status = MxLWare608_OEM_ReadRegister(u8TunerIndex,devId, IF_FREQ_SEL_REG, &readData);

    if (ifOutCfg.manualFreqSet == MXL_ENABLE)
    {
        TUNER_DBG(("%s, IF Freq = %d \n", __FUNCTION__, ifOutCfg.manualIFOutFreqInKHz));

        // IF out manual setting : bit<5>
        readData |= 0x20;
        status = MxLWare608_OEM_WriteRegister(u8TunerIndex,devId, IF_FREQ_SEL_REG, readData);

        // Manual IF freq set
        ifFcw = (UINT16)(ifOutCfg.manualIFOutFreqInKHz * 8192 / 216000);
        control = (ifFcw & 0xFF); // Get low 8 bit
        status |= MxLWare608_OEM_WriteRegister(u8TunerIndex,devId, IF_FCW_LOW_REG, control);

        control = ((ifFcw >> 8) & 0x0F); // Get high 4 bit
        status |= MxLWare608_OEM_WriteRegister(u8TunerIndex,devId, IF_FCW_HIGH_REG, control);
    }
    ...
    if (status == MXL_SUCCESS)
    {
        ...
        if (ifOutCfg.ifInversion <= MXL_ENABLE)
        {
            ...
            status |= MxLWare608_OEM_WriteRegister(u8TunerIndex,devId, IF_PATH_GAIN_REG, control);
        }
        else
            status |= MXL_INVALID_PARAMETER;
    }
...
MXL_STATUS MxLWare608_API_ReqTunerAGCLock(UINT8 u8TunerIndex,UINT8 devId, MXL_BOOL* agcLockStatusPtr)
{
    MXL_STATUS status = MXL_SUCCESS;
    UINT8 regData = 0;
    ...
    if (agcLockStatusPtr)
    {
        status = MxLWare608_OEM_ReadRegister(u8TunerIndex,devId, AGC_SAGCLOCK_STATUS_REG, &regData);
...
MXL_STATUS MxLWare608_API_ReqTunerLockStatus(UINT8 u8TunerIndex,UINT8 devId, MXL_BOOL* rfLockPtr,
                                                          MXL_BOOL* refLockPtr)
{
    MXL_STATUS status = MXL_SUCCESS;
    UINT8 regData = 0;
    ...
    {
        status = MxLWare608_OEM_ReadRegister(u8TunerIndex,devId, RF_REF_STATUS_REG, &regData);

        if ((regData & 0x02) == 0x02) rfLockStatus = MXL_LOCKED;
        if ((regData & 0x01) == 0x01) refLockStatus = MXL_LOCKED;

        TUNER_DBG((" RfSynthStatus = %d, RefSynthStatus = %d", (UINT8)rfLockStatus,
                                                                   (UINT8)refLockStatus));
...
MXL_STATUS MxLWare608_API_ReqTunerRxPower(UINT8 u8TunerIndex,UINT8 devId, SINT16* rxPwrPtr)
{
    UINT8 status = MXL_SUCCESS;
    UINT8 regData = 0;
    UINT16 tmpData = 0;
    ...
    if (rxPwrPtr)
    {
        ...
        status = MxLWare608_OEM_ReadRegister(u8TunerIndex,devId, RFPIN_RB_LOW_REG, &regData);
        tmpData = regData;

        // RF input power high <1:0>
        status |= MxLWare608_OEM_ReadRegister(u8TunerIndex,devId, RFPIN_RB_HIGH_REG, &regData);
        ...
        *rxPwrPtr = (tmpData & 0x01FF) * 25;  //100 times dBm
        ...
        TUNER_DBG((" Rx power = %d times of 0.01dBm \n", *rxPwrPtr));
    }
    else
        status = MXL_INVALID_PARAMETER;
