...
static MXL_STATUS MxL661_ConfigDevReset(UINT8 u8TunerIndex, PMXL661_RESET_CFG_T resetCfg)
{
  ...
  status = MxLWare661_OEM_WriteRegister(u8TunerIndex, resetCfg->I2cSlaveAddr, AIC_RESET_REG, 0x00);
...
static MXL_STATUS MxL661_ConfigDevOverwriteDefault(UINT8 u8TunerIndex, PMXL661_OVERWRITE_DEFAULT_CFG_T overDefaultPtr)
{
  UINT8 status = MXL_TRUE;
  UINT8 readData = 0;

  MxL_DLL_DEBUG0("MXL661_DEV_OVERWRITE_DEFAULT_CFG \n");
  status |= MxL661_Ctrl_ProgramRegisters(u8TunerIndex, overDefaultPtr->I2cSlaveAddr, MxL661_OverwriteDefaults);

  status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, overDefaultPtr->I2cSlaveAddr, 0x00, 0x01);
  status |= MxLWare661_OEM_ReadRegister(u8TunerIndex, overDefaultPtr->I2cSlaveAddr, 0x31, &readData);
  readData &= 0x2F;
  readData |= 0xD0;
  status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, overDefaultPtr->I2cSlaveAddr, 0x31, readData);
  status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, overDefaultPtr->I2cSlaveAddr, 0x00, 0x00);


  /* If Single supply 3.3v is used */
  if (MXL_ENABLE == overDefaultPtr->SingleSupply_3_3V)
    status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, overDefaultPtr->I2cSlaveAddr, MAIN_REG_AMP, 0x04);
...
static MXL_STATUS MxL661_ConfigDevPowerMode(UINT8 u8TunerIndex, PMXL661_PWR_MODE_CFG_T pwrModePtr)
{
  UINT8 status = MXL_TRUE;
  UINT8 i2cAddr;
  ...
  switch(pwrModePtr->PowerMode)
  {
    ...
    case MXL661_PWR_MODE_ACTIVE:
      status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, i2cAddr, TUNER_ENABLE_REG, MXL_ENABLE);
      status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, i2cAddr, START_TUNE_REG, MXL_ENABLE);
      break;

    case MXL661_PWR_MODE_STANDBY:
      status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, i2cAddr, START_TUNE_REG, MXL_DISABLE);
      status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, i2cAddr, TUNER_ENABLE_REG, MXL_DISABLE);
      break;
...
static MXL_STATUS MxL661_ConfigDevXtalSet(UINT8 u8TunerIndex, PMXL661_XTAL_SET_CFG_T xtalSetPtr)
{
  UINT8 status = MXL_TRUE;
  UINT8 control = 0;
  ...
  status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, xtalSetPtr->I2cSlaveAddr, XTAL_CAP_CTRL_REG, control);
  ...
  {
    control |= 0x40;
    status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, xtalSetPtr->I2cSlaveAddr, XTAL_ENABLE_DIV_REG, control);
    status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, xtalSetPtr->I2cSlaveAddr, XTAL_EXT_BIAS_REG, 0x80);
  }
  else
  {
    control &= 0x01;
    status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, xtalSetPtr->I2cSlaveAddr, XTAL_ENABLE_DIV_REG, control);
    status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, xtalSetPtr->I2cSlaveAddr, XTAL_EXT_BIAS_REG, 0x0A);
  }

  // Main regulator re-program
  if (MXL_ENABLE == xtalSetPtr->SingleSupply_3_3V)
    status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, xtalSetPtr->I2cSlaveAddr, MAIN_REG_AMP, 0x14);
...
static MXL_STATUS MxL661_ConfigDevIfOutSet(UINT8 u8TunerIndex, PMXL661_IF_OUT_CFG_T ifOutPtr)
{
  UINT16 ifFcw;
  UINT8 status = MXL_TRUE;
  UINT8 readData = 0;
  ...
  status |= MxLWare661_OEM_ReadRegister(u8TunerIndex, ifOutPtr->I2cSlaveAddr, IF_FREQ_SEL_REG, &readData);
  ...
  if (ifOutPtr->ManualFreqSet == MXL_ENABLE)
  {
    ...
    readData |= 0x20;
    status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, ifOutPtr->I2cSlaveAddr, IF_FREQ_SEL_REG, readData);
    ...
    status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, ifOutPtr->I2cSlaveAddr, IF_FCW_LOW_REG, control);

    control = ((ifFcw >> 8) & 0x0F); // Get high 4 bit
    status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, ifOutPtr->I2cSlaveAddr, IF_FCW_HIGH_REG, control);
  }
  else
  {
  ...
  status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, ifOutPtr->I2cSlaveAddr, IF_PATH_GAIN_REG, control);
...
static MXL_STATUS  MxL661_ConfigDevGPO(UINT8 u8TunerIndex, PMXL661_GPO_CFG_T gpoParamPtr)
{
  UINT8 status = MXL_TRUE;
  UINT8 regData = 0;
  ...
  status |= MxLWare661_OEM_ReadRegister(u8TunerIndex, gpoParamPtr->I2cSlaveAddr, GPO_SETTING_REG, &regData);
  ...
  if (MXL_FALSE != status)
    status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, gpoParamPtr->I2cSlaveAddr, GPO_SETTING_REG, regData);
...
static MXL_STATUS MxL661_GetDeviceInfo(UINT8 u8TunerIndex, PMXL661_DEV_INFO_T DevInfoPtr)
{
  UINT8 status = MXL_TRUE;
  UINT8 readBack = 0;
  UINT8 i2cAddr = 0;
  ...
  status |= MxLWare661_OEM_ReadRegister(u8TunerIndex, i2cAddr, CHIP_ID_REQ_REG, &readBack);
  DevInfoPtr->ChipId = (readBack & 0xFF);

  status |= MxLWare661_OEM_ReadRegister(u8TunerIndex, i2cAddr, CHIP_VERSION_REQ_REG, &readBack);
...
static MXL_STATUS  MxL661_GetDeviceGpoState(UINT8 u8TunerIndex, PMXL661_GPO_INFO_T gpoPtr)
{
  ...
  status = MxLWare661_OEM_ReadRegister(u8TunerIndex, gpoPtr->I2cSlaveAddr, GPO_SETTING_REG, &regData);
...
static MXL_STATUS  MxL661_GetDevicePllState(UINT8 u8TunerIndex, PMXL661_PLL_INFO_T pllPtr)
{
  UINT8 status = MXL_TRUE;
  UINT8 condition[3] = {0};
  ...
  for (k = 0; k < 7; k++)
  {
    ...
    status |= MxLWare661_OEM_ReadRegister(u8TunerIndex, pllPtr->I2cSlaveAddr, regAddr[k], &regData[k]);
  }
...
static MXL_STATUS MxL661_ConfigTunerPowerUp(UINT8 u8TunerIndex, PMXL661_POWER_UP_CFG_T topMasterCfgPtr)
{
  UINT8 status = MXL_TRUE;
  UINT8 regData = 0;
  ...
  status = MxLWare661_OEM_WriteRegister(u8TunerIndex, topMasterCfgPtr->I2cSlaveAddr, TUNER_ENABLE_REG, regData);
...
static MXL_STATUS MxL661_ConfigTunerSequenceSet(UINT8 u8TunerIndex, PMXL661_START_TUNE_CFG_T sequnCfgPtr)
{
  UINT8 status = MXL_TRUE;
  UINT8 regData = 0;
  UINT8 idx = 0;
  ...
  status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, sequnCfgPtr->I2cSlaveAddr, START_TUNE_REG, 0x00);
  ...
  if (sequnCfgPtr->StartTune == MXL_ENABLE)
  {
    ...
    status |= MxLWare661_OEM_ReadRegister(u8TunerIndex, sequnCfgPtr->I2cSlaveAddr, 0x6E, &regData);
    status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, sequnCfgPtr->I2cSlaveAddr, 0x6E, (regData | 0x80));

    status |= MxLWare661_OEM_ReadRegister(u8TunerIndex, sequnCfgPtr->I2cSlaveAddr, 0x14, &regData);
    status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, sequnCfgPtr->I2cSlaveAddr, 0x14, (regData & 0xDF));

    // Bit <0> 1 : start , 0 : abort calibrations
    status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, sequnCfgPtr->I2cSlaveAddr, START_TUNE_REG, 0x01);

    MxLWare661_OEM_Sleep(10);

    // Resume DIG_ANA_XTAL_CLK_EN_BYPS = 0 after start tune
    status |= MxLWare661_OEM_ReadRegister(u8TunerIndex, sequnCfgPtr->I2cSlaveAddr, 0x6E, &regData0);
    status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, sequnCfgPtr->I2cSlaveAddr, 0x6E, (regData0 & 0x7F));

#ifdef MXL661_CUSTOMER_SPECIFIC_SETTING_3

    status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, sequnCfgPtr->I2cSlaveAddr, 0x14, (regData | 0x20));

    status |= MxLWare661_OEM_ReadRegister(u8TunerIndex, sequnCfgPtr->I2cSlaveAddr, 0x13, &regData);

     // Check if signal type is Analog
    if (MxL661xtalSpurCfg[idx].appMode == MXL661_ANA_NTSC_MODE)
    {
      // Apply N+1 blocker settings
      status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, sequnCfgPtr->I2cSlaveAddr, DFE_FINETUNE_STEP_REG, 0x83);
    }
    else
    {
      // Undo N+1 blocker settings if app mode is not NTSC
      status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, sequnCfgPtr->I2cSlaveAddr, DFE_FINETUNE_STEP_REG, 0x80);
    }
    regData &= 0xFC; // Set Bit<1:0> = 00
    status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, sequnCfgPtr->I2cSlaveAddr, 0x13, regData);
...
static MXL_STATUS MxL661_ConfigTunerAtvHpfLpf(UINT8 u8TunerIndex, PMXL661_TUNER_HPF_LPF_CFG_T atvTunerHpfLpfCfgPtr)
{
  ...
  status = MxLWare661_OEM_ReadRegister(u8TunerIndex, atvTunerHpfLpfCfgPtr->I2cSlaveAddr, CHIP_VERSION_REQ_REG, &readData);
  if(readData != 0x4)
  {
  ...
  status = MxL661_GetRSSI(u8TunerIndex, atvTunerHpfLpfCfgPtr->I2cSlaveAddr, &dfeRSSIOnePercentdB);  // Get RSSI
  ...
  {
    status |= MxL661_GetRFStrenth(u8TunerIndex, atvTunerHpfLpfCfgPtr->I2cSlaveAddr, &RFStrOnePercentdB);
    status |= MxL661_GetRSSI(u8TunerIndex, atvTunerHpfLpfCfgPtr->I2cSlaveAddr, &dfeRSSIOnePercentdB); if(status != MXL_TRUE) return (MXL_STATUS)status;
    ...
    {
      // enable HPF
      status |= MxLWare661_OEM_ReadRegister(u8TunerIndex, atvTunerHpfLpfCfgPtr->I2cSlaveAddr, HPF_CTRL_REG, &readData);
      readData |= 0x04;
      status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, atvTunerHpfLpfCfgPtr->I2cSlaveAddr, HPF_CTRL_REG, readData);
      MxL_DLL_DEBUG0("HPF is on \n");

      do {
        MxLWare661_OEM_Sleep(WAIT_TIME_FOR_RSSI_STEP_MS);
        status |= MxLWare661_OEM_ReadRegister(u8TunerIndex, atvTunerHpfLpfCfgPtr->I2cSlaveAddr, RF_RSSI_DB_RB, &readData);
        if (readData != 0)
          break;
        waitTimeCnt --;
      }while (waitTimeCnt > 0);
    ...
    {
      // enable LPF
      status |= MxLWare661_OEM_ReadRegister(u8TunerIndex, atvTunerHpfLpfCfgPtr->I2cSlaveAddr, LPF_CTRL_REG, &readData);
      readData |= 0x08;
      status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, atvTunerHpfLpfCfgPtr->I2cSlaveAddr, LPF_CTRL_REG, readData);
      ...
      status |= MxL661_GetRFStrenth(u8TunerIndex, atvTunerHpfLpfCfgPtr->I2cSlaveAddr, &RFStrOnePercentdB); // Get RF Strenth
      status |= MxL661_GetRSSI(u8TunerIndex, atvTunerHpfLpfCfgPtr->I2cSlaveAddr, &dfeRSSIOnePercentdB); // Get RSSI
      if((RFStrOnePercentdB - RF_RSSI_OFFSET) < (dfeRSSIOnePercentdB - DFE_RSSI_OFFSET)) {
        MxL_DLL_DEBUG0("Warning: RF Strength is smaller than RSSI. \n");
        return (MXL_STATUS)status;
...
static MXL_STATUS MxL661_ConfigTunerApplicationModeSet(UINT8 u8TunerIndex, PMXL661_TUNER_MODE_CFG_T modeSetPtr)
{
  UINT8 status = MXL_TRUE;
  UINT8 dfeRegData = 0;
...

  {
    ...
    switch(modeSetPtr->SignalMode)
    {
      ...
      case MXL661_ANA_NTSC_MODE:
        ...
        status |= MxL661_Ctrl_ProgramRegisters(u8TunerIndex, modeSetPtr->I2cSlaveAddr, tmpRegTable);

        if (modeSetPtr->IFOutFreqinKHz < HIGH_IF_35250_KHZ)
        {
          // Low power IF dependent settings
#ifdef MXL661_CUSTOMER_SPECIFIC_SETTING_1
          status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, modeSetPtr->I2cSlaveAddr, DIG_ANA_IF_CFG_0, 0xFE);
#else
          status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, modeSetPtr->I2cSlaveAddr, DIG_ANA_IF_CFG_0, 0x7E);
#endif
          status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, modeSetPtr->I2cSlaveAddr, DIG_ANA_IF_CFG_1, 0x10);
          ...
          status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, modeSetPtr->I2cSlaveAddr, 0xDD, dfeRegData);

        }
        else
        {
          // High power IF dependent settings
          status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, modeSetPtr->I2cSlaveAddr, DIG_ANA_IF_CFG_0, 0xD9);
          status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, modeSetPtr->I2cSlaveAddr, DIG_ANA_IF_CFG_1, 0x16);
      ...
      case MXL661_ANA_PAL_BG:
        ...
        if (modeSetPtr->IFOutFreqinKHz < HIGH_IF_35250_KHZ)
        {
          ...
          status |= MxL661_Ctrl_ProgramRegisters(u8TunerIndex, modeSetPtr->I2cSlaveAddr, MxL661_Pal_RfLutSwpLIF);
        }
        else
        {
          // High power IF dependent settings
      ...
      case MXL661_ANA_PAL_I:
        status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, modeSetPtr->I2cSlaveAddr, 0x0C, 0x0B);
        ...
        else
        {
          ...
          status |= MxL661_Ctrl_ProgramRegisters(u8TunerIndex, modeSetPtr->I2cSlaveAddr, MxL661_PalI_RfLutSwpHIF);
      ...
      case MXL661_ANA_PAL_D:
        status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, modeSetPtr->I2cSlaveAddr, 0x0C, 0x0C);
        ...
        else
        {
          ...
          status |= MxL661_Ctrl_ProgramRegisters(u8TunerIndex, modeSetPtr->I2cSlaveAddr, MxL661_PalD_RfLutSwpHIF);
      ...
      case MXL661_DIG_J83B:
        ...
        status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, modeSetPtr->I2cSlaveAddr, DFE_CSF_SS_SEL, dfeRegData);
      ...
      case MXL661_DIG_ISDBT_ATSC:
        ...
        {
          // Low power IF dependent settings
          status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, modeSetPtr->I2cSlaveAddr, DIG_ANA_IF_CFG_0, 0xF9);
          status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, modeSetPtr->I2cSlaveAddr, DIG_ANA_IF_CFG_1, 0x18);
          status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, modeSetPtr->I2cSlaveAddr, DIG_ANA_IF_PWR, 0xF1);
        }
        else
        {
          // High power IF dependent settings
          status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, modeSetPtr->I2cSlaveAddr, DIG_ANA_IF_CFG_0, 0xD9);
          status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, modeSetPtr->I2cSlaveAddr, DIG_ANA_IF_CFG_1, 0x16);
          status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, modeSetPtr->I2cSlaveAddr, DIG_ANA_IF_PWR, 0xB1);
        }
        ...
        status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, modeSetPtr->I2cSlaveAddr, DFE_DACIF_GAIN, dfeRegData);
      ...
      case MXL661_DIG_DVB_T_DTMB:
        ...
        if (modeSetPtr->IFOutFreqinKHz < HIGH_IF_35250_KHZ)
        {
          // Low power IF dependent settings
          status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, modeSetPtr->I2cSlaveAddr, DIG_ANA_IF_CFG_0, 0xFE);
          status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, modeSetPtr->I2cSlaveAddr, DIG_ANA_IF_CFG_1, 0x10); /* If it low IF, change here */
          status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, modeSetPtr->I2cSlaveAddr, DIG_ANA_IF_PWR, 0xB1); /* If it low IF, change here */
        }
        else
        {
          // High power IF dependent settings
  ...
  if (MXL_FALSE != status)
  {
    status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, modeSetPtr->I2cSlaveAddr, XTAL_EXT_BIAS_REG, 0x0A);

    // XTAL calibration
    status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, modeSetPtr->I2cSlaveAddr, XTAL_CALI_SET_REG, 0x00);
    status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, modeSetPtr->I2cSlaveAddr, XTAL_CALI_SET_REG, 0x01);
    ...
    if (modeSetPtr->SignalMode <= MXL661_ANA_SECAM_L)
    {
      ...
      status |= MxLWare661_OEM_ReadRegister(u8TunerIndex, modeSetPtr->I2cSlaveAddr, CHIP_VERSION_REQ_REG, &regData);
      if((regData & 0xFF) == 0x4)//MxL661 V4
      {
        // Program 0x0169 register with 0x01
        status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, modeSetPtr->I2cSlaveAddr, PAGE_CHANGE_REG, 0x01);
        status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, modeSetPtr->I2cSlaveAddr, DFE_SEQ_SPARE2_REG, 0x01);
        status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, modeSetPtr->I2cSlaveAddr, PAGE_CHANGE_REG, 0x00);
        MxL_DLL_DEBUG0("Program 0x0169 register with 0x01 for MxL661 v4. \n");
      }
      else if((regData & 0xFF) == 0x2)
      {
        status |= MxLWare661_OEM_ReadRegister(u8TunerIndex, modeSetPtr->I2cSlaveAddr,0x5F, &regData);
      ...
      for (idx = 0; idx < MAX_MXL661_DEVICES; idx++)
      {
        ...
        if (modeSetPtr->SignalMode == MXL661_ANA_NTSC_MODE)
        {
          if (MxL661ifFcwCfg[idx].i2cAddr == modeSetPtr->I2cSlaveAddr)
          {
            ...
            {
              status |= MxLWare661_OEM_ReadRegister(u8TunerIndex, modeSetPtr->I2cSlaveAddr, IF_FREQ_SEL_REG, &regData);
              status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, modeSetPtr->I2cSlaveAddr, IF_FREQ_SEL_REG, (regData | 0x20));
            ...
            status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, modeSetPtr->I2cSlaveAddr, IF_FCW_LOW_REG, regData);

            regData = (UINT8)((ifFcw >> 8) & 0x000F);
            status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, modeSetPtr->I2cSlaveAddr, IF_FCW_HIGH_REG, regData);
        ...
        else
        {
          ...
          if (MxL661ifFcwCfg[idx].i2cAddr == modeSetPtr->I2cSlaveAddr)
          {
            ...
            if (MxL661ifFcwCfg[idx].if_fcw_byp == 0)
            {
              ...
              status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, modeSetPtr->I2cSlaveAddr, IF_FREQ_SEL_REG, regData);
            }
            else
            {
              // IF out manual setting : bit<5>
        ...
        if (MxL661xtalSpurCfg[idx].i2cAddr == modeSetPtr->I2cSlaveAddr)
        {
          ...
          {
            // change page to 1
            status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, modeSetPtr->I2cSlaveAddr, 0x00, 0x01);

            // read reg ana_dig_refsx_xtal_amp_rb
            status |= MxLWare661_OEM_ReadRegister(u8TunerIndex, modeSetPtr->I2cSlaveAddr, 0xA2, &MxL661xtalSpurCfg[idx].xtalAmp_Rb);

            // read reg ana_dig_refsx_xtal_amp_lin_rb
            status |= MxLWare661_OEM_ReadRegister(u8TunerIndex, modeSetPtr->I2cSlaveAddr, 0xA1, &MxL661xtalSpurCfg[idx].xtalAmp_LinRb);

            MxL661xtalSpurCfg[idx].xtalAmp_LinRb &= 0x30;
            MxL661xtalSpurCfg[idx].xtalAmp_LinRb >>= 4;

            // change page to 0
            status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, modeSetPtr->I2cSlaveAddr, 0x00, 0x00);
...
static MXL_STATUS MxL661_ConfigTunerAgcSet(UINT8 u8TunerIndex, PMXL661_AGC_SET_CFG_T agcSetPtr)
{
  UINT8 status = MXL_TRUE;
  UINT8 regData = 0;
  ...
  status |= MxLWare661_OEM_ReadRegister(u8TunerIndex, agcSetPtr->I2cSlaveAddr, AGC_CONFIG_REG, &regData);
  regData &= 0xF2; // Clear bits <3:2> & <0>
  regData = (UINT8) (regData | (agcSetPtr->AgcType << 2) | agcSetPtr->AgcSel);
  status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, agcSetPtr->I2cSlaveAddr, AGC_CONFIG_REG, regData);

  // AGC set point <6:0>
  status |= MxLWare661_OEM_ReadRegister(u8TunerIndex, agcSetPtr->I2cSlaveAddr, AGC_SET_POINT_REG, &regData);
  regData &= 0x80; // Clear bit <6:0>
  regData |= agcSetPtr->SetPoint;
  status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, agcSetPtr->I2cSlaveAddr, AGC_SET_POINT_REG, regData);

  // AGC Polarity <4>
  status |= MxLWare661_OEM_ReadRegister(u8TunerIndex, agcSetPtr->I2cSlaveAddr, AGC_FLIP_REG, &regData);
  regData &= 0xEF; // Clear bit <4>
  regData |= (agcSetPtr->AgcPolarityInverstion << 4);
  status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, agcSetPtr->I2cSlaveAddr, AGC_FLIP_REG, regData);
...
static MXL_STATUS MxL661_ConfigTunerFineTune(UINT8 u8TunerIndex, PMXL661_FINE_TUNE_CFG_T fineTunePtr)
{
  UINT8 status = MXL_TRUE;
  UINT8 regData = 0;
  ...
  status |= MxLWare661_OEM_ReadRegister(u8TunerIndex, fineTunePtr->I2cSlaveAddr, DFE_FINETUNE_STEP_RB,  &regData);
  ...
  status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, fineTunePtr->I2cSlaveAddr, DFE_FINETUNE_STEP_REG, regData);
#else

  MxL_DLL_DEBUG0("MXL661_TUNER_FINE_TUNE_CFG, fine tune = %d", fineTunePtr->ScaleUp);

  status |= MxLWare661_OEM_ReadRegister(u8TunerIndex, fineTunePtr->I2cSlaveAddr, DFE_FINETUNE_STEP_REG, &regData);
  if ((regData & 0x80) == 0x80)  // Fine tune is by passed
  {
    regData &= 0x7F;
    status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, fineTunePtr->I2cSlaveAddr, DFE_FINETUNE_STEP_REG, regData); // Set Bit<7> = 0
  }
#endif

  status |= MxLWare661_OEM_ReadRegister(u8TunerIndex, fineTunePtr->I2cSlaveAddr, FINE_TUNE_SET_REG, &regData);

  // Fine tune <1:0>
  regData &= 0xFC;
  status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, fineTunePtr->I2cSlaveAddr, FINE_TUNE_SET_REG, regData);
...
static MXL_STATUS MxL661_ConfigTunerEnableFineTune(UINT8 u8TunerIndex, PMXL661_ENABLE_FINE_TUNE_CFG_T fineTunePtr)
{
  UINT8 status = MXL_TRUE;
  ...
  status |= MxLWare661_OEM_ReadRegister(u8TunerIndex, fineTunePtr->I2cSlaveAddr, FINE_TUNE_CTRL_REG_0, &fineTuneReg1);
  status |= MxLWare661_OEM_ReadRegister(u8TunerIndex, fineTunePtr->I2cSlaveAddr, FINE_TUNE_CTRL_REG_1, &fineTuneReg2);
  ...
  status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, fineTunePtr->I2cSlaveAddr, FINE_TUNE_CTRL_REG_0, fineTuneReg1);
  status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, fineTunePtr->I2cSlaveAddr, FINE_TUNE_CTRL_REG_1, fineTuneReg2);
...
static MXL_STATUS MxL661_ConfigTunerChanTune(UINT8 u8TunerIndex, PMXL661_TUNER_TUNE_CFG_T tuneParamPtr)
{
  UINT32 frequency;
  UINT32 freq = 0;
  UINT8 status = MXL_TRUE;
  ...
  {
    status |= MxLWare661_OEM_ReadRegister(u8TunerIndex, tuneParamPtr->I2cSlaveAddr, CHIP_VERSION_REQ_REG, &regData);
    if(regData != 0x4)
      MxL_DLL_DEBUG0(" It's not the MxL661 chip, don't need set HPF and LPF to default. \n");
    else
    {
      status |= MxLWare661_OEM_ReadRegister(u8TunerIndex, tuneParamPtr->I2cSlaveAddr, HPF_CTRL_REG, &regData);
      regData &= 0xFB;
      status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, tuneParamPtr->I2cSlaveAddr, HPF_CTRL_REG, regData);


      status |= MxLWare661_OEM_ReadRegister(u8TunerIndex, tuneParamPtr->I2cSlaveAddr, LPF_CTRL_REG, &regData);
      regData &= 0xF7;
      status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, tuneParamPtr->I2cSlaveAddr, LPF_CTRL_REG, regData);
    }
  }
  ...
  if ((tuneParamPtr->SignalMode <= MXL661_ANA_SECAM_L) && (tuneParamPtr->TuneType == MXL661_VIEW_MODE))
  {
    ...
    {
      status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, tuneParamPtr->I2cSlaveAddr, 0x7C, 0x1F);
    ...
    {
      status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, tuneParamPtr->I2cSlaveAddr, 0x7C, 0x9F);
    ...
    status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, tuneParamPtr->I2cSlaveAddr, 0x00, 0x01);
    status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, tuneParamPtr->I2cSlaveAddr, 0x31, regData);
    status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, tuneParamPtr->I2cSlaveAddr, 0x00, 0x00);
    ...
    switch(tuneParamPtr->SignalMode)
    {
      ...
      case MXL661_ANA_NTSC_MODE:
        ...
        if (MXL661_XTAL_16MHz == tuneParamPtr->XtalFreqSel)
        {
          if (MXL661_NTSC_CARRIER_NA == tuneParamPtr->NtscCarrier)
          {
            ...
            else
            {
              ...
              status |= MxL661_Ctrl_ProgramRegisters(u8TunerIndex, tuneParamPtr->I2cSlaveAddr, MxL661_Ntsc_HRCRfLutSwpLIF);
            }
          }
          else
          {
        ...
        if (freqLutPtr)
          status |= MxL661_Ctrl_SetRfFreqLutReg(u8TunerIndex, tuneParamPtr->I2cSlaveAddr, tuneParamPtr->FreqInHz, freqLutPtr);
      ...
      case MXL661_ANA_PAL_D:
        ...
        {
          status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, tuneParamPtr->I2cSlaveAddr, 0x93, 0x33);
          status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, tuneParamPtr->I2cSlaveAddr, 0x94, 0x44);
          status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, tuneParamPtr->I2cSlaveAddr, 0xBA, 0x40);
          status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, tuneParamPtr->I2cSlaveAddr, 0x5D, 0x0E);
        }
        else
        {
          status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, tuneParamPtr->I2cSlaveAddr, 0x93, 0x44);
          status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, tuneParamPtr->I2cSlaveAddr, 0x94, 0x55);
          status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, tuneParamPtr->I2cSlaveAddr, 0xBA, 0x50);
          status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, tuneParamPtr->I2cSlaveAddr, 0x5D, 0x0F);
        }
      ...
      case MXL661_ANA_PAL_BG:
        ...
        status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, tuneParamPtr->I2cSlaveAddr, 0x0C, regData);
        ...
        if((regData & 0xFF) == 0x4)//MxL661 V4
        {
          ...
          status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, tuneParamPtr->I2cSlaveAddr, PAGE_CHANGE_REG, 0x01);
          status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, tuneParamPtr->I2cSlaveAddr, DFE_SEQ_SPARE2_REG, 0x00);
          status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, tuneParamPtr->I2cSlaveAddr, PAGE_CHANGE_REG, 0x00);
          MxL_DLL_DEBUG0("Program 0x0169 register with 0x00 for MxL661 v4. \n");
        }
        else if((regData & 0xFF) == 0x2)
        {
          status |= MxLWare661_OEM_ReadRegister(u8TunerIndex, tuneParamPtr->I2cSlaveAddr,0x5F, &regData);
          if (regData != 0x00) ////MxL661 V3
          {
          // Program 0x0169 register with 0x00
          status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, tuneParamPtr->I2cSlaveAddr, PAGE_CHANGE_REG, 0x01);
        ...
        if (MXL661_XTAL_16MHz == tuneParamPtr->XtalFreqSel)
        {
          ...
          if (MXL661_ANA_TV_DIG_CABLE_BW_7MHz == tuneParamPtr->BandWidth)
          {
            if (tuneParamPtr->IFOutFreqinKHz < HIGH_IF_35250_KHZ)
            {
              ...
              status |= MxL661_Ctrl_ProgramRegisters(u8TunerIndex, tuneParamPtr->I2cSlaveAddr, MxL661_Pal_RfLutSwpLIF);
            }
            else
            {
              freqLutPtr = MXL661_PAL_BG_7MHZ_LUT_XTAL_16MHZ_HIF;
              status |= MxL661_Ctrl_ProgramRegisters(u8TunerIndex, tuneParamPtr->I2cSlaveAddr, MxL661_PalBG_7MHzBW_RfLutSwpHIF);
            }
          ...
          else
          {
            ...
            else
            {
              ...
              status |= MxL661_Ctrl_ProgramRegisters(u8TunerIndex, tuneParamPtr->I2cSlaveAddr, MxL661_PalBG_8MHzBW_RfLutSwpHIF);
            }
          }
  ...
  // Tune mode <0>
  status |= MxLWare661_OEM_ReadRegister(u8TunerIndex, tuneParamPtr->I2cSlaveAddr, TUNE_MODE_REG, &regData);
  regData &= 0xFE;

  if (MXL661_SCAN_MODE == tuneParamPtr->TuneType) regData |= 0x01;

  status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, tuneParamPtr->I2cSlaveAddr, TUNE_MODE_REG, regData);

  // Bandwidth <7:0>
  status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, tuneParamPtr->I2cSlaveAddr, CHAN_TUNE_BW_REG, (UINT8)tuneParamPtr->BandWidth);
  ...
  // Set RF
  status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, tuneParamPtr->I2cSlaveAddr, CHAN_TUNE_LOW_REG, (UINT8)(freq & 0xFF));
  status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, tuneParamPtr->I2cSlaveAddr, CHAN_TUNE_HI_REG, (UINT8)((freq >> 8 ) & 0xFF));
  ...
  if (tuneParamPtr->SignalMode <= MXL661_ANA_SECAM_L)
  {
    ...
    for (idx = 0; idx < MAX_MXL661_DEVICES; idx++)
    {
      if (MxL661xtalSpurCfg[idx].i2cAddr == tuneParamPtr->I2cSlaveAddr)
      {
        ...
        status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, tuneParamPtr->I2cSlaveAddr, XTAL_EXT_BIAS_REG, regData);
  ...
  if ((tuneParamPtr->SignalMode <= MXL661_ANA_SECAM_L) && (tuneParamPtr->TuneType == MXL661_VIEW_MODE))
  {
    ...
    regData = 0;
    status |= MxLWare661_OEM_ReadRegister(u8TunerIndex, tuneParamPtr->I2cSlaveAddr, FINE_TUNE_CTRL_REG_1, &regData);
    regData |= 0x20;
    status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, tuneParamPtr->I2cSlaveAddr, FINE_TUNE_CTRL_REG_1, regData);
  ...
  if ((tuneParamPtr->SignalMode <= MXL661_ANA_SECAM_L) && (tuneParamPtr->TuneType == MXL661_VIEW_MODE))
  {
    status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, tuneParamPtr->I2cSlaveAddr, PAGE_CHANGE_REG, 1);

    status |= MxLWare661_OEM_ReadRegister(u8TunerIndex, tuneParamPtr->I2cSlaveAddr, DIG_ANA_GINJO_LT_REG, &regData);
    if (tuneParamPtr->FreqInHz >= APP_MODE_FREQ_HZ_THRESHOLD_1)
      regData |= 0x10; // Bit<4> = 1
    else
      regData &= 0xEF; // Bit<4> = 0
    status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, tuneParamPtr->I2cSlaveAddr, DIG_ANA_GINJO_LT_REG, regData);
    status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, tuneParamPtr->I2cSlaveAddr, PAGE_CHANGE_REG, 0);

    if (tuneParamPtr->FreqInHz >= APP_MODE_FREQ_HZ_THRESHOLD_1)
      status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, tuneParamPtr->I2cSlaveAddr, FINE_TUNE_INIT1_REG, 0x26);
    else
      status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, tuneParamPtr->I2cSlaveAddr, FINE_TUNE_INIT1_REG, 0x48);
  }
#endif
  return (MXL_STATUS)status;
}
...
static MXL_STATUS MxL661_ConfigSpurShiftAdjustment(UINT8 u8TunerIndex, PMXL661_SPUR_SHIFT_ADJ_CFG_T spurShiftAdjPtr)
{
  UINT8 status = MXL_TRUE;
  UINT8 regData = 0;
  ...
  {
    status |= MxLWare661_OEM_ReadRegister(u8TunerIndex, spurShiftAdjPtr->I2cSlaveAddr, 0xEA, &regData);
    regData |= 0x20;
    status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, spurShiftAdjPtr->I2cSlaveAddr, 0xEA, regData);

    status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, spurShiftAdjPtr->I2cSlaveAddr,
                                 0xEB,
                                 spurShiftAdjPtr->SpurShiftingClkAdjValue);

    status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, spurShiftAdjPtr->I2cSlaveAddr, START_TUNE_REG, 0x00);
    status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, spurShiftAdjPtr->I2cSlaveAddr, START_TUNE_REG, 0x01);
...
static MXL_STATUS MxL661_ConfigHlfrfSettings(UINT8 u8TunerIndex, PMXL661_HLFRF_CFG_T hlfrfCfgPtr)
{
  UINT8 status = MXL_TRUE;
  UINT8 regData0 = 0;
  UINT8 regData1 = 0;
  ...
  status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, hlfrfCfgPtr->I2cSlaveAddr, PAGE_CHANGE_REG, 0x01);
  status |= MxLWare661_OEM_ReadRegister(u8TunerIndex, hlfrfCfgPtr->I2cSlaveAddr, DIG_ANA_GINJO_LT_REG, &regData0);
  ...
  status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, hlfrfCfgPtr->I2cSlaveAddr, DIG_ANA_GINJO_LT_REG, regData0);
  status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, hlfrfCfgPtr->I2cSlaveAddr, PAGE_CHANGE_REG, 0x00);

  status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, hlfrfCfgPtr->I2cSlaveAddr, 0xA9, regData1);
...
static MXL_STATUS MxL661_ConfigTunerAfc(UINT8 u8TunerIndex, PMXL661_TUNER_AFC_CFG_T afcCfgPtr)
{
  ...
  u32FreqOffsetKHz = (UINT32)afcCfgPtr->AfcRangeInKHz;
    ...
    status = MxLWare661_OEM_ReadRegister(u8TunerIndex, afcCfgPtr->I2cSlaveAddr, FINE_TUNE_SET_REG, &lowByte);
       lowByte |= 0x04;
       status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, afcCfgPtr->I2cSlaveAddr, FINE_TUNE_SET_REG, lowByte);
     }
     else
     {
       status = MxLWare661_OEM_ReadRegister(u8TunerIndex, afcCfgPtr->I2cSlaveAddr, FINE_TUNE_SET_REG, &lowByte);
       ...
       status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, afcCfgPtr->I2cSlaveAddr, AFC_RANGE_SET_HIGH_REG, highByte);
       status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, afcCfgPtr->I2cSlaveAddr, AFC_RANGE_SET_LOW_REG, lowByte);
  }
...
static MXL_STATUS MxL661_ConfigTunerBalunless(UINT8 u8TunerIndex, PMXL661_TUNER_BALUNLESS_CFG_T balunlessCfgPtr)
{
    UINT8 status = MXL_TRUE;
    UINT8 regData;
    ...
    {
        status |= MxLWare661_OEM_ReadRegister(u8TunerIndex, balunlessCfgPtr->I2cSlaveAddr, DFE_AGC_TOPMAXRF_REG, &regData);
        if(regData != 0x1E)
          status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, balunlessCfgPtr->I2cSlaveAddr, DFE_AGC_TOPMAXRF_REG, 0x1E);
        ...
        status |= MxLWare661_OEM_ReadRegister(u8TunerIndex, balunlessCfgPtr->I2cSlaveAddr, CHIP_VERSION_REQ_REG, &regData);
        if(regData != 0x4)
            MxL_DLL_DEBUG0("This version chip doesn't support HPF and LPF config \n");
        else
        {
            status |= MxLWare661_OEM_ReadRegister(u8TunerIndex, balunlessCfgPtr->I2cSlaveAddr, HPF_CTRL_REG, &regData);
            if(balunlessCfgPtr->EnableHPF == MXL_ENABLE)  // If enable HPF, then do the below flow
                regData |= 0x04;
            else
                regData &= 0xFB;
            status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, balunlessCfgPtr->I2cSlaveAddr, HPF_CTRL_REG, regData);


            status |= MxLWare661_OEM_ReadRegister(u8TunerIndex, balunlessCfgPtr->I2cSlaveAddr, LPF_CTRL_REG, &regData);
            if(balunlessCfgPtr->EnableLPF == MXL_ENABLE)  // If enable LPF, then do the below flow
                regData |= 0x08;
            else
                regData &= 0xF7;
              status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, balunlessCfgPtr->I2cSlaveAddr, LPF_CTRL_REG, regData);
        }
    }
...
static MXL_STATUS MxL661_ConfigTunerAtvDelay(UINT8 u8TunerIndex, PMXL661_TUNER_ATV_DELAY_CFG_T atvDelayCfgPtr)
{
  UINT32 tmpData;
  ...
  status |= MxLWare661_OEM_ReadRegister(u8TunerIndex, atvDelayCfgPtr->I2cSlaveAddr, CHIP_VERSION_REQ_REG, &regData);
  if((regData & 0xFF) != 0x4)
  {
  ...
  status = MxLWare661_OEM_WriteRegister(u8TunerIndex, atvDelayCfgPtr->I2cSlaveAddr, DFE_DP_DELAY_LOW_REG, regData);

  regData = (UINT8)(((tmpData / delayPrecisionInNs) >> 8) & 0x01);
  status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, atvDelayCfgPtr->I2cSlaveAddr, DFE_DP_DELAY_HIGH_REG, regData);
...
static MXL_STATUS MxL661_GetTunerSignalType(UINT8 u8TunerIndex, PMXL661_SIGNAL_TYPE_T tuneSigTypePtr)
{
  MXL_STATUS status = MXL_TRUE;
  UINT8 regData = 0;

  status = MxLWare661_OEM_ReadRegister(u8TunerIndex, tuneSigTypePtr->I2cSlaveAddr, SIGNAL_TYPE_REG, &regData);
...
static MXL_STATUS MxL661_GetTunerLockStatus(UINT8 u8TunerIndex, PMXL661_TUNER_LOCK_STATUS_T tunerLockStatusPtr)
{
  MXL_STATUS status = MXL_TRUE;
  UINT8 regData = 0;
  ...
  status = MxLWare661_OEM_ReadRegister(u8TunerIndex, tunerLockStatusPtr->I2cSlaveAddr, RF_REF_STATUS_REG, &regData);
...
static MXL_STATUS MxL661_GetTunerAgcLockStatus(UINT8 u8TunerIndex, PMXL661_TUNER_AGC_LOCK_T agcLockStatusPtr)
{
  ...
  status = MxLWare661_OEM_ReadRegister(u8TunerIndex, agcLockStatusPtr->I2cSlaveAddr, AGC_SAGCLOCK_STATUS_REG, &regData);
...
static MXL_STATUS MxL661_GetTunerRxPower(UINT8 u8TunerIndex, PMXL661_TUNER_RX_PWR_T rxPwrPtr)
{
  UINT8 status = MXL_TRUE;
  UINT8 regData = 0;
  UINT16 tmpData = 0;
  ...
  status = MxLWare661_OEM_ReadRegister(u8TunerIndex, rxPwrPtr->I2cSlaveAddr, RFPIN_RB_LOW_REG, &regData);
  tmpData = regData;

  // RF input power high <1:0>
  status |= MxLWare661_OEM_ReadRegister(u8TunerIndex, rxPwrPtr->I2cSlaveAddr, RFPIN_RB_HIGH_REG, &regData);
...
static MXL_STATUS MxL661_GetTunerAfcCorrection(UINT8 u8TunerIndex, PMXL661_TUNER_AFC_CORRECTION_T afcCorrectionPtr)
{
  UINT32 tmpData = 0;
  UINT8 status = MXL_TRUE;
  UINT8 regData = 0;
  ...
  status |= MxLWare661_OEM_ReadRegister(u8TunerIndex, afcCorrectionPtr->I2cSlaveAddr, RFPIN_RB_HIGH_REG, &regData);
  afcLock = (regData & 0x10) >> 4;

  // AFC offset correction readback <7:0> correponding to bit<7:0>
  status |= MxLWare661_OEM_ReadRegister(u8TunerIndex, afcCorrectionPtr->I2cSlaveAddr, DFE_CTRL_ACCUM_LOW_REG, &regData);

  // AFC offset correction readback <15:8> correponding to bit<7:0>
  status |= MxLWare661_OEM_ReadRegister(u8TunerIndex, afcCorrectionPtr->I2cSlaveAddr, DFE_CTRL_ACCUM_MID_REG, &regData2);

  // AFC offset correction readback <17:16> correponding to bit<1:0>
  status |= MxLWare661_OEM_ReadRegister(u8TunerIndex, afcCorrectionPtr->I2cSlaveAddr, DFE_CTRL_ACCUM_HI_REG, &regData3);
...
static MXL_STATUS MxL661_GetTunerAtvDelay(UINT8 u8TunerIndex, PMXL661_TUNER_ATV_DELAY_CFG_T atvDelayInfoPtr)
{
  UINT32 tmpData = 0;
  UINT16 delayPrecisionInNs;
  UINT8 status = MXL_TRUE;
  ...
  status |= MxLWare661_OEM_ReadRegister(u8TunerIndex, atvDelayInfoPtr->I2cSlaveAddr, DFE_DP_DELAY_HIGH_REG, &readData);
  tmpData = readData & 0x01;

  status |= MxLWare661_OEM_ReadRegister(u8TunerIndex, atvDelayInfoPtr->I2cSlaveAddr, DFE_DP_DELAY_LOW_REG, &readData);
...
static MXL_STATUS MxL661_SetTeletextSpurMitigation(UINT8 u8TunerIndex, PMXL661_TELETEXT_SPUR_CFG_T teletextSpurCfgPtr)
{
  ...
  status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, teletextSpurCfgPtr->I2cSlaveAddr, DFE_FDC_IIR0_1, 0x77);
  status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, teletextSpurCfgPtr->I2cSlaveAddr, DFE_FDC_IIR2_3, 0x77);
  ...
  {
    // Change Page to 0
    status = MxLWare661_OEM_WriteRegister(u8TunerIndex, teletextSpurCfgPtr->I2cSlaveAddr, 0x00, 0x01);

    // Read dfe_fdc_offset_i
    status |= MxLWare661_OEM_ReadRegister(u8TunerIndex, teletextSpurCfgPtr->I2cSlaveAddr, DFE_FDC_OFFSET_I_LSB, &regData);
    dcCalWordi = (regData & 0xFF);

    status |= MxLWare661_OEM_ReadRegister(u8TunerIndex, teletextSpurCfgPtr->I2cSlaveAddr, DFE_FDC_OFFSET_I_MSB, &regData);
    ...
    status |= MxLWare661_OEM_ReadRegister(u8TunerIndex, teletextSpurCfgPtr->I2cSlaveAddr, DFE_FDC_OFFSET_Q_LSB, &regData);
    dcCalWordq = (regData & 0xFF);

    status |= MxLWare661_OEM_ReadRegister(u8TunerIndex, teletextSpurCfgPtr->I2cSlaveAddr, DFE_FDC_OFFSET_Q_MSB, &regData);
    ...
    // Change Page to 1
    status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, teletextSpurCfgPtr->I2cSlaveAddr, 0x00, 0x00);
  ...
  else
  {
    ...
    status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, teletextSpurCfgPtr->I2cSlaveAddr, DFE_FDC_IIR0_1, 0x56);
    status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, teletextSpurCfgPtr->I2cSlaveAddr, DFE_FDC_IIR2_3, 0x44);
  }
...
MXL_STATUS MxLWare661_API_ConfigDevice(UINT8 u8TunerIndex, MXL661_COMMAND_T *ParamPtr)
{
  MXL_STATUS status = MXL_TRUE;
  ...
  switch (cmdType)
  {
    case MXL661_DEV_SOFT_RESET_CFG:
      status = MxL661_ConfigDevReset(u8TunerIndex, &ParamPtr->MxLIf.cmdResetCfg);
      break;

    case MXL661_DEV_OVERWRITE_DEFAULT_CFG:
      status = MxL661_ConfigDevOverwriteDefault(u8TunerIndex, &ParamPtr->MxLIf.cmdOverwriteDefault);
      break;

    case MXL661_DEV_XTAL_SET_CFG:
      status = MxL661_ConfigDevXtalSet(u8TunerIndex, &ParamPtr->MxLIf.cmdXtalCfg);
      break;

    case MXL661_DEV_POWER_MODE_CFG:
      status = MxL661_ConfigDevPowerMode(u8TunerIndex, &ParamPtr->MxLIf.cmdPwrModeCfg);
      break;

    case MXL661_DEV_IF_OUT_CFG:
      status = MxL661_ConfigDevIfOutSet(u8TunerIndex, &ParamPtr->MxLIf.cmdIfOutCfg);
      break;

    case MXL661_DEV_GPO_CFG:
      status = MxL661_ConfigDevGPO(u8TunerIndex, &ParamPtr->MxLIf.cmdGpoCfg);
      break;
...
MXL_STATUS MxLWare661_API_GetDeviceStatus(UINT8 u8TunerIndex, MXL661_COMMAND_T *ParamPtr)
{
  MXL_STATUS status = MXL_TRUE;
  MXL661_CMD_TYPE_E cmdType;
  ...
  switch (cmdType)
  {
    case MXL661_DEV_ID_VERSION_REQ:
      status = MxL661_GetDeviceInfo(u8TunerIndex, &ParamPtr->MxLIf.cmdDevInfoReq);
      break;

    case MXL661_DEV_GPO_STATE_REQ:
      status = MxL661_GetDeviceGpoState(u8TunerIndex, &ParamPtr->MxLIf.cmdGpoReq);
      break;

    case MXL661_DEV_PLL_STATE_REQ:
      status = MxL661_GetDevicePllState(u8TunerIndex, &ParamPtr->MxLIf.cmdPllReq);
      break;
...
MXL_STATUS MxLWare661_API_ConfigTuner(UINT8 u8TunerIndex, MXL661_COMMAND_T *ParamPtr)
{
  MXL_STATUS status = MXL_TRUE;
  MXL661_CMD_TYPE_E cmdType;
  ...
  switch (cmdType)
  {
    case MXL661_TUNER_POWER_UP_CFG:
      status = MxL661_ConfigTunerPowerUp(u8TunerIndex, &ParamPtr->MxLIf.cmdTunerPoweUpCfg);
      break;

    case MXL661_TUNER_START_TUNE_CFG:
      status = MxL661_ConfigTunerSequenceSet(u8TunerIndex, &ParamPtr->MxLIf.cmdStartTuneCfg);
      break;

    case MXL661_TUNER_MODE_CFG:
      status = MxL661_ConfigTunerApplicationModeSet(u8TunerIndex, &ParamPtr->MxLIf.cmdModeCfg);
      break;

    case MXL661_TUNER_AGC_CFG:
      status = MxL661_ConfigTunerAgcSet(u8TunerIndex, &ParamPtr->MxLIf.cmdAgcSetCfg);
      break;

    case MXL661_TUNER_FINE_TUNE_CFG:
      status = MxL661_ConfigTunerFineTune(u8TunerIndex, &ParamPtr->MxLIf.cmdFineTuneCfg);
      break;

    case MXL661_TUNER_ENABLE_FINE_TUNE_CFG:
      status = MxL661_ConfigTunerEnableFineTune(u8TunerIndex, &ParamPtr->MxLIf.cmdEnableFineTuneCfg);
      break;

    case MXL661_TUNER_CHAN_TUNE_CFG:
      status = MxL661_ConfigTunerChanTune(u8TunerIndex, &ParamPtr->MxLIf.cmdChanTuneCfg);
      break;

    case MXL661_TUNER_SPUR_SHIFT_ADJ_CFG:
      status = MxL661_ConfigSpurShiftAdjustment(u8TunerIndex, &ParamPtr->MxLIf.cmdSuprShiftAdjCfg);
      break;

    case MXL661_TUNER_HLFRF_CFG:
      status = MxL661_ConfigHlfrfSettings(u8TunerIndex, &ParamPtr->MxLIf.cmdHlfrfCfg);
      break;

    case MXL661_TUNER_AFC_CFG:
      status = MxL661_ConfigTunerAfc(u8TunerIndex, &ParamPtr->MxLIf.cmdAfcCfg);
      break;

#ifdef MXL661_ENABLE_TELETEXT_SPUR_FEATURE
    case MXL661_TUNER_TELETEXT_SPUR_CFG:
      status = MxL661_SetTeletextSpurMitigation(u8TunerIndex, &ParamPtr->MxLIf.cmdTeletextSpurCfg);
      break;
#endif

    case MXL661_TUNER_BALUNLESS_CFG:
      status = MxL661_ConfigTunerBalunless(u8TunerIndex, &ParamPtr->MxLIf.cmdBalunlessCfg);
      break;

    case MXL661_TUNER_ATV_DELAY_CFG:
      status = MxL661_ConfigTunerAtvDelay(u8TunerIndex, &ParamPtr->MxLIf.cmdAtvDelayCfg);
      break;

    case MXL661_TUNER_HPF_LPF_CFG:
      status = MxL661_ConfigTunerAtvHpfLpf(u8TunerIndex, &ParamPtr->MxLIf.cmdTunerAtvFilterCfg);
      break;
...
MXL_STATUS MxLWare661_API_GetTunerStatus(UINT8 u8TunerIndex, MXL661_COMMAND_T *ParamPtr)
{
  MXL_STATUS status;
  MXL661_CMD_TYPE_E cmdType;
  ...
  switch (cmdType)
  {
    case MXL661_TUNER_SIGNAL_TYPE_REQ:
      status = MxL661_GetTunerSignalType(u8TunerIndex, &ParamPtr->MxLIf.cmdTunerSignalTypeReq);
      break;

    case MXL661_TUNER_LOCK_STATUS_REQ:
      status = MxL661_GetTunerLockStatus(u8TunerIndex, &ParamPtr->MxLIf.cmdTunerLockReq);
      break;

    case MXL661_TUNER_AGC_LOCK_REQ:
      status = MxL661_GetTunerAgcLockStatus(u8TunerIndex, &ParamPtr->MxLIf.cmdTunerAgcLockReq);
      break;

    case MXL661_TUNER_RX_PWR_REQ:
      status = MxL661_GetTunerRxPower(u8TunerIndex, &ParamPtr->MxLIf.cmdTunerPwrReq);
      break;

    case MXL661_TUNER_AFC_CORRECTION_REQ:
      status = MxL661_GetTunerAfcCorrection(u8TunerIndex, &ParamPtr->MxLIf.cmdTunerAfcCorectionReq);
      break;

    case MXL661_TUNER_ATV_DELAY_REQ:
      status = MxL661_GetTunerAtvDelay(u8TunerIndex, &ParamPtr->MxLIf.cmdAtvDelayCfg);
      break;
