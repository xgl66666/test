...
static TUNER_MS_INIT_PARAM InitParam[MAX_FRONTEND_NUM*R858_TUNER_MAX_NUM]; //MAX_FRONTEND_NUM
static MS_BOOL bR858_Init_ALL[MAX_FRONTEND_NUM] = {0};
SLAVE_ID_USAGE* pstR858_slave_ID_TBL = NULL;
static SLAVE_ID_USAGE R858_possible_slave_ID[3] =
{
    {0xD4, FALSE},
    {0x14, FALSE},
    {0xFF, FALSE}
...
R858_Share_Xtal_Type R858_SHARE_XTAL = R858_NO_SHARE_XTAL;
...
UINT8 R858_iniArray[2][R858_REG_NUM] = {     //Tuner1/2
    ...
    //  0x1C  0x1D  0x1E  0x1F  0x20  0x21  0x22  0x23  0x24  0x25
    	0x45, 0x06, 0xA5, 0x2A, 0x0B, 0xE8, 0xC8, 0x00, 0xD2, 0x80,
    //  0x26  0x27  0x28  0x29  0x2A  0x2B  0x2C  0x2D  0x2E  0x2F
			0x22, 0x02, 0x00, 0xD7, 0xA5, 0xF6, 0x77, 0x1A},
...
UINT8 R858_LPF_CAL_Array[2][R858_REG_NUM] = {   //tuner1/2
    {  0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,  //read only
       ...
       0xAD, 0xF0, 0x0F, 0xE0, 0xC8, 0x07, 0xD2, 0x00,
       0xC0, 0xE1, 0x11, 0x17, 0xA1, 0xD6, 0x73, 0x18},
     {
       0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,  //read only
...
UINT8 R858_IMR_CAL_Array[2][R858_REG_NUM] = {   //tuner1/2
   {  0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,  //read only
      ...
      0x00, 0x40, 0xD1, 0x00, 0x90, 0x25, 0x00, 0x08},
   {
      0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,  //read only
...
static UINT8  R858_Initial_done_flag[R858_MAX_NUM][2] = {{FALSE, FALSE},{FALSE, FALSE}};
static UINT8  R858_IMR_done_flag[R858_MAX_NUM][2] = {{FALSE, FALSE},{FALSE, FALSE}};
...
static MS_U32 u32CurFreq[R858_MAX_NUM][2];
...
//Mapping to Rafael defined tuner number
static R858_ExtTunerNum_Type _R858TunerNumMapping(MS_U8 u8TunerIndex)
{
    MS_U8 u8DevIndex = u8TunerIndex/R858_TUNER_MAX_NUM;
    MS_U8 i = 0;

    for(i=0;i<R858_MAX_NUM;i++)
    {
       if((InitParam[u8DevIndex].u8SlaveID) == R858_ADDRESS[i][0])
           break;
    }

    return (R858_ExtTunerNum_Type)i;
}

//Mapping to Mstar defined tuner index
static MS_U8 _I2CPortMapping(R858_ExtTunerNum_Type R858_Num)
{
    MS_U8 i = 0;

    for(i=0;i<MAX_FRONTEND_NUM;i++)
    {
       if(R858_ADDRESS[R858_Num][0] == InitParam[i].u8SlaveID)
           break;
    }
...
static MS_BOOL I2C_Read_Len(R858_ExtTunerNum_Type R858_Num, I2C_LEN_TYPE *I2C_Info)
{
    ...
    ePort = getI2CPort(_I2CPortMapping(R858_Num));
...
R858_Freq_Info_Type R858_Freq_Sel(UINT32 LO_freq, UINT32 RF_freq, R858_Standard_Type R858_Standard)
{
    ...
    if(LO_freq<580000)
        ...
        R858_Freq_Info.TF_HPF_BPF = 1;		//3 => 110
...
R858_SysFreq_Info_Type R858_SysFreq_Sel(R858_Standard_Type R858_Standard,UINT32 RF_freq)
{
    ...
    case R858_DVB_T2_1_7M_IF_5M:
        ...
        R858_SysFreq_Info.FILT_3TH_LPF_GAIN=0;			//R17[7:6]	[normal (0), +0.5dB (1), +1dB (2), +1.5dB (3)] ? Description different
...
R858_ErrCode R858_IMR(R858_ExtTunerNum_Type R858_Num, R858_IntTunerNum_Type IntTunerNum, UINT8 IMR_MEM, UINT8 IM_Flag ,UINT8 Rev_Mode)
{
    ...
    UNUSED UINT8 imr_auto_result2 = 0;
    UINT8 imr_done_1 = 0;
...
R858_ErrCode R858_PLL(R858_ExtTunerNum_Type R858_Num, R858_IntTunerNum_Type IntTunerNum, UINT32 LO_Freq, R858_Standard_Type R858_Standard)
{
    ...
    // PLL LDOA=2.2V
    R858_I2C.RegAddr = 25;
	R858_Array[R858_Num][IntTunerNum][R858_I2C.RegAddr] = (R858_Array[R858_Num][IntTunerNum][R858_I2C.RegAddr] & 0xF3) | 0x04; // R25[3:2]=00
    ...
    */
	...
	if(R858_Xtal == 24000)
	{
		if((u2XalDivJudge>=48000) && (u2XalDivJudge<114000))
		{
	...
	if(R858_Xtal == 24000)
	{
		...
		{
			if((u2XalDivJudge>=48000) && (u2XalDivJudge<114000)) //48 to 198MHz
			{
	...
	else
	{
		...
		else  //DTV
		{
			...
			if((u2XalDivJudge>=48000) && (u2XalDivJudge<114000)) //48 to 114MHz
			{
	...
	{
		PLL_Ref = R858_Xtal;
		R858_Array[R858_Num][IntTunerNum][28] = (R858_Array[R858_Num][IntTunerNum][28] & 0x9F) | 0x00; //1st_div2=0, 2nd_div2=0	 R29[5:4]
	}
	else if(R858_XtalDiv==XTAL_DIV1_2)
	{
		PLL_Ref = R858_Xtal/2;
		R858_Array[R858_Num][IntTunerNum][28] = (R858_Array[R858_Num][IntTunerNum][28] & 0x9F) | 0x20; //1st_div2=0(R29[4]), 2nd_div2=1(R29[5])
	}
	else if(R858_XtalDiv==XTAL_DIV2_1)
	{
		PLL_Ref = R858_Xtal/2;
		R858_Array[R858_Num][IntTunerNum][28] = (R858_Array[R858_Num][IntTunerNum][28] & 0x9F) | 0x20; //1st_div2=1(R29[4]), 2nd_div2=0(R29[5])
	}
	else if(R858_XtalDiv==XTAL_DIV4)  //24MHz
	{
		PLL_Ref = R858_Xtal/4;
		R858_Array[R858_Num][IntTunerNum][28] = (R858_Array[R858_Num][IntTunerNum][28] & 0x9F) | 0x40; //2nd_div2=1, 1st_div2=1  R29[5:4]
	}
	...
	//IQ_deg R44[3]
	R858_I2C.RegAddr = 44;
	R858_Array[R858_Num][IntTunerNum][R858_I2C.RegAddr] = (R858_Array[R858_Num][IntTunerNum][R858_I2C.RegAddr] & 0xE7) | (IQdeg<<3);
...
R858_ErrCode R858_MUX(R858_ExtTunerNum_Type R858_Num, R858_IntTunerNum_Type IntTunerNum, UINT32 LO_KHz, UINT32 RF_KHz, R858_Standard_Type R858_Standard)
{
    ...
    R858_I2C.RegAddr = 12;
	R858_Array[R858_Num][IntTunerNum][R858_I2C.RegAddr] = (R858_Array[R858_Num][IntTunerNum][R858_I2C.RegAddr] & 0xF8) | (Freq_Info1.TF_HPF_BPF & 0x07) ;  // R12[0]
    R858_I2C.Data = R858_Array[R858_Num][IntTunerNum][R858_I2C.RegAddr];
    if(I2C_Write(R858_Num, &R858_I2C) != RT_Success)
        return RT_Fail;
    ...
    //TF_HPF_CNR => R12[2:1]
	R858_I2C.RegAddr = 11;
	R858_Array[R858_Num][IntTunerNum][R858_I2C.RegAddr] = (R858_Array[R858_Num][IntTunerNum][R858_I2C.RegAddr] & 0x3F) | (Freq_Info1.TF_HPF_CNR << 6);  // R12[2:1]
    R858_I2C.Data = R858_Array[R858_Num][IntTunerNum][R858_I2C.RegAddr];
    if(I2C_Write(R858_Num, &R858_I2C) != RT_Success)
        return RT_Fail;
...
R858_ErrCode R858_SetFrequency(R858_ExtTunerNum_Type R858_Num, R858_IntTunerNum_Type IntTunerNum, R858_Set_Info R858_INFO)
{
	...
	R858_I2C_Len.I2cAddr=R858_ADDRESS[R858_Num][IntTunerNum];
	 ...
	 else
	 {
	  ...
	  R858_Array[R858_Num][IntTunerNum][R858_I2C.RegAddr] = (R858_Array[R858_Num][IntTunerNum][R858_I2C.RegAddr] & 0xDF) | (Img_R<<5);  //R17[2]
	  R858_I2C.Data = R858_Array[R858_Num][IntTunerNum][R858_I2C.RegAddr];
	  if(I2C_Write(R858_Num, &R858_I2C) != RT_Success)
		 return RT_Fail;
...
R858_ErrCode R858_Standby(R858_ExtTunerNum_Type R858_Num, R858_IntTunerNum_Type IntTunerNum, R858_LT_Type R858_LT)
{
	...
	if(R858_SBY_flag[R858_Num][IntTunerNum]==0)
	{
		for(i=8; i<R858_REG_NUM; i++)  //store R8~R47
		{
		 ...
		 for(i=48; i<R858_REG_NUM; i++)  //store tuner1 R42~R47
		{
...
R858_ErrCode R858_Wakeup(R858_ExtTunerNum_Type R858_Num, R858_IntTunerNum_Type IntTunerNum)
{
	...
	if(R858_SBY_flag[R858_Num][IntTunerNum]==1)
	{
		...
		R858_I2C_Len.I2cAddr=R858_ADDRESS[R858_Num][R858_TUNER1];
		R858_I2C.I2cAddr=R858_ADDRESS[R858_Num][R858_TUNER1];
		if(R858_SBY_flag[R858_Num][1-IntTunerNum]==0) //S-W(W-S) to W-W
		{
			if(R858_LNA_VTHVTL_Change_flag[R858_Num]== 1)// do nothing
			{
			}
...
R858_ErrCode R858_GetRfRssi(R858_ExtTunerNum_Type R858_Num, R858_IntTunerNum_Type IntTunerNum, UINT32 RF_Freq_Khz, int *RfLevelDbm, UINT8 *fgRfMaxGain)
{
	//UINT8 bPulseFlag;
...
MS_BOOL MDrv_Tuner_R858_Init(MS_U8 u8TunerIndex,TUNER_MS_INIT_PARAM* pParam)
{
    ...
    R858_Num = _R858TunerNumMapping(u8TunerIndex);
    ...
    if(bR858_Init_ALL[R858_Num] == FALSE)
    {
        TUNER_DBG(("R858 Index %d init..... \n", (int)R858_Num));
        R858_INFO.R858_Standard = R858_DVB_C_8M_IF_5M;
        R858_INFO.R858_Standard_2 = R858_DVB_C_8M_IF_5500;
        if(RT_Fail == R858_Init_ALL(R858_Num, R858_INFO))
        {
            TUNER_ERR(("R858 Init FAIL..... \n"));
            return FALSE;
        }
        bR858_Init_ALL[R858_Num] = TRUE;
    }
    else
    {
        TUNER_DBG(("R858 Index %d have been inited..... \n", (int)R858_Num));
    }
...
MS_BOOL MDrv_Tuner_R858_SetTuner(MS_U8 u8TunerIndex, MS_U32 u32Freq, MS_U8 feBw)
{
    ...
    else if(DVBC == *InitParam[u8TunerIndex].pCur_Broadcast_type)
    {
           ...
           {
           case TUNER_BW_MODE_6MHZ:
                R858_Info.R858_Standard_2 = R858_DVB_C_6M_IF_5500;
    ...
    u32CurFreq[R858_Num][IntTunerNum] = u32Freq; //backup
    //Loop Through
    R858_Info.R858_LT = LT_ON; // Loop through ON
    //Clk output
    R858_Info.R858_ClkOutMode = CLK_OUT_ON; // No tuner clock output for other IC
...
MS_BOOL R858_Extension_Function(MS_U8 u8TunerIndex, TUNER_EXT_FUNCTION_TYPE fuction_type, void *data)
{
    ...
    switch(fuction_type)
    {
        case TUNER_EXT_FUNC_GET_POWER_LEVEL:
            if( RT_Success != R858_GetRfRssi(R858_Num, IntTunerNum,u32CurFreq[R858_Num][IntTunerNum], &RfLevelDbm, &RfGainflag))
                bret = FALSE;

            *(int*)(data)= (RfLevelDbm/1000);
            break;

         case TUNER_EXT_FUNC_GET_RFAGC_STATUS:
            if( RT_Success != R858_GetRfRssi(R858_Num, IntTunerNum,u32CurFreq[R858_Num][IntTunerNum], &RfLevelDbm, (UINT8*)(data)))
                 bret = FALSE;
         ...
         case TUNER_EXT_FUNC_UNDER_EXT_DMD_TEST:
          ...
          case TUNER_EXT_FUNC_FINALIZE:
             R858_Initial_done_flag[R858_Num][IntTunerNum] = FALSE;
             bR858_Init_ALL[R858_Num] = FALSE;
             break;
        default:
...
MS_BOOL MDrv_Tuner_R858_GetTunerIF(MS_U8 u8TunerIndex, MS_U32* pu32IF_Freq)
{
   ...
   R858_Sys_Info = R858_Sys_Sel(R858_pre_standard[R858_Num][IntTunerNum]);
   *pu32IF_Freq = (MS_U32)R858_Sys_Info.IF_KHz;
    return TRUE;
}

#define R858_CHIP_ID CHIP_ID
MS_BOOL R858_I2C_SLAVE_ID_CHK(MS_U8 u8TunerIndex, MS_U8* pu8SlveID_1st, MS_U8* pu8SlveID_2nd)
{
    MS_U8 i, j,u8I2C_Port, u8MaxI2CPort;
    MS_BOOL bRet = FALSE;
    ...
    if(pstR858_slave_ID_TBL == NULL)
    {
        pstR858_slave_ID_TBL = (SLAVE_ID_USAGE *)malloc(sizeof(R858_possible_slave_ID) * u8MaxI2CPort);
        if(NULL == pstR858_slave_ID_TBL)
        {
            return FALSE;
        }
        else
        {
            ...
            {
                for(j=0; j< (sizeof(R858_possible_slave_ID)/sizeof(SLAVE_ID_USAGE)); j++)
                {
                    pSlaveIDTBL = (pstR858_slave_ID_TBL + i*sizeof(R858_possible_slave_ID)/sizeof(SLAVE_ID_USAGE) + j);
                    memcpy(pSlaveIDTBL, &R858_possible_slave_ID[j], sizeof(SLAVE_ID_USAGE));
                 }
            }
    ...
    do
    {
        pSlaveIDTBL = pstR858_slave_ID_TBL + u8I2C_Port*sizeof(R858_possible_slave_ID)/sizeof(SLAVE_ID_USAGE) + i;
        if(pSlaveIDTBL->bInUse)
        {
            TUNER_DBG(("[R858]I2C Slave ID 0x%x Have Used on the same I2C Port\n", pSlaveIDTBL->u8SlaveID));
        }
        ...
        {
            R858_I2C_Len.I2cAddr= pSlaveIDTBL->u8SlaveID;
            R858_I2C_Len.RegAddr= 0;
            R858_I2C_Len.Len = 1;
            R858_I2C_Len.Data[0] = 0;
            InitParam[u8TunerIndex].u8SlaveID = R858_I2C_Len.I2cAddr;
            if(I2C_Read_Len(_R858TunerNumMapping(u8TunerIndex*R858_MAX_NUM),&R858_I2C_Len) != RT_Success)
            {
            ...
            else
            {
                 ...
                 *pu8SlveID_1st = pSlaveIDTBL->u8SlaveID;
                 *pu8SlveID_2nd = R858_ADDRESS[_R858TunerNumMapping(u8TunerIndex*R858_MAX_NUM)][1];
        ...
        i++;
        //printf("### try slave ID 0x%x\n", (pSlaveIDTBL->stID_Tbl[i].u8SlaveID_1st));
    }while((pSlaveIDTBL->u8SlaveID) != 0xFF);

    //if(u8TunerIndex == (MAX_FRONTEND_NUM - 1))//Last I2C port have tried
    //{
    //   printf("#### free pstR858_slave_ID_TBL\n");
    //   free(pstR858_slave_ID_TBL);
    //   pstR858_slave_ID_TBL = NULL;
    //}
    return bRet;
...
{
    UINT8 regData = 0;
    MS_U8 u8slaveID_1st, u8slaveID_2nd;

    InitParam[u8TunerIndex].u8SlaveID = 0;
    InitParam[u8TunerIndex].u8SlaveID_EXT = 0;
#if 1
    if(R858_I2C_SLAVE_ID_CHK(u8TunerIndex, &u8slaveID_1st, &u8slaveID_2nd))
    {
       InitParam[u8TunerIndex].u8SlaveID = u8slaveID_1st;
       InitParam[u8TunerIndex].u8SlaveID_EXT = u8slaveID_2nd;
       TUNER_ERR(("[R858]Got tuner slave ID 0x%02x and 0x%02x\n", u8slaveID_1st, u8slaveID_2nd));
    }
    else
    {
        TUNER_ERR(("[R858]All slave IDs have tried\n"));
        return FALSE;
    }
 #else

     R858_I2C_Len.I2cAddr= R858_ADDRESS[u8TunerIndex][0];
     R858_I2C_Len.RegAddr= 0;
     R858_I2C_Len.Len = 1;
     R858_I2C_Len.Data[0] = 0;
    ...
    {
        //R858_ADDRESS[u8TunerIndex][0] = u8slaveID_1st;
        //R858_ADDRESS[u8TunerIndex][1] = u8slaveID_2nd;
        if(NULL != pu32channel_cnt)
            *(pu32channel_cnt) += R858_TUNER_MAX_NUM;
    }
