...
#if IF_THIS_TUNER_INUSE(TUNER_AV2017)
#define  TUNER_AV2017_SLAVE_ID      0xC6 //0xC4 // 0xC0//0xC6
...
static MS_BOOL AV2017_WriteReg(MS_U8 u8TunerIndex, MS_U8 u8SlaveID, MS_U8 u8Addr, MS_U8 u8Data)
{
    MS_BOOL bRet=TRUE;
    MS_U8 u8Value[2];
    MS_IIC_PORT ePort;
    ...
    if (!bRet)
    {
        TUNER_ERR(("AV2017_WriteReg fail \n"));
    }
    return bRet;
}
static MS_BOOL AV2017_ReadReg(MS_U8 u8TunerIndex, MS_U8 u8SlaveID, MS_U8 u8Addr, MS_U8 *u8Data)
{
    MS_BOOL bRet = TRUE;
    MS_IIC_PORT ePort;
    ePort = getI2CPort(u8TunerIndex);
    ...
    if (!bRet)
    {
        TUNER_ERR(("AV2017_ReadReg fail \n"));
    }
    return bRet;
}

static void AV2017_SlaveID_Check(MS_U8 u8TunerIndex)
{
     ...
     MS_U8 regValue;
      ...
      {
          regValue=(char) (0x38);
          if(AV2017_WriteReg(u8TunerIndex, _u8SlaveID,0,regValue))
          {
               regValue = 0;
               if(AV2017_ReadReg(u8TunerIndex,_u8SlaveID,0,&regValue))
               {
                     if(regValue == 0x38)
                     {
                         break;
      ...
      TUNER_DBG(("\n\n@@@@@@@@@@@@@ AV2017_SlaveID_Check[ 0x%x ] \n\n",_u8SlaveID));
}

static MS_BOOL AV2017_Init(MS_U8 u8TunerIndex) 
{
    MS_BOOL bRet=TRUE;
    MS_U8 index;

    AV2017_SlaveID_Check(u8TunerIndex);

    for (index=0; index < 12; index++)
    {
        bRet&=AV2017_WriteReg(u8TunerIndex, _u8SlaveID, TunerInitialSetting[0][index], TunerInitialSetting[1][index]);
    }
    MsOS_DelayTask(1);
    ...
    MsOS_DelayTask(1);
    bRet&=AV2017_WriteReg(u8TunerIndex, _u8SlaveID, TunerInitialSetting[0][12], TunerInitialSetting[1][12]);
    MsOS_DelayTask(100);
    for (index=0; index < 12; index++)
    {
        bRet&=AV2017_WriteReg(u8TunerIndex, _u8SlaveID, TunerInitialSetting[0][index], TunerInitialSetting[1][index]);
...
MS_BOOL MDrv_Tuner_AV2017_Initial(MS_U8 u8TunerIndex,TUNER_MS_INIT_PARAM* pParam)
{
    return AV2017_Init(u8TunerIndex);
}

MS_BOOL MDrv_Tuner_AV2017_SetFreq_S2(MS_U8 u8TunerIndex,MS_U32 u32CenterFreq, MS_U32 u32SymbolRate_Hz)
{
    MS_BOOL bRet=TRUE;
    ...
    if (u32SymbolRate_Hz == 0 || u32SymbolRate_Hz > 55000) //auto-scan mode
    {
	    bAutoScan = TRUE;
    }
    //MsOS_DelayTask(50);
    ...
    // Send Reg0 ->Reg4
    MsOS_DelayTask(5);
    bRet&=AV2017_WriteReg(u8TunerIndex, _u8SlaveID, 0x00, u8Reg[0]);
    bRet&=AV2017_WriteReg(u8TunerIndex, _u8SlaveID, 0x01, u8Reg[1]);
    bRet&=AV2017_WriteReg(u8TunerIndex, _u8SlaveID, 0x02, u8Reg[2]);
    bRet&=AV2017_WriteReg(u8TunerIndex, _u8SlaveID, 0x03, u8Reg[3]);
    MsOS_DelayTask(5);
    // Sequence 5
    // Send Reg5
    bRet&=AV2017_WriteReg(u8TunerIndex, _u8SlaveID, 0x05, u8Reg[5]);
    MsOS_DelayTask(5);
    ...
    {
         u8Reg[6] = 0x06;
         bRet&=AV2017_WriteReg(u8TunerIndex, _u8SlaveID, 0x25, u8Reg[6]);
         MsOS_DelayTask(5);
	...
	u8Reg[7] = 0xD6;
         bRet&=AV2017_WriteReg(u8TunerIndex, _u8SlaveID, 0x0C, u8Reg[7]);
         //MsOS_DelayTask(5);
    }
    return bRet;
}
MS_BOOL MDrv_Tuner_AV2017_CheckLock(MS_U8 u8TunerIndex)
{
    MS_BOOL bRet=TRUE;
    MS_U8 u8Data;

    bRet&=AV2017_ReadReg(u8TunerIndex, _u8SlaveID, 0x0B, &u8Data);
    if (bRet==FALSE)
    {
        return bRet;
    }
...
void MDrv_Tuner_AV2017_SetRolloff(MS_U8 u8Rolloff)
{
    sCurRolloff = u8Rolloff;
}

MS_BOOL AV2017_Extension_Function(MS_U8 u8TunerIndex, TUNER_EXT_FUNCTION_TYPE fuction_type, void *data)
{
    ...
    switch(fuction_type)
    {
         ...
         case TUNER_EXT_FUNC_POWER_ON_OFF:
             bret &= AV2017_ReadReg(u8TunerIndex,_u8SlaveID, 0x0C, &regData);
            if(FALSE == *(MS_BOOL *)data)   //power off
            {
               regData |= (0x1<<5);
               bret &= AV2017_WriteReg(u8TunerIndex,_u8SlaveID, 0x0C, regData);
            }
            else
            {
               if((regData & (0x1<<5)) >>5)
               {
                  bret &= AV2017_Init(u8TunerIndex);
               }
            }
            break;
...
#define AV2017_CHIP_ID_0 0x21
#define AV2017_CHIP_ID_1 0x15
MS_BOOL MDrv_Tuner_AV2017_CheckExist(MS_U8 u8TunerIndex, MS_U32* pu32channel_cnt)
{
    MS_U8 regData = 0;
    MS_U8 regData2 = 0;
    MS_U8 regData3 = 0;
    ...
    for (;i<5;i++)
    {
        if(!AV2017_WriteReg(u8TunerIndex, _u8SlaveID, 0x0C, 0xF6))
            continue;
        if(!AV2017_WriteReg(u8TunerIndex, _u8SlaveID, 0x1C, 0x00))
            continue;
        if(!AV2017_ReadReg(u8TunerIndex, _u8SlaveID, 0x0, &regData))
            continue;
        if(!AV2017_ReadReg(u8TunerIndex, _u8SlaveID, 0x33, &regData2))
            continue;
        if(!AV2017_ReadReg(u8TunerIndex, _u8SlaveID, 0x34, &regData3))
            continue;
        TUNER_DBG(("[av2018] read id =0x%x reg0x33=0x%x reg0x34=0x%x\n",regData,regData2,regData3));
        
        if ( (regData2 == AV2017_CHIP_ID_0) && (regData3 == AV2017_CHIP_ID_1) )
        {
...
DRV_TUNER_TABLE_TYPE GET_TUNER_ENTRY_NODE(TUNER_AV2017) DDI_DRV_TUNER_TABLE_ENTRY(tunertab) =      
{                                                                       
    .name               = "TUNER_AV2017",          // demod system name
    .data               = TUNER_AV2017,            // private data value
    .Init               = MDrv_Tuner_AV2017_Initial,
    .SetFreq            = MDrv_Tuner_Null_SetFreq,
    .SetFreqS2          = MDrv_Tuner_AV2017_SetFreq_S2,
    .GetLock            = MDrv_Tuner_AV2017_CheckLock,
    .SetTuner           = MDrv_Tuner_Null_SetTuner,
    .CheckExist         = MDrv_Tuner_AV2017_CheckExist,
    .GetTunerIF         = MDRV_Tuner_Null_GetTunerIF,
    .Extension_Function = AV2017_Extension_Function
};
