...
MXL_STATUS MxL661_Ctrl_ProgramRegisters(UINT8 u8TunerIndex, UINT8 I2cAddr, PMXL661_REG_CTRL_INFO_T ctrlRegInfoPtr)
{
  ...
  while (status == MXL_TRUE)
  {
    ...
    {
      status = MxLWare661_OEM_ReadRegister(u8TunerIndex, I2cAddr,ctrlRegInfoPtr[i].regAddr, &tmp);
    ...
    status = MxLWare661_OEM_WriteRegister(u8TunerIndex, I2cAddr,ctrlRegInfoPtr[i].regAddr, tmp);
    if (status != MXL_TRUE) break;
...
MXL_STATUS MxL661_Ctrl_WriteRegField(UINT8 u8TunerIndex, UINT8 I2cAddr, PMXL661_REG_CTRL_INFO_T ctrlRegInfoPtr)
{
  MXL_STATUS status;
  UINT8 tmp = 0;
  ...
  {
      status = MxLWare661_OEM_ReadRegister(u8TunerIndex, I2cAddr, ctrlRegInfoPtr->regAddr, &tmp);
  ...
  status = MxLWare661_OEM_WriteRegister(u8TunerIndex, I2cAddr, ctrlRegInfoPtr->regAddr, tmp);
...
MXL_STATUS MxL661_Ctrl_SetRfFreqLutReg(UINT8 u8TunerIndex, UINT8 i2cAddress, UINT32 FreqInHz, PMXL661_CHAN_DEPENDENT_FREQ_TABLE_T freqLutPtr)
{
  ...
  for (idx = 0; idx < MxL661_SPUR_REGISTER.SpurRegNum; idx++)
    status |= MxLWare661_OEM_WriteRegister(u8TunerIndex, i2cAddress, MxL661_SPUR_REGISTER.SpurRegAddr[idx], regSetData[idx]);
...
--|
--|-----------------------------------------------------------------------------*/
MXL_STATUS MxL661_GetRFStrenth(UINT8 u8TunerIndex, UINT8 I2cTunerAddr, SINT32 *pRFStrength)
{
  ...
  status |= MxLWare661_OEM_ReadRegister(u8TunerIndex, I2cTunerAddr, RF_RSSI_DB_RB, &readData);
  dfeRfrssiDbCal = (readData & 0xFF);
  dfeRfrssiDbCal = ((dfeRfrssiDbCal*100 + 8) / 16);

  status |= MxLWare661_OEM_ReadRegister(u8TunerIndex, I2cTunerAddr, AGC_ATTN_LNA_BO, &readData);
...
--|
--|-----------------------------------------------------------------------------*/
MXL_STATUS MxL661_GetRSSI(UINT8 u8TunerIndex, UINT8 I2cTunerAddr, SINT32 *pdfeRSSI)
{
  ...
  // Read dfe_agc_slope
  status |= MxLWare661_OEM_ReadRegister(u8TunerIndex, I2cTunerAddr, AGC_SLOPE_REG, &readData);
  defAgcSlope = (readData & 0x07);
  defAgcSlope = ((defAgcSlope*100 + 2) / 4);

  // Read dfe_agc_gaincodeoffset
  status |= MxLWare661_OEM_ReadRegister(u8TunerIndex, I2cTunerAddr, AGC_OFFSET_REG, &readData);
  dfeAgcGainOffset = (readData & 0x3F);

  // Read total_gain_code
  status |= MxLWare661_OEM_ReadRegister(u8TunerIndex, I2cTunerAddr, AGC_TOTALGAIN_RB_L, &readData);
  dfeAgcTotalGain = (readData & 0xFF);
  status |= MxLWare661_OEM_ReadRegister(u8TunerIndex, I2cTunerAddr, AGC_TOTALGAIN_RB_H, &readData);
  dfeAgcTotalGain |= ((readData & 0x1F) << 8);

  // Read RSSIDig2
  status |= MxLWare661_OEM_ReadRegister(u8TunerIndex, I2cTunerAddr, DIG2_RSSI_DB_RB_L, &readData);
  dfeRSSIDig2 = (readData & 0xFF);
  status |= MxLWare661_OEM_ReadRegister(u8TunerIndex, I2cTunerAddr, DIG2_RSSI_DB_RB_H, &readData);
...
#endif /* (FRONTEND_TUNER_TYPE == TUNER_MXL661) */
