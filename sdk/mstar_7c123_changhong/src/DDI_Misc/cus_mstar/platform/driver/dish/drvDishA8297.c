...
static MS_BOOL _read_Status_Reg(MS_U8 u8DishIndex, MS_U8 u8Addr, MS_U8* pu8Data)
{
    ...
    u8BuffTemp[0] = u8Addr;
    bRet &= MDrv_IIC_WriteBytes(_get_I2C_port(u8DishIndex), A8297_SLAVE_ADDR, 0, NULL, 1, &u8BuffTemp[0]);
    bRet &= MDrv_IIC_ReadBytes(_get_I2C_port(u8DishIndex),A8297_SLAVE_ADDR_R, 0, NULL, 1, &u8Data);
    *pu8Data = u8Data;
    return bRet;
}
...
MS_BOOL MDrv_Dish_SetLNBPower(MS_U8 u8DishIndex, DISH_LNBPWR_TYPE enLNBPwr)
{
    ...
    u8Data &= (~(0xf << CTRL_REG_SHIFT[u8Index]));
    
      switch(enLNBPwr)
      {
           case EN_LNBPWR_TYPE_OFF:
               u8Data |= DATA_LNBPOWER_OFF << CTRL_REG_SHIFT[u8Index];
               break;
           case EN_LNBPWR_TYPE_13V:
               u8Data |= DATA_13V_OUT<< CTRL_REG_SHIFT[u8Index];
               break;
           case EN_LNBPWR_TYPE_18V:
               u8Data |= DATA_18V_OUT<< CTRL_REG_SHIFT[u8Index];
               break;
           default:
           u8Data = DATA_13V_OUT<< CTRL_REG_SHIFT[u8Index];
