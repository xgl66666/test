...
static DEMOD_MS_INIT_PARAM MS_DVBT_INIT_PARAM;
...
MS_BOOL MDrv_MStar_DVBT_Demod_Restart(MS_U8 u8DemodIndex, DEMOD_MS_FE_CARRIER_PARAM* pParam,MS_U32 u32BroadCastType)
{
    ...
    if(!MS_DVBT_INIT_PARAM.pstTunertab->GetTunerIF(u8DemodIndex, &u32IF_Freq))
    {
        u32IF_Freq = _u32IFrequency;
        DMD_DBG(("INTERN_DVBT Get Tuner IF FAIL, use default Tuner IF %d kHz\n",(int)u32IF_Freq));
    }
    else
    {
       DMD_DBG(("INTERN_DVBT Get Tuner IF %d kHz\n", (int)u32IF_Freq));
    }

    //MDrv_DMD_DVBT_SetConfigHPLP(BW, FALSE, bPalBG, bLPSel);

    MDrv_DMD_DVBT_SetConfigHPLPSetIF(BW, FALSE, bPalBG, bLPSel, u32IF_Freq, 45474, (MS_U8)FRONTEND_DEMOD_IQ_SWAP);
...
MS_BOOL MDrv_MStar_DVBT_Demod_I2C_ByPass(MS_U8 u8DemodIndex,MS_BOOL bOn)
{
  ...
  {
      return GET_DEMOD_ENTRY_NODE(DEMOD_MSINTERN_DVBT).I2CByPassPreSetting(u8DemodIndex,bOn);
  }
  else
