...
#endif
#endif
DRV_DEMOD_TABLE_TYPE GET_DEMOD_ENTRY_NODE(DEMOD_MSKENYA_DVBC) DDI_DRV_TABLE_ENTRY(demodtab);

static MS_BOOL bInited = FALSE;
static MS_S32 _s32MutexId = -1;
static DEMOD_MS_INIT_PARAM KenyaDVBC_InitParam;
static MS_U32 _u32IFrequency = 36130;//KHz for demod restart&init
...
MS_BOOL MDrv_Kenya_Demod_Init(MS_U8 u8DemodIndex,DEMOD_MS_INIT_PARAM* pParam)
{
...
MS_BOOL MDrv_Kenya_Demod_GetSNR(MS_U8 u8DemodIndex,float *pfSNR)
{
    MS_BOOL ret;
...
MS_BOOL MDrv_Kenya_Demod_Restart(MS_U8 u8DemodIndex, DEMOD_MS_FE_CARRIER_PARAM* pParam,MS_U32 u32BroadCastType)
{
    ...
    if(!KenyaDVBC_InitParam.pstTunertab->GetTunerIF(u8DemodIndex, &u32IF_Freq))
    {
        u32IF_Freq = _u32IFrequency;
        DMD_DBG(("Kenya Internal DVBC get Tuner IF fail, use default IF = %d kHz\n",(int)u32IF_Freq));
    }
    else
    {
        DMD_DBG(("Kenya Internal DVBC get Tuner IF OK, IF = %d kHz\n",(int)u32IF_Freq));
    }
    ...
    //{
        ...
        switch(pParam->CabParam.eBandWidth)
        {
            case DEMOD_CAB_BW_6M:
                 MDrv_DMD_DVBC_GetDSPReg(REG_DVBC_BW, &u8reg_data);
                 MDrv_DMD_DVBC_SetDSPReg(REG_DVBC_BW, u8reg_data | 0x1);
                 break;
            case DEMOD_CAB_BW_8M:
            default:
                 MDrv_DMD_DVBC_GetDSPReg(REG_DVBC_BW, &u8reg_data);
                 MDrv_DMD_DVBC_SetDSPReg(REG_DVBC_BW, u8reg_data & 0xFE);
                 break;
        }
#endif
#endif
...
MS_BOOL MDrv_Kenya_Demod_I2C_ByPass(MS_U8 u8DemodIndex,MS_BOOL bOn)
{
  if(GET_DEMOD_ENTRY_NODE(DEMOD_MSKENYA_DVBC).I2CByPassPreSetting != NULL)
  {
      return GET_DEMOD_ENTRY_NODE(DEMOD_MSKENYA_DVBC).I2CByPassPreSetting(u8DemodIndex,bOn);
  }
  else
...
DRV_DEMOD_TABLE_TYPE GET_DEMOD_ENTRY_NODE(DEMOD_MSKENYA_DVBC) DDI_DRV_TABLE_ENTRY(demodtab) =
{
     ...
     .I2CByPass                    = MDrv_Kenya_Demod_I2C_ByPass,
