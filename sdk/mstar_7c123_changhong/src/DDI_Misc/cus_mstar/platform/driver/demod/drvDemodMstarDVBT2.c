...
#define DVBT2_TDI_LEN 0x600000
#define DVBT2_DJB_LEN 0xC8000
#define DVBT2_EQ_LEN  0x96000
#if defined(CHIP_K5AP)
/*Because DVB-T and DVB-T2 F/W merge together ,so F/W size is bigger*/
#define DVBT2_FW_LEN 0x20000
#else
#define DVBT2_FW_LEN  0x8000
#endif

#if defined(CHIP_K5TN) || defined(CHIP_K5AP)
typedef enum
{
    COFDM_FEC_LOCK,
    COFDM_TPS_LOCK,
...
static MS_U32 u32INTERNAL_DVBT2_EQ_ADR = 0;
static MS_U32 u32INTERNAL_DVBT2_TDI_ADR = 0;
static MS_U32 u32INTERNAL_DVBT2_DJB_ADR = 0;
static MS_U32 u32INTERNAL_DVBT2_FW_ADR = 0;
...
DEMOD_MS_INIT_PARAM MS_DVBT2_INIT_PARAM;
...
#else
    if((u32INTERNAL_DVBT2_EQ_ADR == 0) || (u32INTERNAL_DVBT2_TDI_ADR == 0) ||\
        (u32INTERNAL_DVBT2_DJB_ADR == 0) || (u32INTERNAL_DVBT2_FW_ADR == 0))
    {
       return FALSE;
    }
    else
...
#if (defined(UFO_DEMOD_DVBT2_SUPPORT_DMD_INT) && INTERN_DVBT2_SUPPORT_DMD_INT)||\
    (defined(UFO_DEMOD_DVBT_SUPPORT_DMD_INT) && INTERN_DVBT_SUPPORT_DMD_INT)
static void _mdrv_msdvbt2_demod_cb(MS_U8 u8arg)
{
    if(MS_DVBT2_INIT_PARAM.fpCB!= NULL)
    {
        MS_DVBT2_INIT_PARAM.fpCB(MS_DVBT2_INIT_PARAM.u8FrontendIndex,u8arg);
    }
}
#endif
...
MS_BOOL MDrv_MSDVBT2_Demod_Init(MS_U8 u8DemodIndex,DEMOD_MS_INIT_PARAM* pParam)
{
    ...
    memcpy(&MS_DVBT2_INIT_PARAM, pParam, sizeof(DEMOD_MS_INIT_PARAM));
    ...
    if(ret == TRUE)
    {
        ...
        #if (defined(UFO_DEMOD_DVBT2_SUPPORT_DMD_INT) && INTERN_DVBT2_SUPPORT_DMD_INT)
        MDrv_DMD_DVBT2_Reg_INT_CB(_mdrv_msdvbt2_demod_cb);
        #endif
        #if (defined(UFO_DEMOD_DVBT_SUPPORT_DMD_INT) && INTERN_DVBT_SUPPORT_DMD_INT)
        MDrv_DMD_DVBT_Reg_INT_CB(_mdrv_msdvbt2_demod_cb);
        #endif
    }
...
#if MS_DVBT2_INUSE
MS_BOOL MDrv_MSDVBT2_Demod_GetNoChannelFlag(COFDM_LOCK_STATUS eStatus)
{
    MS_U16        u16Address = 0;
    MS_U8         cData = 0;
    MS_U8         cBitMask = 0;
...
MS_BOOL MDrv_MSDVBT2_Demod_GetLock(MS_U8 u8DemodIndex, EN_LOCK_STATUS *peLockStatus)
{
    ...
    if(E_DEMOD_TYPE_T == eCurDemodType)
    {
        ...
        doneFlag = MDrv_MSDVBT2_Demod_GetNoChannelFlag(COFDM_DETECT_DONE_FLAG);
        if (doneFlag)
        {
            tChannelFlag = MDrv_MSDVBT2_Demod_GetNoChannelFlag(COFDM_DVBT_NOCH_FLAG);
            t2ChannelFlag = MDrv_MSDVBT2_Demod_GetNoChannelFlag(COFDM_DVBT2_NOCH_FLAG);
...
MS_BOOL MDrv_MSDVBT2_Demod_Get_Packet_Error(MS_U8 u8DemodIndex, MS_U16 *u16_data)
{
    MS_BOOL ret= TRUE;
    ...
    if(E_DEMOD_TYPE_T == eCurDemodType)
        ret = MDrv_DMD_DVBT_GetPacketErr(u16_data);
#if MS_DVBT2_INUSE
    else
        ret = MDrv_DMD_DVBT2_GetPacketErr(u16_data);
#endif
    HB_ReleaseMutex(_s32MutexId);
    return ret;
}
...
MS_BOOL MDrv_MSDVBT2_Demod_GetPWR(MS_U8 u8DemodIndex, MS_S32 *ps32Signal)
{
    ...
    ret = MS_DVBT2_INIT_PARAM.pstTunertab->Extension_Function(u8DemodIndex, TUNER_EXT_FUNC_GET_POWER_LEVEL, ps32Signal);
...
MS_BOOL MDrv_MSDVBT2_Demod_GetSSI(MS_U8 u8DemodIndex, MS_U16 *pu16SSI)
{
    ...
    ret &= MS_DVBT2_INIT_PARAM.pstTunertab->Extension_Function(u8DemodIndex, TUNER_EXT_FUNC_GET_POWER_LEVEL, &PowerLevel);
    ...
    else
    {
         if(E_DEMOD_TYPE_T == eCurDemodType)
             ...
             ret &= MDrv_DMD_DVBT2_GetSignalStrengthWithRFPower(pu16SSI, fRFPowerDbm);
#endif
    }
...
MS_BOOL MDrv_MSDVBT2_Demod_GetParam(MS_U8 u8DemodIndex,DEMOD_MS_FE_CARRIER_PARAM* pParam)
{
    MS_BOOL ret = TRUE;
    MS_U16 u16Info;
    if (HB_ObtainMutex(_s32MutexId, COFDMDMD_MUTEX_TIMEOUT) == FALSE)
    ...
    if(E_DEMOD_TYPE_T == eCurDemodType)
    {
        ret &= MDrv_DMD_DVBT_GetTPSInfo(&u16Info);
        if(ret)
        {
           ...
           pParam->TerParam.eConstellation = u16Info&0x3;
           pParam->TerParam.eHierarchy = (u16Info & (0x7 << 3))>>3;
           pParam->TerParam.eLPCodeRate = (u16Info & (0x7 <<6))>>6;
           pParam->TerParam.eHPCodeRate = (u16Info & (0x7 << 9))>>9;
           pParam->TerParam.eGuardInterval= (u16Info & (0x3 << 12))>>12;
           pParam->TerParam.eFFT_Mode = (u16Info & (0x3<<14))>>14;
        }
    }
#if MS_DVBT2_INUSE
    else
    {
        ret &= MDrv_DMD_DVBT2_GetL1Info(&u16Info, T2_MODUL_MODE);
        pParam->TerParam.eConstellation = u16Info & 0x7;
        
        ret &= MDrv_DMD_DVBT2_GetL1Info(&u16Info, T2_CODE_RATE);
        pParam->TerParam.eHierarchy = 0;
        pParam->TerParam.eHPCodeRate = u16Info & 0x7;
        pParam->TerParam.eLPCodeRate = pParam->TerParam.eHPCodeRate;
         ...
         ret &= MDrv_DMD_DVBT2_GetL1Info(&u16Info, T2_GUARD_INTERVAL);
         pParam->TerParam.eGuardInterval= u16Info & 0x7;
         
         ret &= MDrv_DMD_DVBT2_GetL1Info(&u16Info, T2_FFT_VALUE);
         pParam->TerParam.eFFT_Mode = u16Info & 0x07;
    }
#endif
...
MS_BOOL MDrv_MSDVBT2_Demod_Restart(MS_U8 u8DemodIndex, DEMOD_MS_FE_CARRIER_PARAM* pParam,MS_U32 u32BroadCastType)
{
    ...
    if ( (u32BroadCastType == DVBT) && (MDrv_MSDVBT2_Demod_GetCurrentDemodType(u8DemodIndex) != E_DEMOD_TYPE_T) )
    {
        MDrv_MSDVBT2_Demod_SetCurrentDemodType(u8DemodIndex,E_DEMOD_TYPE_T);
    }
    else if ( (u32BroadCastType == DVBT2) && (MDrv_MSDVBT2_Demod_GetCurrentDemodType(u8DemodIndex) != E_DEMOD_TYPE_T2) )
    {
        MDrv_MSDVBT2_Demod_SetCurrentDemodType(u8DemodIndex,E_DEMOD_TYPE_T2);
    }
    else
    {
        DMD_DBG((" Don't need to reload FW\n"));
    }
#endif
    ...
    //if(cur_BW != pParam->TerParam.eBandWidth)
    {
        ...
        if(E_DEMOD_TYPE_T == MDrv_MSDVBT2_Demod_GetCurrentDemodType(u8DemodIndex))
        {
...
{
    if (E_DEMOD_TYPE_T2 == MDrv_MSDVBT2_Demod_GetCurrentDemodType(u8DemodIndex))
...
MS_BOOL DEMOD_MSDVBT2_Extension_Function(MS_U8 u8DemodIndex, DEMOD_EXT_FUNCTION_TYPE fuction_type, void *data)
{
    ...
    case DEMOD_EXT_FUNC_SET_BUFFER_ADDR:
        ...
        u32INTERNAL_DVBT2_DJB_ADR = u32INTERNAL_DVBT2_TDI_ADR + (MS_U32)DVBT2_TDI_LEN;
        u32INTERNAL_DVBT2_FW_ADR =  u32INTERNAL_DVBT2_DJB_ADR +(MS_U32)DVBT2_DJB_LEN;
        u32INTERNAL_DVBT2_EQ_ADR = u32INTERNAL_DVBT2_FW_ADR + (MS_U32)DVBT2_FW_LEN;
        break;

        case DEMOD_EXT_FUNC_SET_INTERRUPT_CALLBACK:
            MS_DVBT2_INIT_PARAM.fpCB = (fpDemodCB)(data);
 #if (defined(UFO_DEMOD_DVBT_SUPPORT_DMD_INT) && INTERN_DVBT_SUPPORT_DMD_INT)
            MDrv_DMD_DVBT_Reg_INT_CB(_mdrv_msdvbt2_demod_cb);
 #endif
 #if (defined(UFO_DEMOD_DVBT2_SUPPORT_DMD_INT) && INTERN_DVBT2_SUPPORT_DMD_INT)
...
{
    if(GET_DEMOD_ENTRY_NODE(DEMOD_MSINTERN_DVBT2).I2CByPassPreSetting != NULL)
    {
        return GET_DEMOD_ENTRY_NODE(DEMOD_MSINTERN_DVBT2).I2CByPassPreSetting(u8DemodIndex,bOn);
    }
    else
...
DRV_DEMOD_TABLE_TYPE GET_DEMOD_ENTRY_NODE(DEMOD_MSINTERN_DVBT2) DDI_DRV_TABLE_ENTRY(demodtab) =
{
...
#if ((defined(UFO_DEMOD_DVBT_SUPPORT_DMD_INT) && INTERN_DVBT_SUPPORT_DMD_INT)||\
    (defined(UFO_DEMOD_DVBT2_SUPPORT_DMD_INT) && INTERN_DVBT2_SUPPORT_DMD_INT))
     .SupportINT                   = TRUE,
#else
     .SupportINT                   = FALSE,
#endif
    ...
    .GetParam                     = MDrv_MSDVBT2_Demod_GetParam,
    ...
    .Get_Packet_Error             = MDrv_MSDVBT2_Demod_Get_Packet_Error,
