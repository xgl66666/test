...
DRV_DEMOD_TABLE_TYPE GET_DEMOD_ENTRY_NODE(DEMOD_MSKAISER_DVBC)  DDI_DRV_TABLE_ENTRY(demodtab) ;
...
static DEMOD_MS_INIT_PARAM Kaiser_DVBC_INIT_PARAM;
...
MS_BOOL MDrv_Demod_Dual_Public_Init(void )
{
    ...
    static MS_U8 u8DMD_DVBC_InitExt[]={
       ...
       (MS_U8)(36167>>8),
       (MS_U8)(36167>>0),
       (MS_U8)(24000>>24), // FS Frequency 45474 => 24000 K3
       (MS_U8)(24000>>16),
       (MS_U8)(24000>>8),
       (MS_U8)(24000>>0),
       0, // IQ Swap
...
MS_BOOL MDrv_Demod_MSDVBC_51_Init(MS_U8 u8DemodIndex,DEMOD_MS_INIT_PARAM* pParam)
{
    ...
    Kaiser_DVBC_INIT_PARAM.pstTunertab = pParam->pstTunertab;
    switch (u8DemodIndex)
    {
        case 0:
...
MS_BOOL MDrv_Demod_MSDVBC_51_Restart(MS_U8 u8DemodIndex, DEMOD_MS_FE_CARRIER_PARAM* pParam,MS_U32 u32BroadCastType)
{
    ...
    if(!Kaiser_DVBC_INIT_PARAM.pstTunertab->GetTunerIF(u8DemodIndex,&u32IF_Freq))
    {
       u32IF_Freq = _u32IFrequency;
       DMD_DBG(("Kaiser DVB-C demod get Tuner IF FAIL, use default IF %d kHz\n", (int)u32IF_Freq));
    }
    else
    {
       DMD_DBG(("Kaiser DVB-C demod get Tuner IF %d kHz\n", (int)u32IF_Freq));
    }
...
{
  if(GET_DEMOD_ENTRY_NODE(DEMOD_MSKAISER_DVBC).I2CByPassPreSetting != NULL)
  {
      return GET_DEMOD_ENTRY_NODE(DEMOD_MSKAISER_DVBC).I2CByPassPreSetting(u8DemodIndex,bOn);
  }
  else
...
DRV_DEMOD_TABLE_TYPE GET_DEMOD_ENTRY_NODE(DEMOD_MSKAISER_DVBC)  DDI_DRV_TABLE_ENTRY(demodtab) =
{
     .name                         = "DEMOD_MSKAISER_DVBC",
     .data                         = DEMOD_MSKAISER_DVBC,
