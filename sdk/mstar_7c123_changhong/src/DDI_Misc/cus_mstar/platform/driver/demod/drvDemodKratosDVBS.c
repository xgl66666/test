...
static MS_BOOL bIsBlindGetTPList[MAX_FRONTEND_NUM];
...
MS_BOOL MDrv_Demod_Kratos_DVBS_Init(MS_U8 u8DemodIndex,DEMOD_MS_INIT_PARAM* pParam)
{
    ...
    bIsBlindGetTPList[u8DemodIndex] = FALSE;
...
MS_BOOL MDrv_Demod_Kratos_DVBS_Restart(MS_U8 u8DemodIndex, DEMOD_MS_FE_CARRIER_PARAM* pParam,MS_U32 u32BroadCastType)
{
    ...
    //{
        ...
        if(    bIsBlindGetTPList[u8DemodIndex])
        {
            if(FALSE == MDrv_DMD_DVBS_BlindScan_Config(pParam->SatParam.u32SymbolRate, eModulationType, 0, FRONTEND_DEMOD_IQ_SWAP, FALSE))
            {
...
MS_BOOL MDrv_Demod_Kratos_BlindScan_Start(MS_U8 u8DemodIndex, MS_U16 u16StartFreq,MS_U16 u16EndFreq)
{
    ...
    bIsBlindGetTPList[u8DemodIndex] = TRUE;
