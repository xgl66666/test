...
BOOL _Mdrv_MulProtCommon_AddShot(U32 u32Value, BOOL bNegtive)
{
    if(SHOTLIST_FULL())
    {
        IR_MULTI_PROTOCOL_PRINT("Error buffer full\n");
        return FALSE;
    }
    else
    {
...
{
    IR_MULTI_PROTOCOL_PRINT("-----dump data----\n");
    if(!SHOTLIST_EMPTY())
    {
        U8 u8Index=0;
        U32 u32ShotValue;
        ...
        {
            if(bNegShot==TRUE)
                IR_MULTI_PROTOCOL_PRINT("N[%d]: %u\n", u8Index, u32ShotValue);
            else
                IR_MULTI_PROTOCOL_PRINT("P[%d]: %u\n", u8Index, u32ShotValue);

            u8Index++;
        }
    }
    IR_MULTI_PROTOCOL_PRINT("-----dump end----\n");
}
#endif
