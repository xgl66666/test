...
/***************************************************************************
  ...
  *
  **************************************************************************/
#define AllOCATE_TX_RING	NUM_OF_TX_RING*2  /* For Two wifi device  */
#define DescMBuf_SIZE (AllOCATE_TX_RING*3)
	...
	OUT	PNDIS_PHYSICAL_ADDRESS PhysicalAddress)
{
	...
	for (i = 0; i < DescMBuf_SIZE;i++)
	{
		if (DescMBuf[i].pMBuf == NULL)
		{
			...
			//HAL_DCACHE_INVALIDATE();//for gx cpu
			if (Cached)
...
static ULONG FirstTx_SIZE[AllOCATE_TX_RING][TX_RING_SIZE * TX_DMA_1ST_BUFFER_SIZE];
	...
	OUT	PNDIS_PHYSICAL_ADDRESS PhysicalAddress)
{
	...
	FirstTx_Index++;
	if (FirstTx_Index == AllOCATE_TX_RING)
		FirstTx_Index = 0;
        ...
        pPkt = (PECOS_PKT_BUFFER) pPacket;
       	...
       	switch (pPkt->MemPoolType)
       	{
            case MemPool_TYPE_MBUF:
                ...
                break;
            default:
				diag_printf("%s Fuck!! Is anything wrong?\n",__FUNCTION__);
                kfree(pPkt);
                break;
        }
    }
	...
	IN int size)
{
    ...
    pPacket = (PECOS_PKT_BUFFER) RTMP_AllocateRxPacketBuffer(pAd, NULL, size, FALSE, &AllocVa, &AllocPa);
    return (PNDIS_PACKET) pPacket;    
}
	...
	IN	ULONG	Length)
{
    ...
    pPacket = (PECOS_PKT_BUFFER) RTMP_AllocateRxPacketBuffer(pAd, NULL, Length, FALSE, &AllocVa, &AllocPa);
    return (PNDIS_PACKET) pPacket;
}
	...
	IN	UCHAR			FromWhichBSSID)
{
	...
	/* copy header (maybe +VLAN tag) */
	NdisMoveMemory(pPacket->pDataPtr, pHeader802_3, HdrLen);
	skb_put(pPacket, HdrLen + VLAN_Size);

	/* copy data body */
	NdisMoveMemory(pPacket->pDataPtr + HdrLen , pData, DataSize);
	skb_put(pPacket, DataSize);
	pPacket->net_dev = get_netdev_from_bssid(pAd, FromWhichBSSID);
    diag_printf("%s\n",__FUNCTION__);
    return (PNDIS_PACKET) pPacket;
