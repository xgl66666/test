    ...
    IN  PSTRING          arg)
{
	...
	*/
	...
	mt7601_connect_status = 0; //switch ssid
    return success;
}
...
next:
	if (bIsPrintAllMAC)
	{
		...
		if (pBufMac != NULL)
		{
			...
			printk("MacAddr=0x%04x, MacValue=0x%x\n", macAddr, macValue);
...
// ++Alan test
	if (wrq->u.data.length > 1) /*No parameters. */
	{
		...
		sprintf(msg, "\n");
	    /*Parsing Read or Write */
		while ((this_char = mtk_simple_strsep((char **)&ptr, ",")) != NULL)
		{
		if (!*this_char)
			goto next;
	...
	IN	PBSS_ENTRY				pBssEntry)
{
	...
	if (pBssEntry->Rssi >= -50)
		...
		pSignal->ChannelQuality = (UINT8)(24 + ((pBssEntry->Rssi + 80) * 26)/10);
	else if (pBssEntry->Rssi >= -90)   /* between -80 ~ -90dbm */
        pSignal->ChannelQuality = (UINT8)((pBssEntry->Rssi + 90) * 26)/10;
	else
		pSignal->ChannelQuality = 0;

    pSignal->Rssi = (UINT8)(pBssEntry->Rssi);
	...
	IN	ULONG					Data)
{
	...
	// for GX
	sprintf(pData,"%02x:%02x:%02x:%02x:%02x:%02x",PRINT_MAC(pAd->CurrentAddress));
	return NDIS_STATUS_SUCCESS;
}
