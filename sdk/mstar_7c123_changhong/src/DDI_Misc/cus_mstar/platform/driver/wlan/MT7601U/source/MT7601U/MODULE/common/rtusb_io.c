	...
	IN	UINT32			TransferBufferLength)
{
	...
	{
		DBGPRINT(RT_DEBUG_TRACE, ("MCU has entered sleep mode\n"));
		return NDIS_STATUS_FAILURE;
	}
	else
	{
		...
		do {
			 ...
			 if(RET==-ESHUTDOWN) //add for mstar,if host driver think device is down,we just stop wifi
			{
				RTMP_SET_FLAG(pAd, fRTMP_ADAPTER_NIC_NOT_EXIST);
				DBGPRINT(RT_DEBUG_ERROR, ("%s get ESHUTDOWN Error,shut down device\n",__FUNCTION__));
			    break;
			}
