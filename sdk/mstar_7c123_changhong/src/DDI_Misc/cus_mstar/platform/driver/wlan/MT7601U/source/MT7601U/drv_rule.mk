...
ifeq ($(DRV_NAME),DEMOD)
ifeq ($(INTERN_DVBS_SUPPORT),enable)
DRV_SRC     += $(wildcard $(CURDIR)/*.c)
else
DRV_SRC     += $(filter-out $(wildcard $(CURDIR)/drvDMD_INTERN_DVBS.c), $(wildcard $(CURDIR)/*.c))
endif
else
DRV_SRC     += $(wildcard $(CURDIR)/*.c)
endif

ifeq ($(CUSTOMER_SUPPORT),y)
CUSTOMER_DRV_NAME = AUDIO AUDSP
ifeq ($(DRV_NAME),$(filter $(DRV_NAME),$(CUSTOMER_DRV_NAME)))
HAL_ROOT = $(CUS_HAL)
endif
else
HAL_ROOT = $(DDIHAL)
...
HAL_SRC     = $(foreach dir, $(HAL_DIR), $(wildcard $(HAL_ROOT)/$(CHIP)/$(dir)/*.c))
HAL_SRC_S   = $(foreach dir, $(HAL_DIR), $(wildcard $(HAL_ROOT)/$(CHIP)/$(dir)/*.s))
...
HAL_IMPORT_SRC = $(foreach dir, $(HAL_DIR), $(wildcard $(HAL_ROOT)/$(CHIP)/$(dir)/obj_import/$(OS_TYPE)/$(MCU_TYPE)/*.src))
HAL_IMPORT_OBJ = $(foreach file, $(notdir $(patsubst %.src, %.o, $(HAL_IMPORT_SRC))), $(OBJ_PATH)/$(file))
DRV_INCDIR  = $(CURDIR) $(foreach dir,$(DRV_SUB),$(CURDIR)/$(dir)) $(foreach dir,$(HAL_DIR),$(HAL_ROOT)/$(CHIP)/$(dir))
DRV_CC_INCS = $(foreach dir,$(DRV_INCDIR),-I$(dir))
...
VPATH = $(DRV_SUB) $(foreach dir, $(HAL_DIR), $(HAL_ROOT)/$(CHIP)/$(dir))
...
all : $(DRV_OBJ) $(DRV_IMPORT_OBJ) $(HAL_OBJ_S) $(HAL_OBJ) $(HAL_IMPORT_OBJ) $(HAL_OBJ_T) $(HAL_IMPORT_OBJ_T) $(DRV_OBJ_T) $(DRV_IMPORT_OBJ_T)
	@echo "DRV_IMPORT_OBJ="$(DRV_IMPORT_OBJ)
	@echo "HAL_OBJ_S="$(HAL_OBJ_S)
	@echo "HAL_OBJ="$(HAL_OBJ)
	@echo "HAL_IMPORT_OBJ="$(HAL_IMPORT_OBJ)
	@echo "HAL_OBJ_T="$(HAL_OBJ_T)
	@echo "HAL_IMPORT_OBJ_T="$(HAL_IMPORT_OBJ_T)
	@echo "DRV_OBJ_T="$(DRV_OBJ_T)
	@echo "DRV_IMPORT_OBJ_T="$(DRV_IMPORT_OBJ_T)
ifneq ($(SW_API_FLOW_ENABLE),TRUE)
	@echo "SW_API_FLOW_ENABLE is ture"
	@$(OBJCOPY) --add-section .mmodule_version=version_info $(word 1, $(DRV_OBJ))
endif
	@echo "MAKE_TYPE=""$(MAKE_TYPE)"
	@echo "LINK_TYPE=""$(LINK_TYPE)"
	@echo "BUILDING_API=""$(BUILDING_API)"
	@echo "INC_BUILD=""$(INC_BUILD)"
	@echo "WIFI_DRIVER_LIB="
	@echo "$(DDI_PATH)/lib$(LIB_NAME).$(LIB_EXT)"
ifneq ($(MAKE_TYPE),lint)
ifeq ($(LINK_TYPE),dynamic)
ifneq ($(BUILDING_API),1)
    ifeq ($(INC_BUILD),enable)
	@$(CC) $(CFG_CC_OPTS) $(LD_LIBS_INC) $(LD_OPTS) $(DRV_OBJ) $(DRV_IMPORT_OBJ) $(HAL_OBJ_S) $(HAL_OBJ) $(HAL_IMPORT_OBJ) -g -o $(DDI_PATH)/lib$(LIB_NAME).$(LIB_EXT)
    else
	@$(CC) $(CFG_CC_OPTS) $(LD_OPTS) $(DRV_OBJ) $(DRV_IMPORT_OBJ) $(HAL_OBJ_S) $(HAL_OBJ) $(HAL_IMPORT_OBJ) -g -o $(DDI_PATH)/lib$(LIB_NAME).$(LIB_EXT)
    endif
endif
else
...
$(DRV_OBJ)  :  %.o : %.c
	@echo CC_OPTS=$(CC_OPTS)  > /dev/null
	@echo CC_DEFS=$(CC_DEFS) > /dev/null
	@echo CC_INCS=$(CC_INCS) > /dev/null
	@echo CC_INCS=$(CFLAGS) > /dev/null
	@echo EXTRA_CFLAGS= $(EXTRA_CFLAGS) > /dev/null
	@echo WFLAGS=$(WFLAGS) > /dev/null
ifneq ($(MAKE_TYPE),lint)
	@echo [CC] $(notdir $<)
	$(CC) $(WFLAGS) $(EXTRA_CFLAGS) $(CC_OPTS) $(CC_DEFS) $(CC_INCS) $(DRV_CC_INCS) -o $@ $<;
...
setup:
ifeq ($(LINK_TYPE),dynamic)
	@echo [drv_rule.mk][lib$(LIB_NAME).so]
else
	@echo [drv_rule.mk][lib$(LIB_NAME).a]
endif

clean :
#	@echo "Clean WiFi Driver..."
	@rm -f $(HAL_OBJ_S)
	@rm -f $(HAL_OBJ)
	@rm -f $(HAL_IMPORT_OBJ)
	@rm -f $(DRV_OBJ)
