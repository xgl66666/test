...
//				 ("CNTL - Association successful on BSS #%ld\n", pAd->MlmeAux.BssIdx));
			diag_printf("CNTL - Association successful on BSS #%ld\n", pAd->MlmeAux.BssIdx);
		mt7601_connect_status = 23;	
		DBGPRINT(RT_DEBUG_ERROR, ("%s: mt7601_connect_status =%d\n",  __FUNCTION__,mt7601_connect_status));
		/*
		*for wep and open ,when assocation is done , connection is setup
		*/
		if (pAd->StaCfg.AuthMode <= Ndis802_11AuthModeAutoSwitch)
			{
	...
	for (i = 0; i < total_bssNr; i++) 
	{
		...
		if (bDelEntry)
		{
			...
			if ( MAC_ADDR_EQUAL(pBss->Bssid,pAd->CommonCfg.Bssid))
				diag_printf("delete current bssid\n");	
