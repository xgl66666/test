...
loadfw_protect:
	...
	DBGPRINT(RT_DEBUG_TRACE, ("<%s:%d>:MACValue=%d\n", __FUNCTION__,__LINE__,MACValue));
...
/* ++ dump firmware ++ */
			...
			DBGPRINT(RT_DEBUG_INFO, ("%s: loading ilm before submit URB\n", __FUNCTION__));
			Status = RTUSB_SUBMIT_URB(pURB);
			DBGPRINT(RT_DEBUG_INFO, ("%s: loading ilm after submit URB\n", __FUNCTION__));
			if (Status)
			{
			...
			DBGPRINT(RT_DEBUG_INFO, ("%s: delay point 1\n", __FUNCTION__));
			MACValue++;
			RtmpOsMsDelay(100);
			RTMP_IO_WRITE32(pAd, TX_CPU_PORT_FROM_FCE_CPU_DESC_INDEX, MACValue);
			DBGPRINT(RT_DEBUG_INFO, ("%s: delay point 3\n", __FUNCTION__));
...
/* ++ dump firmware ++ */
	...
	while (1)
	{
		...
		if (SentLen > 0)
		{
			...
			DBGPRINT(RT_DEBUG_INFO, ("%s: loading dlm before submit URB\n", __FUNCTION__));
			Status = RTUSB_SUBMIT_URB(pURB);
			DBGPRINT(RT_DEBUG_INFO, ("%s: loading dlm before submit URB\n", __FUNCTION__));
			if (Status)
			{
