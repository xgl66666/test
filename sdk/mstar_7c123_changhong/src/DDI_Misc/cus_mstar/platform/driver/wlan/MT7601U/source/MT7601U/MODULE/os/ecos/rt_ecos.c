...
unsigned long mtk_simple_strtoul(const char *cp,char **endp,unsigned int base)
{
...
{
        if(*cp=='-')
                return -mtk_simple_strtoul(cp+1,endp,base);
        return mtk_simple_strtoul(cp,endp,base);
}
...
#endif
        pTask->task_killed = 0;
	cyg_thread_delete(pTask->threadID);
...
{
    DBGPRINT(RT_DEBUG_ERROR, ("%s: Eddy Notice\n", __FUNCTION__));
	if_detach(&pNetDev->sc_arpcom.ac_if);
}
	...
	IN RTMP_OS_NETDEV_OP_HOOK	*pDevOpHook)
{
	...
	/*register ifnet to system*/
	pNetDev->funs->eth_drv->init(pNetDev,&pDevOpHook->devAddr[0]);
	return NDIS_STATUS_SUCCESS;
}
...
#endif
	os_free_mem(NULL,pAd); /* pci_free_consistent(os_cookie->pci_dev,sizeof(RTMP_ADAPTER),pAd,os_cookie->pAd_pa); */
	if (os_cookie)
...
void rt_init_completion(RTMP_OS_COMPLETION *comp )
{
	comp->done = 0;
...
void rt_complete(RTMP_OS_COMPLETION *comp )
{
	comp->done =1;
...
void send_monitor_packets(IN PNET_DEV pNetDev,
			  ...
			  IN UINT32 MaxRssi)
{
 /*for monitor mode,we don't support on gx platform*/
	return ;
}
