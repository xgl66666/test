...
/*	pObj = (POS_COOKIE) pAd->OS_Cookie; */
	...
	memcpy(name,extra,strlen(extra));
	memset(extra, 0x00, IW_PRIV_SIZE_MASK);

	wrqin.u.data.pointer = name;
	wrqin.u.data.length = strlen(name);

	pIoctlShow->pData = (CHAR *)extra;
	pIoctlShow->MaxSize = IW_PRIV_SIZE_MASK;
	pIoctlShow->InfType = INT_MAIN;
	RTMP_STA_IoctlHandle(pAd, &wrqin, CMD_RTPRIV_IOCTL_SHOW, SHOW_CFG_VALUE,
...
/*			memcpy(wrqin->u.name, pAd->CurrentAddress, ETH_ALEN); */
			RTMP_STA_IoctlHandle(pAd, NULL, CMD_RTPRIV_IOCTL_STA_SIOCGIFHWADDR,
							0, wrqin->u.data.pointer, 0, INT_MAIN);
		...
		case RTPRIV_IOCTL_SHOW:
			rt_private_show(net_dev, NULL, wrq, wrqin->u.data.pointer);
			break;
		case RTPRIV_IOCTL_STATISTICS:
			rt_private_get_statistics(net_dev, NULL, wrq, wrqin->u.data.pointer);
				break;
...
/*			RTMPIoctlGetSiteSurvey(pAd, wrq); */
		    break;
#if 1//def DBG//by ziqiang for debug
...
/*			RTMPIoctlRF(pAd, wrq); */
       ...
       // case SIOCETHTOOL:
        //        break;
		default:
			DBGPRINT(RT_DEBUG_ERROR, ("IOCTL::unknown IOCTL's cmd = 0x%08x, 0x%08x\n", cmd,SIOCGIFHWADDR));
			Status = -EOPNOTSUPP;
			break;
	}
...
)
{
    ...
    //flag == 1 -> RadioOff  flag == 0 -> RadioOn
...
INT rt28xx_get_connected_ap_bssid(
	IN	PNET_DEV         net_dev,
	IN	OUT	struct ifreq	*rq
	)
{
	...
	struct iwreq        *wrqin = (struct iwreq *) rq;
	PSTRING extra=wrqin->u.data.pointer;
	RT_CMD_STA_IOCTL_SHOW IoctlShow, *pIoctlShow = &IoctlShow;
	GET_PAD_FROM_NET_DEV(pAd, net_dev);
	char name[128];
	...
	RTMP_STA_IoctlHandle(pAd, &wrqin, CMD_RTPRIV_IOCTL_SHOW,SHOW_CONN_STATUS,
						pIoctlShow, 0, INT_MAIN);
