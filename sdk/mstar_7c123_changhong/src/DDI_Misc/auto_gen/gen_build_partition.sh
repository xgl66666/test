...
if [ "$FLASH_TYPE" == "SPI" ]; then
    ...
    if [ "$FS_TYPE" == "ubifs" ] || [ "$FS_TYPE" == "ramdisk" ]; then
        ...
        if [ "$CHIP" == "kiwi" ] || [ "$CHIP" == "k5tn" ] || [ "$CHIP" == "k1c" ] || [ "$CHIP" == "k5ap" ]; then
            let UBI_PEB_SIZE=$(($NAND_PAGE_SIZE * $NAND_BLOCK_PAGE_CNT))
        else
