...
if [ -f $REF_INFO ]; then
        ...
        if [ "$FLASH_TYPE" == "SPI" ]; then
            ...
            if [ "$CHIP" == "kano" ] || [ "$CHIP" == "kayla" ] || [ "$CHIP" == "curry" ] || [ "$CHIP" == "kris" ] || [ "$CHIP" == "k6" ] || [ "$CHIP" == "k6lite" ] || [ "$CHIP" == "k5tn" ] || [ "$CHIP" == "c2p" ] || [ "$CHIP" == "k7u" ] || [ "$CHIP" == "k1c" ] || [ "$CHIP" == "k5ap" ]; then
                RFS_TAR=../../../bsp/linux/linux_kernel/${LINUX_KERNEL_VERSION}/ROOTFS_${RFS_TAR}.${TAR_EXT_NAME}
            else
