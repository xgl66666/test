...
if [ "$OS_TYPE" == "ecos" ]; then
    ...
    if [ "$FS_TYPE" == "ramdisk" ] && [ "$3" == "ROOTFS" ]; then
        exit 0;
    fi

    need_to_signed=false
    OPTEE_NANDINFO_ADDR=28100000
    OPTEE_PAIRPAGEMAP_ADDR=28200000
    OPTEE_PARTITION_ADDR=28300000

    # set image size
    if [ "$FS_TYPE" == "ramdisk" ] && [ "$3" == "kernel" ]; then
    ...
    # set TFTP address
    if [ "$TOOLCHAIN" == "arm-none-linux-gnueabi" ]; then
        TFTP_ADR=20400000
    elif [ "$TOOLCHAIN" == "aarch64-linux-gnu" ]; then
        TFTP_ADR=25000000
...
if [ "$OS_TYPE" == "ecos" ]; then
    if [ "$5" == "TFTP" ]; then
        echo "tftp \$(App_Dram_Address) ./${IMAGENAME}.zip" >> $FILE_PATH
    else
        echo "fatload usb 0 \$(App_Dram_Address) $5/${IMAGENAME}.zip" >> $FILE_PATH
    fi
    echo "spi_wrc \$(App_Dram_Address) 100000 \$(SIZE_APP)" >> $FILE_PATH
else # [ "$OS_TYPE" == "ecos" ]; then
    if [ "$FS_TYPE" == "yaffs2" ] && [ "$3" != "kernel" ]; then
        printf "mw.b 0x${TFTP_ADR} 0xff 0x%x\n" ${IMG_SIZE} >> $FILE_PATH
    fi

    if [ "$3" == "kernel" ]; then
        if [ "$DDI_TEE" == "enable" ] || [ "$MI_DRM_SUPPORT_TEE" == "enable" ] || [ "$DDI_SECURE_UBOOT" == "enable" ]; then
        ...
        if [ "$5" == "NAND" ] || [ "$5" == "EMMC" ] || [ "$5" == "SPI" ]; then
            echo "tftp 0x${TFTP_ADR} $5/bl31.aes" >> $FILE_PATH
        else
            echo "fatload usb 0 ${TFTP_ADR} $5/bl31.aes" >> $FILE_PATH
        fi

    elif [ "$3" == "writecis" ]; then
        if [ "$5" == "NAND" ]; then
            echo "tftp 0x${OPTEE_NANDINFO_ADDR} $5/NANDINFO.nni" >> $FILE_PATH
            echo "tftp 0x${OPTEE_PAIRPAGEMAP_ADDR} $5/PAIRPAGEMAP_v2.ppm" >> $FILE_PATH
            echo "tftp 0x${OPTEE_PARTITION_ADDR} $5/PARTITION_v2_default.pni" >> $FILE_PATH
        else
            echo "fatload usb 0 0x${OPTEE_NANDINFO_ADDR} $5/NANDINFO.nni" >> $FILE_PATH
            echo "fatload usb 0 0x${OPTEE_PAIRPAGEMAP_ADDR} $5/PAIRPAGEMAP_v2.ppm" >> $FILE_PATH
            echo "fatload usb 0 0x${OPTEE_PARTITION_ADDR} $5/PARTITION_v2_default.pni" >> $FILE_PATH
        fi
        echo "ncisbl 0x${OPTEE_NANDINFO_ADDR} 0x${OPTEE_PAIRPAGEMAP_ADDR} 0x${OPTEE_PARTITION_ADDR}" >> $FILE_PATH
        ...
        if [ "$5" == "NAND" ] || [ "$5" == "EMMC" ] || [ "$5" == "SPI" ]; then
            echo "tftp 0x${TFTP_ADR} $5/RTPM.aes" >> $FILE_PATH
        else
            echo "fatload usb 0 ${TFTP_ADR} $5/RTPM.aes" >> $FILE_PATH
        fi
    else
        if [ "$5" == "NAND" ] || [ "$5" == "EMMC" ] || [ "$5" == "SPI" ]; then
    ...
    if [ "$FLASH_TYPE" == "SPI" ]; then
        ...
        if [ "$FS_TYPE" == "ext4" ]; then
            ...
            if [ "$3" == "kernel" ]; then
                if [ "$LINUX_KERNEL_VERSION" == "3.10.40_64bit" ] || [ "$LINUX_KERNEL_VERSION" == "3.10.86_64bit" ] || [ "$LINUX_KERNEL_VERSION" == "4.9_64bit" ] || [ "$CHIP" == "muji" ]; then
                    echo "setenv bootcmd mmc read.p 0x23000000 DTB ${DTB_SIZE}\;mmc read.p 0x25000000 KL \${filesize}\; bootm 0x25000000" >> $FILE_PATH
                elif [ "$LINUX_KERNEL_VERSION" == "3.10.23" ]; then
                    echo "setenv bootcmd mmc read.p 0x81000000 KL \${filesize}\; bootm 0x81000000" >> $FILE_PATH
                else
            ...
            if [ "$3" == "kernel" ]; then
                if [ "$CHIP" == "kaiser" ]; then
                    echo "setenv bootcmd mmc read.p 0x25000000 KL \${filesize}\; bootm 0x25000000" >> $FILE_PATH
                elif [ "$LINUX_KERNEL_VERSION" == "3.10.40_64bit" ] || [ "$LINUX_KERNEL_VERSION" == "3.10.86_64bit" ] || [ "$LINUX_KERNEL_VERSION" == "4.9_64bit" ]; then
                    echo "setenv bootcmd mmc read.p 0x23000000 DTB 0x1000\;mmc read.p 0x25000000 KL \${filesize}\; bootm 0x25000000" >> $FILE_PATH
                elif [ "$LINUX_KERNEL_VERSION" == "3.10.23" ] ; then
                    echo "setenv bootcmd mmc read.p 0x81000000 KL \${filesize}\; bootm 0x81000000" >> $FILE_PATH
                elif [ "$LINUX_KERNEL_VERSION" == "3.10.40" ] || [ "$LINUX_KERNEL_VERSION" == "3.10.86" ]; then
                    echo "setenv bootcmd mmc read.p 0x25000000 KL \${filesize}\; bootm 0x25000000" >> $FILE_PATH
                fi
            fi
            echo "saveenv" >> $FILE_PATH
        ...
        if [ "$DDI_TEE" == "enable" -o "$MI_DRM_SUPPORT_TEE" == "enable" ] && [ "$3" == "tee" ]; then
            if [ "$5" == "NAND" ] || [ "$5" == "EMMC" ] || [ "$5" == "SPI" ]; then
        ...
        if [ "$DDI_TEE" == "enable" -o "$MI_DRM_SUPPORT_TEE" == "enable" ] && [ "$3" == "tee" -o "$3" == "kernel" ]; then
            signed_partition="$3"
            need_to_signed=true
        ...
        if [ "$DDI_TEE" == "enable" -o "$MI_DRM_SUPPORT_TEE" == "enable" -o "$DDI_SECURE_UBOOT" == "enable" ] && [ "$3" == "kernel" ]; then
            echo "nand erase.part KL" >> $FILE_PATH
            printf "nand write.e 0x${TFTP_ADR} KL \$(filesize)"  >> $FILE_PATH
            echo >> $FILE_PATH

        elif [ "$DDI_TEE" == "enable" -o "$MI_DRM_SUPPORT_TEE" == "enable" ] && [ "$3" == "tee" ]; then
        ...
        elif [ "$DDI_OPTEE" == "enable" -o "$MI_DRM_SUPPORT_OPTEE" == "enable" ] && [ "$3" == "optee" ]; then
            echo "nand erase.part OPTEE" >> $FILE_PATH
            printf "nand write.e 0x${TFTP_ADR} OPTEE \$(filesize)"  >> $FILE_PATH
            echo >> $FILE_PATH
            echo >> $FILE_PATH

        elif [ "$DDI_OPTEE" == "enable" -o "$MI_DRM_SUPPORT_OPTEE" == "enable" ] && [ "$3" == "writecis" ]; then
            echo >> $FILE_PATH

        elif [ "$DDI_ECC_SW_OBFUSCATION" == "enable" ] && [ "$3" == "RTPM" ]; then
            echo "nand erase.part RTPM" >> $FILE_PATH
            printf "nand write.e 0x${TFTP_ADR} RTPM \$(filesize)"  >> $FILE_PATH
            echo >> $FILE_PATH

        elif [ "$DDI_OPTEE" == "enable" -o "$MI_DRM_SUPPORT_OPTEE" == "enable" ] && [ "$3" == "armfw" ]; then
            echo "nand erase.part ARMFW" >> $FILE_PATH
            printf "nand write.e 0x${TFTP_ADR} ARMFW 0x%x\n" ${IMG_SIZE} >> $FILE_PATH
            echo >> $FILE_PATH
        elif [ "$FS_TYPE" == "ubifs" ] || [ "$FS_TYPE" == "ramdisk" ]; then
            echo "ubi part UBI" >> $FILE_PATH
            printf "ubi write 0x${TFTP_ADR} $4 0x%x\n" ${IMG_SIZE} >> $FILE_PATH
        elif [ "$FS_TYPE" == "jffs2" ] || [ "$FS_TYPE" == "sqsh" ] || [ "$FS_TYPE" == "cramfs" ]; then
