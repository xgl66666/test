...
elif [ "$LINUX_KERNEL_VERSION" == "3.1.10" ] || [ "$LINUX_KERNEL_VERSION" == "3.10.23" ] || [ "$LINUX_KERNEL_VERSION" == "3.10.23.CMA" ] || [ "$LINUX_KERNEL_VERSION" == "3.10.40" ]
	then
...
if [ "$BOOT_MODE" == "EMMC" ] || [ "$BOOT_MODE" == "NAND" ]
	then
...
if [ "$FLASH_TYPE" == "SPI" ]
	...
	if [ "$CHIP" == "kaiser" ] || [ "$CHIP" == "keltic" ] || [ "$CHIP" == "clippers" ] || [ "$CHIP" == "muji" ] || [ "$CHIP" == "keres" ] || [ "$CHIP" == "kirin" ] || [ "$CHIP" == "kratos" ] || [ "$CHIP" == "kano" ] || [ "$CHIP" == "kiwi" ] || [ "$CHIP" == "kastor" ]
		then
...
function cal_size()
{
  ...
  if [ "$CHIP" == "kiwi" ]; then
      let CAL_SIZE=$((tmp2_size + $RESERVED_SIZE +$2 + 0x3000000))
  else
...
if [ "$FLASH_TYPE" == "SPI" ]
	then
        ...
        if [ "$FS_TYPE" == "sqsh" ]; then
            if [ "$CHIP" == "kratos" ]; then
                # get CONF_SIZE
        		cal_size ../usb_update/Target/image/${CFG_FOLDER_NAME}.sqsh $EXT_CONF_SIZE
        		printf "CONF_SIZE=0x%08X\n" $CAL_SIZE >> ${VAR_LIST}

        		#get MWLIB_SIZE
        		if [ "$RELEASE_MW_LIB" == "enable" ]; then
        			cal_size ../usb_update/Target/image/mwlib.sqsh $EXT_MWLIB_SIZE
        			printf "MWLIB_SIZE=0x%08X\n" $CAL_SIZE >> ${VAR_LIST}
        		fi

                # get LIB_SIZE
        		if [ "$LINK_TYPE" == "dynamic" ]; then
        			cal_size ../usb_update/Target/image/lib_dynamic.sqsh $EXT_LIB_SIZE
		...
		# get KERNEL_SIZE
		cal_size ${KERNEL_IMG_PATH}/${KERNEL_IMG_NAME} 0
		...
		if [ "$CHIP" == "muji" ]
			then
				# get DBT_SIZE
				# cal_size ${KERNEL_IMG_PATH}/DTB.dtb $EXT_DBT_SIZE
				printf "DTB_SIZE=0x1000\n" >> ${VAR_LIST}
		fi
