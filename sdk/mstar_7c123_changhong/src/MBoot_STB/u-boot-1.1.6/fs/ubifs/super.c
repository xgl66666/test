			...
			void *data)
{
    ...
    s->s_type = type;
    strncpy(s->s_id, type->name, (sizeof(s->s_id)-1));
...
static int validate_inode(struct ubifs_info *c, const struct inode *inode)
{
    ...
    if (ui)
    {
        if (ui->compr_type >= UBIFS_COMPR_TYPES_CNT) { //ui->compr_type < 0 ||
...
static int mount_ubifs(struct ubifs_info *c)
{
	...
	snprintf(c->bgt_name, sizeof(c->bgt_name), BGT_NAME_PATTERN, c->vi.ubi_num, c->vi.vol_id);
...
int ubifs_mount(char *vol_name)
{
	 ...
	 */
	flags = MS_RDONLY;
	strncat(name, vol_name, (sizeof(name)-strlen(name)));
	data = NULL;
	mnt = NULL;
