 ...
 */
struct t_testTune TuneFakeChan[1] =
{
...
void TestHelper_DMD_Config(unsigned char *ucFeID, tTDAL_DMD_TerSourceState SourceState)
{
	...
	if(TestHelper_DMD_CurrentIdx < NUMBER_OF_DMD)
	{
		...
		for(idx = 0; idx < (TestHelper_DMD_CurrentIdx+1) && idx < DMD_Capability.uiFENumber; idx++)
		{
		    eFeId = (tTDAL_DMD_FE) idx;
...
void TestHelper_DMD_Tune(unsigned char *ucFeID)
{
	...
	TestHelper_DMD_TuneParam(TuneChannel[gChannel].name, &DMD_TuneParam, &uiFrequency);
...
void TestHelper_DMD_GetInfo(unsigned char *ucFeID)
{
	...
	if (DMD_Info.TunData.Sat.Modulation == eTDAL_DMD_MOD_QPSK)
	{
